SELECT DISTINCT 
	...
	T6.UNIT_NAME_RYAKU AS NISUGATA_UNIT_NAME_RYAKU,
	CASE 
	WHEN (SELECT COUNT(*) FROM T_ZAIKO_HINMEI_HURIWAKE tzhh WHERE T1.SYSTEM_ID = tzhh.SYSTEM_ID AND T1.DETAIL_SYSTEM_ID = tzhh.DETAIL_SYSTEM_ID AND T1.SEQ = tzhh.SEQ AND tzhh.DENSHU_KBN_CD = '2') > 1 THEN '複数在庫品目' 
	ELSE T7.ZAIKO_HINMEI_NAME 
	END AS ZAIKO_HINMEI_NAME
		

FROM dbo.T_SHUKKA_DETAIL AS T1 
LEFT JOIN dbo.M_YOUKI AS T2 ON T1.YOUKI_CD = T2.YOUKI_CD AND T2.DELETE_FLG = 0
LEFT JOIN dbo.T_MANIFEST_ENTRY AS T3 ON T1.SYSTEM_ID = T3.RENKEI_SYSTEM_ID AND T1.DETAIL_SYSTEM_ID=T3.RENKEI_MEISAI_SYSTEM_ID AND T3.RENKEI_DENSHU_KBN_CD='2' AND T3.DELETE_FLG = 0
LEFT JOIN dbo.M_DENPYOU_KBN AS T4 ON T1.DENPYOU_KBN_CD = T4.DENPYOU_KBN_CD AND T4.DELETE_FLG = 0
LEFT JOIN dbo.M_UNIT AS T5 ON T1.UNIT_CD = T5.UNIT_CD AND T5.DELETE_FLG = 0
LEFT JOIN dbo.M_UNIT AS T6 ON T1.NISUGATA_UNIT_CD = T6.UNIT_CD AND T6.DELETE_FLG = 0 
LEFT JOIN
 (SELECT TZ.SYSTEM_ID,TZ.DETAIL_SYSTEM_ID,TZ.SEQ,TZ.ZAIKO_HINMEI_NAME FROM T_ZAIKO_HINMEI_HURIWAKE TZ 
   INNER JOIN (SELECT SYSTEM_ID,DETAIL_SYSTEM_ID,SEQ,DENSHU_KBN_CD FROM T_ZAIKO_HINMEI_HURIWAKE
	 ...
	 ON TZ.SYSTEM_ID = TT.SYSTEM_ID AND TZ.DETAIL_SYSTEM_ID = TT.DETAIL_SYSTEM_ID AND TZ.SEQ = TT.SEQ AND TT.DENSHU_KBN_CD = '2') AS T7
