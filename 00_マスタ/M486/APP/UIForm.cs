// $Id: UIForm.cs 33429 2014-10-27 07:56:47Z y-hosokawa@takumi-sys.co.jp $
using System;
...
using Shougun.Core.Master.HikiaiGenbaIchiran.Logic;
...
namespace Shougun.Core.Master.HikiaiGenbaIchiran.APP
{
    public partial class UIForm : Shougun.Core.Common.IchiranCommon.APP.IchiranSuperForm
    {
        ...
        {
            if (this.customDataGridView1.Columns.Contains(this.businessLogic.KEY_FLG))
            {
                this.customDataGridView1.Columns[this.businessLogic.KEY_FLG].Visible = false;
            }
            ...
            if (this.customDataGridView1.Columns.Contains(this.businessLogic.KEY_ID2))
            {
                this.customDataGridView1.Columns[this.businessLogic.KEY_ID2].Visible = false;
            }
        }
                ...
                if (!this.DesignMode)
                {
                    ...
                    if (isClear)
                    {
                        sql = "select " + this.logic.SelectQeury +
                                " from M_HIKIAI_GENBA " +
                                " LEFT JOIN M_SHAIN ON M_SHAIN.SHAIN_CD = M_HIKIAI_GENBA.EIGYOU_TANTOU_CD" +
                                " LEFT JOIN M_TORIHIKISAKI ON M_TORIHIKISAKI.TORIHIKISAKI_CD = M_HIKIAI_GENBA.TORIHIKISAKI_CD AND M_HIKIAI_GENBA.HIKIAI_TORIHIKISAKI_USE_FLG = 0 " +
                                " LEFT JOIN M_HIKIAI_TORIHIKISAKI ON M_HIKIAI_TORIHIKISAKI.TORIHIKISAKI_CD = M_HIKIAI_GENBA.TORIHIKISAKI_CD AND M_HIKIAI_GENBA.HIKIAI_TORIHIKISAKI_USE_FLG = 1 " +
                                " LEFT JOIN M_GYOUSHA ON M_GYOUSHA.GYOUSHA_CD = M_HIKIAI_GENBA.GYOUSHA_CD AND M_HIKIAI_GENBA.HIKIAI_GYOUSHA_USE_FLG = 0 " +
                                " LEFT JOIN M_HIKIAI_GYOUSHA ON M_HIKIAI_GYOUSHA.GYOUSHA_CD = M_HIKIAI_GENBA.GYOUSHA_CD AND M_HIKIAI_GENBA.HIKIAI_GYOUSHA_USE_FLG = 1 " +
                                " where 1 = 0 " +
                                " order by " + orderBy;
                    }
                    ...
                    this.Table = this.businessLogic.daoGenba.GetDateForStringSql(sql);
                }
            }
            catch (Exception ex)
            {
        ...
        public void PopupAfterTorihikisakiCd()
        {
            try
            {
                ...
                if (this.businessLogic.SearchsetTorihikisaki(null) == -1)
                {
                    return;
                }
        ...
        internal void GYOUSHA_CD_Enter(object sender, EventArgs e)
        {
            try
            {
                ...
                this.PreviousValue = this.GYOUSHA_CD.Text;
            }
            catch (Exception ex)
            {
                LogUtility.Fatal("GYOUSHA_CD_Enter", ex);
                throw ex;
            }
            finally
            {
        ...
        internal void GYOUSHA_CD_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                ...
                //値の変更をチェック
                if (!this.PreviousValue.Equals(this.GYOUSHA_CD.Text))
                {
                    this.HIKIAI_GYOUSHA_USE_FLG.Text = "0";
                    //取引先拠点セット
                    this.businessLogic.IsSetDataFromPopup = false;
                    this.businessLogic.SearchsetGyousha(e);
                }
            }
            catch (Exception ex)
            {
        ...
        /// 取引先CD検索ポップアップ後の処理を実施
        /// </summary>
        public void PopupAfterGyoushaCd()
        {
            try
            {
                LogUtility.DebugMethodStart();
                ...
                if (!this.PreviousValue.Equals(this.GYOUSHA_CD.Text))
                {
                    ...
                    //業者セット
                    this.businessLogic.IsSetDataFromPopup = true;
                    if (this.businessLogic.SearchsetGyousha(null) == -1)
                    {
                        return;
                    }
                    if (this.ActiveControl != null)
            ...
            {
                LogUtility.Error("PopupAfterGyoushaCd", ex);
                throw;
            }
            finally
            {
        ...
        internal void GENBA_CD_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                ...
                this.businessLogic.GENBA_CD_Validating(sender, e);
            }
            catch (Exception ex)
            {
