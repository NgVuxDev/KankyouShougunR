...
using GenbamemoBunruiHoshu.APP;
using GenbamemoBunruiHoshu.Validator;
...
namespace GenbamemoBunruiHoshu.Logic
{
    /// <summary>
    /// 現場メモ分類入力画面のビジネスロジック
    /// </summary>
    public class GenbamemoBunruiHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "GenbamemoBunruiHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_BUNRUI_DATA_SQL = "GenbamemoBunruiHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_BUNRUI_DATA_SQL = "GenbamemoBunruiHoshu.Sql.GetGenbamemoBunruiDataSql.sql";

        /// <summary>
        /// 現場メモ分類入力画面Form
        /// </summary>
        private GenbamemoBunruiHoshuForm form;
        ...
        private M_GENBAMEMO_BUNRUI[] entitys;
        ...
        /// 現場メモ分類マスタのDao
        /// </summary>
        private IM_GENBAMEMO_BUNRUIDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_GENBAMEMO_BUNRUI SearchString { get; set; }
        ...
        public GenbamemoBunruiHoshuLogic(GenbamemoBunruiHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_BUNRUI_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_BUNRUI_DATA_SQL, new M_GENBAMEMO_BUNRUI());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_GENBAMEMO_BUNRUI[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_GENBAMEMO_BUNRUI();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_GENBAMEMO_BUNRUI>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.GenbamemoBunruiHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_GENBAMEMO_BUNRUI> addList = new List<M_GENBAMEMO_BUNRUI>();
                foreach (var bunruiEntity in bunruiEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.GenbamemoBunruiHoshuConstans.GENBAMEMO_BUNRUI_CD) && n.Value.ToString().Equals(bunruiEntity.GENBAMEMO_BUNRUI_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.GenbamemoBunruiHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                GenbamemoBunruiHoshuValidator vali = new GenbamemoBunruiHoshuValidator();
                bool result = vali.GenbamemoBunruiCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_GENBAMEMO_BUNRUI bunruiEntity in this.entitys)
                        {
                            M_GENBAMEMO_BUNRUI entity = this.dao.GetDataByCd(bunruiEntity.GENBAMEMO_BUNRUI_CD);
                            if (entity == null)
                            {
        ...
        public override bool Equals(object other)
        {
            ...
            GenbamemoBunruiHoshuLogic localLogic = other as GenbamemoBunruiHoshuLogic;
        ...
        {
            M_GENBAMEMO_BUNRUI entity = new M_GENBAMEMO_BUNRUI();
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.GENBAMEMO_BUNRUI_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["GENBAMEMO_BUNRUI_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
