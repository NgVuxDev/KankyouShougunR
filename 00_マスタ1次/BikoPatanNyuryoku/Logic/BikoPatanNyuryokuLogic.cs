// $Id: BikoPatanNyuryokuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using BikoPatanNyuryoku.APP;
using BikoPatanNyuryoku.Validator;
...
namespace BikoPatanNyuryoku.Logic
{
    /// <summary>
    /// 備考パターン保守画面のビジネスロジック
    /// </summary>
    public class BikoPatanNyuryokuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "BikoPatanNyuryoku.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_BIKO_DATA_SQL = "BikoPatanNyuryoku.Sql.GetIchiranDataSql.sql";

        private readonly string GET_BIKO_DATA_SQL = "BikoPatanNyuryoku.Sql.GetBikoPatanNyuryokuDataSql.sql";

        /// <summary>
        /// 備考パターン保守画面Form
        /// </summary>
        private BikoPatanNyuryokuForm form;
        ...
        private M_BIKO_PATAN_NYURYOKU[] entitys;
        ...
        /// 備考パターンマスタのDao
        /// </summary>
        private IM_BIKO_PATAN_NYURYOKUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_BIKO_PATAN_NYURYOKU SearchString { get; set; }
        ...
        public BikoPatanNyuryokuLogic(BikoPatanNyuryokuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_BIKO_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_BIKO_DATA_SQL, new M_BIKO_PATAN_NYURYOKU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_BIKO_PATAN_NYURYOKU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_BIKO_PATAN_NYURYOKU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_BIKO_PATAN_NYURYOKU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.BikoPatanNyuryokuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_BIKO_PATAN_NYURYOKU> addList = new List<M_BIKO_PATAN_NYURYOKU>();
                foreach (var bikoEntity in bikoEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.BikoPatanNyuryokuConstans.BIKO_CD) && n.Value.ToString().Equals(bikoEntity.BIKO_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.BikoPatanNyuryokuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                BikoPatanNyuryokuValidator vali = new BikoPatanNyuryokuValidator();
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_BIKO_PATAN_NYURYOKU bikoEntity in this.entitys)
                        {
                            M_BIKO_PATAN_NYURYOKU entity = this.dao.GetDataByCd(bikoEntity.BIKO_CD);
                            if (entity == null)
                            {
        ...
        public override bool Equals(object other)
        {
            ...
            BikoPatanNyuryokuLogic localLogic = other as BikoPatanNyuryokuLogic;
        ...
        {
            M_BIKO_PATAN_NYURYOKU entity = new M_BIKO_PATAN_NYURYOKU();
