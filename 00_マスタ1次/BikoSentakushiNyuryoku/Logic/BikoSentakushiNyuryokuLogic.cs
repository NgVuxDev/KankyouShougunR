// $Id: BikoSentakushiNyuryokuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using BikoSentakushiNyuryoku.APP;
using BikoSentakushiNyuryoku.Validator;
...
namespace BikoSentakushiNyuryoku.Logic
{
    /// <summary>
    /// 備考パターン保守画面のビジネスロジック
    /// </summary>
    public class BikoSentakushiNyuryokuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "BikoSentakushiNyuryoku.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_BIKO_DATA_SQL = "BikoSentakushiNyuryoku.Sql.GetIchiranDataSql.sql";

        private readonly string GET_BIKO_DATA_SQL = "BikoSentakushiNyuryoku.Sql.GetBikoSentakushiNyuryokuDataSql.sql";

        /// <summary>
        /// 備考パターン保守画面Form
        /// </summary>
        private BikoSentakushiNyuryokuForm form;
        ...
        private M_BIKO_SENTAKUSHI_NYURYOKU[] entitys;
        ...
        /// 備考パターンマスタのDao
        /// </summary>
        private IM_BIKO_SENTAKUSHI_NYURYOKUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_BIKO_SENTAKUSHI_NYURYOKU SearchString { get; set; }
        ...
        public BikoSentakushiNyuryokuLogic(BikoSentakushiNyuryokuForm targetForm)
        {
            LogUtility.DebugMethodStart(targetForm);

            this.form = targetForm;
            this.dao = DaoInitUtility.GetComponent<IM_BIKO_SENTAKUSHI_NYURYOKUDao>();
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_BIKO_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_BIKO_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_BIKO_DATA_SQL, new M_BIKO_SENTAKUSHI_NYURYOKU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_BIKO_SENTAKUSHI_NYURYOKU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_BIKO_SENTAKUSHI_NYURYOKU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_BIKO_SENTAKUSHI_NYURYOKU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.BikoSentakushiNyuryokuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var bikoEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_BIKO_SENTAKUSHI_NYURYOKU> addList = new List<M_BIKO_SENTAKUSHI_NYURYOKU>();
                foreach (var bikoEntity in bikoEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.BikoSentakushiNyuryokuConstans.BIKO_CD) && n.Value.ToString().Equals(bikoEntity.BIKO_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.BikoSentakushiNyuryokuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), bikoEntity);
                            addList.Add(bikoEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                BikoSentakushiNyuryokuValidator vali = new BikoSentakushiNyuryokuValidator();
                bool result = vali.BikoCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
            ...
            try
            {
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    if (gcRwos.Cells["DELETE_FLG"].Value != null && gcRwos.Cells["DELETE_FLG"].Value.ToString() == "True")
                    {
                        ...
                        //    continue;
                        //}
                        bikoCd += gcRwos.Cells["BIKO_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(bikoCd))
                {
                    bikoCd = bikoCd.Substring(0, bikoCd.Length - 1);
                    strList = bikoCd.Split(',');
                    ...
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        msgLogic.MessageBoxShow("E258", "備考パターン", "備考パターンCD", strName);
        ...
        public void Preview()
        {
            ...
            MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
            msgLogic.MessageBoxShow("C011", "備考パターン一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_BIKO_SENTAKUSHI_NYURYOKU bikoEntity in this.entitys)
                        {
                            M_BIKO_SENTAKUSHI_NYURYOKU entity = this.dao.GetDataByCd(bikoEntity.BIKO_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (bikoEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(bikoEntity);
                            }
                            else
                            {
                                this.dao.Update(bikoEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            BikoSentakushiNyuryokuLogic localLogic = other as BikoSentakushiNyuryokuLogic;
        ...
        {
            M_BIKO_SENTAKUSHI_NYURYOKU entity = new M_BIKO_SENTAKUSHI_NYURYOKU();
