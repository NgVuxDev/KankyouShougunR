...
namespace BunruiHoshu.Validator
{
    ...
    /// 分類保守検証ロジック
    /// </summary>
    public class BunruiHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public BunruiHoshuValidator()
        {
        }
        ...
        public bool BunruiCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.BunruiHoshuConstans.BUNRUI_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.BunruiHoshuConstans.BUNRUI_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowBunruiCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string bunruiCd = row.Field<string>(Const.BunruiHoshuConstans.BUNRUI_CD);
                ...
                {
                    msgLogic.MessageBoxShow("E022", "入力された分類CD");
                    result = false;
                }
            }
