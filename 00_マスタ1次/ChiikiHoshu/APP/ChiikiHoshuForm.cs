// $Id: ChiikiHoshuForm.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
namespace ChiikiHoshu.APP
{
    ...
    /// </summary>
    [Implementation]
    public partial class ChiikiHoshuForm : SuperForm
    {
        /// <summary>
        /// 地域保守画面ロジック
        /// </summary>
        private ChiikiHoshuLogic logic;
        internal MessageBoxShowLogic errmessage = new MessageBoxShowLogic();

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public ChiikiHoshuForm()
            : base(WINDOW_ID.M_CHIIKI, WINDOW_TYPE.ICHIRAN_WINDOW_FLAG)
        {
            InitializeComponent();

            // 画面タイプなど引数値は変更となるが基本的にやることは変わらない
            this.logic = new ChiikiHoshuLogic(this);
        ...
        {
            if (e.CellName.Equals(Const.ChiikiHoshuConstans.CHIIKI_CD))
            {
        ...
        /// 都道府県選択後処理
        /// </summary>
        public virtual void TodoufukenPopupAfter()
        {
            string cd = string.Empty;
            if (this.Ichiran.EditingControl != null)
            {
                cd = this.Ichiran.EditingControl.Text;
                if (!string.IsNullOrWhiteSpace(cd))
                {
                    cd = string.Format("{0:D2}", Int16.Parse(cd));
                    this.Ichiran.EditingControl.Text = cd;
                }
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ChiikiHoshuForm_Shown(object sender, EventArgs e)
        {
            // 主キーを非活性にする
            this.logic.EditableToPrimaryKey();
        }
