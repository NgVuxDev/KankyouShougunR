// $Id: ChiikiHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace ChiikiHoshu.Validator
{
    /// <summary>
    /// 地域保守検証ロジック
    /// </summary>
    public class ChiikiHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public ChiikiHoshuValidator()
        {
        }
        ...
        public bool ChiikiCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.ChiikiHoshuConstans.CHIIKI_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.ChiikiHoshuConstans.CHIIKI_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    {
                        string shainCd = row.Field<string>(Const.ChiikiHoshuConstans.CHIIKI_CD);
