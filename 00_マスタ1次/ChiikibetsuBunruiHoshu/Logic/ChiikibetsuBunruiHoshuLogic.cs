// $Id: ChiikibetsuBunruiHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using ChiikibetsuBunruiHoshu.APP;
using ChiikibetsuBunruiHoshu.Validator;
...
namespace ChiikibetsuBunruiHoshu.Logic
{
    /// <summary>
    /// 地域別分類保守画面のビジネスロジック
    /// </summary>
    public class ChiikibetsuBunruiHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ChiikibetsuBunruiHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_CHIIKIBETSU_BUNRUI_DATA_SQL = "ChiikibetsuBunruiHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_CHIIKIBETSU_BUNRUI_DATA_SQL = "ChiikibetsuBunruiHoshu.Sql.GetChiikibetsuBunruiDataSql.sql";

        private readonly string GET_CHIIKIBETSU_BUNRUI_BY_CD_SQL = "ChiikibetsuBunruiHoshu.Sql.GetChiikibetsuBunruiByCdSql.sql";

        private readonly string GET_CHIIKIBETSU_BUNRUI_STRUCT_SQL = "ChiikibetsuBunruiHoshu.Sql.GetIchiranStructSql.sql";

        private readonly string GET_CHIIKIBETSU_BUNRUI_ALL_SQL = "ChiikibetsuBunruiHoshu.Sql.GetIchiranALLSql.sql";

        /// <summary>
        /// 地域別分類保守画面Form
        /// </summary>
        private ChiikibetsuBunruiHoshuForm form;
        ...
        /// 地域別分類マスタエンティティ
        /// </summary>
        private M_CHIIKIBETSU_BUNRUI[] entitys;
        ...
        /// 地域別分類のDao
        /// </summary>
        private IM_CHIIKIBETSU_BUNRUIDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_CHIIKIBETSU_BUNRUI SearchString { get; set; }
        ...
        public ChiikibetsuBunruiHoshuLogic(ChiikibetsuBunruiHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                // 対象データを抽出する
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_BUNRUI_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_BUNRUI_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);

                // M_CHIIKIBETSU_BUNRUIに存在しない項目での絞り込みはデータ取得後に行う
                if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
                {
                    switch (this.form.CONDITION_VALUE.DBFieldsName)
                    {
                        ...
                        case ("HOUKOKUSHO_BUNRUI_NAME_RYAKU"):
                            var table = (
                ...
                // 対象外も含め、全データを抽出する
                M_CHIIKIBETSU_BUNRUI searchParam = new M_CHIIKIBETSU_BUNRUI();
                searchParam.CHIIKI_CD = this.form.CHIIKI_CD.Text;
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_BUNRUI_DATA_SQL, searchParam);
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_CHIIKIBETSU_BUNRUI[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_CHIIKIBETSU_BUNRUI();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_CHIIKIBETSU_BUNRUI>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ChiikibetsuBunruiHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_CHIIKIBETSU_BUNRUI> addList = new List<M_CHIIKIBETSU_BUNRUI>();
                foreach (var CHIIKIEntity in chiikiEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuBunruiHoshuConstans.HOUKOKUSHO_BUNRUI_CD) && n.Value.ToString().Equals(CHIIKIEntity.HOUKOKUSHO_BUNRUI_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuBunruiHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ChiikibetsuBunruiHoshuValidator vali = new ChiikibetsuBunruiHoshuValidator();
        ...
        public void Preview()
        {
            ...
            msgLogic.MessageBoxShow("C011", "地域別分類一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_CHIIKIBETSU_BUNRUI CHIIKIEntity in this.entitys)
                        {
                            dt = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_BUNRUI_BY_CD_SQL, CHIIKIEntity);
                            if (dt.Rows.Count <= 0)
                            {
        ...
        public override bool Equals(object other)
        {
            ...
            ChiikibetsuBunruiHoshuLogic localLogic = other as ChiikibetsuBunruiHoshuLogic;
        ...
        internal bool SetIchiran(bool isRead)
        {
            try
            {
                    ...
                    for (int i = 0; i < this.SearchResult.Rows.Count; i++)
                    {
                        ...
                        foreach (DataRow dr in this.SearchResultAll.Rows)
                        {
                            if (SearchResult.Rows[i]["CHIIKI_CD"].ToString() == dr["CHIIKI_CD"].ToString()
                                && SearchResult.Rows[i]["HOUKOKUSHO_BUNRUI_CD"].ToString() == dr["HOUKOKUSHO_BUNRUI_CD"].ToString())
                            {
                                DataRow row = table.NewRow();
                                row["CHIIKI_CD"] = dr["CHIIKI_CD"];
                                row["HOUKOKUSHO_BUNRUI_CD"] = dr["HOUKOKUSHO_BUNRUI_CD"];
                                row["HOUKOKUSHO_BUNRUI_NAME_RYAKU"] = dr["HOUKOKUSHO_BUNRUI_NAME_RYAKU"];
                                row["HOUKOKU_BUNRUI_CD"] = dr["HOUKOKU_BUNRUI_CD"];
                                row["HOUKOKU_BUNRUI_NAME"] = dr["HOUKOKU_BUNRUI_NAME"];
                                row["SEKIMEN_KBN"] = false;
                                row["TOKUTEI_YUUGAI_KBN"] = false;
                        ...
                        if (!isKey)
                        {
                            ...
                            row["HOUKOKUSHO_BUNRUI_CD"] = SearchResult.Rows[i]["HOUKOKUSHO_BUNRUI_CD"];
                            row["HOUKOKUSHO_BUNRUI_NAME_RYAKU"] = SearchResult.Rows[i]["HOUKOKUSHO_BUNRUI_NAME_RYAKU"];
                            row["HOUKOKU_BUNRUI_CD"] = SearchResult.Rows[i]["HOUKOKU_BUNRUI_CD"];
                            row["HOUKOKU_BUNRUI_NAME"] = SearchResult.Rows[i]["HOUKOKU_BUNRUI_NAME"];
                            row["SEKIMEN_KBN"] = false;
                            row["TOKUTEI_YUUGAI_KBN"] = false;
                            row["DELETE_FLG"] = false;
                            table.Rows.Add(row);
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M242", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        private void EventInit()
        {
            ...
            //報告書分類読込ボタン(process1)イベント生成
            parentForm.bt_process1.Click += new EventHandler(this.ReadHoukokushoBunrui);
        ...
        {
            M_CHIIKIBETSU_BUNRUI entity = new M_CHIIKIBETSU_BUNRUI();
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
            {
                ...
                {
                    // M_CHIIKIBETSU_BUNRUIに存在しない項目は検索条件に含めない
                    switch (this.form.CONDITION_VALUE.DBFieldsName)
                    {
                        case ("HOUKOKUSHO_BUNRUI_NAME_RYAKU"):
                            break;
        ...
        private void UpdateRelationInfo(M_CHIIKIBETSU_BUNRUI entity, bool isDelete)
        {
            if (entity == null
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(proc))
                    {
                        if (proc.Equals("1"))
                        {
                            this.ReadHoukokushoBunrui(sender, e);
                        }
                    }
                }
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void ReadHoukokushoBunrui(object sender, EventArgs e)
        {
            try
            {
                ...
                // 問い合わせ確認
                if (msgLogic.MessageBoxShow("C066", "報告用分類", "報告書分類") != DialogResult.Yes)
                {
                    this.form.CHIIKI_CD.Focus();
                    return;
                }
                ...
                // DataTable構造作成
                this.SearchResult = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_BUNRUI_STRUCT_SQL, null);
                this.SearchResultAll = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_BUNRUI_ALL_SQL, null);
                ...
                // 報告書分類読込処理
                M_HOUKOKUSHO_BUNRUI[] bunrui = DaoInitUtility.GetComponent<IM_HOUKOKUSHO_BUNRUIDao>().GetAllValidData(new M_HOUKOKUSHO_BUNRUI());
                if (bunrui != null && bunrui.Length > 0)
                {
                    for (int i = 0; i < bunrui.Length; i++)
                    {
                        DataRow row = this.SearchResult.NewRow();
                        row["CHIIKI_CD"] = this.form.CHIIKI_CD.Text;
                        row["HOUKOKUSHO_BUNRUI_CD"] = bunrui[i].HOUKOKUSHO_BUNRUI_CD;
                        row["HOUKOKUSHO_BUNRUI_NAME_RYAKU"] = bunrui[i].HOUKOKUSHO_BUNRUI_NAME_RYAKU;
                        row["HOUKOKU_BUNRUI_CD"] = bunrui[i].HOUKOKUSHO_BUNRUI_CD.Substring(2, 4);
                        row["HOUKOKU_BUNRUI_NAME"] = bunrui[i].HOUKOKUSHO_BUNRUI_NAME;
                        row["SEKIMEN_KBN"] = false;
                        row["TOKUTEI_YUUGAI_KBN"] = false;
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.HOUKOKUSHO_BUNRUI_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["HOUKOKUSHO_BUNRUI_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
