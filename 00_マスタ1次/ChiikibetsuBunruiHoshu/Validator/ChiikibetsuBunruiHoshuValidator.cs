// $Id: ChiikibetsuBunruiHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace ChiikibetsuBunruiHoshu.Validator
{
    ...
    /// 地域分類保守検証ロジック
    /// </summary>
    public class ChiikibetsuBunruiHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public ChiikibetsuBunruiHoshuValidator()
        {
        }
        ...
        public bool CHIIKICDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.ChiikibetsuBunruiHoshuConstans.HOUKOKUSHO_BUNRUI_CD] as GcCustomAlphaNumTextBoxCell;
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.ChiikibetsuBunruiHoshuConstans.HOUKOKUSHO_BUNRUI_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                if (dt != null)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowChiikibetsuBunruiCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string gyoushuCd = row.Field<string>(Const.ChiikibetsuBunruiHoshuConstans.HOUKOKUSHO_BUNRUI_CD);
