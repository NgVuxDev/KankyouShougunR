// $Id: ChiikibetsuJuushoHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using ChiikibetsuJuushoHoshu.APP;
using ChiikibetsuJuushoHoshu.Validator;
...
namespace ChiikibetsuJuushoHoshu.Logic
{
    /// <summary>
    /// 地域別住所保守画面のビジネスロジック
    /// </summary>
    public class ChiikibetsuJuushoHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ChiikibetsuJuushoHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_CHIIKIBETSU_JUUSHO_DATA_SQL = "ChiikibetsuJuushoHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_CHIIKIBETSU_JUUSHO_DATA_SQL = "ChiikibetsuJuushoHoshu.Sql.GetChiikibetsuJuushoDataSql.sql";

        private readonly string GET_CHIIKIBETSU_JUUSHO_BY_CD_SQL = "ChiikibetsuJuushoHoshu.Sql.GetChiikibetsuJuushoByCdSql.sql";

        private readonly string GET_CHIIKIBETSU_JUUSHO_STRUCT_SQL = "ChiikibetsuJuushoHoshu.Sql.GetIchiranStructSql.sql";

        private readonly string GET_CHIIKIBETSU_JUUSHO_ALL_SQL = "ChiikibetsuJuushoHoshu.Sql.GetIchiranAllDataSql.sql";

        /// <summary>
        /// 地域別住所保守画面Form
        /// </summary>
        private ChiikibetsuJuushoHoshuForm form;
        ...
        /// 地域別業種マスタエンティティ
        /// </summary>
        private M_CHIIKIBETSU_JUUSHO[] entitys;
        ...
        /// 地域別住所のDao
        /// </summary>
        private IM_CHIIKIBETSU_JUUSHODao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_CHIIKIBETSU_JUUSHO SearchString { get; set; }
        ...
        public ChiikibetsuJuushoHoshuLogic(ChiikibetsuJuushoHoshuForm targetForm)
        {
            LogUtility.DebugMethodStart(targetForm);

            this.form = targetForm;
            this.dao = DaoInitUtility.GetComponent<IM_CHIIKIBETSU_JUUSHODao>();
        ...
        public int Search()
        {
            try
            {
                ...
                // 対象データを抽出する
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_JUUSHO_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_JUUSHO_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                // M_CHIIKIBETSU_JUUSHOに存在しない項目での絞り込みはデータ取得後に行う
                ...
                // 対象外も含め、全データを抽出する
                M_CHIIKIBETSU_JUUSHO searchParam = new M_CHIIKIBETSU_JUUSHO();
                searchParam.CHIIKI_CD = this.form.CHIIKI_CD.Text;
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_JUUSHO_DATA_SQL, searchParam);
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_CHIIKIBETSU_JUUSHO[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_CHIIKIBETSU_JUUSHO();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_CHIIKIBETSU_JUUSHO>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ChiikibetsuJuushoHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_CHIIKIBETSU_JUUSHO> addList = new List<M_CHIIKIBETSU_JUUSHO>();
                foreach (var CHIIKIEntity in CHIIKIEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuJuushoHoshuConstans.CHANGE_CHIIKI_CD) && n.Value.ToString().Equals(CHIIKIEntity.CHANGE_CHIIKI_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuJuushoHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ChiikibetsuJuushoHoshuValidator vali = new ChiikibetsuJuushoHoshuValidator();
        ...
        public void Preview()
        {
            ...
            msgLogic.MessageBoxShow("C011", "地域別住所一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_CHIIKIBETSU_JUUSHO CHIIKIEntity in this.entitys)
                        {
                            dt = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_JUUSHO_BY_CD_SQL, CHIIKIEntity);
                            if (dt.Rows.Count <= 0)
                            {
        ...
        public override bool Equals(object other)
        {
            ...
            ChiikibetsuJuushoHoshuLogic localLogic = other as ChiikibetsuJuushoHoshuLogic;
        ...
        internal bool SetIchiran(bool isRead)
        {
            try
            {
                    ...
                    for (int i = 0; i < this.SearchResult.Rows.Count; i++)
                    {
                        ...
                        foreach (DataRow dr in this.SearchResultAll.Rows)
                        {
                            if (SearchResult.Rows[i]["CHIIKI_CD"].ToString() == dr["CHIIKI_CD"].ToString()
                                && SearchResult.Rows[i]["CHANGE_CHIIKI_CD"].ToString() == dr["CHANGE_CHIIKI_CD"].ToString())
                            {
                                DataRow row = table.NewRow();
                                row["CHIIKI_CD"] = dr["CHIIKI_CD"];
                                row["CHANGE_CHIIKI_CD"] = dr["CHANGE_CHIIKI_CD"];
                                row["HOUKOKU_JUUSHO_CD"] = SearchResult.Rows[i]["HOUKOKU_JUUSHO_CD"];
                                row["HOUKOKU_JUUSHO_NAME"] = SearchResult.Rows[i]["HOUKOKU_JUUSHO_NAME"];
                                row["CHIIKI_NAME_RYAKU"] = SearchResult.Rows[i]["CHIIKI_NAME_RYAKU"];
                                row["CHIIKIBETSU_JUUSHO_BIKOU"] = SearchResult.Rows[i]["CHIIKIBETSU_JUUSHO_BIKOU"];
                        ...
                        if (!isKey)
                        {
                            ...
                            row["CHANGE_CHIIKI_CD"] = SearchResult.Rows[i]["CHANGE_CHIIKI_CD"];
                            row["HOUKOKU_JUUSHO_CD"] = SearchResult.Rows[i]["HOUKOKU_JUUSHO_CD"];
                            row["HOUKOKU_JUUSHO_NAME"] = SearchResult.Rows[i]["HOUKOKU_JUUSHO_NAME"];
                            row["CHIIKI_NAME_RYAKU"] = SearchResult.Rows[i]["CHIIKI_NAME_RYAKU"];
                            row["CHIIKIBETSU_JUUSHO_BIKOU"] = SearchResult.Rows[i]["CHIIKIBETSU_JUUSHO_BIKOU"];
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M240", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        private void EventInit()
        {
            ...
            //地域読込ボタン(process1)イベント生成
            parentForm.bt_process1.Click += new EventHandler(this.ReadChiiki);
        ...
        {
            M_CHIIKIBETSU_JUUSHO entity = new M_CHIIKIBETSU_JUUSHO();
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
            {
                if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.ItemDefinedTypes))
                {
                    if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
                    {
                        if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.ItemDefinedTypes))
                        {
                            // M_CHIIKIBETSU_JUUSHOに存在しない項目は検索条件に含めない
                            switch (this.form.CONDITION_VALUE.DBFieldsName)
                            {
                                case ("CHIIKI_NAME_RYAKU"):
                                    break;
        ...
        private void UpdateRelationInfo(M_CHIIKIBETSU_JUUSHO entity, bool isDelete)
        {
            if (entity == null
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void ReadChiiki(object sender, EventArgs e)
        {
            try
            {
                ...
                // 問い合わせ確認
                if (msgLogic.MessageBoxShow("C066", "報告用住所", "地域") != DialogResult.Yes)
                {
                    this.form.CHIIKI_CD.Focus();
                    return;
                }
                ...
                // DataTable構造作成
                this.SearchResult = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_JUUSHO_STRUCT_SQL, null);
                this.SearchResultAll = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_JUUSHO_ALL_SQL, null);
                ...
                // 地域読込処理
                M_CHIIKI[] chiiki = DaoInitUtility.GetComponent<IM_CHIIKIDao>().GetAllValidData(new M_CHIIKI());
                if (chiiki != null && chiiki.Length > 0)
                {
                    for (int i = 0; i < chiiki.Length; i++)
                    {
                        DataRow row = this.SearchResult.NewRow();
                        row["CHIIKI_CD"] = this.form.CHIIKI_CD.Text;
                        row["CHANGE_CHIIKI_CD"] = chiiki[i].CHIIKI_CD;
                        row["CHIIKI_NAME_RYAKU"] = chiiki[i].CHIIKI_NAME_RYAKU;
                        row["HOUKOKU_JUUSHO_CD"] = chiiki[i].CHIIKI_CD.Substring(2, 4);
                        row["HOUKOKU_JUUSHO_NAME"] = chiiki[i].CHIIKI_NAME;
                        row["CHIIKIBETSU_JUUSHO_BIKOU"] = chiiki[i].CHIIKI_BIKOU;
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.CHANGE_CHIIKI_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["CHANGE_CHIIKI_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
