// $Id: ChiikibetsuShisetsuHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using ChiikibetsuShisetsuHoshu.APP;
using ChiikibetsuShisetsuHoshu.Validator;
...
namespace ChiikibetsuShisetsuHoshu.Logic
{
    /// <summary>
    /// 地域別施設保守画面のビジネスロジック
    /// </summary>
    public class ChiikibetsuShisetsuHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ChiikibetsuShisetsuHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_CHIIKIBETSU_SHISETSU_DATA_SQL = "ChiikibetsuShisetsuHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_CHIIKIBETSU_SHISETSU_DATA_SQL = "ChiikibetsuShisetsuHoshu.Sql.GetChiikibetsuShisetsuDataSql.sql";

        private readonly string GET_CHIIKIBETSU_SHISETSU_BY_CD_SQL = "ChiikibetsuShisetsuHoshu.Sql.GetChiikibetsuShisetsuByCdSql.sql";

        private readonly string GET_CHIIKIBETSU_SHISETSU_STRUCT_SQL = "ChiikibetsuShisetsuHoshu.Sql.GetIchiranStructSql.sql";

        /// <summary>
        /// 地域別施設保守画面Form
        /// </summary>
        private ChiikibetsuShisetsuHoshuForm form;
        ...
        /// 地域別施設マスタエンティティ
        /// </summary>
        private M_CHIIKIBETSU_SHISETSU[] entitys;
        ...
        /// 地域別施設のDao
        /// </summary>
        private IM_CHIIKIBETSU_SHISETSUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_CHIIKIBETSU_SHISETSU SearchString { get; set; }
        ...
        public ChiikibetsuShisetsuHoshuLogic(ChiikibetsuShisetsuHoshuForm targetForm)
        {
            LogUtility.DebugMethodStart(targetForm);

            this.form = targetForm;
            this.dao = DaoInitUtility.GetComponent<IM_CHIIKIBETSU_SHISETSUDao>();
        ...
        public int Search()
        {
            try
            {
                ...
                // 対象データを抽出する
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_SHISETSU_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_SHISETSU_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                // M_CHIIKIBETSU_SHISETSUに存在しない項目での絞り込みはデータ取得後に行う
                ...
                // 対象外も含め、全データを抽出する
                M_CHIIKIBETSU_SHISETSU searchParam = new M_CHIIKIBETSU_SHISETSU();
                searchParam.CHIIKI_CD = this.form.CHIIKI_CD.Text;
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHISETSU_DATA_SQL, searchParam);
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_CHIIKIBETSU_SHISETSU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_CHIIKIBETSU_SHISETSU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_CHIIKIBETSU_SHISETSU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ChiikibetsuShisetsuHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_CHIIKIBETSU_SHISETSU> addList = new List<M_CHIIKIBETSU_SHISETSU>();
                foreach (var CHIIKIEntity in CHIIKIEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuShisetsuHoshuConstans.SHOBUN_HOUHOU_CD) && n.Value.ToString().Equals(CHIIKIEntity.SHOBUN_HOUHOU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuShisetsuHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ChiikibetsuShisetsuHoshuValidator vali = new ChiikibetsuShisetsuHoshuValidator();
        ...
        public void Preview()
        {
            ...
            msgLogic.MessageBoxShow("C011", "地域別施設一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_CHIIKIBETSU_SHISETSU CHIIKIEntity in this.entitys)
                        {
                            dt = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHISETSU_BY_CD_SQL, CHIIKIEntity);
                            if (dt.Rows.Count <= 0)
                            {
        ...
        public override bool Equals(object other)
        {
            ...
            ChiikibetsuShisetsuHoshuLogic localLogic = other as ChiikibetsuShisetsuHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M239", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            M_CHIIKIBETSU_SHISETSU entity = new M_CHIIKIBETSU_SHISETSU();
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
            {
                ...
                {
                    // M_CHIIKIBETSU_SHISETSUに存在しない項目は検索条件に含めない
                    switch (this.form.CONDITION_VALUE.DBFieldsName)
                    {
                        case ("SHOBUN_HOUHOU_NAME_RYAKU"):
                            break;
        ...
        private void UpdateRelationInfo(M_CHIIKIBETSU_SHISETSU entity, bool isDelete)
        {
            if (entity == null
        ...
        public void ReadShobunHouhou(object sender, EventArgs e)
        {
            try
            {
                ...
                // 問い合わせ確認
                if (msgLogic.MessageBoxShow("C066", "報告用施設", "処分方法") != DialogResult.Yes)
                {
                    this.form.CHIIKI_CD.Focus();
                    return;
                }
                ...
                // DataTable構造作成
                this.SearchResult = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHISETSU_STRUCT_SQL, null);
                this.SearchResultAll = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHISETSU_STRUCT_SQL, null);
                ...
                if (shobun != null && shobun.Length > 0)
                {
                    for (int i = 0; i < shobun.Length; i++)
                    {
                        ...
                        row["SHOBUN_HOUHOU_NAME_RYAKU"] = shobun[i].SHOBUN_HOUHOU_NAME_RYAKU;
                        row["HOUKOKU_SHISETSU_CD"] = string.Empty;
                        row["HOUKOKU_SHISETSU_NAME"] = string.Empty;
                        row["CHIIKIBETSU_SHISETSU_BIKOU"] = string.Empty;
