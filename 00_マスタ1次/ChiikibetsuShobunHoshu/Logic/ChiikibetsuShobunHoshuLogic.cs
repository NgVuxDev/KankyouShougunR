// $Id: ChiikibetsuShobunHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using ChiikibetsuShobunHoshu.APP;
using ChiikibetsuShobunHoshu.Validator;
...
namespace ChiikibetsuShobunHoshu.Logic
{
    /// <summary>
    /// 地域別処分保守画面のビジネスロジック
    /// </summary>
    public class ChiikibetsuShobunHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ChiikibetsuShobunHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_CHIIKIBETSU_SHOBUN_DATA_SQL = "ChiikibetsuShobunHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_CHIIKIBETSU_SHOBUN_DATA_SQL = "ChiikibetsuShobunHoshu.Sql.GetChiikibetsuShobunDataSql.sql";

        private readonly string GET_CHIIKIBETSU_SHOBUN_BY_CD_SQL = "ChiikibetsuShobunHoshu.Sql.GetChiikibetsuShobunByCdSql.sql";

        private readonly string GET_CHIIKIBETSU_SHOBUN_STRUCT_SQL = "ChiikibetsuShobunHoshu.Sql.GetIchiranStructSql.sql";

        private readonly string GET_CHIIKIBETSU_SHOBUN_ALL_SQL = "ChiikibetsuShobunHoshu.Sql.GetIchiranAllDataSql.sql";

        /// <summary>
        /// 地域別処分保守画面Form
        /// </summary>
        private ChiikibetsuShobunHoshuForm form;
        ...
        /// 地域別処分マスタエンティティ
        /// </summary>
        private M_CHIIKIBETSU_SHOBUN[] entitys;
        ...
        /// 地域別処分のDao
        /// </summary>
        private IM_CHIIKIBETSU_SHOBUNDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_CHIIKIBETSU_SHOBUN SearchString { get; set; }
        ...
        public ChiikibetsuShobunHoshuLogic(ChiikibetsuShobunHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                // 対象データを抽出する
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_SHOBUN_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_CHIIKIBETSU_SHOBUN_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);

                // M_CHIIKIBETSU_SHOBUNに存在しない項目での絞り込みはデータ取得後に行う
                ...
                // 対象外も含め、全データを抽出する
                M_CHIIKIBETSU_SHOBUN searchParam = new M_CHIIKIBETSU_SHOBUN();
                searchParam.CHIIKI_CD = this.form.CHIIKI_CD.Text;
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHOBUN_DATA_SQL, searchParam);
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_CHIIKIBETSU_SHOBUN[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_CHIIKIBETSU_SHOBUN();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_CHIIKIBETSU_SHOBUN>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ChiikibetsuShobunHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_CHIIKIBETSU_SHOBUN> addList = new List<M_CHIIKIBETSU_SHOBUN>();
                foreach (var CHIIKIEntity in CHIIKIEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuShobunHoshuConstans.SHOBUN_HOUHOU_CD) && n.Value.ToString().Equals(CHIIKIEntity.SHOBUN_HOUHOU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ChiikibetsuShobunHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ChiikibetsuShobunHoshuValidator vali = new ChiikibetsuShobunHoshuValidator();
        ...
        public void Preview()
        {
            ...
            msgLogic.MessageBoxShow("C011", "地域別処分一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_CHIIKIBETSU_SHOBUN CHIIKIEntity in this.entitys)
                        {
                            dt = dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHOBUN_BY_CD_SQL, CHIIKIEntity);
                            if (dt.Rows.Count <= 0)
                            {
        ...
        public override bool Equals(object other)
        {
            ...
            ChiikibetsuShobunHoshuLogic localLogic = other as ChiikibetsuShobunHoshuLogic;
        ...
        internal bool SetIchiran(bool isRead)
        {
            try
            {
                    ...
                    for (int i = 0; i < this.SearchResult.Rows.Count; i++)
                    {
                        ...
                        foreach (DataRow dr in this.SearchResultAll.Rows)
                        {
                            if (SearchResult.Rows[i]["CHIIKI_CD"].ToString() == dr["CHIIKI_CD"].ToString()
                                && SearchResult.Rows[i]["SHOBUN_HOUHOU_CD"].ToString() == dr["SHOBUN_HOUHOU_CD"].ToString())
                            {
                                DataRow row = table.NewRow();
                                row["CHIIKI_CD"] = dr["CHIIKI_CD"];
                                row["SHOBUN_HOUHOU_CD"] = dr["SHOBUN_HOUHOU_CD"];
                                row["SHOBUN_HOUHOU_NAME"] = dr["SHOBUN_HOUHOU_NAME"];
                                row["HOUKOKU_SHOBUN_HOUHOU_CD"] = dr["SHOBUN_HOUHOU_CD"];
                                row["HOUKOKU_SHOBUN_HOUHOU_NAME"] = dr["SHOBUN_HOUHOU_NAME"];
                                row["SHOBUN_MOKUTEKI_CD"] = string.Empty;
                                row["CHIIKIBETSU_SHOBUN_BIKOU"] = dr["CHIIKIBETSU_SHOBUN_BIKOU"];
                        ...
                        if (!isKey)
                        {
                            ...
                            row["SHOBUN_HOUHOU_CD"] = SearchResult.Rows[i]["SHOBUN_HOUHOU_CD"];
                            row["SHOBUN_HOUHOU_NAME"] = SearchResult.Rows[i]["SHOBUN_HOUHOU_NAME"];
                            row["HOUKOKU_SHOBUN_HOUHOU_CD"] = SearchResult.Rows[i]["SHOBUN_HOUHOU_CD"];
                            row["HOUKOKU_SHOBUN_HOUHOU_NAME"] = SearchResult.Rows[i]["SHOBUN_HOUHOU_NAME"];
                            row["SHOBUN_MOKUTEKI_CD"] = string.Empty;
                            row["CHIIKIBETSU_SHOBUN_BIKOU"] = SearchResult.Rows[i]["CHIIKIBETSU_SHOBUN_BIKOU"];
                            row["DELETE_FLG"] = false;
                            table.Rows.Add(row);
                        }
                    }
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M241", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        private void EventInit()
        {
            ...
            //処分方法読込ボタン(process1)イベント生成
            parentForm.bt_process1.Click += new EventHandler(this.ReadShobunHouhou);
        ...
        {
            M_CHIIKIBETSU_SHOBUN entity = new M_CHIIKIBETSU_SHOBUN();
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
            {
                if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.ItemDefinedTypes))
                {
                    if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
                    {
                        if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.ItemDefinedTypes))
                        {
                            // M_CHIIKIBETSU_SHOBUNに存在しない項目は検索条件に含めない
                            switch (this.form.CONDITION_VALUE.DBFieldsName)
                            {
                                case ("SHOBUN_HOUHOU_NAME"):
                                    break;
        ...
        private void UpdateRelationInfo(M_CHIIKIBETSU_SHOBUN entity, bool isDelete)
        {
            if (entity == null
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(proc))
                    {
                        if (proc.Equals("1"))
                        {
                            this.ReadShobunHouhou(sender, e);
                        }
                    }
                }
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void ReadShobunHouhou(object sender, EventArgs e)
        {
            try
            {
                ...
                // 問い合わせ確認
                if (msgLogic.MessageBoxShow("C066", "報告用処分方法", "処分方法") != DialogResult.Yes)
                {
                    this.form.CHIIKI_CD.Focus();
                    return;
                }
                ...
                // DataTable構造作成
                this.SearchResult = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHOBUN_STRUCT_SQL, null);
                this.SearchResultAll = this.dao.GetDataBySqlFile(this.GET_CHIIKIBETSU_SHOBUN_ALL_SQL, null);
                ...
                // 処分方法読込処理
                M_SHOBUN_HOUHOU[] shobun = DaoInitUtility.GetComponent<IM_SHOBUN_HOUHOUDao>().GetAllValidData(new M_SHOBUN_HOUHOU());
                if (shobun != null && shobun.Length > 0)
                {
                    for (int i = 0; i < shobun.Length; i++)
                    {
                        DataRow row = this.SearchResult.NewRow();
                        row["CHIIKI_CD"] = this.form.CHIIKI_CD.Text;
                        row["SHOBUN_HOUHOU_CD"] = shobun[i].SHOBUN_HOUHOU_CD;
                        row["SHOBUN_HOUHOU_NAME"] = shobun[i].SHOBUN_HOUHOU_NAME_RYAKU;
                        row["HOUKOKU_SHOBUN_HOUHOU_CD"] = shobun[i].SHOBUN_HOUHOU_CD;
                        row["HOUKOKU_SHOBUN_HOUHOU_NAME"] = shobun[i].SHOBUN_HOUHOU_NAME;
                        row["SHOBUN_MOKUTEKI_CD"] = string.Empty;
                        row["CHIIKIBETSU_SHOBUN_BIKOU"] = shobun[i].SHOBUN_HOUHOU_BIKOU;
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.SHOBUN_HOUHOU_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["SHOBUN_HOUHOU_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
