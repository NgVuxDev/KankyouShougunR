// $Id: ChiikibetsuShobunHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace ChiikibetsuShobunHoshu.Validator
{
    ...
    /// 地域処分保守検証ロジック
    /// </summary>
    public class ChiikibetsuShobunHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public ChiikibetsuShobunHoshuValidator()
        {
        }
        ...
        public bool CHIIKICDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.ChiikibetsuShobunHoshuConstans.SHOBUN_HOUHOU_CD] as GcCustomAlphaNumTextBoxCell;
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.ChiikibetsuShobunHoshuConstans.SHOBUN_HOUHOU_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowChiikibetsuShobunCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string shobunCd = row.Field<string>(Const.ChiikibetsuShobunHoshuConstans.SHOBUN_HOUHOU_CD);
