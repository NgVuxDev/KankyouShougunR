// $Id: DenManiHaikiNameHoshuForm.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
using System.Windows.Forms;
using DenManiHaikiNameHoshu.Logic;
...
namespace DenManiHaikiNameHoshu.APP
{
    ...
    /// </summary>
    [Implementation]
    public partial class DenManiHaikiNameHoshuForm : SuperForm
    {
        /// <summary>
        /// メニュー権限保守画面ロジック
        /// </summary>
        private DenManiHaikiNameHoshuLogic logic;
        internal MessageBoxShowLogic errmessage = new MessageBoxShowLogic();

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public DenManiHaikiNameHoshuForm()
            : base(WINDOW_ID.M_DENSHI_HAIKI_NAME, WINDOW_TYPE.ICHIRAN_WINDOW_FLAG)
        {
            InitializeComponent();

            // 画面タイプなど引数値は変更となるが基本的にやることは変わらない
            this.logic = new DenManiHaikiNameHoshuLogic(this);
        ...
        public virtual void Search(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(this.EDI_MEMBER_ID.Text))
            {
                messageShowLogic.MessageBoxShow("E001", "加入者番号CD");
                this.EDI_MEMBER_ID.Focus();
            }
            else
            {
        ...
        {
            if (e.CellName.Equals(Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD))
            {
                bool isNoErr = true;
                ...
                if (e.FormattedValue != null && !string.IsNullOrWhiteSpace(e.FormattedValue.ToString())
                    && !this.Ichiran[e.RowIndex, Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD].ReadOnly
                    && !this.logic.beforeHaikiCd.Equals(this.Ichiran[e.RowIndex, Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD].Value))
                {
                    this.Ichiran[e.RowIndex, Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME].Value = this.logic.GetHaikiName(e.FormattedValue.ToString());
                }
            }
        ...
        {
            if (!string.IsNullOrEmpty(this.EDI_MEMBER_ID.Text) && !this.logic.SetJigyoushaName(this.EDI_MEMBER_ID.Text))
            {
        ...
        private void Ichiran_CellEnter(object sender, CellEventArgs e)
        {
            ...
            if (e.CellName.Equals(Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD))
            {
                if (this.Ichiran[e.RowIndex, Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD].Value != null)
                {
                    this.logic.beforeHaikiCd = Convert.ToString(this.Ichiran[e.RowIndex, Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD].Value);
                }
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public virtual void DenManiHaikiNameHoshuForm_UserRegistCheck(object sender, r_framework.Event.RegistCheckEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DenManiHaikiNameHoshuForm_Shown(object sender, EventArgs e)
        {
            // 主キーを非活性にする
            this.logic.EditableToPrimaryKey();
        }
