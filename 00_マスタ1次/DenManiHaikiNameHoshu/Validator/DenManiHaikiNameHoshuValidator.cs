// $Id: DenManiHaikiNameHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace DenManiHaikiNameHoshu.Validator
{
    ...
    /// 電子廃棄物名称保守検証ロジック
    /// </summary>
    public class DenManiHaikiNameHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public DenManiHaikiNameHoshuValidator()
        {
        }
        ...
        public bool HaikiNameCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DenManiHaikiNameCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string haikiNameCd = row.Field<string>(Const.DenManiHaikiNameHoshuConstans.HAIKI_NAME_CD);

                        GcCustomAlphaNumTextBoxCell cell = new GcCustomAlphaNumTextBoxCell();
                        cell.Value = haikiNameCd;
                        list.Add(cell);
                    }
                    cells.AddRange(list);
                }
                ...
                {
                    msgLogic.MessageBoxShow("E022", "入力された電子廃棄物名称CD");
                    result = false;
                }
            }
