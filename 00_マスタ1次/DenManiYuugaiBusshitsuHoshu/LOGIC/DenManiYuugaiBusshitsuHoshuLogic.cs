// $Id: DenManiYuugaiBusshitsuHoshuLogic.cs 15601 2014-02-04 06:08:35Z ogawa@takumi-sys.co.jp $
using System;
...
using DenManiYuugaiBusshitsuHoshu.APP;
using DenManiYuugaiBusshitsuHoshu.Validator;

namespace DenManiYuugaiBusshitsuHoshu.Logic
{
    /// <summary>
    /// 有害物質保守画面のビジネスロジック
    /// </summary>
    public class DenManiYuugaiBusshitsuHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "DenManiYuugaiBusshitsuHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_YUUGAI_BUSSHITSU_DATA_SQL = "DenManiYuugaiBusshitsuHoshu.Sql.GetIchiranDenManiYuugaiBusshitsuDataSql.sql";

        private readonly string GET_YUUGAI_BUSSHITSU_DATA_SQL = "DenManiYuugaiBusshitsuHoshu.Sql.GetDenManiYuugaiBusshitsuDataSql.sql";

        /// <summary>
        /// 有害物質保守画面Form
        /// </summary>
        private DenManiYuugaiBusshitsuHoshuForm form;
        ...
        private M_DENSHI_YUUGAI_BUSSHITSU[] entitys;
        ...
        /// 有害物質のDao
        /// </summary>
        private IM_DENSHI_YUUGAI_BUSSHITSUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_DENSHI_YUUGAI_BUSSHITSU SearchString { get; set; }
        ...
        public DenManiYuugaiBusshitsuHoshuLogic(DenManiYuugaiBusshitsuHoshuForm targetForm)
        {
            LogUtility.DebugMethodStart(targetForm);

            this.form = targetForm;
            this.dao = DaoInitUtility.GetComponent<IM_DENSHI_YUUGAI_BUSSHITSUDao>();
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_YUUGAI_BUSSHITSU_DATA_SQL
                                                                       , this.SearchString
                                                                       , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_YUUGAI_BUSSHITSU_DATA_SQL
                                                                       , this.SearchString
                                                                       , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_YUUGAI_BUSSHITSU_DATA_SQL, new M_DENSHI_YUUGAI_BUSSHITSU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_DENSHI_YUUGAI_BUSSHITSU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_DENSHI_YUUGAI_BUSSHITSU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_DENSHI_YUUGAI_BUSSHITSU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.DenManiYuugaiBusshitsuHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_DENSHI_YUUGAI_BUSSHITSU> addList = new List<M_DENSHI_YUUGAI_BUSSHITSU>();
                foreach (var denybEntity in denybEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.DenManiYuugaiBusshitsuHoshuConstans.YUUGAI_BUSSHITSU_CD) && n.Value.ToString().Equals(denybEntity.YUUGAI_BUSSHITSU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.DenManiYuugaiBusshitsuHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                DenManiYuugaiBusshitsuHoshuValidator vali = new DenManiYuugaiBusshitsuHoshuValidator();
                bool result = vali.YuugaiBusshitsuCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_DENSHI_YUUGAI_BUSSHITSU yuugaiBusshitsuEntity in this.entitys)
                        {
                            M_DENSHI_YUUGAI_BUSSHITSU entity = this.dao.GetDataByCd(yuugaiBusshitsuEntity.YUUGAI_BUSSHITSU_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (yuugaiBusshitsuEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(yuugaiBusshitsuEntity);
                            }
                            else
                            {
                                this.dao.Update(yuugaiBusshitsuEntity);
                            }

                            UpdateRelationInfo(yuugaiBusshitsuEntity, false);
                        }
                        // トランザクション終了
                        tran.Commit();
                    }
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    using (var tran = new Transaction())
                    {
                        foreach (M_DENSHI_YUUGAI_BUSSHITSU yuugaiBusshitsuEntity in this.entitys)
                        {
                            ...
                            {
                                this.dao.Update(yuugaiBusshitsuEntity);
                                UpdateRelationInfo(yuugaiBusshitsuEntity, true);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            DenManiYuugaiBusshitsuHoshuLogic localLogic = other as DenManiYuugaiBusshitsuHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M319", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            M_DENSHI_YUUGAI_BUSSHITSU entity = new M_DENSHI_YUUGAI_BUSSHITSU();
        ...
        private void UpdateRelationInfo(M_DENSHI_YUUGAI_BUSSHITSU entity, bool isDelete)
        {
            if (entity == null
                || string.IsNullOrEmpty(entity.YUUGAI_BUSSHITSU_CD))
            {
                return;
            }
        }
        ...
        /// <summary>
        /// テーブル固定値定義書に存在するデータの場合、削除、名称、適用期間を変更不可にする
        /// </summary>
        ...
        internal void SetFixedRow(Row row)
        {
            ...
            {
                foreach (var columnName in Const.DenManiYuugaiBusshitsuHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            ...
            {
                catchErr = false;
                var objValue = row[Const.DenManiYuugaiBusshitsuHoshuConstans.YUUGAI_BUSSHITSU_CD].Value;

                if (row != null && objValue != null)
                {
                    var strCd = objValue.ToString();
                    if (Const.DenManiYuugaiBusshitsuHoshuConstans.fixedRowList.Contains(strCd))
                    {
                        return true;
                    }
                }
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.YUUGAI_BUSSHITSU_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["YUUGAI_BUSSHITSU_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
