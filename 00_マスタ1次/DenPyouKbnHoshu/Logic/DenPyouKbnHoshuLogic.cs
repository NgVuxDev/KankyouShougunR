// $Id: DenPyouKbnHoshuLogic.cs 18834 2014-04-09 06:51:03Z sc.m.moriya@willwave.jp $
using System;
...
using DenPyouKbnHoshu.APP;
using DenPyouKbnHoshu.Validator;
...
namespace DenPyouKbnHoshu.Logic
{
/// <summary>
/// 伝票区分画面のビジネスロジック
/// </summary>
public class DenPyouKbnHoshuLogic : IBuisinessLogic
{
    #region フィールド

    private readonly string ButtonInfoXmlPath = "DenPyouKbnHoshu.Setting.ButtonSetting.xml";

    private readonly string GET_ICHIRAN_DENPYOU_KBN_DATA_SQL = "DenPyouKbnHoshu.Sql.GetIchiranDataSql.sql";

    private readonly string GET_DENPYOU_KBN_DATA_SQL = "DenPyouKbnHoshu.Sql.GetDenPyouKbnDataSql.sql";

    private readonly string CHECK_DELETE_DENPYOU_KBN_SQL = "DenPyouKbnHoshu.Sql.CheckDeleteDenpyouKbnSql.sql";
    /// <summary>
    ///伝票区分画面Form
    /// </summary>
    private DenPyouKbnHoshuForm form;
    ...
    private M_DENPYOU_KBN[] entitys;

    /// <summary>
    /// 伝票区分のDao
    /// </summary>
    private IM_DENPYOU_KBNDao dao;
    ...
    /// 検索条件
    /// </summary>
    public M_DENPYOU_KBN SearchString { get; set; }
    ...
    public DenPyouKbnHoshuLogic(DenPyouKbnHoshuForm targetForm)
    {
    ...
    public int Search()
    {
        try
        {
            ...
            this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_DENPYOU_KBN_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
            this.SearchResultAll = dao.GetDataBySqlFile(this.GET_DENPYOU_KBN_DATA_SQL, new M_DENPYOU_KBN());
    ...
    public bool CreateEntity(bool isDelete)
    {
        try
        {
            ...
            var entityList = new M_DENPYOU_KBN[this.form.Ichiran.Rows.Count];
            for (int i = 0; i < entityList.Length; i++)
            {
                entityList[i] = new M_DENPYOU_KBN();
            }

            var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_DENPYOU_KBN>(entityList);
            ...
            foreach (DataColumn column in dt.Columns)
            {
                ...
                if (column.ColumnName.Equals(Const.DenPyouKbnHoshuConstans.TIME_STAMP))
                {
                    column.Unique = false;
                }
            }
            ...
            var depyoukbnEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

            List<M_DENPYOU_KBN> addList = new List<M_DENPYOU_KBN>();
            foreach (var depyoukbnEntity in depyoukbnEntityList)
            {
                foreach (Row row in this.form.Ichiran.Rows)
                {
                    if (row.Cells.Any(n => (n.DataField.Equals(Const.DenPyouKbnHoshuConstans.DENPYOU_KBN_CD) && n.Value.ToString().Equals(Convert.ToString(depyoukbnEntity.DENPYOU_KBN_CD)))) &&
                        row.Cells.Any(n => (n.DataField.Equals(Const.DenPyouKbnHoshuConstans.DELETE_FLG) && (n.FormattedValue == null || bool.Parse(n.FormattedValue.ToString()) == isDelete))))
                    {
                        MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), depyoukbnEntity);
                        addList.Add(depyoukbnEntity);
                        break;
                    }
                }
            }
    ...
    public bool DuplicationCheck()
    {
        try
        {
            ...
            DenPyouKbnHoshuValidator vali = new DenPyouKbnHoshuValidator();
            var result = vali.DenPyouKbnCDValidator(this.form.Ichiran, this.SearchResult, this.SearchResultAll, this.isAllSearch);
    ...
    public bool Preview()
    {
        try
        {
            ...
            msgLogic.MessageBoxShow("C011", "伝票区分一覧表");
    ...
    public virtual void Regist(bool errorFlag)
    {
        try
        {
            ...
            if (!errorFlag)
            {
                ...
                {
                    foreach (M_DENPYOU_KBN DenPyouKbnEntity in this.entitys)
                    {
                        M_DENPYOU_KBN m = this.dao.GetDataByCd(Convert.ToString(DenPyouKbnEntity.DENPYOU_KBN_CD));
                        if (m == null)
                        {
                            this.dao.Insert(DenPyouKbnEntity);
                        }
                        else
                        {
                            this.dao.Update(DenPyouKbnEntity);
                        }
                    }
                    // トランザクション終了
                    tran.Commit();
    ...
    public virtual void LogicalDelete()
    {
        try
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                using (var tran = new Transaction())
                {
                    foreach (M_DENPYOU_KBN depyoukbnEntity in this.entitys)
                    {
                        M_DENPYOU_KBN entity = this.dao.GetDataByCd(depyoukbnEntity.DENPYOU_KBN_CD.ToString());
                        if (entity != null)
                        {
                            this.dao.Update(depyoukbnEntity);
                        }
                    }
                    // トランザクション終了
                    tran.Commit();
    ...
    public override bool Equals(object other)
    {
        ...
        DenPyouKbnHoshuLogic localLogic = other as DenPyouKbnHoshuLogic;
    ...
    /// </summary>
    private MasterBaseForm BaseFormInit()
    {
        LogUtility.DebugMethodStart();
    ...
    {
        M_DENPYOU_KBN entity = new M_DENPYOU_KBN();
        ...
        if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
        {
            ...
            {
                if ("System.Data.SqlTypes.SqlInt16".Equals(this.form.CONDITION_VALUE.ItemDefinedTypes) && !string.IsNullOrEmpty(this.form.CONDITION_VALUE.Text))
                {
                    try
                    {
                        Convert.ToInt16(this.form.CONDITION_VALUE.Text);
                        // 検索条件の設定
                ...
                else if ("dateTime".Equals(this.form.CONDITION_VALUE.ItemDefinedTypes) && !string.IsNullOrEmpty(this.form.CONDITION_VALUE.Text))
                {
                    try
                    {
                        Convert.ToDateTime(this.form.CONDITION_VALUE.Text);
                        // 検索条件の設定
                        entity.SetValue(this.form.CONDITION_VALUE);
                    }
                    catch (Exception e)
    ...
    {
        var objValue = row[Const.DenPyouKbnHoshuConstans.DENPYOU_KBN_CD].Value;

        if (this.CheckFixedRow(row))
        {
            foreach (var columnName in Const.DenPyouKbnHoshuConstans.fixedColumnList)
            {
    ...
    internal bool CheckFixedRow(Row row)
    {
        ...
        if (row != null && objValue != null && int.TryParse(objValue.ToString(), out iValue))
        {
            ...
            if (Const.DenPyouKbnHoshuConstans.fixedRowList.Contains(iValue))
            {
                return true;
            }
        }
    ...
    public bool CheckDelete()
    {
        ...
        bool ret = true;
        var DENPYOU_KBN_CD = string.Empty;
        string[] strList;
        ...
        foreach (Row gcRwos in this.form.Ichiran.Rows)
        {
            ...
            {
                DENPYOU_KBN_CD += gcRwos.Cells["DENPYOU_KBN_CD"].Value.ToString() + ",";
            }
        }

        if (!string.IsNullOrEmpty(DENPYOU_KBN_CD))
        {
            strList = DENPYOU_KBN_CD.Split(',');
            DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_DENPYOU_KBN_SQL, strList);
            if (dtTable != null && dtTable.Rows.Count > 0)
            {
                ...
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("E258", "伝票区分", "伝票区分CD", strName);
