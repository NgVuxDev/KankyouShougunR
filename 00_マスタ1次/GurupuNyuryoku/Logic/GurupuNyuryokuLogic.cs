// $Id: GurupuNyuryokuUriageLogic.cs 99999 2025-03-17 09:22:52Z nguyenanhvu $
using System;
...
using GurupuNyuryoku.APP;
using GurupuNyuryoku.Validator;



namespace GurupuNyuryoku.Logic
{
    /// <summary>
    /// グループ保守画面のビジネスロジック
    /// </summary>
    public class GurupuNyuryokuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "GurupuNyuryoku.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_DATA_SQL = "GurupuNyuryoku.Sql.GetIchiranDataSql.sql";

        private readonly string GET_GURUPU_NYURYOKU_DATA_SQL = "GurupuNyuryoku.Sql.GetGurupuNyuryokuDataSql.sql";

        private readonly string CHECK_DELETE_GURUPU_SQL = "GurupuNyuryoku.Sql.CheckDeleteGurupuSql.sql";

        /// <summary>
        /// グループ保守画面Form
        /// </summary>
        private GurupuNyuryokuForm form;
        ...
        private M_GURUPU_NYURYOKU[] entitys;
        ...
        /// グループのDao
        /// </summary>
        private IM_GURUPU_NYURYOKUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_GURUPU_NYURYOKU SearchString { get; set; }
        ...
        public GurupuNyuryokuLogic(GurupuNyuryokuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                //this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_DATA_SQL
                //                , this.SearchString
                //                , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                //this.SearchResultAll = dao.GetDataBySqlFile(this.GET_GURUPU_NYURYOKU_DATA_SQL, new M_GURUPU_NYURYOKU());
        ...
        public DataTable GetSearchResult()
        {
            //checkDensyuCd();
            SetSearchString();
            var searchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_DATA_SQL
                                                        , this.SearchString
                                                        , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked
                                                        );
            return searchResult;
        ...
        public void SetSearchResultAll()
        {
            //checkDensyuCd();
            SetSearchString();
            M_GURUPU_NYURYOKU searchParams = new M_GURUPU_NYURYOKU();
            searchParams.DENPYOU_KBN_CD = this.SearchString.DENPYOU_KBN_CD;
            this.SearchResultAll = dao.GetDataBySqlFile(this.GET_GURUPU_NYURYOKU_DATA_SQL, searchParams);
        }
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_GURUPU_NYURYOKU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_GURUPU_NYURYOKU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_GURUPU_NYURYOKU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.GurupuNyuryokuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var GurupuEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_GURUPU_NYURYOKU> addList = new List<M_GURUPU_NYURYOKU>();
                foreach (var GurupuEntity in GurupuEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        //if (row.Cells.Any(n => (n.DataField.Equals(Const.GurupuNyuryokuConstans.GURUPU_CD) && n.Value.ToString().Equals(Convert.ToString(GurupuEntity.GURUPU_CD)))) &&
                        //    row.Cells.Any(n => (n.DataField.Equals(Const.GurupuNyuryokuConstans.DELETE_FLG) && (n.FormattedValue == null || bool.Parse(n.FormattedValue.ToString()) == isDelete))))
                        //{
                            //20250318
                            if (parentForm.bt_process1.Text == "[1] 支払")
                            {
                                GurupuEntity.DENPYOU_KBN_CD = 1;
                            }
                            else
                            {
                                GurupuEntity.DENPYOU_KBN_CD = 2;
                            }

                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), GurupuEntity);
                            addList.Add(GurupuEntity);
                            break;
                        //}
                    }
                }
        ...
        public bool CheckDelete()
        {
            ...
            foreach (Row gcRwos in this.form.Ichiran.Rows)
            {
                ...
                {
                    GurupuCd += gcRwos.Cells["GURUPU_CD"].Value.ToString() + ",";
                }
            }

            if (!string.IsNullOrEmpty(GurupuCd))
            {
                GurupuCd = GurupuCd.Substring(0, GurupuCd.Length - 1);
                strList = GurupuCd.Split(',');
                DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_GURUPU_SQL, strList);
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                GurupuNyuryokuValidator vali = new GurupuNyuryokuValidator();
                bool result = vali.GurupuCDValidator(this.form.Ichiran, this.SearchResult, this.SearchResultAll, this.isAllSearch);
        ...
        public bool Preview()
        {
            try
            {
                LogUtility.DebugMethodStart();
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("C011", "グループ一覧表");
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M265", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_GURUPU_NYURYOKU GurupuEntity in this.entitys)
                        {
                            M_GURUPU_NYURYOKU entity = this.dao.GetDataByCdAndDencd(GurupuEntity.GURUPU_CD, GurupuEntity.DENPYOU_KBN_CD.Value);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (GurupuEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(GurupuEntity);
                            }
                            else
                            {
                                //20250324
                                GurupuEntity.GURUPU_ID = entity.GURUPU_ID;
                                this.dao.Update(GurupuEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            GurupuNyuryokuLogic localLogic = other as GurupuNyuryokuLogic;
        ...
        private void SetSearchString()
        {
            ...
            M_GURUPU_NYURYOKU entity = new M_GURUPU_NYURYOKU();

            if (titleControl.Text == "グループ入力(売上)")
            {
        ...
        //    // ゼロパディング後、テキストへ設定
        //    e.Value = String.Format("{0:D" + GurupuNyuryokuUriageConstans.CD_MAXLENGTH + "}", int.Parse(e.Value.ToString()));
        //}
        ...
        internal bool SetFixedIchiran()
        {
            try
            {
                // 明細行にフォーカスがあると、固定値をSelectable=falseにする際に
                // フォーカスの位置がどんどんずれていく。
                // 結果、適用終了日にフォーカスインして、初期値のDateTime.Nowが設定されてしまう
                this.form.Ichiran.CurrentCell = null;
        ...
        {
            var objValue = row[Const.GurupuNyuryokuConstans.GURUPU_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.GurupuNyuryokuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null && int.TryParse(objValue.ToString(), out iValue))
                {
                    ...
                    if (Const.GurupuNyuryokuConstans.fixedRowList.Contains(iValue))
                    {
                        return true;
                    }
                }
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.GURUPU_CD).Where(s => !s.Equals(DBNull.Value)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["GURUPU_CD"]).Where(c => !c.Value.Equals(DBNull.Value)).ToList().
                                            ForEach(c =>
                                            {
        ...
        {
            if (this.form.CONDITION_VALUE.DBFieldsName.Equals(Const.GurupuNyuryokuConstans.GURUPU_CD))
            {
        ...
        //    M_GURUPU_NYURYOKU entity = new M_GURUPU_NYURYOKU();
        //    //システム設定より画面Title取得
        //    //if (titleControl.Text == "グループ入力")
        //    //{
        //    //    switch (this.entitySysInfo.GURUPU_NYURYOKU_DEFAULT.Value.ToString())
        //    //    {
        //    //        case "1":
        //    //            titleControl.Text = "グループ入力（売上）";
        //    //            break;
