// $Id: GurupuNyuryokuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
using System;
...
namespace GurupuNyuryoku.Validator
{
    ...
    /// 単位保守検証ロジック
    /// </summary>
    public class GurupuNyuryokuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public GurupuNyuryokuValidator()
        {
        }
        ...
        /// <returns></returns>
        public bool GurupuCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            // 要実装方法検討。現段階は仮実装。

            bool result = true;

            GcCustomNumericTextBox2Cell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.GurupuNyuryokuConstans.GURUPU_CD] as GcCustomNumericTextBox2Cell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.GurupuNyuryokuConstans.GURUPU_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    {
                        Int16 gurupuCd = row.Field<Int16>(Const.GurupuNyuryokuConstans.GURUPU_CD);
