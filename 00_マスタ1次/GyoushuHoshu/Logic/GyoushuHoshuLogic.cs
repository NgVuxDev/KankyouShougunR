// $Id: GyoushuHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
namespace GyoushuHoshu.Logic
{
    /// <summary>
    /// 業種保守画面のビジネスロジック
    /// </summary>
    public class GyoushuHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "GyoushuHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_GYOUSHU_DATA_SQL = "GyoushuHoshu.Sql.GetIchiranGyoushudataSql.sql";

        private readonly string GET_GYOUSHU_DATA_SQL = "GyoushuHoshu.Sql.GetGyoushudataSql.sql";

        private readonly string CHECK_DELETE_GYOUSHU_SQL = "GyoushuHoshu.Sql.CheckDeleteGyoushuSql.sql";

        /// <summary>
        /// 業種保守画面Form
        /// </summary>
        private GyoushuHoshuForm form;
        ...
        /// 検索条件
        /// </summary>
        public M_GYOUSHU SearchString { get; set; }
        ...
        public GyoushuHoshuLogic(GyoushuHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_GYOUSHU_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_GYOUSHU_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_GYOUSHU_DATA_SQL, new M_GYOUSHU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_GYOUSHU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_GYOUSHU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_GYOUSHU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.GyoushuHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var GyoushuEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_GYOUSHU> addList = new List<M_GYOUSHU>();
                foreach (var gyoushuEntity in GyoushuEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.GyoushuHoshuConstans.GYOUSHU_CD) && n.Value.ToString().Equals(gyoushuEntity.GYOUSHU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.GyoushuHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), gyoushuEntity);
                            addList.Add(gyoushuEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                GyoushuHoshuValidator vali = new GyoushuHoshuValidator();
                bool result = vali.GyoushuCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
            ...
            try
            {
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    if (gcRwos.Cells["DELETE_FLG"].Value != null && gcRwos.Cells["DELETE_FLG"].Value.ToString() == "True")
                    {
                        ...
                        gyoushuCd += gcRwos.Cells["GYOUSHU_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(gyoushuCd))
                {
                    gyoushuCd = gyoushuCd.Substring(0, gyoushuCd.Length - 1);
                    strList = gyoushuCd.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(CHECK_DELETE_GYOUSHU_SQL, strList);
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        msgLogic.MessageBoxShow("E258", "業種", "業種CD", strName);
        ...
        //    MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
        //    msgLogic.MessageBoxShow("C011", "業種一覧表");
        ...
        public bool CancelCondition()
        {
            try
            {
                ...
                ClearCondition();
                bool catchEr = SetIchiran();

                LogUtility.DebugMethodEnd(catchEr);
                return catchEr;
            }
            catch (Exception ex)
            {
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_GYOUSHU gyoushuEntity in this.entitys)
                        {
                            M_GYOUSHU entity = this.dao.GetDataByCd(gyoushuEntity.GYOUSHU_CD);
                            if (entity == null)
                            {
                                //削除チェックが付けられている場合は、新規登録を行わない
                                if (gyoushuEntity.DELETE_FLG)
                                {
                                    continue;
                                }

                                this.dao.Insert(gyoushuEntity);
                            }
                            else
                            {
                                this.dao.Update(gyoushuEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            GyoushuHoshuLogic localLogic = other as GyoushuHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M233", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            M_GYOUSHU entity = new M_GYOUSHU();
