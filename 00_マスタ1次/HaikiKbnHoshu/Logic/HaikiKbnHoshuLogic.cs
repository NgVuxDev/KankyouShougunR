// $Id: HaikiKbnHoshuLogic.cs 18759 2014-04-08 09:18:10Z sc.h.hashimoto $
using System;
...
using HaikiKbnHoshu.APP;
using HaikiKbnHoshu.Const;
using HaikiKbnHoshu.Validator;
...
namespace HaikiKbnHoshu.Logic
{
    /// <summary>
    /// 廃棄物区分保守画面のビジネスロジック
    /// </summary>
    public class HaikiKbnHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "HaikiKbnHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_DATA_SQL = "HaikiKbnHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_HAYIKEBUTU_DATA_SQL = "HaikiKbnHoshu.Sql.GetHaikiKbnDataSql.sql";

        private readonly string CHECK_DELETE_HANKI_KBN_SQL = "HaikiKbnHoshu.Sql.CheckDeleteHankiKbnSql.sql";

        /// <summary>
        /// 廃棄物区分保守画面Form
        /// </summary>
        private HaikiKbnHoshuForm form;
        ...
        /// 検索条件
        /// </summary>
        public M_HAIKI_KBN SearchString { get; set; }
        ...
        public HaikiKbnHoshuLogic(HaikiKbnHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_HAYIKEBUTU_DATA_SQL, new M_HAIKI_KBN());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_HAIKI_KBN[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_HAIKI_KBN();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_HAIKI_KBN>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.HaikiKbnHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var haikikbnEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_HAIKI_KBN> addList = new List<M_HAIKI_KBN>();
                foreach (var haikikbnEntity in haikikbnEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.HaikiKbnHoshuConstans.HAIKI_KBN_CD) && n.Value.ToString().Equals(Convert.ToString(haikikbnEntity.HAIKI_KBN_CD)))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.HaikiKbnHoshuConstans.DELETE_FLG) && (n.FormattedValue == null || bool.Parse(n.FormattedValue.ToString()) == isDelete))))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), haikikbnEntity);
                            addList.Add(haikikbnEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                HaikiKbnHoshuValidator vali = new HaikiKbnHoshuValidator();
                bool result = vali.HaikiKbnCDValidator(this.form.Ichiran, this.SearchResult, this.SearchResultAll, this.isAllSearch);
        ...
        public bool CheckDelete()
        {
            try
            {
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    ...
                    {
                        HaikiKbnCd += gcRwos.Cells["HAIKI_KBN_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(HaikiKbnCd))
                {
                    strList = HaikiKbnCd.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_HANKI_KBN_SQL, strList);
        ...
        public bool Preview()
        {
            try
            {
                LogUtility.DebugMethodStart();
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("C011", "廃棄物区分一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_HAIKI_KBN haikiKbnEntity in this.entitys)
                        {
                            M_HAIKI_KBN entity = this.dao.GetDataByCd(haikiKbnEntity.HAIKI_KBN_CD.ToString());
                            if (entity == null)
                            {
                                this.dao.Insert(haikiKbnEntity);
                            }
                            else
                            {
                                this.dao.Update(haikiKbnEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            HaikiKbnHoshuLogic localLogic = other as HaikiKbnHoshuLogic;
        ...
        {
            M_HAIKI_KBN entity = new M_HAIKI_KBN();
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
            {
                if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.ItemDefinedTypes))
                {
                    if ("System.Data.SqlTypes.SqlInt16".Equals(this.form.CONDITION_VALUE.ItemDefinedTypes) && !string.IsNullOrEmpty(this.form.CONDITION_VALUE.Text))
                    {
                        try
                        {
                            entity.HAIKI_KBN_CD = Int16.Parse(this.form.CONDITION_VALUE.Text);
                        }
                        catch (Exception e)
                        {
                            return ;
        ...
        //    // ゼロパディング後、テキストへ設定
        //    e.Value = String.Format("{0:D" + HaikiKbnHoshuConstans.CD_MAXLENGTH + "}", int.Parse(e.Value.ToString()));
        //}
        ...
        {
            var objValue = row[Const.HaikiKbnHoshuConstans.HAIKI_KBN_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.HaikiKbnHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row,out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null && int.TryParse(objValue.ToString(), out iValue))
                {
                    ...
                    if (Const.HaikiKbnHoshuConstans.fixedRowList.Contains(iValue))
                    {
                        return true;
                    }
                }
