// $Id: HaikiNameHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace HaikiNameHoshu.Validator
{
    ...
    /// 廃棄物名称保守検証ロジック
    /// </summary>
    public class HaikiNameHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public HaikiNameHoshuValidator()
        {
        }
        ...
        public bool HaikiNameCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.HaikiNameHoshuConstans.HAIKI_NAME_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.HaikiNameHoshuConstans.HAIKI_NAME_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowHaikiNameCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string haikiNameCd = row.Field<string>(Const.HaikiNameHoshuConstans.HAIKI_NAME_CD);
                ...
                {
                    msgLogic.MessageBoxShow("E022", "入力された廃棄物名称CD");
                    result = false;
                }
            }
