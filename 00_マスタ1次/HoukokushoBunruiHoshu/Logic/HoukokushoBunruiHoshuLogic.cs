// $Id: HoukokushoBunruiHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using HoukokushoBunruiHoshu.APP;
using HoukokushoBunruiHoshu.Validator;
...
namespace HoukokushoBunruiHoshu.Logic
{
    /// <summary>
    /// 報告書分類保守画面のビジネスロジック
    /// </summary>
    public class HoukokushoBunruiHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "HoukokushoBunruiHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_HOUKOKUSHO_BUNRUI_DATA_SQL = "HoukokushoBunruiHoshu.Sql.GetHoukokushoBunruidataSql.sql";

        private readonly string GET_ICHIRAN_HOUKOKUSHO_BUNRUI_DATA_SQL = "HoukokushoBunruiHoshu.Sql.GetIchiranHoukokushoBunruiDataSql.sql";

        private readonly string CHECK_DELETE_HOUKOKUSHO_BUNRUI_SQL = "HoukokushoBunruiHoshu.Sql.CheckDeleteHoukokushoBunruiSql.sql";

        /// <summary>
        /// 報告書分類保守画面Form
        /// </summary>
        private HoukokushoBunruiHoshuForm form;
        ...
        private M_HOUKOKUSHO_BUNRUI[] entitys;
        ...
        /// 検索条件
        /// </summary>
        public M_HOUKOKUSHO_BUNRUI SearchString { get; set; }
        ...
        public HoukokushoBunruiHoshuLogic(HoukokushoBunruiHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_HOUKOKUSHO_BUNRUI_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_HOUKOKUSHO_BUNRUI_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_HOUKOKUSHO_BUNRUI_DATA_SQL, new M_HOUKOKUSHO_BUNRUI());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_HOUKOKUSHO_BUNRUI[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_HOUKOKUSHO_BUNRUI();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_HOUKOKUSHO_BUNRUI>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.HoukokushoBunruiHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var houkokushoBunruiEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_HOUKOKUSHO_BUNRUI> addList = new List<M_HOUKOKUSHO_BUNRUI>();
                foreach (var houkokushoBunruiEntity in houkokushoBunruiEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.HoukokushoBunruiHoshuConstans.HOUKOKUSHO_BUNRUI_CD) && n.Value.ToString().Equals(houkokushoBunruiEntity.HOUKOKUSHO_BUNRUI_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.HoukokushoBunruiHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), houkokushoBunruiEntity);
                            addList.Add(houkokushoBunruiEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                HoukokushoBunruiHoshuValidator vali = new HoukokushoBunruiHoshuValidator();
                bool result = vali.HoukokushoBunruiCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public bool CheckDelete()
        {
            try
            {
                ...
                bool ret = true;
                var houkokushoBunruiCd = string.Empty;
                string[] strList;
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    ...
                    {
                        houkokushoBunruiCd += gcRwos.Cells["HOUKOKUSHO_BUNRUI_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(houkokushoBunruiCd))
                {
                    houkokushoBunruiCd = houkokushoBunruiCd.Substring(0, houkokushoBunruiCd.Length - 1);
                    strList = houkokushoBunruiCd.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_HOUKOKUSHO_BUNRUI_SQL, strList);
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        msgLogic.MessageBoxShow("E258", "報告書分類", "報告書分類CD", strName);
        ...
        public bool Preview()
        {
            try
            {
                ...
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("C011", "報告書分類一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_HOUKOKUSHO_BUNRUI houkokushoEntity in this.entitys)
                        {
                            M_HOUKOKUSHO_BUNRUI entity = this.dao.GetDataByCd(houkokushoEntity.HOUKOKUSHO_BUNRUI_CD);
                            if (entity == null)
                            {
                                //削除チェックが付けられている場合は、新規登録を行わない
                                if (houkokushoEntity.DELETE_FLG)
                                {
                                    continue;
                                }

                                this.dao.Insert(houkokushoEntity);
                            }
                            else
                            {
                                this.dao.Update(houkokushoEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            HoukokushoBunruiHoshuLogic localLogic = other as HoukokushoBunruiHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M246", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            var objValue = row[Const.HoukokushoBunruiHoshuConstans.HOUKOKUSHO_BUNRUI_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.HoukokushoBunruiHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null)
                {
                    ...
                    if (Const.HoukokushoBunruiHoshuConstans.fixedRowList.Contains(strCd))
                    {
                        return true;
                    }
                }
