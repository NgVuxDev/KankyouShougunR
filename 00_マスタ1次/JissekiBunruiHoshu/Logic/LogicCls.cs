// $Id: LogicCls.cs 50440 2015-05-22 10:02:49Z gai $
using System;
...
using JissekiBunruiHoshu.APP;
using JissekiBunruiHoshu.Validator;
...
namespace JissekiBunruiHoshu.Logic
{
    /// <summary>
    /// 実績分類入力画面のビジネスロジック
    /// </summary>
    public class LogicCls : IBuisinessLogic
    {
        ...
        private readonly string CHECK_DELETE_JISSEKI_BUNRUI_SQL = "JissekiBunruiHoshu.Sql.CheckDeleteJissekiBunruiSql.sql";

        /// <summary>
        /// ボタン情報XMLパス
        /// </summary>
        private readonly string ButtonInfoXmlPath = "JissekiBunruiHoshu.Setting.ButtonSetting.xml";
        ...
        /// 実績分類エンティティ配列
        /// </summary>
        private M_JISSEKI_BUNRUI[] entitys;
        ...
        /// 実績分類のDao
        /// </summary>
        private IM_JISSEKI_BUNRUIDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_JISSEKI_BUNRUI SearchString { get; set; }
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetJissekiBunruiData(new M_JISSEKI_BUNRUI());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_JISSEKI_BUNRUI[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_JISSEKI_BUNRUI();
                }

                var dataBinderLogic = new DataBinderLogic<M_JISSEKI_BUNRUI>(entityList);
                ...
                var jissekiBunruiEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                var addList = new List<M_JISSEKI_BUNRUI>();
                foreach (var jissekiBunruiEntity in jissekiBunruiEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ConstCls.JISSEKI_BUNRUI_CD) && n.Value.ToString().Equals(jissekiBunruiEntity.JISSEKI_BUNRUI_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ConstCls.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), jissekiBunruiEntity);
                            addList.Add(jissekiBunruiEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                JissekiBunruiValidator vali = new JissekiBunruiValidator();
                bool result = vali.BumonCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public bool CheckDelete()
        {
            try
            {
                ...
                bool ret = true;
                var jissekiBunruiCd = string.Empty;
                string[] strList;
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    if (gcRwos.Cells["DELETE_FLG"].Value != null && gcRwos.Cells["DELETE_FLG"].Value.ToString() == "True")
                    {
                        ...
                        jissekiBunruiCd += gcRwos.Cells["JISSEKI_BUNRUI_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(jissekiBunruiCd))
                {
                    jissekiBunruiCd = jissekiBunruiCd.Substring(0, jissekiBunruiCd.Length - 1);
                    strList = jissekiBunruiCd.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_JISSEKI_BUNRUI_SQL, strList);
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        msgLogic.MessageBoxShow("E258", "実績分類", "実績分類CD", strName);
        ...
        public bool Preview()
        {
            try
            {
                ...
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("C011", "実績分類一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_JISSEKI_BUNRUI jissekiBunruiEntity in this.entitys)
                        {
                            M_JISSEKI_BUNRUI entity = this.dao.GetDataByCd(jissekiBunruiEntity.JISSEKI_BUNRUI_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (jissekiBunruiEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(jissekiBunruiEntity);
                            }
                            else
                            {
                                this.dao.Update(jissekiBunruiEntity);
                            }
                        }
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                if (r_framework.Authority.Manager.CheckAuthority("M655", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            M_JISSEKI_BUNRUI entity = new M_JISSEKI_BUNRUI();
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.JISSEKI_BUNRUI_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["JISSEKI_BUNRUI_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
