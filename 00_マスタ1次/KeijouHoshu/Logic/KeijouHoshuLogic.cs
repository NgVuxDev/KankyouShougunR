// $Id: KeijouHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using KeijouHoshu.APP;
using KeijouHoshu.Validator;
...
namespace KeijouHoshu.Logic
{
    /// <summary>
    /// 形状保守画面のビジネスロジック
    /// </summary>
    public class KeijouHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "KeijouHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_KEIJOU_DATA_SQL = "KeijouHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_KEIJOU_DATA_SQL = "KeijouHoshu.Sql.GetKeijouDataSql.sql";

        /// <summary>
        /// 形状保守画面Form
        /// </summary>
        private KeijouHoshuForm form;
        ...
        private M_KEIJOU[] entitys;

        /// <summary>
        /// 形状のDao
        /// </summary>
        private IM_KEIJOUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_KEIJOU SearchString { get; set; }
        ...
        public KeijouHoshuLogic(KeijouHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                SetSearchString();
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_KEIJOU_DATA_SQL
                                                                        , this.SearchString
                                                                        , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_KEIJOU_DATA_SQL
                                                        , this.SearchString
                                                        , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_KEIJOU_DATA_SQL, new M_KEIJOU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_KEIJOU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_KEIJOU();
                }
                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_KEIJOU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.KeijouHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var keijouEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);
                List<M_KEIJOU> addList = new List<M_KEIJOU>();
                foreach (var keijouEntity in keijouEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.KeijouHoshuConstans.KEIJOU_CD) && n.Value.ToString().Equals(keijouEntity.KEIJOU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.KeijouHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), keijouEntity);
                            addList.Add(keijouEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                KeijouHoshuValidator vali = new KeijouHoshuValidator();
                bool result = vali.KeijouCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        //    msgLogic.MessageBoxShow("C011", "形状一覧表");
        //    MessageBox.Show("未実装");
        //}
        ...
        public bool CancelCondition()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M258", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_KEIJOU keijouEntity in this.entitys)
                        {
                            M_KEIJOU entity = this.dao.GetDataByCd(keijouEntity.KEIJOU_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (keijouEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(keijouEntity);
                            }
                            else
                            {
                                this.dao.Update(keijouEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            KeijouHoshuLogic localLogic = other as KeijouHoshuLogic;
        ...
        {
            var objValue = row[Const.KeijouHoshuConstans.KEIJOU_CD].Value;

            if (this.CheckFixedRow(row))
            {
                foreach (var columnName in Const.KeijouHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row)
        {
            ...
            if (row != null && objValue != null)
            {
                ...
                if (Const.KeijouHoshuConstans.fixedRowList.Contains(strCd))
                {
                    return true;
                }
            }
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.KEIJOU_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["KEIJOU_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
