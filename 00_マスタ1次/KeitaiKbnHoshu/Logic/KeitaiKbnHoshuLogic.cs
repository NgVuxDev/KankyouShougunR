// $Id: KeitaiKbnHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using KeitaiKbnHoshu.APP;
using KeitaiKbnHoshu.Validator;
...
namespace KeitaiKbnHoshu.Logic
{
    /// <summary>
    /// 形態区分保守画面のビジネスロジック
    /// </summary>
    public class KeitaiKbnHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "KeitaiKbnHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_KEITAI_KBN_DATA_SQL = "KeitaiKbnHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_KEITAI_KBN_DATA_SQL = "KeitaiKbnHoshu.Sql.GetKeitaiKbnDataSql.sql";

        /// <summary>
        /// 形態区分保守画面Form
        /// </summary>
        private KeitaiKbnHoshuForm form;
        ...
        /// 検索条件
        /// </summary>
        public M_KEITAI_KBN SearchString { get; set; }
        ...
        public KeitaiKbnHoshuLogic(KeitaiKbnHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_KEITAI_KBN_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_KEITAI_KBN_DATA_SQL
                                            , this.SearchString
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_KEITAI_KBN_DATA_SQL, new M_KEITAI_KBN());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_KEITAI_KBN[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_KEITAI_KBN();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_KEITAI_KBN>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.KeitaiKbnHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var keitaikbnEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_KEITAI_KBN> addList = new List<M_KEITAI_KBN>();
                foreach (var keitaikbnEntity in keitaikbnEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.KeitaiKbnHoshuConstans.KEITAI_KBN_CD) && n.Value.ToString().Equals(Convert.ToString(keitaikbnEntity.KEITAI_KBN_CD)))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.KeitaiKbnHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), keitaikbnEntity);
                            addList.Add(keitaikbnEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                KeitaiKbnHoshuValidator vali = new KeitaiKbnHoshuValidator();
                bool result = vali.keitaikbnCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public bool Preview()
        {
            try
            {
                ...
                msgLogic.MessageBoxShow("C011", "形態区分一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_KEITAI_KBN keitaikbnEntity in this.entitys)
                        {
                            M_KEITAI_KBN entity = this.dao.GetDataByCd(keitaikbnEntity.KEITAI_KBN_CD.ToString());
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (keitaikbnEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(keitaikbnEntity);
                            }
                            else
                            {
                                this.dao.Update(keitaikbnEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            KeitaiKbnHoshuLogic localLogic = other as KeitaiKbnHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M249", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            M_KEITAI_KBN entity = new M_KEITAI_KBN();
        ...
        //    // ゼロパディング後、テキストへ設定
        //    e.Value = String.Format("{0:D" + KeitaiKbnHoshuConstans.CD_MAXLENGTH + "}", int.Parse(e.Value.ToString()));
        //}
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.KEITAI_KBN_CD).Where(s => !s.Value.Equals(DBNull.Value)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["KEITAI_KBN_CD"]).Where(c => !c.Value.Equals(DBNull.Value)).ToList().
                                            ForEach(c =>
                                            {
        ...
        {
            if (this.form.CONDITION_VALUE.DBFieldsName.Equals(Const.KeitaiKbnHoshuConstans.KEITAI_KBN_CD)
                || this.form.CONDITION_VALUE.DBFieldsName.Equals(Const.KeitaiKbnHoshuConstans.DENSHU_KBN_CD))
            {
