// $Id: KihonHinmeiTankaHoshuValidator.cs 31332 2014-10-01 07:56:54Z y-hosokawa@takumi-sys.co.jp $
using System;
...
namespace KihonHinmeiTankaHoshu.Validator
{
    ...
    /// 基本品名単価保守検証ロジック
    /// </summary>
    public class KihonHinmeiTankaHoshuValidator
    {
        /// <summary>
        /// 最大日付（適用終了日が未入力時に使用）
        /// </summary>
        ...
        /// コンストラクタ
        /// </summary>
        public KihonHinmeiTankaHoshuValidator()
        {
        }
        ...
        /// <returns></returns>
        public bool KihonHinmeitanCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            // 要実装方法検討。現段階は仮実装。
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    {
                        object denshukbncd = row[Const.KihonHinmeiTankaHoshuConstans.DENSHU_KBN_CD];
                        object hinmeicd = row[Const.KihonHinmeiTankaHoshuConstans.HINMEI_CD];
                        object unitcd = row[Const.KihonHinmeiTankaHoshuConstans.UNIT_CD];
                        object unpancd = row[Const.KihonHinmeiTankaHoshuConstans.UNPAN_GYOUSHA_CD];
                        object nioroshigyoucd = row[Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GYOUSHA_CD];
                        object nioroshigencd = row[Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GENBA_CD];
                        object tekiyoubegin = row[Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN];
                        object tekiyouend = row[Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END];

                        // 修正対象が本当に修正されているかチェックする
                        if (row[Const.KihonHinmeiTankaHoshuConstans.SYS_ID] != null && !row[Const.KihonHinmeiTankaHoshuConstans.SYS_ID].ToString().Equals(string.Empty))
                        {
                            DataRow[] dr = dtAll.Select(String.Format("SYS_ID = '{0}'", row[Const.KihonHinmeiTankaHoshuConstans.SYS_ID].ToString()));
                            if (dr.Length > 0
                                && ((bool)dr[0][Const.KihonHinmeiTankaHoshuConstans.DELETE_FLG]).Equals(((bool)row[Const.KihonHinmeiTankaHoshuConstans.DELETE_FLG]))
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.DENSHU_KBN_CD].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.DENSHU_KBN_CD].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.HINMEI_CD].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.HINMEI_CD].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.UNIT_CD].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.UNIT_CD].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.UNPAN_GYOUSHA_CD].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.UNPAN_GYOUSHA_CD].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GYOUSHA_CD].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GYOUSHA_CD].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GENBA_CD].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GENBA_CD].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.TANKA].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.TANKA].ToString())
                                && dr[0][Const.KihonHinmeiTankaHoshuConstans.BIKOU].ToString().Equals(row[Const.KihonHinmeiTankaHoshuConstans.BIKOU].ToString())
                                && (dr[0][Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN] == null ? string.Empty : dr[0][Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN].ToString()) == (row[Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN] == null ? string.Empty : row[Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN].ToString())
                                && (dr[0][Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END] == null ? string.Empty : dr[0][Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END].ToString()) == (row[Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END] == null ? string.Empty : row[Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END].ToString()))
                            {
                                row.RejectChanges();
                            }
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowKihonHinmeiTankaCompare());
                    foreach (DataRow row in rows)
                    {
                        isChange.Add(false);

                        //品名CD
                        listHim.Add(row.Field<string>(Const.KihonHinmeiTankaHoshuConstans.HINMEI_CD));

                        //伝種区分CD
                        if (row.Field<Object>(Const.KihonHinmeiTankaHoshuConstans.DENSHU_KBN_CD) == null)
                        {
                            listDenshu.Add(null);
                        }
                        else
                        {
                            listDenshu.Add(row.Field<Int16>(Const.KihonHinmeiTankaHoshuConstans.DENSHU_KBN_CD));
                        }

                        //単位CD
                        if (row.Field<Object>(Const.KihonHinmeiTankaHoshuConstans.UNIT_CD) == null)
                        {
                            listUnit.Add(null);
                        }
                        else
                        {
                            listUnit.Add(row.Field<Int16>(Const.KihonHinmeiTankaHoshuConstans.UNIT_CD));
                        }

                        //運搬業者CD
                        listUnpan.Add(row.Field<string>(Const.KihonHinmeiTankaHoshuConstans.UNPAN_GYOUSHA_CD));

                        //荷降先業者CD
                        listNGyousha.Add(row.Field<string>(Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GYOUSHA_CD));

                        //荷降先現場CD
                        listNGenba.Add(row.Field<string>(Const.KihonHinmeiTankaHoshuConstans.NIOROSHI_GENBA_CD));

                        //適用開始日
                        if (row.Field<Object>(Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN) == null)
                        {
                            listBegin.Add(null);
                        }
                        else
                        {
                            listBegin.Add(row.Field<DateTime>(Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_BEGIN));
                        }

                        //適用終了日
                        if (row.Field<Object>(Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END) == null)
                        {
                            listEnd.Add(null);
                        }
                        else
                        {
                            listEnd.Add(row.Field<DateTime>(Const.KihonHinmeiTankaHoshuConstans.TEKIYOU_END));
                        }
                ...
                for (int i = 0; i < listHim.Count; i++)
                {
                    ...
                    for (int j = 0; j < listHim.Count; j++)
                    {
                        ...
                        if (allDataTable.Rows[i][Const.KihonHinmeiTankaHoshuConstans.DELETE_FLG].Equals(true)
                            && allDataTable.Rows[i][Const.KihonHinmeiTankaHoshuConstans.SYS_ID].ToString().Equals("")
                            || (allDataTable.Rows[j][Const.KihonHinmeiTankaHoshuConstans.DELETE_FLG].Equals(true)
                            && allDataTable.Rows[j][Const.KihonHinmeiTankaHoshuConstans.SYS_ID].ToString().Equals("")))
                        {
                            continue;
                        }
