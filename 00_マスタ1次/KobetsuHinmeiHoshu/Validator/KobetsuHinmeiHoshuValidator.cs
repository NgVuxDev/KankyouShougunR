// $Id: KobetsuHinmeiHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace KobetsuHinmeiHoshu.Validator
{
    ...
    /// 分類保守検証ロジック
    /// </summary>
    public class KobetsuHinmeiHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public KobetsuHinmeiHoshuValidator()
        {
        }
        ...
        public bool HinmeiCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.KobetsuHinmeiHoshuConstans.HINMEI_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.KobetsuHinmeiHoshuConstans.HINMEI_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowKobetsuHinmeiCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string bunruiCd = row.Field<string>(Const.KobetsuHinmeiHoshuConstans.HINMEI_CD);
                ...
                {
                    msgLogic.MessageBoxShow("E003", "品名CD", control.Value.ToString());
                    result = false;
                }
            }
            return result;
