// $Id: ManiFestShuRuiHoshuLogic.cs 18911 2014-04-10 04:23:30Z sc.m.moriya@willwave.jp $
using System;
...
using ManiFestShuRuiHoshu.APP;
using ManiFestShuRuiHoshu.Const;
using ManiFestShuRuiHoshu.Validator;
...
namespace ManiFestShuRuiHoshu.Logic
{
    /// <summary>
    ///マニフェスト種類画面のビジネスロジック
    /// </summary>
    public class ManiFestShuRuiHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ManiFestShuRuiHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_MANIFEST_SHURUI_DATA_SQL = "ManiFestShuRuiHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_MANIFEST_SHURUI_DATA_SQL = "ManiFestShuRuiHoshu.Sql.GetManiFestShuRuiDataSql.sql";

        private readonly string CHECK_DELETE_MANIFESUTO_SURUI_SQL = "ManiFestShuRuiHoshu.Sql.CheckDeleteManiShuruiSql.sql";

        /// <summary>
        ///マニフェスト種類画面Form
        /// </summary>
        private ManiFestShuRuiHoshuForm form;
        ...
        private M_MANIFEST_SHURUI[] entitys;

        /// <summary>
        /// マニフェスト種類のDao
        /// </summary>
        private IM_MANIFEST_SHURUIDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_MANIFEST_SHURUI SearchString { get; set; }
        ...
        public ManiFestShuRuiHoshuLogic(ManiFestShuRuiHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_MANIFEST_SHURUI_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_MANIFEST_SHURUI_DATA_SQL, new M_MANIFEST_SHURUI());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_MANIFEST_SHURUI[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_MANIFEST_SHURUI();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_MANIFEST_SHURUI>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ManiFestShuRuiHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var manifestshuruiEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_MANIFEST_SHURUI> addList = new List<M_MANIFEST_SHURUI>();
                foreach (var manifestshuruiEntity in manifestshuruiEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ManiFestShuRuiHoshuConstans.MANIFEST_SHURUI_CD) && n.Value.ToString().Equals(Convert.ToString(manifestshuruiEntity.MANIFEST_SHURUI_CD)))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ManiFestShuRuiHoshuConstans.DELETE_FLG) && (n.FormattedValue == null || bool.Parse(n.FormattedValue.ToString()) == isDelete))))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), manifestshuruiEntity);
                            addList.Add(manifestshuruiEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ManiFestShuRuiHoshuValidator vali = new ManiFestShuRuiHoshuValidator();
                bool result = vali.manifestshuruiCDValidator(this.form.Ichiran, this.SearchResult, this.SearchResultAll, this.isAllSearch);
        ...
        //    msgLogic.MessageBoxShow("C011", "マニフェスト種類一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_MANIFEST_SHURUI manifestshuruiEntity in this.entitys)
                        {
                            M_MANIFEST_SHURUI entity = this.dao.GetDataByCd(manifestshuruiEntity.MANIFEST_SHURUI_CD.ToString());
                            if (entity == null)
                            {
                                this.dao.Insert(manifestshuruiEntity);
                            }
                            else
                            {
                                this.dao.Update(manifestshuruiEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        /// <returns></returns>
        public bool Equals(ManiFestShuRuiHoshuLogic other)
        {
        ...
        {
            M_MANIFEST_SHURUI entity = new M_MANIFEST_SHURUI();
        ...
        //    // ゼロパディング後、テキストへ設定
        //    e.Value = String.Format("{0:D" + ManiFestShuRuiHoshuConstans.CD_MAXLENGTH + "}", int.Parse(e.Value.ToString()));
        //}
        ...
        {
            var objValue = row[Const.ManiFestShuRuiHoshuConstans.MANIFEST_SHURUI_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.ManiFestShuRuiHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row,out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null && int.TryParse(objValue.ToString(), out iValue))
                {
                    ...
                    if (Const.ManiFestShuRuiHoshuConstans.fixedRowList.Contains(iValue))
                    {
                        return true;
                    }
                }
        ...
        public bool CheckDelete()
        {
            try
            {
                ...
                bool ret = true;
                var MANIFEST_SHURUI_CD = string.Empty;
                string[] strList;
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    ...
                    {
                        MANIFEST_SHURUI_CD += gcRwos.Cells["MANIFEST_SHURUI_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(MANIFEST_SHURUI_CD))
                {
                    strList = MANIFEST_SHURUI_CD.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_MANIFESUTO_SURUI_SQL, strList);
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                        msgLogic.MessageBoxShow("E258", "マニフェスト種類", "マニフェスト種類CD", strName);
