// $Id: NisugataHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using NisugataHoshu.APP;
using NisugataHoshu.Validator;
...
namespace NisugataHoshu.Logic
{
    /// <summary>
    /// 荷姿保守画面のビジネスロジック
    /// </summary>
    public class NisugataHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "NisugataHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_NISUGATA_DATA_SQL = "NisugataHoshu.Sql.GetNisugataDataSql.sql";

        private readonly string GET_ICHIRAN_NISUGATA_DATA_SQL = "NisugataHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string CHECK_DELETE_NISUGATA_SQL = "NisugataHoshu.Sql.CheckDeleteNisugataSql.sql";

        /// <summary>
        /// 荷姿保守画面Form
        /// </summary>
        private NisugataHoshuForm form;
        ...
        /// 検索条件
        /// </summary>
        public M_NISUGATA SearchString { get; set; }
        ...
        public NisugataHoshuLogic(NisugataHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_NISUGATA_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_NISUGATA_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_NISUGATA_DATA_SQL, new M_NISUGATA());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_NISUGATA[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_NISUGATA();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_NISUGATA>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.NisugataHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var nisugataEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_NISUGATA> addList = new List<M_NISUGATA>();
                foreach (var nisugataEntity in nisugataEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.NisugataHoshuConstans.NISUGATA_CD) && n.Value.ToString().Equals(nisugataEntity.NISUGATA_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.NisugataHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), nisugataEntity);
                            addList.Add(nisugataEntity);
                            break;
                        }
                    }
                }
            ...
            try
            {
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    if (gcRwos.Cells["DELETE_FLG"].Value != null && gcRwos.Cells["DELETE_FLG"].Value.ToString() == "True")
                    {
                        ...
                        nisugataCd += gcRwos.Cells["NISUGATA_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(nisugataCd))
                {
                    nisugataCd = nisugataCd.Substring(0, nisugataCd.Length - 1);
                    strList = nisugataCd.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_NISUGATA_SQL, strList);
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        msgLogic.MessageBoxShow("E258", "荷姿", "荷姿CD", strName);
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                NisugataHoshuValidator vali = new NisugataHoshuValidator();
                bool result = vali.NisugataCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public bool Preview()
        {
            try
            {
                ...
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("C011", "荷姿一覧表");
        ...
        public bool CancelCondition()
        {
            try
            {
                ...
                //enable funtion button when F7 press
                // 権限チェック
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_NISUGATA nisugataEntity in this.entitys)
                        {
                            M_NISUGATA entity = this.dao.GetDataByCd(nisugataEntity.NISUGATA_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (nisugataEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(nisugataEntity);
                            }
                            else
                            {
                                this.dao.Update(nisugataEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            NisugataHoshuLogic localLogic = other as NisugataHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M259", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
                    FunctionControl.ControlFunctionButton((MasterBaseForm)this.form.ParentForm, true);
                }
                else
        ...
        {
            M_NISUGATA entity = new M_NISUGATA();
        ...
        {
            var objValue = row[Const.NisugataHoshuConstans.NISUGATA_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.NisugataHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null)
                {
                    ...
                    if (Const.NisugataHoshuConstans.fixedRowList.Contains(strCd))
                    {
                        return true;
                    }
                }
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.NISUGATA_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["NISUGATA_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
