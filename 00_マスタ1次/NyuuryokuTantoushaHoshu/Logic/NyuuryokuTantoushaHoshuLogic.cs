// $Id: NyuuryokuTantoushaHoshuLogic.cs 19014 2014-04-11 04:23:12Z sc.h.hashimoto $
using System;
...
using NyuuryokuTantoushaHoshu.APP;
using NyuuryokuTantoushaHoshu.Dto;
...
namespace NyuuryokuTantoushaHoshu.Logic
{
    /// <summary>
    /// 入力担当者保守画面のビジネスロジック
    /// </summary>
    public class NyuuryokuTantoushaHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "NyuuryokuTantoushaHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_NYUURYOKUTANTOUSHA_DATA_SQL = "NyuuryokuTantoushaHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_NYUURYOKUTANTOUSHA_DATA_SQL = "NyuuryokuTantoushaHoshu.Sql.GetNyuuryokuTantoushaDataSql.sql";

        private readonly string GET_ICHIRAN_SHAIN_DATA_SQL = "NyuuryokuTantoushaHoshu.Sql.GetIchiranShaindataSql.sql";

        /// <summary>
        /// 入力担当者保守画面Form
        /// </summary>
        private NyuuryokuTantoushaHoshuForm form;
        ...
        private M_NYUURYOKU_TANTOUSHA[] entitys;
        ...
        /// 入力担当者のDao
        /// </summary>
        private IM_NYUURYOKU_TANTOUSHADao dao;
        ...
        /// 検索条件(Dto)
        /// </summary>
        public NyuuryokuTantoushaHoshuDto nyuuryokuTantoushaHoshuDto { get; set; }
        ...
        public NyuuryokuTantoushaHoshuLogic(NyuuryokuTantoushaHoshuForm targetForm)
        {
            LogUtility.DebugMethodStart(targetForm);

            this.form = targetForm;
            this.dao = DaoInitUtility.GetComponent<IM_NYUURYOKU_TANTOUSHADao>();
            this.daoShain = DaoInitUtility.GetComponent<IM_SHAINDao>();
            this.nyuuryokuTantoushaHoshuDto = new NyuuryokuTantoushaHoshuDto();
        ...
        public int Search()
        {
            try
            {
                ...
                // プロパティ設定有無チェック
                if (this.nyuuryokuTantoushaHoshuDto.PropertiesShainExistCheck())
                {
                    // 社員マスタの条件で取得
                    this.SearchResult = daoShain.GetIchiranDataSqlFile(this.GET_ICHIRAN_SHAIN_DATA_SQL
                                            , this.nyuuryokuTantoushaHoshuDto.ShainSearchString
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                }
                else
                {
                    // 入力担当者マスタの条件で取得
                    this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_NYUURYOKUTANTOUSHA_DATA_SQL
                                            , this.nyuuryokuTantoushaHoshuDto.nyuuryokuTantoushaSearchString
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_TEKIYOU.Checked
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_TEKIYOUGAI.Checked);
                }

                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_NYUURYOKUTANTOUSHA_DATA_SQL, new M_NYUURYOKU_TANTOUSHA());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_NYUURYOKU_TANTOUSHA[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_NYUURYOKU_TANTOUSHA();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_NYUURYOKU_TANTOUSHA>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.NyuuryokuTantoushaHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_NYUURYOKU_TANTOUSHA> addList = new List<M_NYUURYOKU_TANTOUSHA>();
                if (this.form.Ichiran.DataSource != null)
                {
                    foreach (DataRow row in ((DataTable)this.form.Ichiran.DataSource).Rows)
                    {
                        M_NYUURYOKU_TANTOUSHA ent = new M_NYUURYOKU_TANTOUSHA();
                        ent.SHAIN_CD = row[Const.NyuuryokuTantoushaHoshuConstans.SHAIN_CD].ToString();
                        ent.NYUURYOKU_TANTOUSHA_BIKOU = row[Const.NyuuryokuTantoushaHoshuConstans.NYUURYOKU_TANTOUSHA_BIKOU].ToString();
                        ...
                        ent.DELETE_FLG = (bool)row[Const.NyuuryokuTantoushaHoshuConstans.DELETE_FLG];
                        addList.Add(ent);
                    }
                }
        ...
        public bool Preview()
        {
            try
            {
                ...
                msgLogic.MessageBoxShow("C011", "入力担当者一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_NYUURYOKU_TANTOUSHA nyuuryokutantoushaEntity in this.entitys)
                        {
                            if (!String.IsNullOrEmpty(nyuuryokutantoushaEntity.SHAIN_CD))
                            {
                                M_NYUURYOKU_TANTOUSHA entity = this.dao.GetDataByCd(nyuuryokutantoushaEntity.SHAIN_CD);
                                if (entity != null)
                                {
                                    nyuuryokutantoushaEntity.DELETE_FLG = entity.DELETE_FLG;

                                    this.dao.Update(nyuuryokutantoushaEntity);
                                }
                            }
                        }
                        // トランザクション終了
        ...
        public override bool Equals(object other)
        {
            ...
            NyuuryokuTantoushaHoshuLogic localLogic = other as NyuuryokuTantoushaHoshuLogic;
        ...
        {
            M_NYUURYOKU_TANTOUSHA entity = new M_NYUURYOKU_TANTOUSHA();
            ...
            nyuuryokuTantoushaHoshuDto.ShainSearchString = entityShain;
            nyuuryokuTantoushaHoshuDto.nyuuryokuTantoushaSearchString = entity;
        }
