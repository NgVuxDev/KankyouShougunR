// $Id: ShobunHouhouHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using ShobunHouhouHoshu.APP;
using ShobunHouhouHoshu.Validator;
using Shougun.Core.Common.BusinessCommon;

namespace ShobunHouhouHoshu.Logic
{
    /// <summary>
    /// 処分方法画面のビジネスロジック
    /// </summary>
    public class ShobunHouhouHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ShobunHouhouHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_SHOBUN_DATA_SQL = "ShobunHouhouHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_SHOBUNHOUHOU_DATA_SQL = "ShobunHouhouHoshu.Sql.GetShobunHouhoudataSql.sql";

        private readonly string CHECK_DELETE_SHOBUN_SQL = "ShobunHouhouHoshu.Sql.CheckDeleteShobunSql.sql";

        /// <summary>
        /// 処分方法画面Form
        /// </summary>
        private ShobunHouhouHoshuForm form;
        ...
        private M_SHOBUN_HOUHOU[] entitys;
        ...
        /// 処分方法のDao
        /// </summary>
        private IM_SHOBUN_HOUHOUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_SHOBUN_HOUHOU SearchString { get; set; }
        ...
        public ShobunHouhouHoshuLogic(ShobunHouhouHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_SHOBUN_DATA_SQL
                                                 , this.SearchString
                                                 , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_SHOBUNHOUHOU_DATA_SQL, new M_SHOBUN_HOUHOU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_SHOBUN_HOUHOU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_SHOBUN_HOUHOU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_SHOBUN_HOUHOU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ShobunHouhouHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var ShobunHouhouHoshuEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_SHOBUN_HOUHOU> addList = new List<M_SHOBUN_HOUHOU>();
                foreach (var ShobunHouhouHoshuEntity in ShobunHouhouHoshuEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ShobunHouhouHoshuConstans.SHOBUN_HOUHOU_CD) && n.Value.ToString().Equals(ShobunHouhouHoshuEntity.SHOBUN_HOUHOU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ShobunHouhouHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), ShobunHouhouHoshuEntity);
                            addList.Add(ShobunHouhouHoshuEntity);
                            break;
                        }
                    }
                }
        ...
        public bool CheckDelete()
        {
            ...
            foreach (Row gcRwos in this.form.Ichiran.Rows)
            {
                ...
                {
                    shobunCd += gcRwos.Cells["SHOBUN_HOUHOU_CD"].Value.ToString() + ",";
                }
            }

            if (!string.IsNullOrEmpty(shobunCd))
            {
                shobunCd = shobunCd.Substring(0, shobunCd.Length - 1);
                strList = shobunCd.Split(',');
                DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_SHOBUN_SQL, strList);
                if (dtTable != null && dtTable.Rows.Count > 0)
                {
                    ...
                    msgLogic.MessageBoxShow("E258", "処分", "処分CD", strName);
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ShobunHouhouHoshuValidator vali = new ShobunHouhouHoshuValidator();
                bool result = vali.ShobunHouhouCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        //    MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
        //    msgLogic.MessageBoxShow("C011", "処分方法一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_SHOBUN_HOUHOU ShobunHouhouHoshuEntity in this.entitys)
                        {
                            M_SHOBUN_HOUHOU entity = this.dao.GetDataByCd(ShobunHouhouHoshuEntity.SHOBUN_HOUHOU_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (ShobunHouhouHoshuEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(ShobunHouhouHoshuEntity);
                            }
                            else
                            {
                                this.dao.Update(ShobunHouhouHoshuEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public virtual void LogicalDelete()
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    using (var tran = new Transaction())
                    {
                        foreach (M_SHOBUN_HOUHOU shobunhouhouEntity in this.entitys)
                        {
                            M_SHOBUN_HOUHOU entity = this.dao.GetDataByCd(shobunhouhouEntity.SHOBUN_HOUHOU_CD);
                            if (entity != null)
                            {
                                this.dao.Update(shobunhouhouEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            ShobunHouhouHoshuLogic localLogic = other as ShobunHouhouHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M269", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            var objValue = row[Const.ShobunHouhouHoshuConstans.SHOBUN_HOUHOU_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.ShobunHouhouHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null)
                {
                    ...
                    if (Const.ShobunHouhouHoshuConstans.fixedRowList.Contains(strCd))
                    {
                        return true;
                    }
                }
