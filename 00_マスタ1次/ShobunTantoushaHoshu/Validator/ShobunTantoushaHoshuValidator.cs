// $Id: ShobunTantoushaHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace ShobunTantoushaHoshu.Validator
{
    ...
    /// 処分担当者保守検証ロジック
    /// </summary>
    public class ShobunTantoushaHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public ShobunTantoushaHoshuValidator()
        {
        }
        /// <summary>
        /// 社員CD重複チェック
        ...
        public bool ShainCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.ShobunTantoushaHoshuConstans.SHAIN_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.ShobunTantoushaHoshuConstans.SHAIN_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    {
                        string shainCd = row.Field<string>(Const.ShobunTantoushaHoshuConstans.SHAIN_CD);
