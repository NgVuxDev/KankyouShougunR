// $Id: ShuruiHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using ShuruiHoshu.APP;
using ShuruiHoshu.Validator;

namespace ShuruiHoshu.Logic
{
    /// <summary>
    /// 種類保守画面のビジネスロジック
    /// </summary>
    public class ShuruiHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "ShuruiHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_SHURUI_DATA_SQL = "ShuruiHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_SHURUI_DATA_SQL = "ShuruiHoshu.Sql.GetShuruidataSql.sql";

        private readonly string CHECK_DELETE_SHURUI_SQL = "ShuruiHoshu.Sql.CheckDeleteShuruiSql.sql";

        /// <summary>
        /// 種類保守画面Form
        /// </summary>
        private ShuruiHoshuForm form;
        ...
        /// 検索条件
        /// </summary>
        public M_SHURUI SearchString { get; set; }
        ...
        public ShuruiHoshuLogic(ShuruiHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_SHURUI_DATA_SQL
                                                , this.SearchString
                                                , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_SHURUI_DATA_SQL
                                                , this.SearchString
                                                , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_SHURUI_DATA_SQL, new M_SHURUI());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_SHURUI[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_SHURUI();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_SHURUI>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.ShuruiHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var shuruiEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_SHURUI> addList = new List<M_SHURUI>();
                foreach (var shuruiEntity in shuruiEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.ShuruiHoshuConstans.SHURUI_CD) && n.Value.ToString().Equals(shuruiEntity.SHURUI_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.ShuruiHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), shuruiEntity);
                            addList.Add(shuruiEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                ShuruiHoshuValidator vali = new ShuruiHoshuValidator();
                bool result = vali.ShuruiCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
            ...
            try
            {
                ...
                foreach (Row gcRwos in this.form.Ichiran.Rows)
                {
                    if (gcRwos.Cells["DELETE_FLG"].Value != null && gcRwos.Cells["DELETE_FLG"].Value.ToString() == "True")
                    {
                        ...
                        shuruiCd += gcRwos.Cells["SHURUI_CD"].Value.ToString() + ",";
                    }
                }

                if (!string.IsNullOrEmpty(shuruiCd))
                {
                    shuruiCd = shuruiCd.Substring(0, shuruiCd.Length - 1);
                    strList = shuruiCd.Split(',');
                    DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_SHURUI_SQL, strList);
                    if (dtTable != null && dtTable.Rows.Count > 0)
                    {
                        ...
                        msgLogic.MessageBoxShow("E258", "種類", "種類CD", strName);
        ...
        //    MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
        //    msgLogic.MessageBoxShow("C011", "種類一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_SHURUI shuruiEntity in this.entitys)
                        {
                            M_SHURUI entity = this.dao.GetDataByCd(shuruiEntity.SHURUI_CD);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (shuruiEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(shuruiEntity);
                            }
                            else
                            {
                                this.dao.Update(shuruiEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            ShuruiHoshuLogic localLogic = other as ShuruiHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M234", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.SHURUI_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["SHURUI_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
