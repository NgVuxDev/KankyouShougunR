// $Id: TegatahokanshaHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
...
namespace TegatahokanshaHoshu.Validator
{
    ...
    /// 手形保管者検証ロジック
    /// </summary>
    public class TegatahokanshaHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public TegatahokanshaHoshuValidator()
        {
        }
        ...
        public bool ShainCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.TegatahokanshaHoshuConstans.SHAIN_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.TegatahokanshaHoshuConstans.SHAIN_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowTegatahokanshaCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string shainCd = row.Field<string>(Const.TegatahokanshaHoshuConstans.SHAIN_CD);
