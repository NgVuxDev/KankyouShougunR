// $Id: UnitHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using UnitHoshu.APP;
using UnitHoshu.Validator;

namespace UnitHoshu.Logic
{
    /// <summary>
    /// 単位保守画面のビジネスロジック
    /// </summary>
    public class UnitHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "UnitHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_DATA_SQL = "UnitHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_HAYIKEBUTU_DATA_SQL = "UnitHoshu.Sql.GetUnitDataSql.sql";

        private readonly string CHECK_DELETE_UNIT_SQL = "UnitHoshu.Sql.CheckDeleteUnitSql.sql";

        /// <summary>
        /// 単位保守画面Form
        /// </summary>
        private UnitHoshuForm form;
        ...
        /// 検索条件
        /// </summary>
        public M_UNIT SearchString { get; set; }
        ...
        public UnitHoshuLogic(UnitHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_DATA_SQL
                                                , this.SearchString
                                                , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_HAYIKEBUTU_DATA_SQL, new M_UNIT());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_UNIT[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_UNIT();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_UNIT>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.UnitHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var unitEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_UNIT> addList = new List<M_UNIT>();
                foreach (var unitEntity in unitEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.UnitHoshuConstans.UNIT_CD) && n.Value.ToString().Equals(Convert.ToString(unitEntity.UNIT_CD)))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.UnitHoshuConstans.DELETE_FLG) && (n.FormattedValue == null || bool.Parse(n.FormattedValue.ToString()) == isDelete))))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), unitEntity);
                            addList.Add(unitEntity);
                            break;
                        }
                    }
                }
        ...
        public bool CheckDelete()
        {
            ...
            foreach (Row gcRwos in this.form.Ichiran.Rows)
            {
                ...
                {
                    unitCd += gcRwos.Cells["UNIT_CD"].Value.ToString() + ",";
                }
            }

            if (!string.IsNullOrEmpty(unitCd))
            {
                unitCd = unitCd.Substring(0, unitCd.Length - 1);
                strList = unitCd.Split(',');
                DataTable dtTable = dao.GetDataBySqlFileCheck(this.CHECK_DELETE_UNIT_SQL, strList);
                if (dtTable != null && dtTable.Rows.Count > 0)
                {
                    ...
                    msgLogic.MessageBoxShow("E258", "単位", "単位CD", strName);
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                UnitHoshuValidator vali = new UnitHoshuValidator();
                bool result = vali.UnitCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        public bool Preview()
        {
            try
            {
                LogUtility.DebugMethodStart();
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                msgLogic.MessageBoxShow("C011", "単位一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_UNIT unitEntity in this.entitys)
                        {
                            M_UNIT entity = this.dao.GetDataByCd(unitEntity.UNIT_CD.Value);
                            if (entity == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (unitEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(unitEntity);
                            }
                            else
                            {
                                this.dao.Update(unitEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
        ...
        public override bool Equals(object other)
        {
            ...
            UnitHoshuLogic localLogic = other as UnitHoshuLogic;
        ...
        //    // ゼロパディング後、テキストへ設定
        //    e.Value = String.Format("{0:D" + UnitHoshuConstans.CD_MAXLENGTH + "}", int.Parse(e.Value.ToString()));
        //}
        ...
        {
            var objValue = row[Const.UnitHoshuConstans.UNIT_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.UnitHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null && int.TryParse(objValue.ToString(), out iValue))
                {
                    ...
                    if (Const.UnitHoshuConstans.fixedRowList.Contains(iValue))
                    {
                        return true;
                    }
                }
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.UNIT_CD).Where(s => !s.Equals(DBNull.Value)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["UNIT_CD"]).Where(c => !c.Value.Equals(DBNull.Value)).ToList().
                                            ForEach(c =>
                                            {
        ...
        {
            if (this.form.CONDITION_VALUE.DBFieldsName.Equals(Const.UnitHoshuConstans.UNIT_CD))
            {
