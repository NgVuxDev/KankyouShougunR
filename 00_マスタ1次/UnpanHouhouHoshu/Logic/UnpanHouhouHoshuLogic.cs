// $Id: UnpanHouhouHoshuLogic.cs 51723 2015-06-08 06:14:52Z hoangvu@e-mall.co.jp $
using System;
...
using UnpanHouhouHoshu.APP;
using UnpanHouhouHoshu.Validator;

namespace UnpanHouhouHoshu.Logic
{
    /// <summary>
    /// 運搬方法画面のビジネスロジック
    /// </summary>
    public class UnpanHouhouHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "UnpanHouhouHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_UNPANHOUHOU_DATA_SQL = "UnpanHouhouHoshu.Sql.GetIchiranDataSql.sql";

        private readonly string GET_UNPAN_HOUHOU_DATA_SQL = "UnpanHouhouHoshu.Sql.GetUnpanHouhouDataSql.sql";

        /// <summary>
        /// 運搬方法画面Form
        /// </summary>
        private UnpanHouhouHoshuForm form;
        ...
        private M_UNPAN_HOUHOU[] entitys;
        ...
        /// 運搬方法のDao
        /// </summary>
        private IM_UNPAN_HOUHOUDao dao;
        ...
        /// 検索条件
        /// </summary>
        public M_UNPAN_HOUHOU SearchString { get; set; }
        ...
        public UnpanHouhouHoshuLogic(UnpanHouhouHoshuForm targetForm)
        {
        ...
        public int Search()
        {
            try
            {
                ...
                this.SearchResult = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_UNPANHOUHOU_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultCheck = dao.GetIchiranDataSqlFile(this.GET_ICHIRAN_UNPANHOUHOU_DATA_SQL
                                                            , this.SearchString
                                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                this.SearchResultAll = dao.GetDataBySqlFile(this.GET_UNPAN_HOUHOU_DATA_SQL, new M_UNPAN_HOUHOU());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_UNPAN_HOUHOU[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_UNPAN_HOUHOU();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_UNPAN_HOUHOU>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.UnpanHouhouHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                var unpanEntityList = dataBinderLogic.CreateEntityForDataTable(this.form.Ichiran);

                List<M_UNPAN_HOUHOU> addList = new List<M_UNPAN_HOUHOU>();
                foreach (var unpanEntity in unpanEntityList)
                {
                    foreach (Row row in this.form.Ichiran.Rows)
                    {
                        if (row.Cells.Any(n => (n.DataField.Equals(Const.UnpanHouhouHoshuConstans.UNPAN_HOUHOU_CD) && n.Value.ToString().Equals(unpanEntity.UNPAN_HOUHOU_CD))) &&
                            row.Cells.Any(n => (n.DataField.Equals(Const.UnpanHouhouHoshuConstans.DELETE_FLG) && bool.Parse(n.FormattedValue.ToString()) == isDelete)))
                        {
                            MasterCommonLogic.SetFooterProperty(MasterCommonLogic.GetCurrentShain(this.form), unpanEntity);
                            addList.Add(unpanEntity);
                            break;
                        }
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            try
            {
                ...
                UnpanHouhouHoshuValidator vali = new UnpanHouhouHoshuValidator();
                bool result = vali.UnpanHouhouCDValidator(this.form.Ichiran, this.SearchResultCheck, this.SearchResultAll, this.isAllSearch);
        ...
        //    msgLogic.MessageBoxShow("C011", "運搬方法一覧表");
        //    MessageBox.Show("未実装");
        //    LogUtility.DebugMethodEnd();
        //}
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_UNPAN_HOUHOU unpanEntity in this.entitys)
                        {
                            M_UNPAN_HOUHOU m = this.dao.GetDataByCd(unpanEntity.UNPAN_HOUHOU_CD);
                            if (m == null)
                            {
                                // 削除チェックが付けられている場合は、新規登録を行わない
                                if (unpanEntity.DELETE_FLG)
                                {
                                    continue;
                                }
                                this.dao.Insert(unpanEntity);
                            }
                            else
                            {
                                this.dao.Update(unpanEntity);
                            }
                        }
                        // トランザクション終了
                        tran.Commit();
            ...
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    using (var tran = new Transaction())
                    {
                        foreach (M_UNPAN_HOUHOU unpanEntity in this.entitys)
                        {
                            M_UNPAN_HOUHOU entity = this.dao.GetDataByCd(unpanEntity.UNPAN_HOUHOU_CD.ToString());
                            if (entity != null)
                            {
                                this.dao.Update(unpanEntity);
                            }
        ...
        public override bool Equals(object other)
        {
            ...
            UnpanHouhouHoshuLogic localLogic = other as UnpanHouhouHoshuLogic;
        ...
        internal bool SetIchiran()
        {
            try
            {
                ...
                // 権限チェック
                if (r_framework.Authority.Manager.CheckAuthority("M268", r_framework.Const.WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        {
            M_UNPAN_HOUHOU entity = new M_UNPAN_HOUHOU();
        ...
        {
            var objValue = row[Const.UnpanHouhouHoshuConstans.UNPAN_HOUHOU_CD].Value;

            bool catchErr = false;
            if (this.CheckFixedRow(row, out catchErr) && !catchErr)
            {
                foreach (var columnName in Const.UnpanHouhouHoshuConstans.fixedColumnList)
                {
        ...
        internal bool CheckFixedRow(Row row, out bool catchErr)
        {
            try
            {
                ...
                if (row != null && objValue != null)
                {
                    ...
                    if (Const.UnpanHouhouHoshuConstans.fixedRowList.Contains(strCd))
                    {
                        return true;
                    }
                }
        ...
        internal bool EditableToPrimaryKey()
        {
            ...
            {
                // DBから主キーのListを取得
                var allEntityList = this.dao.GetAllData().Select(s => s.UNPAN_HOUHOU_CD).Where(s => !string.IsNullOrEmpty(s)).ToList();

                // DBに存在する行の主キーを非活性にする
                this.form.Ichiran.Rows.Select(r => r.Cells["UNPAN_HOUHOU_CD"]).Where(c => c.Value != null).ToList().
                                            ForEach(c =>
                                            {
