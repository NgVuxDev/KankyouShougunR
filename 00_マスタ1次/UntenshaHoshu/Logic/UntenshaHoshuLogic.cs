// $Id: UntenshaHoshuLogic.cs 19011 2014-04-11 04:12:53Z sc.h.hashimoto $
using System;
...
using UntenshaHoshu.APP;
using UntenshaHoshu.Dto;
using UntenshaHoshu.Validator;
using MasterCommon.Utility;
using Seasar.Framework.Exceptions;
using Seasar.Dao;

namespace UntenshaHoshu.Logic
{
    /// <summary>
    /// 運転者保守画面のビジネスロジック
    /// </summary>
    public class UntenshaHoshuLogic : IBuisinessLogic
    {
        #region フィールド

        private readonly string ButtonInfoXmlPath = "UntenshaHoshu.Setting.ButtonSetting.xml";

        private readonly string GET_ICHIRAN_UNTENSHA_DATA_SQL = "UntenshaHoshu.Sql.GetIchiranUntenshadataSql.sql";

        private readonly string GET_UNTENSHA_DATA_SQL = "UntenshaHoshu.Sql.GetUntenshadataSql.sql";

        private readonly string GET_ICHIRAN_SHAIN_DATA_SQL = "UntenshaHoshu.Sql.GetIchiranShaindataSql.sql";

        /// <summary>
        /// 運転者保守画面Form
        /// </summary>
        private UntenshaHoshuForm form;
        ...
        /// 検索条件(Dto)
        /// </summary>
        public UntenshaHoshuDto untenshaHoshuDto { get; set; }
        ...
        public UntenshaHoshuLogic(UntenshaHoshuForm targetForm)
        {
            ...
            this.untenshaHoshuDto = new UntenshaHoshuDto();
        ...
        public int Search()
        {
            try
            {
                ...
                // プロパティ設定有無チェック
                if (this.untenshaHoshuDto.PropertiesShainExistCheck())
                {
                    // 社員マスタの条件で取得
                    this.SearchResult = daoShain.GetIchiranDataSqlFile(this.GET_ICHIRAN_SHAIN_DATA_SQL
                                            , this.untenshaHoshuDto.ShainSearchString
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked);
                }
                else
                {
                    // 運転者マスタの条件で取得
                    this.SearchResult = daoUntensha.GetIchiranDataSqlFile(this.GET_ICHIRAN_UNTENSHA_DATA_SQL
                                            , this.untenshaHoshuDto.UntenshaSearchString
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_TEKIYOU.Checked
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_DELETED.Checked
                                            , this.form.ICHIRAN_HYOUJI_JOUKEN_TEKIYOUGAI.Checked);
                }

                this.SearchResultAll = daoUntensha.GetDataBySqlFile(this.GET_UNTENSHA_DATA_SQL, new M_UNTENSHA());
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                var entityList = new M_UNTENSHA[this.form.Ichiran.Rows.Count];
                for (int i = 0; i < entityList.Length; i++)
                {
                    entityList[i] = new M_UNTENSHA();
                }

                var dataBinderLogic = new DataBinderLogic<r_framework.Entity.M_UNTENSHA>(entityList);
                ...
                foreach (DataColumn column in dt.Columns)
                {
                    ...
                    if (column.ColumnName.Equals(Const.UntenshaHoshuConstans.TIME_STAMP))
                    {
                        column.Unique = false;
                    }
                }
                ...
                List<M_UNTENSHA> addList = new List<M_UNTENSHA>();
                if (this.form.Ichiran.DataSource != null)
                {
                    foreach (DataRow row in ((DataTable)this.form.Ichiran.DataSource).Rows)
                    {
                        M_UNTENSHA ent = new M_UNTENSHA();
                        ent.SHAIN_CD = row[Const.UntenshaHoshuConstans.SHAIN_CD].ToString();
                        ent.UNTENSHA_BIKOU = row[Const.UntenshaHoshuConstans.UNTENSHA_BIKOU].ToString();
                        ...
                        ent.DELETE_FLG = (bool)row[Const.UntenshaHoshuConstans.DELETE_FLG];
                        addList.Add(ent);
                    }
                }
        ...
        public bool DuplicationCheck()
        {
            ...
            UntenshaHoshuValidator vali = new UntenshaHoshuValidator();
            bool result = vali.UntenshaCDValidator(this.form.Ichiran, this.SearchResult, this.SearchResultAll, this.isAllSearch);
        ...
        public bool Preview()
        {
            try
            {
                ...
                msgLogic.MessageBoxShow("C011", "運転者一覧表");
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                if (!errorFlag)
                {
                    ...
                    {
                        foreach (M_UNTENSHA untenshaEntity in this.entitys)
                        {
                            if (!String.IsNullOrEmpty(untenshaEntity.SHAIN_CD))
                            {
                                M_UNTENSHA entity = this.daoUntensha.GetDataByCd(untenshaEntity.SHAIN_CD);
                                if (entity == null)
                                {
                                    this.daoUntensha.Insert(untenshaEntity);
                                }
                                else
                                {
                                    this.daoUntensha.Update(untenshaEntity);
                                }
                            }
                        }
                        // トランザクション終了
        ...
        public override bool Equals(object other)
        {
            ...
            UntenshaHoshuLogic localLogic = other as UntenshaHoshuLogic;
        ...
        {
            M_UNTENSHA entityUntensha = new M_UNTENSHA();   // TODO:メインマスタEntity(表示する作成者等があるEntity)
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.DBFieldsName))
            {
                if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.ItemDefinedTypes))
                {
                    ...
                    bool isExist = this.EntityExistCheck(entityUntensha, this.form.CONDITION_VALUE.DBFieldsName);

                    // TODO : true=メインマスタEntityのSetValue使用、false=サブマスタEntityのSetValue使用
                    if (isExist)
                    {
                        // 検索条件の設定(運転者マスタ)
                        entityUntensha.SetValue(this.form.CONDITION_VALUE);
                    }
                    else
                    {
                        // 検索条件の設定(社員マスタ)
            ...
            untenshaHoshuDto.ShainSearchString = entityShain;
            untenshaHoshuDto.UntenshaSearchString = entityUntensha;
        }
