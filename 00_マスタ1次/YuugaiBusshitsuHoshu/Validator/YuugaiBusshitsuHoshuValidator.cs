// $Id: YuugaiBusshitsuHoshuValidator.cs 12324 2013-12-23 12:55:25Z ishibashi $
using System;
...
namespace YuugaiBusshitsuHoshu.Validator
{
    /// <summary>
    /// 有害物質保守検証ロジック
    /// </summary>
    class YuugaiBusshitsuHoshuValidator
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        public YuugaiBusshitsuHoshuValidator()
        {
        }
        ...
        public bool YuugaiBusshitsuCDValidator(GcMultiRow gcMultiRow, DataTable dt, DataTable dtAll, bool isAllSearch)
        {
            ...
            GcCustomAlphaNumTextBoxCell control = gcMultiRow[gcMultiRow.CurrentRow.Index, Const.YuugaiBusshitsuHoshuConstans.YUUGAI_BUSSHITSU_CD] as GcCustomAlphaNumTextBoxCell;
            if (control == null
                || control.Value == null
            ...
            // 重複チェック
            {
                ...
                // 表示分(検索条件による抽出分)
                {
                    ...
                    foreach (Row row in gcMultiRow.Rows)
                    {
                        ...
                        Cell cell = row.Cells[Const.YuugaiBusshitsuHoshuConstans.YUUGAI_BUSSHITSU_CD];
                        if (cell.Selected)
                        {
                            continue;
                        }
                ...
                // 非表示分(検索条件から漏れたデータ)
                {
                    ...
                    var rows = enumRowAll.Except(enumRow, new DataRowYuugaiBusshitsuCompare());

                    var list = new List<GcCustomAlphaNumTextBoxCell>();
                    foreach (DataRow row in rows)
                    {
                        string gyoushuCd = row.Field<string>(Const.YuugaiBusshitsuHoshuConstans.YUUGAI_BUSSHITSU_CD);
