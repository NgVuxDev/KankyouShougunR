...
WHERE
A.NEXT_SYSTEM_ID =   /*data.MANIFEST_SYSTEM_ID*/
AND A.DELETE_FLG = 0               --マニフェスト紐付.削除フラグ
...
--AND H.LAST_SBN_END_DATE IS NOT NULL  --マニフェスト明細.最終処分終了日
--AND H.LAST_SBN_GENBA_CD IS NOT NULL  --マニフェスト明細.最終処分現場CD
AND (C.MANIFEST_ID != '' AND C.MANIFEST_ID IS NOT NULL)
AND NOT EXISTS(
SELECT
*
FROM
