...
--マニフェストパターン
  FROM T_MANIFEST_PT_ENTRY AS TME WITH(NOLOCK)
...
--マニフェストパターン収集運搬
  LEFT JOIN T_MANIFEST_PT_UPN AS TMU WITH(NOLOCK)
...
--マニフェストパターン印字
  LEFT JOIN T_MANIFEST_PT_PRT AS TMP WITH(NOLOCK)
 ...
 WHERE TME.DELETE_FLG = 'false'
   AND TME.SYSTEM_ID = /*data.SYSTEM_ID*/ 
--   AND TME.SEQ = /*data.SEQ*/
