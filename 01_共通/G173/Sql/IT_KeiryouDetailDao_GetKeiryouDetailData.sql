SELECT 
	...
	T5.UNIT_NAME_RYAKU AS NISUGATA_NAME_RYAKU

FROM dbo.T_KEIRYOU_DETAIL AS T1 
LEFT JOIN dbo.M_YOUKI AS T2 ON T1.YOUKI_CD = T2.YOUKI_CD 
LEFT JOIN dbo.T_MANIFEST_ENTRY AS T3 ON T1.SYSTEM_ID = T3.RENKEI_SYSTEM_ID AND T1.DETAIL_SYSTEM_ID=T3.RENKEI_MEISAI_SYSTEM_ID AND T3.RENKEI_DENSHU_KBN_CD='140' 
LEFT JOIN dbo.M_DENPYOU_KBN AS T4 ON T1.DENPYOU_KBN_CD = T4.DENPYOU_KBN_CD 
LEFT JOIN dbo.M_UNIT AS T5 ON T1.NISUGATA_UNIT_CD = T5.UNIT_CD 

WHERE 
 T1.SYSTEM_ID = /*systemId*/0 /*END*/
AND T1.SEQ = /*seq*/0 /*END*/
ORDER BY T1.DETAIL_SYSTEM_ID
