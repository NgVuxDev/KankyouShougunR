...
{
    public class DgvCustomLabeledCheckBoxCell : DgvCustomCheckBoxCell
	{
        ...
        public DgvCustomLabeledCheckBoxCell()
		{
		}

        public virtual void CloneChiled()
		{
            var column = this.OwningColumn as DgvCustomLabeledCheckBoxColumn;

            if (column != null)
            {
                this.LabelText = column.LabelText;
            }
		}
			...
			int rowIndex)
		{
            ...
            _checkboxLocation.Offset(size.Width - cellStyle.Padding.Horizontal - sizeCheckBox.Width - 7, checkY);
			_checkboxHitArea = new Rectangle(location, sizeCheckBox);
			_checkboxHitArea.Offset(-cellBounds.Left, -cellBounds.Top);
			_checkboxHitArea.Width = size.Width - 2;
			_textRectangle = new Rectangle(
				location.X - 2,
				location.Y,
				size.Width - cellStyle.Padding.Horizontal - sizeCheckBox.Width - 5,
				size.Height);
		}
			...
			DataGridViewPaintParts paintParts)
		{
			...
			// Draw the eCell background, if specified.
			...
			// Draw the eCell borders, if specified.
