...
FROM(
    ...
    SELECT 
      ...
      ,CONVERT(VARCHAR,SHUKKA_NUMBER) AS DENPYOU_NUMBER
      ,DENPYOU_DATE
      ,TORIHIKISAKI_NAME AS TORIHIKISAKI_NAME
      ,GYOUSHA_NAME
      ,GENBA_NAME
      ,SYSTEM_ID
      ,'' AS HAIKI_KBN_CD
    ...
    INNER JOIN T_MANIFEST_ENTRY TME ON TME.RENKEI_SYSTEM_ID = TSE.SYSTEM_ID AND TME.RENKEI_DENSHU_KBN_CD = 2 AND TME.DELETE_FLG = 0
    LEFT JOIN M_TORIHIKISAKI MT ON MT.TORIHIKISAKI_CD = TME.TORIHIKISAKI_CD
    WHERE TSE.UKETSUKE_NUMBER = /*data.UKETSUKE_NUMBER*/
    ...
    SELECT
      ...
      ,'' AS HAIKI_KBN_CD
    FROM T_SHUKKA_ENTRY TSE
    INNER JOIN T_UNCHIN_ENTRY UNCHI ON UNCHI.RENKEI_NUMBER = TSE.SHUKKA_NUMBER AND UNCHI.DENSHU_KBN_CD = 2 AND UNCHI.DELETE_FLG = 0
    WHERE TSE.UKETSUKE_NUMBER = /*data.UKETSUKE_NUMBER*/
      AND TSE.DELETE_FLG = 0
      /*IF !data.UNCHIN_FLG*/
      AND 1 = 0
