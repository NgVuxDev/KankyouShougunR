...
namespace Shougun.Core.SalesPayment.UriageSuiiChouhyou
{
    ...
    /// G581 売上推移票 ビジネスロジック
    /// </summary>
    internal class UriageSuiihyouLogicClass : IBuisinessLogic
    {
        /// <summary>
        /// ボタン情報を格納しているＸＭＬファイルのパス（リソース）を保持するフィールド
        /// </summary>
        private readonly string buttonInfoXmlPath = "Shougun.Core.SalesPayment.UriageSuiiChouhyou.Setting.UriageSuiihyouButtonSetting.xml";
        ...
        /// フォーム
        /// </summary>
        private UIForm_UriageSuiihyou form;
        ...
        public UriageSuiihyouLogicClass(UIForm_UriageSuiihyou targetForm)
        {
        ...
        public int Search()
        {
            ...
            var dao = DaoInitUtility.GetComponent<IUriageSuiiHyouDao>();
            var dt = new DataTable();
            string sql;
            if (CreateSuiiHyouDataQuery(out sql))
            {
                dt = dao.GetSuiiHyouDataUriage(sql);
            }
        ...
        internal void CSVPrint(DataTable dt, UriageSuiihyouDtoClass dto)
        {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        private void CreateHeaderDt(UriageSuiihyouDtoClass dto)
        {
        ...
        internal List<M_SHARYOU> GetSharyou(String sharyouCd)
        {
        ...
        internal bool CheckDate(out long monthDiff, out string strSelectDate, out ArrayList arrayPiv)
        {
            ...
            if (date_to.CompareTo(date_from) < 0)
            {
                ...
                {
                    string[] errorMsg = { "売上日付From", "売上日付To" };
                    msglogic.MessageBoxShow("E030", errorMsg);
                }
                else
                {
        ...
        private bool CreateSuiiHyouDataQuery(out string sql)
        {
            ...
            foreach (string col in this.form.FormDataDto.Select)
            {
                switch (col)
                {
                    case "URIAGE_TORIHIKI_KBN_CD":
                        selectIn.Append(col + ",");
                        joinSelect.AppendLine(" MAIN." + col + ",")
                            .AppendLine(" M_TORIHIKI_KBN.TORIHIKI_KBN_NAME URIAGE_TORIHIKI_KBN_NAME,");
                        joinFrom.AppendLine(" LEFT JOIN M_TORIHIKI_KBN ON M_TORIHIKI_KBN.TORIHIKI_KBN_CD = MAIN.URIAGE_TORIHIKI_KBN_CD ");
                        break;
                    case "KYOTEN_CD":
                        selectIn.Append(col + ",");
                        joinSelect.AppendLine(" MAIN." + col + ",")
        ...
        private string CreateBaseSql(string selectGroupBy)
        {
                ...
                this.form.FormDataDto.DenpyouShuruiCd == 5)
            {
                    ...
                    String.IsNullOrEmpty(this.form.FormDataDto.DaikanCdTo))
                {
                    ...
                    sbUnion.Append(CreateUriageBaseSql(this.form.FormDataDto.DenpyouShuruiCd)).Append(CreateBaseWhere(3));
                }
            }
        ...
        private string CreateUkeireBaseSql()
        {
            ...
            baseSql.AppendLine("     INNER JOIN T_UKEIRE_DETAIL DETAIL ");
            ...
            baseSql.AppendLine("     ( ");
            baseSql.AppendLine("      SELECT ");
            baseSql.AppendLine("        DENPYOU.SEIKYUU_NUMBER ");
            ...
            baseSql.AppendLine("      FROM ");
            baseSql.AppendLine("       T_SEIKYUU_DENPYOU DENPYOU ");
            baseSql.AppendLine("       JOIN T_SEIKYUU_DENPYOU_KAGAMI KAGAMI ");
            baseSql.AppendLine("       ON DENPYOU.SEIKYUU_NUMBER = KAGAMI.SEIKYUU_NUMBER ");
            baseSql.AppendLine("       JOIN T_SEIKYUU_DETAIL DETAIL ");
            baseSql.AppendLine("       ON KAGAMI.SEIKYUU_NUMBER = DETAIL.SEIKYUU_NUMBER ");
            baseSql.AppendLine("        AND KAGAMI.KAGAMI_NUMBER = DETAIL.KAGAMI_NUMBER ");
            baseSql.AppendLine("      WHERE ");
            baseSql.AppendLine("          DETAIL.DENPYOU_SHURUI_CD = 1 ");
            baseSql.AppendLine("      AND DENPYOU.DELETE_FLG = 0 ");
            baseSql.AppendLine("     ) SEIKYUU ");
            baseSql.AppendLine("     ON SEIKYUU.DENPYOU_SYSTEM_ID = DETAIL.SYSTEM_ID ");
            baseSql.AppendLine("      AND SEIKYUU.DENPYOU_SEQ = DETAIL.SEQ ");
            baseSql.AppendLine("      AND SEIKYUU.DETAIL_SYSTEM_ID = DETAIL.DETAIL_SYSTEM_ID ");
            baseSql.AppendLine("      AND SEIKYUU.DENPYOU_NUMBER = DETAIL.UKEIRE_NUMBER");
            baseSql.AppendLine("    WHERE");
            baseSql.AppendLine("         ENTRY.DELETE_FLG = 0 ");
            baseSql.AppendLine("     AND ENTRY.TAIRYUU_KBN = 0 ");
            baseSql.AppendLine("     AND DETAIL.DENPYOU_KBN_CD = 1 ");
        ...
        private string CreateShukkaBaseSql()
        {
            ...
            baseSql.AppendLine("       ,DETAIL.DETAIL_SYSTEM_ID ");
            baseSql.AppendLine("       ,DETAIL.DENPYOU_NUMBER ");
            baseSql.AppendLine("      FROM T_SEIKYUU_DENPYOU DENPYOU ");
            baseSql.AppendLine("      JOIN T_SEIKYUU_DENPYOU_KAGAMI KAGAMI ");
            baseSql.AppendLine("      ON DENPYOU.SEIKYUU_NUMBER = KAGAMI.SEIKYUU_NUMBER ");
            baseSql.AppendLine("      JOIN T_SEIKYUU_DETAIL DETAIL ");
            baseSql.AppendLine("      ON KAGAMI.SEIKYUU_NUMBER = DETAIL.SEIKYUU_NUMBER ");
            ...
            baseSql.AppendLine("      AND SEIKYUU.DENPYOU_SEQ = DETAIL.SEQ ");
            baseSql.AppendLine("      AND SEIKYUU.DETAIL_SYSTEM_ID = DETAIL.DETAIL_SYSTEM_ID ");
            baseSql.AppendLine("      AND SEIKYUU.DENPYOU_NUMBER = DETAIL.SHUKKA_NUMBER ");
            baseSql.AppendLine("    WHERE ");
            baseSql.AppendLine("         ENTRY.DELETE_FLG = 0 ");
            ...
            {
                baseSql.Append("KENSHU_URIAGE_DATE");
            }
            else
            {
                baseSql.Append("UPDATE_DATE");
        ...
        /// 伝票種類(3：売上、4：代納、5：全て(デフォルト))
        /// </param>
        /// <returns></returns>
        /// <remarks>
        /// 伝票種類により、売上又は売上(代納)データを取得する
        /// 伝票種類しない場合、全ての売上データを取得する
        /// </remarks>
        //private string CreateUriageBaseSql()
        private string CreateUriageBaseSql(int denpyouShuruiCd = 5)
        // 20150514 伝種「4.代納」追加(不具合一覧(つ) 23) End
        {
            StringBuilder baseSql = new StringBuilder();
            ...
            baseSql.AppendLine("     INNER JOIN T_UR_SH_DETAIL DETAIL ");
            ...
            baseSql.AppendLine("      AND SEIKYUU.DENPYOU_SEQ = DETAIL.SEQ ");
            baseSql.AppendLine("      AND SEIKYUU.DETAIL_SYSTEM_ID = DETAIL.DETAIL_SYSTEM_ID ");
            baseSql.AppendLine("      AND SEIKYUU.DENPYOU_NUMBER = DETAIL.UR_SH_NUMBER ");
            baseSql.AppendLine("    WHERE ");
            baseSql.AppendLine("         ENTRY.DELETE_FLG = 0 ");
            baseSql.AppendLine("     AND DETAIL.DENPYOU_KBN_CD = 1 ");
        ...
        private string CreateBaseWhere(int denpyo, bool kenshu = false)
        {
            ...
            else if (this.form.FormDataDto.DateShuruiCd == 2)
            {
                if (kenshu)
                {
                    whereSb.AppendFormat("     AND ENTRY.KENSHU_URIAGE_DATE >= '{0}' ", this.form.FormDataDto.DateFrom);
                    whereSb.AppendLine(string.Empty);
                    whereSb.AppendFormat("     AND ENTRY.KENSHU_URIAGE_DATE <= '{0}' ", this.form.FormDataDto.DateTo);
                    whereSb.AppendLine(string.Empty);
                }
                else
                {
                    whereSb.AppendFormat("     AND ENTRY.URIAGE_DATE >= '{0}' ", this.form.FormDataDto.DateFrom);
                    whereSb.AppendLine(string.Empty);
                    whereSb.AppendFormat("     AND ENTRY.URIAGE_DATE <= '{0}' ", this.form.FormDataDto.DateTo);
                    whereSb.AppendLine(string.Empty);
                }
            }
            // 入力日付
            ...
            if (denpyo == 2 && kenshu)
            {
                ...
                if (this.form.FormDataDto.ShimeJoukyouCd == 1)
                {
                    ...
                    whereSb.AppendLine("               DENPYOU.SEIKYUU_NUMBER ");
                    ...
                    whereSb.AppendLine("             FROM T_SEIKYUU_DENPYOU DENPYOU ");
                    whereSb.AppendLine("             JOIN T_SEIKYUU_DENPYOU_KAGAMI KAGAMI ");
                    whereSb.AppendLine("               ON DENPYOU.SEIKYUU_NUMBER = KAGAMI.SEIKYUU_NUMBER ");
                    whereSb.AppendLine("             JOIN T_SEIKYUU_DETAIL DETAIL ");
                    whereSb.AppendLine("               ON KAGAMI.SEIKYUU_NUMBER = DETAIL.SEIKYUU_NUMBER ");
            ...
            else
            {
                if (this.form.FormDataDto.ShimeJoukyouCd == 1)
                {
                    whereSb.Append("     AND SEIKYUU.SEIKYUU_NUMBER IS NOT NULL");
                    whereSb.AppendLine(string.Empty);
                }
                else if (this.form.FormDataDto.ShimeJoukyouCd == 2)
                {
                    whereSb.Append("     AND SEIKYUU.SEIKYUU_NUMBER IS NULL");
                    whereSb.AppendLine(string.Empty);
                }
            }
            ...
            {
                whereSb.AppendFormat("     AND ENTRY.URIAGE_TORIHIKI_KBN_CD = {0} ", this.form.FormDataDto.TorihikiKbnCd);
                whereSb.AppendLine(string.Empty);
            }
            // 取引先
