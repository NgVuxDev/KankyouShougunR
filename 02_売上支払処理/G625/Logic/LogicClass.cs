...
namespace Shougun.Core.SalesPayment.ShiharaiJunihyo
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// 支払順位表Dtoを取得・設定します
        /// </summary>
        internal ShiharaiJunihyoDto dto;

        /// <summary>
        /// 支払順位表データテーブルを取得・設定します
        /// </summary>
        internal DataTable ShiharaiJunihyoDataTable { get; set; }
        ...
        public LogicClass(UIForm targetForm)
        {
            ...
            this.dto = new ShiharaiJunihyoDto();
        ...
        public int Search()
        {
            ...
            {
                // 検索条件設定
                this.SetShiharaiJunihyoDto();

                // 検索実行
                DAOClass dao = DaoInitUtility.GetComponent<DAOClass>();
                this.ShiharaiJunihyoDataTable = dao.GetGetShiharaiJunihyoData(this.dto);

                if (this.ShiharaiJunihyoDataTable != null)
                {
                    searchResult = this.ShiharaiJunihyoDataTable.Rows.Count;
                }
        ...
        internal bool CSV()
        {
            try
            {
                ...
                if (this.ShiharaiJunihyoDataTable != null && 0 < this.ShiharaiJunihyoDataTable.Rows.Count)
                {
                    ...
                    for (int i = 0; i < ShiharaiJunihyoDataTable.Rows.Count; i++)
                    {
                        rowTmp = csvDT.NewRow();
                        if (headerTable.Rows[0]["COL_CD_1"] != null && !string.IsNullOrEmpty(headerTable.Rows[0]["COL_CD_1"].ToString()))
                        {
                            if (ShiharaiJunihyoDataTable.Rows[i]["CD_1"] != null && !string.IsNullOrEmpty(ShiharaiJunihyoDataTable.Rows[i]["CD_1"].ToString()))
                            {
                                rowTmp[headerTable.Rows[0]["COL_CD_1"].ToString()] = ShiharaiJunihyoDataTable.Rows[i]["CD_1"].ToString();
                            }
                        }

                        if (headerTable.Rows[0]["COL_NAME_1"] != null && !string.IsNullOrEmpty(headerTable.Rows[0]["COL_NAME_1"].ToString()))
                        {
                            if (ShiharaiJunihyoDataTable.Rows[i]["NAME_1"] != null && !string.IsNullOrEmpty(ShiharaiJunihyoDataTable.Rows[i]["NAME_1"].ToString()))
                            {
                                rowTmp[headerTable.Rows[0]["COL_NAME_1"].ToString()] = ShiharaiJunihyoDataTable.Rows[i]["NAME_1"].ToString();
                            }
                        }

                        if (headerTable.Rows[0]["COL_CD_2"] != null && !string.IsNullOrEmpty(headerTable.Rows[0]["COL_CD_2"].ToString()))
                        {
                            if (ShiharaiJunihyoDataTable.Rows[i]["CD_2"] != null && !string.IsNullOrEmpty(ShiharaiJunihyoDataTable.Rows[i]["CD_2"].ToString()))
                            {
                                rowTmp[headerTable.Rows[0]["COL_CD_2"].ToString()] = ShiharaiJunihyoDataTable.Rows[i]["CD_2"].ToString();
                            }
                        }

                        if (headerTable.Rows[0]["COL_NAME_2"] != null && !string.IsNullOrEmpty(headerTable.Rows[0]["COL_NAME_2"].ToString()))
                        {
                            if (ShiharaiJunihyoDataTable.Rows[i]["NAME_2"] != null && !string.IsNullOrEmpty(ShiharaiJunihyoDataTable.Rows[i]["NAME_2"].ToString()))
                            {
                                rowTmp[headerTable.Rows[0]["COL_NAME_2"].ToString()] = ShiharaiJunihyoDataTable.Rows[i]["NAME_2"].ToString();
                            }
                        }

                        if (ShiharaiJunihyoDataTable.Rows[i]["KINGAKU"] != null && !string.IsNullOrEmpty(ShiharaiJunihyoDataTable.Rows[i]["KINGAKU"].ToString()))
                        {
                            rowTmp["金額"] = Convert.ToDecimal(ShiharaiJunihyoDataTable.Rows[i]["KINGAKU"].ToString()).ToString("#,##0");
                        }
        ...
        internal bool CreateJunihyo()
        {
            try
            {
                ...
                {
                    var headerTable = this.CreateReportHeaderTable();
                    var reportInfo = new ReportInfoR626(headerTable, this.ShiharaiJunihyoDataTable);
                    reportInfo.SetRecord(this.ShiharaiJunihyoDataTable);
                    reportInfo.CreateR626();
                    reportInfo.Title = headerTable.Rows[0]["TITLE"].ToString();
                    reportInfo.ReportID = "R626";
        ...
        /// 検索条件用Dtoに画面の値を設定
        /// </summary>
        private void SetShiharaiJunihyoDto()
        {
            this.dto = new ShiharaiJunihyoDto();
            ...
            else if (this.form.HIDUKE_SHURUI.Text.Equals(ConstClass.DATE_CD_SHIHARAI))
            {
                /* 支払日付 */
                // 支払日付_From
                this.dto.SHIHARAI_DATE_FROM = dateFrom.ToString("yyyy/MM/dd");
                // 支払日付_To
                this.dto.SHIHARAI_DATE_TO = dateTo.ToString("yyyy/MM/dd");
            }
        ...
        private DataTable CreateReportHeaderTable()
        {
            ...
            row["TITLE"] = string.Format("支払順位表（{0}）", title);
            row["JISHA"] = jisha;
        ...
        /// <returns></returns>
        private string CreateJouken1FieldData(ShiharaiJunihyoDto dto)
        {
            if (dto == null)
            {
                return string.Empty;
            ...
            // 日付
            {
                ...
                else if (ConstClass.DATE_CD_SHIHARAI.Equals(this.form.HIDUKE_SHURUI.Text))
                {
                    dateStr = ConstClass.DATE_NAME_SHIHARAI;
                }
