SELECT
	 DATA.*,
	 ISNULL(DATA.KINGAKU, 0) - ISNULL(DATA.PAST_KINGAKU, 0) AS SAGAKU,
	 0 AS ZOKA_RITSU
FROM
(SELECT
	T_UR_SH_ENTRY.SYSTEM_ID,
	T_UR_SH_ENTRY.SEQ,
	T_UR_SH_DETAIL.DETAIL_SYSTEM_ID,
    T_UR_SH_ENTRY.URIAGE_TORIHIKI_KBN_CD,
    CASE T_UR_SH_ENTRY.URIAGE_TORIHIKI_KBN_CD WHEN 1 THEN '現金' WHEN 2 THEN '掛け' ELSE '' END URIAGE_TORIHIKI_KBN_NAME,
...
FROM T_UR_SH_ENTRY
    ...
    LEFT JOIN (SELECT
        ...
        ON SEIKYUU.DENPYOU_SYSTEM_ID = T_UR_SH_DETAIL.SYSTEM_ID
            AND SEIKYUU.DENPYOU_SEQ = T_UR_SH_DETAIL.SEQ
            AND SEIKYUU.DETAIL_SYSTEM_ID = T_UR_SH_DETAIL.DETAIL_SYSTEM_ID
            AND SEIKYUU.DENPYOU_NUMBER = T_UR_SH_DETAIL.UR_SH_NUMBER
...
/*END*/
/*IF dto.DateShurui == 2*/
  AND T_UR_SH_ENTRY.URIAGE_DATE >= /*dto.DateFrom*/''
  AND T_UR_SH_ENTRY.URIAGE_DATE <= /*dto.DateTo*/''
/*END*/
/*IF dto.DateShurui == 3*/
  AND CONVERT(date, T_UR_SH_ENTRY.UPDATE_DATE, 111) >= /*dto.DateFrom*/''
  AND CONVERT(date, T_UR_SH_ENTRY.UPDATE_DATE, 111) <= /*dto.DateTo*/''
/*END*/
...
SELECT
    ...
    M_HINMEI.BUNRUI_CD,
    (SELECT BUNRUI_NAME_RYAKU FROM M_BUNRUI WHERE M_BUNRUI.BUNRUI_CD = M_HINMEI.BUNRUI_CD) AS BUNRUI_NAME,
	(ISNULL(T_UR_SH_DETAIL.KINGAKU, 0) + ISNULL(T_UR_SH_DETAIL.HINMEI_KINGAKU, 0)) - (ISNULL(T_UR_SH_DETAIL.TAX_UCHI, 0) + ISNULL(T_UR_SH_DETAIL.HINMEI_TAX_UCHI, 0)) AS PAST_KINGAKU
FROM T_UR_SH_ENTRY
    JOIN T_UR_SH_DETAIL
        ON T_UR_SH_ENTRY.SYSTEM_ID = T_UR_SH_DETAIL.SYSTEM_ID
            AND T_UR_SH_ENTRY.SEQ = T_UR_SH_DETAIL.SEQ
...
/*IF dto.DateShurui == 1*/
  AND T_UR_SH_ENTRY.DENPYOU_DATE >= /*dto.DateFromPast*/''
  AND T_UR_SH_ENTRY.DENPYOU_DATE <= /*dto.DateToPast*/''
/*END*/
/*IF dto.DateShurui == 2*/
  AND T_UR_SH_ENTRY.URIAGE_DATE >= /*dto.DateFromPast*/''
  AND T_UR_SH_ENTRY.URIAGE_DATE <= /*dto.DateToPast*/''
/*END*/
/*IF dto.DateShurui == 3*/
  AND CONVERT(date, T_UR_SH_ENTRY.UPDATE_DATE, 111) >= /*dto.DateFromPast*/''
  AND CONVERT(date, T_UR_SH_ENTRY.UPDATE_DATE, 111) <= /*dto.DateToPast*/''
/*END*/
