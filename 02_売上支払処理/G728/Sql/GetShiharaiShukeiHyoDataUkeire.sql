...
FROM
(SELECT
	T_UKEIRE_ENTRY.SYSTEM_ID,
	T_UKEIRE_ENTRY.SEQ,
	T_UKEIRE_DETAIL.DETAIL_SYSTEM_ID,
    T_UKEIRE_ENTRY.SHIHARAI_TORIHIKI_KBN_CD,
    CASE T_UKEIRE_ENTRY.SHIHARAI_TORIHIKI_KBN_CD WHEN 1 THEN '現金' WHEN 2 THEN '掛け' ELSE '' END SHIHARAI_TORIHIKI_KBN_NAME,
...
FROM T_UKEIRE_ENTRY
    ...
    LEFT JOIN (SELECT
        ...
        ON SEISAN.DENPYOU_SYSTEM_ID = T_UKEIRE_DETAIL.SYSTEM_ID
            AND SEISAN.DENPYOU_SEQ = T_UKEIRE_DETAIL.SEQ
            AND SEISAN.DETAIL_SYSTEM_ID = T_UKEIRE_DETAIL.DETAIL_SYSTEM_ID
            AND SEISAN.DENPYOU_NUMBER = T_UKEIRE_DETAIL.UKEIRE_NUMBER
WHERE T_UKEIRE_ENTRY.DELETE_FLG = 0
  AND T_UKEIRE_ENTRY.TAIRYUU_KBN = 0
  AND T_UKEIRE_DETAIL.DENPYOU_KBN_CD = 2
...
/*IF dto.TorihikiKbn != 3*/
  AND T_UKEIRE_ENTRY.SHIHARAI_TORIHIKI_KBN_CD = /*dto.TorihikiKbn*/1
/*END*/
...
SELECT
    ...
    M_HINMEI.BUNRUI_CD,
    (SELECT BUNRUI_NAME_RYAKU FROM M_BUNRUI WHERE M_BUNRUI.BUNRUI_CD = M_HINMEI.BUNRUI_CD) AS BUNRUI_NAME,
	(ISNULL(T_UKEIRE_DETAIL.KINGAKU, 0) + ISNULL(T_UKEIRE_DETAIL.HINMEI_KINGAKU, 0)) - (ISNULL(T_UKEIRE_DETAIL.TAX_UCHI, 0) + ISNULL(T_UKEIRE_DETAIL.HINMEI_TAX_UCHI, 0)) AS PAST_KINGAKU
FROM T_UKEIRE_ENTRY
    JOIN T_UKEIRE_DETAIL
        ON T_UKEIRE_ENTRY.SYSTEM_ID = T_UKEIRE_DETAIL.SYSTEM_ID
            AND T_UKEIRE_ENTRY.SEQ = T_UKEIRE_DETAIL.SEQ
...
/*IF dto.DateShurui == 1*/
  AND T_UKEIRE_ENTRY.DENPYOU_DATE >= /*dto.DateFromPast*/''
  AND T_UKEIRE_ENTRY.DENPYOU_DATE <= /*dto.DateToPast*/''
/*END*/
/*IF dto.DateShurui == 2*/
  AND T_UKEIRE_ENTRY.URIAGE_DATE >= /*dto.DateFromPast*/''
  AND T_UKEIRE_ENTRY.URIAGE_DATE <= /*dto.DateToPast*/''
/*END*/
/*IF dto.DateShurui == 3*/
  AND CONVERT(date, T_UKEIRE_ENTRY.UPDATE_DATE, 111) >= /*dto.DateFromPast*/''
  AND CONVERT(date, T_UKEIRE_ENTRY.UPDATE_DATE, 111) <= /*dto.DateToPast*/''
/*END*/
