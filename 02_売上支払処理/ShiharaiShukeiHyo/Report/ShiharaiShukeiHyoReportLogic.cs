...
namespace Shougun.Core.SalesPayment.ShiharaiShukeiHyo
{
    ...
    /// 支払集計表帳票出力ロジッククラス
    /// </summary>
    internal class ShiharaiShukeiHyoReportLogic
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        internal ShiharaiShukeiHyoReportLogic()
        {
        ...
        internal void CreateReport(DataTable dt, ShiharaiShukeiHyoDto dto)
        {
            LogUtility.DebugMethodStart(dt, dto);

            ReportInfoBase reportInfo = new ReportInfoBase(dt);
            reportInfo.Create(ShiharaiShukeiHyoConst.FORM_FILE, ShiharaiShukeiHyoConst.LAYOUT, dt);
            reportInfo.Title = "支払集計表(" + dto.Pattern.PATTERN_NAME + ")";
