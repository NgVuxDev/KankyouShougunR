...
{
    /// <summary>
    /// 受入or出荷明細表作成クラス（伝票日付順）
    /// </summary>
    internal class UkeireShukkaMeisaihyouDenpyouDateReportClass
    {
        /// <summary>
        /// システム設定エンティティ
        /// </summary>
        ...
        /// 帳票テンプレートのパス
        /// </summary>
        private static readonly string OutputFormFullPathName = "./Template/R571_R575DenpyouDate-Form.xml";
        ...
        /// デフォルトコンストラクタ
        /// </summary>
        internal UkeireShukkaMeisaihyouDenpyouDateReportClass()
        {
        ...
        private DataTable EditChouhyouDataTable(DataTable dt, UkeireShukkaMeisaihyouDtoClass dto)
        {
            ...
            // 伝票日付合計（売上金額）
            dt.Columns.Add("GF_DENPYOU_DATE_URIAGE_KINGAKU_VLB");
            // 伝票日付合計（支払金額）
            dt.Columns.Add("GF_DENPYOU_DATE_SHIHARAI_KINGAKU_VLB");
            // 総合計（正味）
            ...
            decimal denpyouDateUriageKingaku = 0;
            decimal denpyouDateShiharaiKingaku = 0;
            decimal shoumi = 0;
            ...
            foreach (DataRow row in ret.Rows)
            {
                ...
                {
                    denpyouDateShoumi = 0;
                    denpyouDateUriageKingaku = 0;
                    denpyouDateShiharaiKingaku = 0;
                    denpyouDate = row["DENPYOU_DATE"].ToString();
                }
                denpyouDateShoumi += ReportInfo.ConvertNullOrEmptyToZero(row["NET_JYUURYOU"]);
                if ("1" == row["DENPYOU_KBN_CD"].ToString())
                {
                    denpyouDateUriageKingaku += ReportInfo.ConvertNullOrEmptyToZero(row["ZEINUKI_KINGAKU"]);
                }
                else if ("2" == row["DENPYOU_KBN_CD"].ToString())
                {
                    denpyouDateShiharaiKingaku += ReportInfo.ConvertNullOrEmptyToZero(row["ZEINUKI_KINGAKU"]);
                }
                row["GF_DENPYOU_DATE_URIAGE_KINGAKU_VLB"] = denpyouDateUriageKingaku;
                row["GF_DENPYOU_DATE_SHIHARAI_KINGAKU_VLB"] = denpyouDateShiharaiKingaku;
                ...
                row["GF_DENPYOU_DATE_URIAGE_KINGAKU_VLB"] = Convert.ToDecimal(row["GF_DENPYOU_DATE_URIAGE_KINGAKU_VLB"]).ToString("#,##0");
                row["GF_DENPYOU_DATE_SHIHARAI_KINGAKU_VLB"] = Convert.ToDecimal(row["GF_DENPYOU_DATE_SHIHARAI_KINGAKU_VLB"]).ToString("#,##0");
