...
{
    /// <summary>
    /// 受入or出荷明細表作成クラス（業者CD順 or 業者フリガナ順）
    /// </summary>
    internal class UkeireShukkaMeisaihyouGyoushaReportClass
    {
        /// <summary>
        /// システム設定エンティティ
        /// </summary>
        ...
        /// 帳票テンプレートのパス
        /// </summary>
        private static readonly string OutputFormFullPathName = "./Template/R571_R575Gyousha-Form.xml";
        ...
        /// デフォルトコンストラクタ
        /// </summary>
        internal UkeireShukkaMeisaihyouGyoushaReportClass()
        {
        ...
        internal void CreateReport(DataTable dt, UkeireShukkaMeisaihyouDtoClass dto)
        {
            ...
            reportInfo.SetGroupVisible("GENBA", false, dto.IsGroupGenba);
            reportInfo.SetGroupVisible("GENBA_NAME", false, dto.IsGroupGenba);
            reportInfo.SetGroupVisible("UNPAN_GYOUSHA", true, false);
            reportInfo.SetGroupVisible("UNPAN_GYOUSHA_NAME", true, false);
            ...
            if (1 == dto.Order)
            {
                reportPopup.ReportCaption += ConstClass.SORT_GYOUSHA_CD_SUB_TITLE;
            }
            else if (2 == dto.Order)
            {
        ...
        private DataTable EditChouhyouDataTable(DataTable dt, UkeireShukkaMeisaihyouDtoClass dto)
        {
            ...
            // 現場合計（売上金額）
            dt.Columns.Add("GF_GENBA_URIAGE_KINGAKU_VLB");
            // 現場合計（支払金額）
            dt.Columns.Add("GF_GENBA_SHIHARAI_KINGAKU_VLB");
            // 業者合計（正味）
            dt.Columns.Add("GF_GYOUSHA_SHOUMI_VLB");
            // 業者合計（売上金額）
            dt.Columns.Add("GF_GYOUSHA_URIAGE_KINGAKU_VLB");
            // 業者合計（支払金額）
            dt.Columns.Add("GF_GYOUSHA_SHIHARAI_KINGAKU_VLB");
            // 総合計（正味）
            ...
            dt.Columns["UNPAN_GYOUSHA_CD"].MaxLength = 24;
            ...
            decimal genbaUriageKingaku = 0;
            decimal genbaShiharaiKingaku = 0;
            decimal gyoushaShoumi = 0;
            decimal gyoushaUriageKingaku = 0;
            decimal gyoushaShiharaiKingaku = 0;
            decimal shoumi = 0;
            ...
            string unpanGenbaCd = ret.Rows[0]["UNPAN_GYOUSHA_CD"].ToString();
            string gyoushaName = ret.Rows[0]["GYOUSHA_NAME"].ToString();
            string genbaName = ret.Rows[0]["GENBA_NAME"].ToString();
            string unpanGyoushaName = ret.Rows[0]["UNPAN_GYOUSHA_NAME"].ToString();
            ...
            foreach (DataRow row in ret.Rows)
            {
                ...
                if (1 == dto.Order)
                {
                    row["FH_TITLE_VLB"] += ConstClass.SORT_GYOUSHA_CD_SUB_TITLE;
                }
                else if (2 == dto.Order)
                {
                ...
                // 集計（業者）
                if (torihikisakiCd != row["TORIHIKISAKI_CD"].ToString() || (gyoushaCd != row["GYOUSHA_CD"].ToString() || gyoushaName != row["GYOUSHA_NAME"].ToString()))
                {
                    genbaShoumi = 0;
                    genbaUriageKingaku = 0;
                    genbaShiharaiKingaku = 0;
                    genbaCd = row["GENBA_CD"].ToString();

                    gyoushaShoumi = 0;
                    gyoushaUriageKingaku = 0;
                    gyoushaShiharaiKingaku = 0;
                ...
                {
                    gyoushaUriageKingaku += ReportInfo.ConvertNullOrEmptyToZero(row["ZEINUKI_KINGAKU"]);
                }
                else if ("2" == row["DENPYOU_KBN_CD"].ToString())
                {
                    gyoushaShiharaiKingaku += ReportInfo.ConvertNullOrEmptyToZero(row["ZEINUKI_KINGAKU"]);
                }
                row["GF_GYOUSHA_URIAGE_KINGAKU_VLB"] = gyoushaUriageKingaku;
                row["GF_GYOUSHA_SHIHARAI_KINGAKU_VLB"] = gyoushaShiharaiKingaku;
                row["GF_GYOUSHA_SHOUMI_VLB"] = gyoushaShoumi;

                // 集計（現場）
                if ((genbaCd != row["GENBA_CD"].ToString() || genbaName != row["GENBA_NAME"].ToString()) || (gyoushaCd != row["GYOUSHA_CD"].ToString() || gyoushaName != row["GYOUSHA_NAME"].ToString()))
                {
                    genbaShoumi = 0;
                    genbaUriageKingaku = 0;
                    genbaShiharaiKingaku = 0;
                ...
                if ("1" == row["DENPYOU_KBN_CD"].ToString())
                {
                    genbaUriageKingaku += ReportInfo.ConvertNullOrEmptyToZero(row["ZEINUKI_KINGAKU"]);
                }
                else if ("2" == row["DENPYOU_KBN_CD"].ToString())
                {
                    genbaShiharaiKingaku += ReportInfo.ConvertNullOrEmptyToZero(row["ZEINUKI_KINGAKU"]);
                }
                row["GF_GENBA_URIAGE_KINGAKU_VLB"] = genbaUriageKingaku;
                row["GF_GENBA_SHIHARAI_KINGAKU_VLB"] = genbaShiharaiKingaku;
                ...
                row["GF_GYOUSHA_URIAGE_KINGAKU_VLB"] = Convert.ToDecimal(row["GF_GYOUSHA_URIAGE_KINGAKU_VLB"]).ToString("#,##0");
                row["GF_GYOUSHA_SHIHARAI_KINGAKU_VLB"] = Convert.ToDecimal(row["GF_GYOUSHA_SHIHARAI_KINGAKU_VLB"]).ToString("#,##0");
                row["GF_GENBA_URIAGE_KINGAKU_VLB"] = Convert.ToDecimal(row["GF_GENBA_URIAGE_KINGAKU_VLB"]).ToString("#,##0");
                row["GF_GENBA_SHIHARAI_KINGAKU_VLB"] = Convert.ToDecimal(row["GF_GENBA_SHIHARAI_KINGAKU_VLB"]).ToString("#,##0");
                ...
                // 集計ヘッダ表示用に業者CD,現場CDを編集
                row["UNPAN_GYOUSHA_CD"] = String.Format("{0,6}", row["TORIHIKISAKI_CD"]) + String.Format("{0,6}", row["GYOUSHA_CD"]) + String.Format("{0,6}", row["GENBA_CD"]) + String.Format("{0,6}", row["UNPAN_GYOUSHA_CD"]);
