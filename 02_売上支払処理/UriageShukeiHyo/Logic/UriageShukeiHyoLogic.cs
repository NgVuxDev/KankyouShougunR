...
namespace Shougun.Core.SalesPayment.UriageShukeiHyo
{
    ...
    /// 売上集計表ロジッククラス
    /// </summary>
    internal class UriageShukeiHyoLogic : IBuisinessLogic
    {
        /// <summary>
        /// 売上集計表画面クラス
        /// </summary>
        private UriageShukeiHyoUIForm form;
        ...
        /// 集計データを取得・設定します
        /// </summary>
        internal List<UriageData> ShukeiDataList { get; private set; }

        /// <summary>
        /// 集計したデータを取得・設定します
        /// </summary>
        internal List<UriageData> ShukeiDataSummaryList { get; private set; }

        /// <summary>
        /// 集計表帳票出力用DTOリストを取得・設定します
        /// </summary>
        internal List<UriageShukeiHyoReportDto> ShukeiHyoReportDtoList { get; private set; }
        ...
        /// コンストラクタ
        /// </summary>
        /// <param name="targetForm">売上集計表画面クラス</param>
        public UriageShukeiHyoLogic(UriageShukeiHyoUIForm targetForm)
        {
        ...
        private void EventInit()
        {
            ...
            /// 20141226 Houkakou 「売上集計表」のダブルクリックを追加する　start
            // 「To」のイベント生成
            ...
            /// 20141226 Houkakou 「売上集計表」のダブルクリックを追加する　end
            ...
            this.form.SHUUKEI_KOUMOKU_CD_TO.MouseDoubleClick += new MouseEventHandler(SHUUKEI_KOUMOKU_CD_TO_MouseDoubleClick); //PhuocLoc 2020/12/08 #136223          
        ...
        public int Search()
        {
            ...
            try
            {
                var dao = DaoInitUtility.GetComponent<UriageShukeiHyoDao>();

                this.ShukeiDataList = new List<UriageData>();

                // 荷降が指定されたら受入・売上／支払が対象
                // 荷積が指定されたら出荷・売上／支払が対象
                // 台貫が指定されたら受入・出荷が対象

                if (this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_UKEIRE || this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_SUBETE)
                {
                ...
                if (this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_SHUKKA || this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_SUBETE)
                {
                ...
                if (this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_URIAGE_SHIHARAI ||
                    this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_DAINOU ||
                    this.form.FormDataDto.DenpyouShurui.ToString() == UriageShukeiHyoConst.DENPYOU_SHURUI_CD_SUBETE)
                {
                ...
                this.ShukeiDataList.ForEach(u =>
                {
                    ...
                    u.URIAGE_DATE = FormatDate(u.URIAGE_DATE);
        ...
        internal bool CSVPrint()
        {
            try
            {
                ...
                {
                    var reportLogic = new UriageShukeiHyoReportLogic();
        ...
        public void Creat_CSV(DataTable dt)
        {
            try
            {
                ...
                head_dt += ",FORMAT_KINGAKU";//VAN 20200330 #134973
                ...
                if (this.errmessage.MessageBoxShow("C013") == DialogResult.Yes)
                {
                    ...
                    // CSV出力
                    csvExport.ConvertDataTableToCsv(csvDT, true, true, "売上集計表", this.form);
                }
            }
            catch (Exception ex)
            {
        ...
        private void CreateReportDtoList()
        {
            ...
            if (this.form.FormDataDto.Pattern.Pattern.HINMEI_DISP_KBN.IsTrue)//VAN 20200330 #134973
            {
                column5 = "品名";
                cd5 = "HINMEI_CD";
                name5 = "HINMEI_NAME";
            ...
            this.ShukeiHyoReportDtoList = this.ShukeiDataList.Select(u => new UriageShukeiHyoReportDto()
                                                             {
                                                                 COLUMN_1 = column1,
                                                                 COLUMN_2 = column2,
                                                                 COLUMN_3 = column3,
                                                                 ...
                                                                 //VAN 20200330 #134973 S
                                                                 NET_JYUURYOU = this.form.FormDataDto.Pattern.Pattern.NET_JYUURYOU_DISP_KBN.IsTrue ? this.ConvertToDecimal(this.GetValue(u, "NET_JYUURYOU")) : 0,
                                                                 SUURYOU = this.form.FormDataDto.Pattern.Pattern.SUURYOU_UNIT_DISP_KBN.IsTrue ? this.ConvertToDecimal(this.GetValue(u, "SUURYOU")) : 0,
                                                                 UNIT_CD = this.form.FormDataDto.Pattern.Pattern.SUURYOU_UNIT_DISP_KBN.IsTrue ? this.ConvertToString(this.GetValue(u, "UNIT_CD")) : string.Empty,
                                                                 UNIT_NAME = this.form.FormDataDto.Pattern.Pattern.SUURYOU_UNIT_DISP_KBN.IsTrue ? this.ConvertToString(this.GetValue(u, "UNIT_NAME")) : string.Empty,
                                                                 //VAN 20200330 #134973 E
        ...
        private void CreateSummaryData()
        {
            ...
            this.ShukeiHyoReportDtoList = this.ShukeiHyoReportDtoList.GroupBy(u => new { u.CD_1, u.CD_2, u.CD_3, u.CD_4, u.CD_5, u.UNIT_CD })
                                                                     .Select(u => new UriageShukeiHyoReportDto()
                                                                     {
        ...
        {
			string dtIssue = this.getDBDateTime().ToString("yyyy/MM/dd HH:mm:ss") + " 発行";
            this.ShukeiHyoReportDtoList.ForEach(u =>
            {
                u.TITLE = "売上集計表（" + this.form.FormDataDto.Pattern.PATTERN_NAME + "）";
            ...
            jouken1.Append(this.form.FormDataDto.ShuukeiKoumoku); //PhuocLoc 2020/12/08 #136223
        ...
        private ButtonSetting[] CreateButtonInfo()
        {
            try
            {
                ...
                return buttonSetting.LoadButtonSetting(thisAssembly, UriageShukeiHyoConst.BUTTON_SETTING_XML);
            }
            catch (Exception e)
            {
                LogUtility.Error(e.Message, e);
