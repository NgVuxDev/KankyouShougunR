...
using Shougun.Core.SalesPayment.Uriagekakutenyuryoku.DAO;
...
namespace Shougun.Core.SalesPayment.Uriagekakutenyuryoku
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// 伝票区分（売上）
        /// </summary>
        private const String DENPYOU_KBN_URIAGE = "1";
        /// <summary>
        /// DB確定区分（確定）
        /// </summary>
        ...
        /// ボタン設定格納ファイル
        /// </summary>
        private readonly string ButtonInfoXmlPath = "Shougun.Core.SalesPayment.Uriagekakutenyuryoku.Setting.ButtonSetting.xml";

        ///// <summary>
        ///// 番採番号
        ///// </summary>
        //Dictionary<string, int> saiban;

        /// <summary>
        /// 検索結果一覧のDao
        /// </summary>
        //private DAOClass daoNyuuSyutuKinIchiranIchiran;
        ...
        /// </summary>
        private UriagekakutenyuryokuForm form;


        /// <summary>
        /// List<T_UKEIRE_DETAIL> mukeiremslist
        /// </summary>
        private List<T_UKEIRE_DETAIL> mukeiremslist;


        /// <summary>
        /// List<T_UKEIRE_ENTRY> mukeireentrylistadd
        /// </summary>
        private List<T_UKEIRE_ENTRY> mukeireentrylistadd;

        /// <summary>
        /// List<T_UKEIRE_ENTRY> mukeireentrylistupdata
        /// </summary>
        private List<T_UKEIRE_ENTRY> mukeireentrylistupdata;

        /// <summary>
        /// List<T_UKEIRE_DETAIL> mshukamslist
        /// </summary>
        ...
        /// <summary>
        /// List<T_UKEIRE_ENTRY> mshukaentrylistadd
        /// </summary>
        private List<T_SHUKKA_ENTRY> mshukaentrylistadd;

        /// <summary>
        /// List<T_UKEIRE_ENTRY> mshukaentrylistupdata
        /// </summary>
        private List<T_SHUKKA_ENTRY> mshukaentrylistupdata;


        /// <summary>
        /// List<T_UR_SH_DETAIL> murshmslist
        /// </summary>
        ...
        /// <summary>
        /// List<T_UR_SH_ENTRY> murshentrylistadd
        /// </summary>
        private List<T_UR_SH_ENTRY> murshentrylistadd;

        /// <summary>
        /// List<T_UR_SH_ENTRY> murshentrylistupdata
        /// </summary>
        ...
        /// コンストラクタ
        /// </summary>
        public LogicClass(UriagekakutenyuryokuForm targetForm)
        {
            ...
            this.isRegist = true;
            //BusinessBaseForm bf = (BusinessBaseForm)targetForm.Parent;
            //IchiranHeaderForm1 ichrHeadF = (IchiranHeaderForm1)bf.Parent;
            //string txt = ichrHeadF.HEADER_KYOTEN_CD.Text;
        ...
        private void EventInit()
        {
            ...
            //Functionボタンのイベント生成
            parentForm.bt_func1.Click += new EventHandler(this.bt_func1_Click);              //確定解除 
            ...
            parentForm.bt_func9.Click += new System.EventHandler(this.bt_func9_Click);     //確定登録
            //並び替えボタン(F10)イベント生成
            ...
            /// 20141203 Houkakou 「売上確定」の日付チェックを追加する　start
            ...
            /// 20141203 Houkakou 「売上確定」の日付チェックを追加する　end
        ...
        private void bt_func1_Click(object sender, EventArgs e)
        {
            ...
            if (dbkoshin != null)
            {
                if (dbkoshin.Rows.Count > 0)
                {
                    ...
                    /// 20141203 Houkakou 「売上確定」の日付チェックを追加する　end
                    PasswordPopupForm pwform = new PasswordPopupForm(this.msysinfo.URIAGE_KAKUTEI_KAIJO_PASSWORD, "1");
        ...
        private void bt_func7_Click(object sender, EventArgs e)
        {
            ...
            this.form.CheckBox_Jyunyu.Checked = false;
            ...
            this.form.radbtn_Subete.Checked = true;
        ...
        {
            //LogUtility.DebugMethodStart();
            if (this.form.CheckBox_Jyunyu.Checked || this.form.CheckBox_Syuka.Checked
                || this.form.CheckBox_Uriageshiharai.Checked)
            {
                /// 20141203 Houkakou 「売上確定」の日付チェックを追加する　start
                if (this.DateCheck())
                {
        ...
        private void bt_func12_Click(object sender, EventArgs e)
        {
            ...
            {
                Properties.Settings.Default.SET_HIDUKE_FROM = DateTime.Parse(this.headForm.HIDUKE_FROM.Value.ToString()).ToShortDateString();          //伝票日付From
            }
            else
            {
            ...
            {
                Properties.Settings.Default.SET_HIDUKE_TO = DateTime.Parse(this.headForm.HIDUKE_TO.Value.ToString()).ToShortDateString();              //伝票日付To
            }
            else
            {
            ...
            Properties.Settings.Default.SET_HIDUKE_KBN = this.headForm.txtNum_HidukeSentaku.Text;   //日付区分
        ...
        private int SaiBan(string tablename, string systemid)
        {
            ...
            searchSaiban = daoIchiran.getdateforstringsql(sql.ToString());
            ...
            if (kubun == "TOUROKU")
            {
                ...
                foreach (DataRow dgvRow in ukeireDt.Rows)
                {
                       ...
                       && !dgvRow["明細・確定区分"].ToString().Equals(dgvRow["比較確定区分"].ToString()))
                    {
                        ...
                        // システムIDか枝番が変わったら別データ
                        if (!dgvRow["システムID"].ToString().Equals(syetemidukeiri) || !dgvRow["枝番"].ToString().Equals(sequkeiri))
                        {
                            ...
                            if (!dgvRow["システムID"].ToString().Equals(syetemidukeiri))
                            {
                                // システムIDが変わったら採番する
                                saiban = SaiBan("T_UKEIRE_ENTRY", dgvRow["システムID"].ToString()) + 1;
                            }
                            else if (dgvRow["システムID"].ToString().Equals(syetemidukeiri) && !dgvRow["枝番"].ToString().Equals(sequkeiri))
                            {
                                // 枝番だけ変わったら＋１でセットする
                                saiban = saiban + 1;
                            }
                            ...
                            foreach (DataRow dr in dbEntry.Rows)
                            {
                                ...
                                ukeireEntryAdd.CREATE_USER = dr["CREATE_USER"].ToString();
                                ukeireEntryAdd.CREATE_DATE = SqlDateTime.Parse(dr["CREATE_DATE"].ToString());
                                ukeireEntryAdd.CREATE_PC = dr["CREATE_PC"].ToString();
                            ...
                            foreach (DataRow dr in dbDetail.Rows)
                            {
                                ...
                                if (this.msysinfo.SYS_KAKUTEI__TANNI_KBN.Equals(SqlInt16.Parse("1")))
                                {
                                    ...
                                    if (DENPYOU_KBN_URIAGE.Equals(dr["DENPYOU_KBN_CD"].ToString()))
                                    {
                                        // 伝票区分が売上の場合
                        ...
                        foreach (T_UKEIRE_DETAIL item in ukeireMsList)
                        {
                            ...
                            {
                                item.KAKUTEI_KBN = SqlInt16.Parse("1");
                        ...
                        row++;
                        syetemidukeiri = dgvRow["システムID"].ToString();
                        sequkeiri = dgvRow["枝番"].ToString();
                    }
                }
                    ...
                    else
                    {
                        ...
                        foreach (T_UKEIRE_ENTRY itemEntry in ukeireEntryListAdd)
                        {
                            if (SqlInt64.Parse(syetemid) == itemEntry.SYSTEM_ID && SqlInt64.Parse(seq) == itemEntry.SEQ)
                            {
                                if (flag == true)
                                {
                                    itemEntry.KAKUTEI_KBN = SqlInt16.Parse("2");
                                }
                                else if (flag == false)
                                {
                                    itemEntry.KAKUTEI_KBN = SqlInt16.Parse("1");
                                }
                            }
                ...
                foreach (DataRow dgvRow in ukeireDt.Rows)
                {
                        ...
                        && !dgvRow["明細・確定区分"].ToString().Equals(dgvRow["比較確定区分"].ToString()))
                    {
                        ...
                        foreach (T_UKEIRE_DETAIL item in ukeireMsList)
                        {
                            ...
                            && item.DETAIL_SYSTEM_ID.ToString().Equals(dgvRow["明細・システムID"].ToString()))
                            {
                                item.KAKUTEI_KBN = SqlInt16.Parse("2");
                                //item.SEQ = SqlInt16.Parse(saiban.ToString());
                            }
            ...
            if (kubun == "TOUROKU")
            {
                ...
                foreach (DataRow dgvRow in shukaDt.Rows)
                {
                        ...
                        && !dgvRow["明細・確定区分"].ToString().Equals(dgvRow["比較確定区分"].ToString()))
                    {
                        DataTable dbDetail = new DataTable();
                        DataTable dbEntry = new DataTable();
                        T_SHUKKA_DETAIL shukaMs = new T_SHUKKA_DETAIL();
                        T_SHUKKA_ENTRY shukaEntryAdd = new T_SHUKKA_ENTRY();
                        T_SHUKKA_ENTRY shukaEntryUpdata = new T_SHUKKA_ENTRY();
                        // WHOカラム設定共通ロジック呼び出し用
                        var dataBind_shukkaMs = new DataBinderLogic<T_SHUKKA_DETAIL>(shukaMs);
                        var dataBind_shukkaEntryAdd = new DataBinderLogic<T_SHUKKA_ENTRY>(shukaEntryAdd);
                        var dataBind_shukkaEntryUpdata = new DataBinderLogic<T_SHUKKA_ENTRY>(shukaEntryUpdata);
                        ...
                        if (!dgvRow["システムID"].ToString().Equals(syetemidukeiri) || !dgvRow["枝番"].ToString().Equals(sequkeiri))
                        {
                            ...
                            {
                                // 論理削除用データセット
                                shukaEntryUpdata = new T_SHUKKA_ENTRY();
                                if (!String.IsNullOrEmpty(dr["SYSTEM_ID"].ToString()))
                                {
                                    shukaEntryUpdata.SYSTEM_ID = SqlInt64.Parse(dr["SYSTEM_ID"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SEQ"].ToString()))
                                {
                                    shukaEntryUpdata.SEQ = SqlInt32.Parse(dr["SEQ"].ToString());
                                }
                                //**********
                                if (!String.IsNullOrEmpty(dr["TAIRYUU_KBN"].ToString()))
                                {
                                    shukaEntryUpdata.TAIRYUU_KBN = SqlBoolean.Parse(dr["TAIRYUU_KBN"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KYOTEN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.KYOTEN_CD = SqlInt16.Parse(dr["KYOTEN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHUKKA_NUMBER"].ToString()))
                                {
                                    shukaEntryUpdata.SHUKKA_NUMBER = SqlInt64.Parse(dr["SHUKKA_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DATE_NUMBER"].ToString()))
                                {
                                    shukaEntryUpdata.DATE_NUMBER = SqlInt32.Parse(dr["DATE_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["YEAR_NUMBER"].ToString()))
                                {
                                    shukaEntryUpdata.YEAR_NUMBER = SqlInt32.Parse(dr["YEAR_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KAKUTEI_KBN"].ToString()))
                                {
                                    shukaEntryUpdata.KAKUTEI_KBN = SqlInt16.Parse(dr["KAKUTEI_KBN"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DENPYOU_DATE"].ToString()))
                                {
                                    shukaEntryUpdata.DENPYOU_DATE = SqlDateTime.Parse(dr["DENPYOU_DATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_DATE"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_DATE = SqlDateTime.Parse(dr["URIAGE_DATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_DATE"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_DATE = SqlDateTime.Parse(dr["SHIHARAI_DATE"].ToString());
                                }
                                shukaEntryUpdata.TORIHIKISAKI_CD = dr["TORIHIKISAKI_CD"].ToString();
                                shukaEntryUpdata.TORIHIKISAKI_NAME = dr["TORIHIKISAKI_NAME"].ToString();
                                shukaEntryUpdata.GYOUSHA_CD = dr["GYOUSHA_CD"].ToString();
                                shukaEntryUpdata.GYOUSHA_NAME = dr["GYOUSHA_NAME"].ToString();
                                shukaEntryUpdata.GENBA_CD = dr["GENBA_CD"].ToString();
                                shukaEntryUpdata.GENBA_NAME = dr["GENBA_NAME"].ToString();
                                shukaEntryUpdata.NIZUMI_GYOUSHA_CD = dr["NIZUMI_GYOUSHA_CD"].ToString();
                                shukaEntryUpdata.NIZUMI_GYOUSHA_NAME = dr["NIZUMI_GYOUSHA_NAME"].ToString();
                                shukaEntryUpdata.NIZUMI_GENBA_CD = dr["NIZUMI_GENBA_CD"].ToString();
                                shukaEntryUpdata.NIZUMI_GENBA_NAME = dr["NIZUMI_GENBA_NAME"].ToString();
                                shukaEntryUpdata.EIGYOU_TANTOUSHA_CD = dr["EIGYOU_TANTOUSHA_CD"].ToString();
                                shukaEntryUpdata.EIGYOU_TANTOUSHA_NAME = dr["EIGYOU_TANTOUSHA_NAME"].ToString();
                                shukaEntryUpdata.NYUURYOKU_TANTOUSHA_CD = dr["NYUURYOKU_TANTOUSHA_CD"].ToString();
                                shukaEntryUpdata.NYUURYOKU_TANTOUSHA_NAME = dr["NYUURYOKU_TANTOUSHA_NAME"].ToString();
                                shukaEntryUpdata.SHARYOU_CD = dr["SHARYOU_CD"].ToString();
                                shukaEntryUpdata.SHARYOU_NAME = dr["SHARYOU_NAME"].ToString();
                                shukaEntryUpdata.SHASHU_CD = dr["SHASHU_CD"].ToString();
                                shukaEntryUpdata.SHASHU_NAME = dr["SHASHU_NAME"].ToString();
                                shukaEntryUpdata.UNPAN_GYOUSHA_CD = dr["UNPAN_GYOUSHA_CD"].ToString();
                                shukaEntryUpdata.UNPAN_GYOUSHA_NAME = dr["UNPAN_GYOUSHA_NAME"].ToString();
                                shukaEntryUpdata.UNTENSHA_CD = dr["UNTENSHA_CD"].ToString();
                                shukaEntryUpdata.UNTENSHA_NAME = dr["UNTENSHA_NAME"].ToString();
                                if (!String.IsNullOrEmpty(dr["NINZUU_CNT"].ToString()))
                                {
                                    shukaEntryUpdata.NINZUU_CNT = SqlInt16.Parse(dr["NINZUU_CNT"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KEITAI_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.KEITAI_KBN_CD = SqlInt16.Parse(dr["KEITAI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DAIKAN_KBN"].ToString()))
                                {
                                    shukaEntryUpdata.DAIKAN_KBN = SqlInt16.Parse(dr["DAIKAN_KBN"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["CONTENA_SOUSA_CD"].ToString()))
                                {
                                    shukaEntryUpdata.CONTENA_SOUSA_CD = SqlInt16.Parse(dr["CONTENA_SOUSA_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["MANIFEST_SHURUI_CD"].ToString()))
                                {
                                    shukaEntryUpdata.MANIFEST_SHURUI_CD = SqlInt16.Parse(dr["MANIFEST_SHURUI_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["MANIFEST_TEHAI_CD"].ToString()))
                                {
                                    shukaEntryUpdata.MANIFEST_TEHAI_CD = SqlInt16.Parse(dr["MANIFEST_TEHAI_CD"].ToString());
                                }
                                shukaEntryUpdata.DENPYOU_BIKOU = dr["DENPYOU_BIKOU"].ToString();
                                shukaEntryUpdata.TAIRYUU_BIKOU = dr["TAIRYUU_BIKOU"].ToString();
                                if (!String.IsNullOrEmpty(dr["UKETSUKE_NUMBER"].ToString()))
                                {
                                    shukaEntryUpdata.UKETSUKE_NUMBER = SqlInt64.Parse(dr["UKETSUKE_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KEIRYOU_NUMBER"].ToString()))
                                {
                                    shukaEntryUpdata.KEIRYOU_NUMBER = SqlInt64.Parse(dr["KEIRYOU_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["RECEIPT_NUMBER"].ToString()))
                                {
                                    shukaEntryUpdata.RECEIPT_NUMBER = SqlInt32.Parse(dr["RECEIPT_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["NET_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.NET_TOTAL = SqlDecimal.Parse(dr["NET_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_SHOUHIZEI_RATE"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_SHOUHIZEI_RATE = SqlDecimal.Parse(dr["URIAGE_SHOUHIZEI_RATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_AMOUNT_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_AMOUNT_TOTAL = SqlDecimal.Parse(dr["URIAGE_AMOUNT_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_SOTO"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_TAX_SOTO = SqlDecimal.Parse(dr["URIAGE_TAX_SOTO"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_UCHI"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_TAX_UCHI = SqlDecimal.Parse(dr["URIAGE_TAX_UCHI"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["URIAGE_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["URIAGE_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_URIAGE_KINGAKU_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.HINMEI_URIAGE_KINGAKU_TOTAL = SqlDecimal.Parse(dr["HINMEI_URIAGE_KINGAKU_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_URIAGE_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.HINMEI_URIAGE_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["HINMEI_URIAGE_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_URIAGE_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.HINMEI_URIAGE_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["HINMEI_URIAGE_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_SHOUHIZEI_RATE"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_SHOUHIZEI_RATE = SqlDecimal.Parse(dr["SHIHARAI_SHOUHIZEI_RATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_AMOUNT_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_AMOUNT_TOTAL = SqlDecimal.Parse(dr["SHIHARAI_AMOUNT_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_SOTO"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_TAX_SOTO = SqlDecimal.Parse(dr["SHIHARAI_TAX_SOTO"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_UCHI"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_TAX_UCHI = SqlDecimal.Parse(dr["SHIHARAI_TAX_UCHI"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["SHIHARAI_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["SHIHARAI_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_SHIHARAI_KINGAKU_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.HINMEI_SHIHARAI_KINGAKU_TOTAL = SqlDecimal.Parse(dr["HINMEI_SHIHARAI_KINGAKU_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_SHIHARAI_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.HINMEI_SHIHARAI_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["HINMEI_SHIHARAI_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_SHIHARAI_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryUpdata.HINMEI_SHIHARAI_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["HINMEI_SHIHARAI_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_ZEI_KEISAN_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_ZEI_KEISAN_KBN_CD = SqlInt16.Parse(dr["URIAGE_ZEI_KEISAN_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_ZEI_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_ZEI_KBN_CD = SqlInt16.Parse(dr["URIAGE_ZEI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TORIHIKI_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.URIAGE_TORIHIKI_KBN_CD = SqlInt16.Parse(dr["URIAGE_TORIHIKI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_ZEI_KEISAN_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_ZEI_KEISAN_KBN_CD = SqlInt16.Parse(dr["SHIHARAI_ZEI_KEISAN_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_ZEI_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_ZEI_KBN_CD = SqlInt16.Parse(dr["SHIHARAI_ZEI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TORIHIKI_KBN_CD"].ToString()))
                                {
                                    shukaEntryUpdata.SHIHARAI_TORIHIKI_KBN_CD = SqlInt16.Parse(dr["SHIHARAI_TORIHIKI_KBN_CD"].ToString());
                                }
                                shukaEntryUpdata.TIME_STAMP = (byte[])(dr["TIME_STAMP"]);

                                //**********
                                // WHOカラム設定
                                dataBind_shukkaEntryUpdata.SetSystemProperty(shukaEntryUpdata, false);
                                shukaEntryUpdata.DELETE_FLG = SqlBoolean.True;

                                // 新規登録用データセット
                                shukaEntryAdd = new T_SHUKKA_ENTRY();
                                if (!String.IsNullOrEmpty(dr["SYSTEM_ID"].ToString()))
                                {
                                    shukaEntryAdd.SYSTEM_ID = SqlInt64.Parse(dr["SYSTEM_ID"].ToString());
                                }
                                shukaEntryAdd.SEQ = SqlInt32.Parse(saiban.ToString());
                                if (!String.IsNullOrEmpty(dr["TAIRYUU_KBN"].ToString()))
                                {
                                    shukaEntryAdd.TAIRYUU_KBN = SqlBoolean.Parse(dr["TAIRYUU_KBN"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KYOTEN_CD"].ToString()))
                                {
                                    shukaEntryAdd.KYOTEN_CD = SqlInt16.Parse(dr["KYOTEN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHUKKA_NUMBER"].ToString()))
                                {
                                    shukaEntryAdd.SHUKKA_NUMBER = SqlInt64.Parse(dr["SHUKKA_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DATE_NUMBER"].ToString()))
                                {
                                    shukaEntryAdd.DATE_NUMBER = SqlInt32.Parse(dr["DATE_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["YEAR_NUMBER"].ToString()))
                                {
                                    shukaEntryAdd.YEAR_NUMBER = SqlInt32.Parse(dr["YEAR_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KAKUTEI_KBN"].ToString()))
                                {
                                    shukaEntryAdd.KAKUTEI_KBN = SqlInt16.Parse(dr["KAKUTEI_KBN"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DENPYOU_DATE"].ToString()))
                                {
                                    shukaEntryAdd.DENPYOU_DATE = SqlDateTime.Parse(dr["DENPYOU_DATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_DATE"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_DATE = SqlDateTime.Parse(dr["URIAGE_DATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_DATE"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_DATE = SqlDateTime.Parse(dr["SHIHARAI_DATE"].ToString());
                                }
                                shukaEntryAdd.TORIHIKISAKI_CD = dr["TORIHIKISAKI_CD"].ToString();
                                shukaEntryAdd.TORIHIKISAKI_NAME = dr["TORIHIKISAKI_NAME"].ToString();
                                shukaEntryAdd.GYOUSHA_CD = dr["GYOUSHA_CD"].ToString();
                                shukaEntryAdd.GYOUSHA_NAME = dr["GYOUSHA_NAME"].ToString();
                                shukaEntryAdd.GENBA_CD = dr["GENBA_CD"].ToString();
                                shukaEntryAdd.GENBA_NAME = dr["GENBA_NAME"].ToString();
                                shukaEntryAdd.NIZUMI_GYOUSHA_CD = dr["NIZUMI_GYOUSHA_CD"].ToString();
                                shukaEntryAdd.NIZUMI_GYOUSHA_NAME = dr["NIZUMI_GYOUSHA_NAME"].ToString();
                                shukaEntryAdd.NIZUMI_GENBA_CD = dr["NIZUMI_GENBA_CD"].ToString();
                                shukaEntryAdd.NIZUMI_GENBA_NAME = dr["NIZUMI_GENBA_NAME"].ToString();
                                shukaEntryAdd.EIGYOU_TANTOUSHA_CD = dr["EIGYOU_TANTOUSHA_CD"].ToString();
                                shukaEntryAdd.EIGYOU_TANTOUSHA_NAME = dr["EIGYOU_TANTOUSHA_NAME"].ToString();
                                shukaEntryAdd.NYUURYOKU_TANTOUSHA_CD = dr["NYUURYOKU_TANTOUSHA_CD"].ToString();
                                shukaEntryAdd.NYUURYOKU_TANTOUSHA_NAME = dr["NYUURYOKU_TANTOUSHA_NAME"].ToString();
                                shukaEntryAdd.SHARYOU_CD = dr["SHARYOU_CD"].ToString();
                                shukaEntryAdd.SHARYOU_NAME = dr["SHARYOU_NAME"].ToString();
                                shukaEntryAdd.SHASHU_CD = dr["SHASHU_CD"].ToString();
                                shukaEntryAdd.SHASHU_NAME = dr["SHASHU_NAME"].ToString();
                                shukaEntryAdd.UNPAN_GYOUSHA_CD = dr["UNPAN_GYOUSHA_CD"].ToString();
                                shukaEntryAdd.UNPAN_GYOUSHA_NAME = dr["UNPAN_GYOUSHA_NAME"].ToString();
                                shukaEntryAdd.UNTENSHA_CD = dr["UNTENSHA_CD"].ToString();
                                shukaEntryAdd.UNTENSHA_NAME = dr["UNTENSHA_NAME"].ToString();
                                if (!String.IsNullOrEmpty(dr["NINZUU_CNT"].ToString()))
                                {
                                    shukaEntryAdd.NINZUU_CNT = SqlInt16.Parse(dr["NINZUU_CNT"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KEITAI_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.KEITAI_KBN_CD = SqlInt16.Parse(dr["KEITAI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DAIKAN_KBN"].ToString()))
                                {
                                    shukaEntryAdd.DAIKAN_KBN = SqlInt16.Parse(dr["DAIKAN_KBN"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["CONTENA_SOUSA_CD"].ToString()))
                                {
                                    shukaEntryAdd.CONTENA_SOUSA_CD = SqlInt16.Parse(dr["CONTENA_SOUSA_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["MANIFEST_SHURUI_CD"].ToString()))
                                {
                                    shukaEntryAdd.MANIFEST_SHURUI_CD = SqlInt16.Parse(dr["MANIFEST_SHURUI_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["MANIFEST_TEHAI_CD"].ToString()))
                                {
                                    shukaEntryAdd.MANIFEST_TEHAI_CD = SqlInt16.Parse(dr["MANIFEST_TEHAI_CD"].ToString());
                                }
                                shukaEntryAdd.DENPYOU_BIKOU = dr["DENPYOU_BIKOU"].ToString();
                                shukaEntryAdd.TAIRYUU_BIKOU = dr["TAIRYUU_BIKOU"].ToString();
                                if (!String.IsNullOrEmpty(dr["UKETSUKE_NUMBER"].ToString()))
                                {
                                    shukaEntryAdd.UKETSUKE_NUMBER = SqlInt64.Parse(dr["UKETSUKE_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KEIRYOU_NUMBER"].ToString()))
                                {
                                    shukaEntryAdd.KEIRYOU_NUMBER = SqlInt64.Parse(dr["KEIRYOU_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["RECEIPT_NUMBER"].ToString()))
                                {
                                    shukaEntryAdd.RECEIPT_NUMBER = SqlInt32.Parse(dr["RECEIPT_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["NET_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.NET_TOTAL = SqlDecimal.Parse(dr["NET_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_SHOUHIZEI_RATE"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_SHOUHIZEI_RATE = SqlDecimal.Parse(dr["URIAGE_SHOUHIZEI_RATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_AMOUNT_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_AMOUNT_TOTAL = SqlDecimal.Parse(dr["URIAGE_AMOUNT_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_SOTO"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_TAX_SOTO = SqlDecimal.Parse(dr["URIAGE_TAX_SOTO"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_UCHI"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_TAX_UCHI = SqlDecimal.Parse(dr["URIAGE_TAX_UCHI"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["URIAGE_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["URIAGE_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_URIAGE_KINGAKU_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.HINMEI_URIAGE_KINGAKU_TOTAL = SqlDecimal.Parse(dr["HINMEI_URIAGE_KINGAKU_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_URIAGE_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.HINMEI_URIAGE_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["HINMEI_URIAGE_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_URIAGE_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.HINMEI_URIAGE_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["HINMEI_URIAGE_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_SHOUHIZEI_RATE"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_SHOUHIZEI_RATE = SqlDecimal.Parse(dr["SHIHARAI_SHOUHIZEI_RATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_AMOUNT_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_AMOUNT_TOTAL = SqlDecimal.Parse(dr["SHIHARAI_AMOUNT_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_SOTO"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_TAX_SOTO = SqlDecimal.Parse(dr["SHIHARAI_TAX_SOTO"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_UCHI"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_TAX_UCHI = SqlDecimal.Parse(dr["SHIHARAI_TAX_UCHI"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["SHIHARAI_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["SHIHARAI_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_SHIHARAI_KINGAKU_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.HINMEI_SHIHARAI_KINGAKU_TOTAL = SqlDecimal.Parse(dr["HINMEI_SHIHARAI_KINGAKU_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_SHIHARAI_TAX_SOTO_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.HINMEI_SHIHARAI_TAX_SOTO_TOTAL = SqlDecimal.Parse(dr["HINMEI_SHIHARAI_TAX_SOTO_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_SHIHARAI_TAX_UCHI_TOTAL"].ToString()))
                                {
                                    shukaEntryAdd.HINMEI_SHIHARAI_TAX_UCHI_TOTAL = SqlDecimal.Parse(dr["HINMEI_SHIHARAI_TAX_UCHI_TOTAL"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_ZEI_KEISAN_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_ZEI_KEISAN_KBN_CD = SqlInt16.Parse(dr["URIAGE_ZEI_KEISAN_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_ZEI_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_ZEI_KBN_CD = SqlInt16.Parse(dr["URIAGE_ZEI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["URIAGE_TORIHIKI_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.URIAGE_TORIHIKI_KBN_CD = SqlInt16.Parse(dr["URIAGE_TORIHIKI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_ZEI_KEISAN_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_ZEI_KEISAN_KBN_CD = SqlInt16.Parse(dr["SHIHARAI_ZEI_KEISAN_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_ZEI_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_ZEI_KBN_CD = SqlInt16.Parse(dr["SHIHARAI_ZEI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHIHARAI_TORIHIKI_KBN_CD"].ToString()))
                                {
                                    shukaEntryAdd.SHIHARAI_TORIHIKI_KBN_CD = SqlInt16.Parse(dr["SHIHARAI_TORIHIKI_KBN_CD"].ToString());
                                }
                                //shukaEntryAdd.TIME_STAMP = (byte[])(dr["TIME_STAMP"]);
                                // WHOカラム設定
                                dataBind_shukkaEntryAdd.SetSystemProperty(shukaEntryAdd, false);
                                shukaEntryAdd.CREATE_USER = dr["CREATE_USER"].ToString();
                                shukaEntryAdd.CREATE_DATE = SqlDateTime.Parse(dr["CREATE_DATE"].ToString());
                                shukaEntryAdd.CREATE_PC = dr["CREATE_PC"].ToString();
                                if (!String.IsNullOrEmpty(dr["DELETE_FLG"].ToString()))
                                {
                                    shukaEntryAdd.DELETE_FLG = SqlBoolean.Parse(dr["DELETE_FLG"].ToString());
                                }
                                shukaEntryListAdd.Add(shukaEntryAdd);
                                shukaEntryListUpdata.Add(shukaEntryUpdata);
                            }

                            // 明細データ（紐づくデータ全件対象）
                            foreach (DataRow dr in dbDetail.Rows)
                            {
                                shukaMs = new T_SHUKKA_DETAIL();
                                //ukeireMs.KAKUTEI_KBN = SqlInt16.Parse("1");
                                //ukeireMs.SYSTEM_ID = SqlInt64.Parse(dgvRow.Cells["システムID"].Value.ToString());
                                //ukeireMs.SEQ = SqlInt32.Parse(dgvRow.Cells["枝番"].Value.ToString());
                                //ukeireMs.DETAIL_SYSTEM_ID = SqlInt64.Parse(dgvRow.Cells["明細システムID"].Value.ToString());
                                if (!String.IsNullOrEmpty(dr["SYSTEM_ID"].ToString()))
                                {
                                    shukaMs.SYSTEM_ID = SqlInt64.Parse(dr["SYSTEM_ID"].ToString());
                                }

                                shukaMs.SEQ = SqlInt32.Parse(saiban.ToString());

                                if (!String.IsNullOrEmpty(dr["DETAIL_SYSTEM_ID"].ToString()))
                                {
                                    shukaMs.DETAIL_SYSTEM_ID = SqlInt64.Parse(dr["DETAIL_SYSTEM_ID"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SHUKKA_NUMBER"].ToString()))
                                {
                                    shukaMs.SHUKKA_NUMBER = SqlInt64.Parse(dr["SHUKKA_NUMBER"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["ROW_NO"].ToString()))
                                {
                                    shukaMs.ROW_NO = SqlInt16.Parse(dr["ROW_NO"].ToString());
                                }
                                // 伝票単位の場合、全明細を確定にする
                                if (this.msysinfo.SYS_KAKUTEI__TANNI_KBN.Equals(SqlInt16.Parse("1")))
                                {
                                    ...
                                    {
                                        // 伝票区分が売上の場合
                                        shukaMs.KAKUTEI_KBN = SqlInt16.Parse(KAKUTEI_KBN_KAKUTEI);
                                    }
                                    else
                                    {
                                        // 伝票区分が支払の場合
                                        ...
                                        {
                                            // 値を保持する
                                            shukaMs.KAKUTEI_KBN = SqlInt16.Parse(dr["KAKUTEI_KBN"].ToString());
                                        }
                                    }
                                }
                                else
                                ...
                                if (!String.IsNullOrEmpty(dr["URIAGESHIHARAI_DATE"].ToString()))
                                {
                                    shukaMs.URIAGESHIHARAI_DATE = SqlDateTime.Parse(dr["URIAGESHIHARAI_DATE"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["STACK_JYUURYOU"].ToString()))
                                {
                                    shukaMs.STACK_JYUURYOU = SqlDecimal.Parse(dr["STACK_JYUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["EMPTY_JYUURYOU"].ToString()))
                                {
                                    shukaMs.EMPTY_JYUURYOU = SqlDecimal.Parse(dr["EMPTY_JYUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["WARIFURI_JYUURYOU"].ToString()))
                                {
                                    shukaMs.WARIFURI_JYUURYOU = SqlDecimal.Parse(dr["WARIFURI_JYUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["WARIFURI_PERCENT"].ToString()))
                                {
                                    shukaMs.WARIFURI_PERCENT = SqlDecimal.Parse(dr["WARIFURI_PERCENT"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["CHOUSEI_JYUURYOU"].ToString()))
                                {
                                    shukaMs.CHOUSEI_JYUURYOU = SqlInt64.Parse(dr["CHOUSEI_JYUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["CHOUSEI_PERCENT"].ToString()))
                                {
                                    shukaMs.CHOUSEI_PERCENT = SqlDecimal.Parse(dr["CHOUSEI_PERCENT"].ToString());
                                }
                                shukaMs.YOUKI_CD = dr["YOUKI_CD"].ToString();
                                if (!String.IsNullOrEmpty(dr["YOUKI_SUURYOU"].ToString()))
                                {
                                    shukaMs.YOUKI_SUURYOU = SqlDecimal.Parse(dr["YOUKI_SUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["YOUKI_JYUURYOU"].ToString()))
                                {
                                    shukaMs.YOUKI_JYUURYOU = SqlDecimal.Parse(dr["YOUKI_JYUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["DENPYOU_KBN_CD"].ToString()))
                                {
                                    shukaMs.DENPYOU_KBN_CD = SqlInt16.Parse(dr["DENPYOU_KBN_CD"].ToString());
                                }
                                shukaMs.HINMEI_CD = dr["HINMEI_CD"].ToString();
                                shukaMs.HINMEI_NAME = dr["HINMEI_NAME"].ToString();
                                if (!String.IsNullOrEmpty(dr["NET_JYUURYOU"].ToString()))
                                {
                                    shukaMs.NET_JYUURYOU = SqlInt64.Parse(dr["NET_JYUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["SUURYOU"].ToString()))
                                {
                                    shukaMs.SUURYOU = SqlDecimal.Parse(dr["SUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["UNIT_CD"].ToString()))
                                {
                                    shukaMs.UNIT_CD = SqlInt16.Parse(dr["UNIT_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["TANKA"].ToString()))
                                {
                                    shukaMs.TANKA = SqlDecimal.Parse(dr["TANKA"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["KINGAKU"].ToString()))
                                {
                                    shukaMs.KINGAKU = SqlDecimal.Parse(dr["KINGAKU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["TAX_SOTO"].ToString()))
                                {
                                    shukaMs.TAX_SOTO = SqlDecimal.Parse(dr["TAX_SOTO"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["TAX_UCHI"].ToString()))
                                {
                                    shukaMs.TAX_UCHI = SqlDecimal.Parse(dr["TAX_UCHI"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_ZEI_KBN_CD"].ToString()))
                                {
                                    shukaMs.HINMEI_ZEI_KBN_CD = SqlInt16.Parse(dr["HINMEI_ZEI_KBN_CD"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_KINGAKU"].ToString()))
                                {
                                    shukaMs.HINMEI_KINGAKU = SqlDecimal.Parse(dr["HINMEI_KINGAKU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_TAX_SOTO"].ToString()))
                                {
                                    shukaMs.HINMEI_TAX_SOTO = SqlDecimal.Parse(dr["HINMEI_TAX_SOTO"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["HINMEI_TAX_UCHI"].ToString()))
                                {
                                    shukaMs.HINMEI_TAX_UCHI = SqlDecimal.Parse(dr["HINMEI_TAX_UCHI"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["MEISAI_BIKOU"].ToString()))
                                {
                                    shukaMs.MEISAI_BIKOU = dr["MEISAI_BIKOU"].ToString();
                                }
                                if (!String.IsNullOrEmpty(dr["NISUGATA_SUURYOU"].ToString()))
                                {
                                    shukaMs.NISUGATA_SUURYOU = SqlDecimal.Parse(dr["NISUGATA_SUURYOU"].ToString());
                                }
                                if (!String.IsNullOrEmpty(dr["NISUGATA_SUURYOU"].ToString()))
                                {
                                    shukaMs.NISUGATA_UNIT_CD = SqlInt16.Parse(dr["NISUGATA_UNIT_CD"].ToString());
                                }
                                //ukeireMs.TIME_STAMP = SqlInt64.Parse(db.Rows[0]["TIME_STAMP"].ToString());
                                //shukaMs.TIME_STAMP = (byte[])(dr["TIME_STAMP"]);
                                // WHOカラム設定
                                dataBind_shukkaMs.SetSystemProperty(shukaMs, false);

                                shukaMsList.Add(shukaMs);
                            }
                        }
                ...
                foreach (DataRow dgvRow in shukaDt.Rows)
                {
                        ...
                        && !dgvRow["明細・確定区分"].ToString().Equals(dgvRow["比較確定区分"].ToString()))
                    {
                        ...
                        if (!dgvRow["システムID"].ToString().Equals(syetemidukeiri) || !dgvRow["枝番"].ToString().Equals(sequkeiri))
                        {
                            ...
                            foreach (DataRow dr in dbEntry.Rows)
                            {
                                //ukeireEntry.SYSTEM_ID = SqlInt64.Parse(dr["SYSTEM_ID"].ToString());
                                shukaEntryUpdata = new T_SHUKKA_ENTRY();
                                if (!String.IsNullOrEmpty(dr["SYSTEM_ID"].ToString()))
                                {
                                    shukaEntryUpdata.SYSTEM_ID = SqlInt64.Parse(dr["SYSTEM_ID"].ToString());
                            ...
                            foreach (DataRow dr in dbDetail.Rows)
                            {
                                ...
                                if (this.msysinfo.SYS_KAKUTEI__TANNI_KBN.Equals(SqlInt16.Parse("1")))
                                {
                                    ...
                                    {
                                        // 伝票区分が売上の場合
                                        shukaMs.KAKUTEI_KBN = SqlInt16.Parse(KAKUTEI_KBN_MIKAKUTEI);
                                    }
                                    else
                                    {
                                        // 伝票区分が支払の場合
                        ...
                        && !dgvRow["明細・確定区分"].ToString().Equals(dgvRow["比較確定区分"].ToString()))
                    {
                        ...
                        if (!dgvRow["システムID"].ToString().Equals(syetemidursh) || !dgvRow["枝番"].ToString().Equals(seqursh))
                        {
                            ...
                            foreach (DataRow dr in dbEntry.Rows)
                            {
                                ...
                                urshEntryAdd.CREATE_USER = dr["CREATE_USER"].ToString();
                                urshEntryAdd.CREATE_DATE = SqlDateTime.Parse(dr["CREATE_DATE"].ToString());
                                urshEntryAdd.CREATE_PC = dr["CREATE_PC"].ToString();
                                if (!String.IsNullOrEmpty(dr["DELETE_FLG"].ToString()))
                                {
        ...
        private void GetMeisaiIchiranData(string kubun)
        {
            ...
            //CustomDataGridView ukeireGridView = new CustomDataGridView();
        ...
        private void bt_process1_Click(object sender, System.EventArgs e)
        {
            ...
            int denshukbn = (int)DENSHU_KBN.URIAGE_KAKUTEI_NYUURYOKU;
            ...
            var callForm1 = (SuperForm)assembly.CreateInstance(
                    ...
                    null,
                //new object[] { this.form.ShainCd, DENSHU_KBN.URIAGE_KAKUTEI_NYUURYOKU.ToString() },
        ...
        {
            //this.form.customDataGridView1.CurrentCell = this.form.customDataGridView1.Rows[0].Cells[3];
        ...
        private void customDataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0 && e.RowIndex >= 0)
            {
                ...
                //DataTable DT = SearchTouRokuTanniKubun();
                //if (DT.Rows.Count > 0)
                //{
                if (this.msysinfo.SYS_KAKUTEI__TANNI_KBN.Equals(SqlInt16.Parse("1")))
                {
                    string systemid = dgvRow.Cells[5].Value.ToString();
                    string seq = dgvRow.Cells[6].Value.ToString();
                    string bunrui = dgvRow.Cells["分類"].Value.ToString();
                    //if (dgvRow.Cells[0].Value.Equals(true) && dgvRow.Cells[4].Value.ToString().Equals("1"))
                    if (dgvRow.Cells[0].Value.Equals(true))
                    {
                        foreach (DataGridViewRow dgvrow in this.form.customDataGridView1.Rows)
                        {

                            if (systemid.Equals(dgvrow.Cells[5].Value.ToString()) &&
                                seq.Equals(dgvrow.Cells[6].Value.ToString()) &&
                                bunrui.Equals(dgvrow.Cells["分類"].Value.ToString()))
                            {
                    ...
                    //else if (dgvRow.Cells[0].Value.Equals(false) && dgvRow.Cells[4].Value.ToString().Equals("1"))
                    else if (dgvRow.Cells[0].Value.Equals(false))
                    {
                        foreach (DataGridViewRow dgvrow in this.form.customDataGridView1.Rows)
                        {
                ...
                else
                {
                    ...
                    //this.form.customDataGridView1.CurrentCell = this.form.customDataGridView1.Rows[0].Cells[2];
                    //this.form.customDataGridView1.CurrentCell = this.form.customDataGridView1.Rows[0].Cells[0];
        ...
        private void MakeWhereSql(StringBuilder sql)
        {
            ...
            if (this.headForm.radbtnDenpyouHiduke.Checked)
            {
                if (this.headForm.HIDUKE_FROM.Value != null)
                {
                    sql.Append(" M1.DENPYOU_DATE >= '" + DateTime.Parse(this.headForm.HIDUKE_FROM.Value.ToString()) + "' ");
                }
                ...
                if (this.headForm.HIDUKE_TO.Value != null)
                {
                    sql.Append(" M1.DENPYOU_DATE <= '" + DateTime.Parse(this.headForm.HIDUKE_TO.Value.ToString()) + "' ");
                }
            }
            else if (this.headForm.radbtnNyuuryokuHiduke.Checked)
            {
                if (this.headForm.HIDUKE_FROM.Value != null)
                {
                    sql.Append(" M1.CREATE_DATE >= '" + DateTime.Parse(this.headForm.HIDUKE_FROM.Value.ToString()) + "' ");
                }
                ...
                if (this.headForm.HIDUKE_TO.Value != null)
                {
                    sql.Append(" M1.CREATE_DATE <= '" + DateTime.Parse(this.headForm.HIDUKE_TO.Value.ToString()) + "' ");
                }
            }

            if (!String.IsNullOrEmpty(this.headForm.KYOTEN_CD.Text))
            {
                sql.Append(" AND M1.KYOTEN_CD = '" + this.headForm.KYOTEN_CD.Text + "' ");
            }
        ...
        private void MakeSearchJuchu(StringBuilder sql)
        {
            ...
            sql.Append(" AND M2.DENPYOU_KBN_CD = 1 ");
            if (this.form.radbtn_Mikakute.Checked)
            {
                // NULLの場合は未確定扱い
                sql.Append(" AND (M2.KAKUTEI_KBN IS NULL OR M2.KAKUTEI_KBN = '" + KAKUTEI_KBN_MIKAKUTEI + "') ");
            }
            if (this.form.radbtn_Kakute.Checked)
            {
        ...
        public int Search()
        {
            try
            {
                ...
                ////全角スペースを半角スペースに変換
                //string beforeString = this.ToHankakuSpace(this.SearchString);
                ////全角英数字を半角変換後の文字列
                //string afterString = this.ToHankaku(beforeString);

                //string[] beforeSeachArray = beforeString.Split(' ');
                //string[] hankakuChangeArray = afterString.Split(' ');

                //string abc = this.selectQuery;
                ...
                {
                    db = daoIchiran.getdateforstringsql(this.mcreateSql);
                }
                ...
                foreach (DataRow row in db.Rows)
                {
                    ...
                    {
                        if (row["明細・確定区分"].ToString().Equals("1"))
                        {
                ...
                //newColumn = this.form.customDataGridView1.Columns["確定区分"];
                //newColumn.ReadOnly = false;
                newColumn.Name = "確定区分";
                DataGridviewCheckboxHeaderCell newheader = new DataGridviewCheckboxHeaderCell();
                newheader.OnCheckBoxClicked += new DataGridViewCheckBoxColumnHeeader.DataGridviewCheckboxHeaderCell.datagridviewcheckboxHeaderEventHander(ch_OnCheckBoxClicked);
                newColumn.HeaderCell = newheader;



                //this.form.customDataGridView1.DataSource = null;
                //newheader.ReadOnly = false;
                //newColumn.HeaderCell = newheader;
                //newColumn.HeaderCell.
                //newColumn.Name        
                //this.form.customDataGridView1.Columns.Add(newColumn);if
                ...
                //this.form.customDataGridView1.Columns[0].HeaderCell.
                ...
                //    //DgvCustomCheckBoxColumn newColumn = new DgvCustomCheckBoxColumn();
                //    //newColumn = this.form.customDataGridView1.Columns["確定区分"];
                //    newColumn1.ReadOnly = false;
                //    newColumn1.Name = dtResult.Columns[i].ColumnName;

                //    this.form.customDataGridView1.Columns.Add(newColumn1);

                //}

                this.searchResult = dtResult;
                //newColumn = (DataGridViewCheckBoxColumn)this.form.customDataGridView1.Columns["確定区分"];
                //newColumn.HeaderCell = newheader;
        ...
        private DataTable SearchUkeireDetail(string system_id, string seq)
        {
            ...
            db = daoIchiran.getdateforstringsql(sql.ToString());
            return db;
        }
        ...
        private void SetFormInit()
        {
            // No.1781
            // システム情報を取得
            this.form.txtNum_HidukeSentaku.Text = this.msysinfo.URIAGE_HYOUJI_JOUKEN.Value.ToString();
            this.form.CheckBox_Syuka.Checked = this.msysinfo.URIAGE_HYOUJI_JOUKEN_SHUKKA.Value;
            this.form.CheckBox_Jyunyu.Checked = this.msysinfo.URIAGE_HYOUJI_JOUKEN_UKEIRE.Value;
            this.form.CheckBox_Uriageshiharai.Checked = this.msysinfo.URIAGE_HYOUJI_JOUKEN_URIAGESHIHARAI.Value;

            this.form.customDataGridView1.TabIndex = 9;
        }
        ...
        internal bool DateCheck()
        {
            try
            {
                ...
                if (date_to.CompareTo(date_from) < 0)
                {
                    ...
                    string strFrom = this.headForm.lab_HidukeNyuuryoku.Text + "From";
                    string strTo = this.headForm.lab_HidukeNyuuryoku.Text + "To";
