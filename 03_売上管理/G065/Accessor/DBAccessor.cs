...
namespace Shougun.Core.SalesManagement.UrikakekinItiranHyo.Accessor
{
    ...
    internal class DBAccessor
    {
        ...
        internal DataTable GetIchiranData(Shougun.Core.Common.Kakepopup.Const.UIConstans.ConditionInfo param, CommonInformation info, Form form, ref bool hadSearched)
        {
            ...
            var torihikisakiSeikyuDt = this.KakekinDao.GetTorihikisakiSeikyuList(param.StartTorihikisakiCD, param.EndTorihikisakiCD);
            ...
            if (kikannaiTbl.Rows.Count == 0)
            {
                ...
                foreach (var torihikisakiCd in torihikisakiCdList)
                {
                    ...
                    var result = getsujiLogic.GetsujiShori(torihikisakiCd, param.StartDay.Year, param.StartDay.Month,
                        isDispAlert, 2, shoriType: GetsujiShoriLogic.GETSUJI_SHORI_TYPE.SEIKYUU);
                    if (!result)
                    {
                        // 計算処理が中断された場合は処理終了
                        return this.meisaiTable;
                ...
                if (getsujiLogic.MonthlyLockUrDatas.ContainsKey(yyyyMM))
                {
                    ...
                    foreach (var lockUrEntity in lockUrEntityList)
                    {
                        var dt = torihikisakiSeikyuDt.AsEnumerable()
                            .Where(n => n["TORIHIKISAKI_CD"].Equals(lockUrEntity.TORIHIKISAKI_CD))
                            .FirstOrDefault();
                        if (dt == null)
                        {
                            continue;
                        ...
                        row["KURIKOSHI_ZANDAKA"] = lockUrEntity.PREVIOUS_MONTH_BALANCE.Value;
                        row["NYUKINGAKU"] = lockUrEntity.NYUUKIN_KINGAKU.Value;
                        row["ZEINUKI_URIAGE"] = lockUrEntity.KINGAKU.Value;
                        // 計算直後は月次調整データは作成されないので、消費税、税込金額、調整前差引残高をそのまま表示
                        row["SHOHIZEI"] = lockUrEntity.TAX.Value;
                        row["ZEIKOMI_URIAGE"] = lockUrEntity.TOTAL_KINGAKU.Value;
                        row["SASHIHIKI_URIAGE_ZANDAKA"] = lockUrEntity.ZANDAKA.Value;
            ...
            foreach (DataRow row in kikannaiTbl.Rows)
            {
                ...
                //var isGenkin = torihikisakiSeikyuDt.AsEnumerable()
        ...
        private bool AllZeroRecord(DataRow row)
        {
            ...
            if (row["KURIKOSHI_ZANDAKA"] != DBNull.Value && Convert.ToDecimal(row["KURIKOSHI_ZANDAKA"]) == decimal.Zero
                && row["NYUKINGAKU"] != DBNull.Value && Convert.ToDecimal(row["NYUKINGAKU"]) == decimal.Zero
                && row["ZEINUKI_URIAGE"] != DBNull.Value && Convert.ToDecimal(row["ZEINUKI_URIAGE"]) == decimal.Zero
                && row["SHOHIZEI"] != DBNull.Value && Convert.ToDecimal(row["SHOHIZEI"]) == decimal.Zero
                && row["ZEIKOMI_URIAGE"] != DBNull.Value && Convert.ToDecimal(row["ZEIKOMI_URIAGE"]) == decimal.Zero
                && row["SASHIHIKI_URIAGE_ZANDAKA"] != DBNull.Value && Convert.ToDecimal(row["SASHIHIKI_URIAGE_ZANDAKA"]) == decimal.Zero)
            {
                return true;
            }
            else
