...
namespace Shougun.Core.SalesManagement.UrikakekinItiranHyo
{
    ...
    /// <summary> R361(売掛金一覧表)帳票を表すクラス・コントロール </summary>
    public class ReportInfoR361 : ReportInfoBase
    {
        #region - Fields -
        ...
        private string taitoru = "売掛金一覧表";                    // タイトル

        // 明細部カラムヘッダ
        private string torisakiCD = "取引先CD";                // 取引先CD
        private string torisakiNameRyaku = "取引先名";         // 取引先名
        private string kurikoshiZandaka = "繰越残高";          // 繰越残高
        private string nyuukinGake = "入金額";                 // 入金額
        private string zeiNukiUriage = "税抜売上金額";         // 税抜売上金額
        private string shouhiZei = "消費税";                   // 消費税
        private string zeiKomiUriage = "税込売上金額";         // 税込売上金額
        private string sashihikiZandaka = "差引残高";          // 差引残高
        ...
        private string nyuukinGoukei = string.Empty;                // 総合計：入金額
        private string zeinukiuriGoukei = string.Empty;             // 総合計：税抜売上
        private string shouhizeiGoukei = string.Empty;              // 総合計：消費税
        private string zeikomiuriGoukei = string.Empty;             // 総合計：税込売上
        private string sasihizanGoukei = string.Empty;              // 総合計：差引残高
        ...
        public ReportInfoR361()
        {
            // パスとレイアウトのデフォルト値を設定しておく

            // 帳票ファイルフルパス
            this.OutputFormFullPathName = "./Template/R361-Form.xml";
        ...
        public void R361_Reprt(DataTable chouhyouData)
        {
            ...
            this.ChouhyouDataTable.Columns.Add("PHY_NYUUKIN_GAKU_VLB");                     // 入金額
            this.ChouhyouDataTable.Columns.Add("PHY_ZEINUKI_URIAGE_VLB");                   // 税抜売上
            this.ChouhyouDataTable.Columns.Add("PHY_SHOUHIZEI_VLB");                        // 消費税
            this.ChouhyouDataTable.Columns.Add("PHY_ZEIKOMI_URIAGE_VLB");                   // 税込売上
            this.ChouhyouDataTable.Columns.Add("PHY_SASHIHIKI_URIAGE_ZANDAKA_VLB");         // 差引売上残高
            ...
            for (int i = 0; i < this.detail.Count; i++)
            {
                ...
                row["PHY_NYUUKIN_GAKU_VLB"] = this.detail[i].NyuukinGaku;                            // 入金額
                row["PHY_ZEINUKI_URIAGE_VLB"] = this.detail[i].ZeinukiUriage;                        // 税抜売上
                row["PHY_SHOUHIZEI_VLB"] = this.detail[i].ShouhiZei;                                 // 消費税                
                row["PHY_ZEIKOMI_URIAGE_VLB"] = this.detail[i].ZeikomiUriage;                        // 税込売上
                row["PHY_SASHIHIKI_URIAGE_ZANDAKA_VLB"] = this.detail[i].SashihikiUriageZandaka;     // 差引売上残高
        ...
        protected override void UpdateFieldsStatus()
        {
            ...
            this.SetFieldName("PHY_NYUUKIN_GAKU_VLB", this.nyuukinGake);                     // 入金額
            this.SetFieldName("PHY_ZEINUKI_URIAGE_VLB", this.zeiNukiUriage);                 // 税抜売上
            this.SetFieldName("PHY_SHOUHIZEI_VLB", this.shouhiZei);                          // 消費税
            this.SetFieldName("PHY_ZEIKOMI_URIAGE_VLB", this.zeiKomiUriage);                 // 税込売上
            this.SetFieldName("PHY_SASHIHIKI_URIAGE_ZANDAKA_VLB", this.sashihikiZandaka);    // 差引売上残高
            ...
            this.SetFieldName("G1F_NYUUKIN_GAKU_TOTAL_CTL", this.nyuukinGoukei);                 // 総合計：入金額
            this.SetFieldName("G1F_ZEINUKI_URIAGE_TOTAL_CTL", this.zeinukiuriGoukei);            // 総合計：税抜売上
            this.SetFieldName("G1F_SHOUHIZEI_TOTAL_CTL", this.shouhizeiGoukei);                  // 総合計：消費税
            this.SetFieldName("G1F_ZEIKOMI_URIAGE_TOTAL_CTL", this.zeikomiuriGoukei);            // 総合計：税込売上
            this.SetFieldName("G1F_SASHIHIKI_URIAGE_ZANDAKA_TOTAL_CTL", this.sasihizanGoukei);   // 総合計：差引残高
        }
        ...
        private void InputDataToMem(DataTable dataTable)
        {
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                else if (list[0] == "2-2")
                {
                    ...
                    this.nyuukinGoukei = list[2];        // 入金額
                    this.zeinukiuriGoukei = list[3];     // 税抜売上
                    this.shouhizeiGoukei = list[4];      // 消費税
                    this.zeikomiuriGoukei = list[5];     // 税込売上
                    this.sasihizanGoukei = list[6];      // 差引残高
                }
            }
        }
        ...
        private class Detail
        {
            ...
            /// <param name="nyuukingaku">入金額を表す文字列</param>
            /// <param name="zeinukiUriage">税抜売上を表す文字列</param>
            /// <param name="shouhiZei">消費税を表す文字列</param>
            /// <param name="zeikomiUriage">税込売上を表す文字列</param>
            /// <param name="sashihikiUriageZandaka">差引売上残高を表す文字列</param>
            /// <param name="shimebi1">締日1を表す文字列</param>
            /// <param name="shimebi2">締日2を表す文字列</param>
            /// <param name="shimebi3">締日3を表す文字列</param>
            public Detail(string torihikisakCD, string torihikisakMei, string kurikosiZndaka, string nyuukingaku, string zeinukiUriage, string shouhiZei, string zeikomiUriage, string sashihikiUriageZandaka, string shimebi1, string shimebi2, string shimebi3)
            {
                this.TorihikisakCD = torihikisakCD;                         // 取引先コード
                this.TorihikisakMei = torihikisakMei;                       // 取引先名称
                this.KurikosiZndaka = kurikosiZndaka;                       // 繰越残高
                this.NyuukinGaku = nyuukingaku;                             // 入金額
                this.ZeinukiUriage = zeinukiUriage;                         // 税抜売上
                this.ShouhiZei = shouhiZei;                                 // 消費税
                this.ZeikomiUriage = zeikomiUriage;                         // 税込売上
                this.SashihikiUriageZandaka = sashihikiUriageZandaka;       // 差引売上残高
            ...
            /// <summary> 入金額を保持するプロパティ </summary>
            public string NyuukinGaku { get; private set; }

            /// <summary> 税抜売上を保持するプロパティ </summary>
            public string ZeinukiUriage { get; private set; }

            /// <summary> 消費税を保持するプロパティ </summary>
            public string ShouhiZei { get; private set; }

            /// <summary> 税込売上を保持するプロパティ </summary>
            public string ZeikomiUriage { get; private set; }

            /// <summary> 差引売上残高を保持するプロパティ </summary>
            public string SashihikiUriageZandaka { get; private set; }
