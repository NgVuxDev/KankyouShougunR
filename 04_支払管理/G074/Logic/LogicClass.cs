...
namespace Shougun.Core.PaymentManagement.KaikakekinItiranHyo
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// 買掛金一覧表画面Form
        /// </summary>
        private KaikakekinItiranHyo.UIForm form;
        ...
        /// DBAccessor
        /// </summary>
        private Shougun.Core.PaymentManagement.KaikakekinItiranHyo.Accessor.DBAccessor dba;
        ...
        public LogicClass(UIForm targetForm)
        {
            ...
            this.dba = new Shougun.Core.PaymentManagement.KaikakekinItiranHyo.Accessor.DBAccessor();
        ...
        internal bool Print()
        {
            ...
            try
            {
                ...
                if (this.form.KaikakekinItiranHyo.RowCount == 0)
                {
                    // データなし時はやらない
                    return ret;
                }
                ...
                foreach (DataGridViewRow row in this.form.KaikakekinItiranHyo.Rows)
                {
                    //DataRow dr;
                    dr = dt.NewRow();
                    ...
                    sBuilder.Append("\",\"");
                    sBuilder.Append(row.Cells["col_ShukkinGaku"].Value.ToString());
                    sBuilder.Append("\",\"");
                    sBuilder.Append(row.Cells["col_ZeinukiShiharai"].Value.ToString());
                    sBuilder.Append("\",\"");
                    sBuilder.Append(row.Cells["col_Shohizei"].Value.ToString());
                    sBuilder.Append("\",\"");
                    sBuilder.Append(row.Cells["col_ZeikomiShiharai"].Value.ToString());
                    sBuilder.Append("\",\"");
                    sBuilder.Append(row.Cells["col_SashihikiShiharaiZandaka"].Value.ToString());
                    sBuilder.Append("\",\"");
                    ...
                    syukkingoukei += decimal.Parse(row.Cells["col_ShukkinGaku"].Value.ToString());
                    zeinukigoukei += decimal.Parse(row.Cells["col_ZeinukiShiharai"].Value.ToString());
                    shohizeigoukei += decimal.Parse(row.Cells["col_Shohizei"].Value.ToString());
                    zeikomigoukei += decimal.Parse(row.Cells["col_ZeikomiShiharai"].Value.ToString());
                    sashihikigoukei += decimal.Parse(row.Cells["col_SashihikiShiharaiZandaka"].Value.ToString());
                }
                ...
                sBuilder.Append("\",\"");
                sBuilder.Append(syukkingoukei.ToString());
                sBuilder.Append("\",\"");
                ...
                ReportInfoR365 report_r365 = new ReportInfoR365();

                report_r365.R365_Reprt(dt);

                // 印刷ポツプアップ画面表示
                using (FormReportPrintPopup report = new FormReportPrintPopup(report_r365))
                {
                    report.ReportCaption = WINDOW_TITLEExt.ToTitleString(WINDOW_ID.T_KAIKAKEKIN_ICHIRANHYOU);
                    report.ShowDialog();
                    report.Dispose();
                }
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (msgLogic.MessageBoxShow("C012") == DialogResult.Yes)
                    {
                        ...
                        CSVExport.ConvertCustomDataGridViewToCsv(this.form.KaikakekinItiranHyo, true, true, WINDOW_TITLEExt.ToTitleString(WINDOW_ID.T_KAIKAKEKIN_ICHIRANHYOU), this.form);
                    }
                }
            }
            catch (Exception ex)
        ...
        internal void SetIchiran()
        {
            ...
            this.form.KaikakekinItiranHyo.IsBrowsePurpose = false;

            int k = this.form.KaikakekinItiranHyo.Rows.Count;
            for (int i = k; i >= 1; i--)
            {
                this.form.KaikakekinItiranHyo.Rows.RemoveAt(this.form.KaikakekinItiranHyo.Rows[i - 1].Index);
            }

            DataView dv = new DataView();
            this.allDataTable.TableName = "買掛金一覧表";
            dv.Table = this.allDataTable;

            //出力区分により、ソート項目を変更する
            if (this.param.OutPutKBN == 2)
            {
                dv.Sort = "SASHIHIKI_ZANDAKA DESC";
            }
            else
            {
            ...
            {
                this.form.KaikakekinItiranHyo.Rows.Add();
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_TorihikisakiCD"].Value = aftsorttable.Rows[i]["TORIHIKISAKI_CD"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_TorihikisakiName"].Value = aftsorttable.Rows[i]["TORIHIKISAKI_NAME"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_KurikosiZandaka"].Value = aftsorttable.Rows[i]["KURIKOSHI_ZANDAKA"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_ShukkinGaku"].Value = aftsorttable.Rows[i]["SHUKKIN_KINGAKU"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_ZeinukiShiharai"].Value = aftsorttable.Rows[i]["SHIHARAI_KINGAKU"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shohizei"].Value = aftsorttable.Rows[i]["SHOHIZEI"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_ZeikomiShiharai"].Value = aftsorttable.Rows[i]["ZEIKOMI_SHIHARAI"];
                this.form.KaikakekinItiranHyo.Rows[i].Cells["col_SashihikiShiharaiZandaka"].Value = aftsorttable.Rows[i]["SASHIHIKI_ZANDAKA"];
                if (SqlInt16.Parse(aftsorttable.Rows[i]["SHIMEBI1"].ToString()).IsNull == true)
                {
                    this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shimebi1"].Value = string.Empty;
                }
                else
                {
                    this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shimebi1"].Value = aftsorttable.Rows[i]["SHIMEBI1"];
                }
                if (SqlInt16.Parse(aftsorttable.Rows[i]["SHIMEBI2"].ToString()).IsNull == true)
                {
                    this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shimebi2"].Value = string.Empty;
                }
                else
                {
                    this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shimebi2"].Value = aftsorttable.Rows[i]["SHIMEBI2"];
                }
                if (SqlInt16.Parse(aftsorttable.Rows[i]["SHIMEBI3"].ToString()).IsNull == true)
                {
                    this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shimebi3"].Value = string.Empty;
                }
                else
                {
                    this.form.KaikakekinItiranHyo.Rows[i].Cells["col_Shimebi3"].Value = aftsorttable.Rows[i]["SHIMEBI3"];
                }
            }

            this.form.KaikakekinItiranHyo.IsBrowsePurpose = true;
        ...
        private void SetInitDisp()
        {
            ...
            this.parentForm.Text = r_framework.Dto.SystemProperty.CreateWindowTitle("買掛金一覧表");
            this.parentForm.lb_hint.Text = "";
            this.form.KaikakekinItiranHyo.Rows.Clear();
            this.ichiranTable = null;
            this.form.KaikakekinItiranHyo.DataSource = this.ichiranTable;
            // 自社名を取得
