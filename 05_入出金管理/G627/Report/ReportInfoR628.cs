...
{
    /// <summary>
    /// ReportInfoR628(入金集計表)
    /// </summary>
    public class ReportInfoR628 : ReportInfoBase
    {
        #region Const

        /// <summary>帳票テンプレート - ファイルパス</summary>
        const string FORM_FULL_PATH_NAME = "./Template/R628-Form.xml";
        ...
        public ReportInfoR628(DataTable headerTable, DataTable detailTable)
        {
        ...
        protected override void UpdateFieldsStatus()
        {
            ...
            if (this.detailTable.Rows[0]["GROUP3_NAME"] == null || string.IsNullOrEmpty(this.detailTable.Rows[0]["GROUP3_NAME"].ToString()))
            {
                this.SetGroupVisible("GROUP3", false, false);
            }

            if (this.detailTable.Rows[0]["GROUP4_NAME"] == null || string.IsNullOrEmpty(this.detailTable.Rows[0]["GROUP4_NAME"].ToString()))
            {
                this.SetGroupVisible("GROUP4", false, false);
            }

            if (this.detailTable.Rows[0]["GROUP5_NAME"] == null || string.IsNullOrEmpty(this.detailTable.Rows[0]["GROUP5_NAME"].ToString()))
            {
                this.SetGroupVisible("GROUP5", false, false);
            }
