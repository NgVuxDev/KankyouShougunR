...
namespace Shougun.Core.ReceiptPayManagement.MiShukkinIchiran
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// ボタン設定XMLファイルパス
        /// </summary>
        private readonly string buttonInfoXmlPath = "Shougun.Core.ReceiptPayManagement.MiShukkinIchiran.Setting.ButtonSetting.xml";
        ...
        public bool CSV(DtoClass dto)
        {
            ...
            try
            {
                ...
                if (dto.Syosiki == ConstClass.SHOSHIKI_T)
                {
                    ...
                    {
                        string[] csvHead = { "営業担当者CD","営業担当者", "出金予定日", "未出金額", "取引先CD", "取引先", 
                                         "精算額", "出金額", "締日", "精算日", "支払方法"};
                    ...
                    {
                        string[] csvHead = { "取引先CD", "取引先",  "出金予定日", "未出金額", "営業担当者CD", "営業担当者", 
                                         "精算額", "出金額", "締日", "精算日", "支払方法"};
                        for (int i = 0; i < csvHead.Length; i++)
                        {
                            csvDT.Columns.Add(csvHead[i]);
                    ...
                    dt = dao.GetMiShukkinIchiranData(dto);
                ...
                else
                {
                    string[] csvHead = { "取引先CD", "取引先",  "業者CD", "業者", "現場CD", "現場", 
                                         "出金予定日", "未出金額",  "営業担当者CD", "営業担当者", "精算額", "出金額", 
                                         "締日", "精算日", "支払方法"};
                    for (int i = 0; i < csvHead.Length; i++)
                    {
                        csvDT.Columns.Add(csvHead[i]);
                    ...
                    dt = dao.GetMiShukkinIchiranData2(dto);
                ...
                if (checkSql == true)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (dt.Rows[i]["MISHUKKIN_GAKU"] != null && !string.IsNullOrEmpty(dt.Rows[i]["MISHUKKIN_GAKU"].ToString()))
                        {
                            rowTmp["未出金額"] = Decimal.Parse(dt.Rows[i]["MISHUKKIN_GAKU"].ToString()).ToString("#,##0");
                        }

                        if (dt.Rows[i]["SEISAN_GAKU"] != null && !string.IsNullOrEmpty(dt.Rows[i]["SEISAN_GAKU"].ToString()))
                        {
                            rowTmp["精算額"] = Decimal.Parse(dt.Rows[i]["SEISAN_GAKU"].ToString()).ToString("#,##0");
                        }
