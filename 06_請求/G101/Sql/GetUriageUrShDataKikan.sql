...
FROM
T_UR_SH_ENTRY T_US
INNER JOIN T_UR_SH_DETAIL T_US_D
...
ON T_US.SYSTEM_ID = HIN_COUNT.SYSTEM_ID 
AND T_US.SEQ = HIN_COUNT.SEQ 
...
WHERE
T_US.TORIHIKISAKI_CD =  /*data.SEIKYU_CD*/
/*IF data.SEIKYUSHIMEBI_FROM != "" && data.SEIKYUSHIMEBI_FROM != null*/
AND CONVERT(DateTime,/*data.SEIKYUSHIMEBI_FROM*/nul, 111) <= T_US.URIAGE_DATE/*END*/
AND T_US.URIAGE_DATE <= CONVERT(DateTime,/*data.SEIKYUSHIMEBI_TO*/null, 111)

AND T_US.DELETE_FLG = '0'
AND T_US.KAKUTEI_KBN = '1' 
AND T_US.URIAGE_TORIHIKI_KBN_CD = '2' 
/*IF data.KYOTEN_CD != 99 */
AND T_US.KYOTEN_CD = /*data.KYOTEN_CD*/
/*END*/
/*IF data.INVOICE_KBN == 2*/
AND (T_US.URIAGE_ZEI_KEISAN_KBN_CD = 2 
/*END*/
/*IF data.INVOICE_KBN == 2*/
OR (T_US.URIAGE_ZEI_KEISAN_KBN_CD = 3 AND (T_US.URIAGE_ZEI_KBN_CD != 1 OR (T_US.URIAGE_ZEI_KBN_CD = 1 AND HIN_COUNT.HIN_NASI = 0))))
/*END*/
/*IF data.INVOICE_KBN != 1*/
AND HIN_COUNT.HIN_SOTO = 0
/*END*/
AND (NOT EXISTS
    (SELECT T_US.SYSTEM_ID, T_US.SEQ
     FROM   T_SEIKYUU_DETAIL
     WHERE  (T_US.SYSTEM_ID = DENPYOU_SYSTEM_ID) 
	 AND (T_US.SEQ = DENPYOU_SEQ)
	 AND (DENPYOU_SHURUI_CD = '3')
	 AND (DELETE_FLG = 0)))
