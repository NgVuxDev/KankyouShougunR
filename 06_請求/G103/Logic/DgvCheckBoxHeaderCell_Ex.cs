...
namespace Shougun.Core.Billing.Seikyuichiran.CustomControls_Ex
{
                ...
                && p.Y >= checkBoxLocation.Y && p.Y <= checkBoxLocation.Y + checkBoxSize.Height)
            {
                if (_isCanCheck)
                {
                    ...
                    foreach (DataGridViewRow dgvRow in this.DataGridView.Rows)
                    {
                        ...
                        if (this.DataGridView.Columns.Contains("INXS_SEIKYUU_KBN"))
                        {
                            inxsSeikyuuKbn = dgvRow.Cells["INXS_SEIKYUU_KBN"].Value.ToString();
                        }
