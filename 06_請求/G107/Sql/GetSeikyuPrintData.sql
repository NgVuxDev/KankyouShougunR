SELECT
  ...
  , TSDKE.DENPYOU_SHURUI_CD_CSV                                    --伝票種類CDCSV用
  ...
  , TSDKE.SHARYOU_NAME AS SHARYOU_NAME				                    --車輛
FROM
  ...
  INNER JOIN (
	SELECT
		...
		,CASE TSDE.DENPYOU_SHURUI_CD 
		      WHEN 10 THEN -1 ELSE TSDE.DENPYOU_SHURUI_CD 
		 END   DENPYOU_SHURUI_CD_CSV                                    --伝票種類CDCSV用
		...
		, TSDE.SHARYOU_NAME AS SHARYOU_NAME 							     --車輛
	FROM
		T_SEIKYUU_DENPYOU_KAGAMI TSDK 
		LEFT JOIN 
        T_SEIKYUU_DETAIL TSDE 
		ON TSDE.SEIKYUU_NUMBER = TSDK.SEIKYUU_NUMBER AND TSDE.KAGAMI_NUMBER = TSDK.KAGAMI_NUMBER
  ) TSDKE 
  ON TSDKE.SEIKYUU_NUMBER = TSD.SEIKYUU_NUMBER 
 WHERE
  ...
  AND (
		 (TSD.SHOSHIKI_KBN != 1 
		 AND (ISNULL(TSDKE.KONKAI_URIAGE_GAKU,0) + 
