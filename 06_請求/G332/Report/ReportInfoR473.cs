...
{
    /// <summary> R473(請求締処理エラー一覧)を表すクラス・コントロール </summary>
    public class ReportInfoR473 : ReportInfoBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ReportInfoR473"/> class.
        /// </summary>
        public ReportInfoR473()
        {
            // 帳票出力フルパスフォーム名
            //this.OutputFormFullPathName = TEMPLATE_PATH + "R473-Form.xml";
            this.OutputFormFullPathName = "./Template/R473-Form.xml";
        ...
        /// </summary>
        public void R473_Report(DataTable chouhyouData)
        {
            ...
            for (int i = 0; i < this.detail.Count; i++)
            {
                ...
                row["PHY_DENPYO_DATE_FLB"] = this.detail[i].Denpyoudate;                   // 伝票日付
                ...
                row["FH_PRINT_DATE_VLB"] = this.detail[i].PrintDate;                       // 発行日付
        ...
        {
            // ヘッダ部
            this.SetFieldName("FH_TITLE_VLB", "請求締処理エラー一覧");                  // タイトル
            this.SetFieldName("FH_CORP_RYAKU_NAME_VLB", this.detail[0].CorpRyakuName);              // 会社名
            this.SetFieldName("FH_PRINT_DATE_VLB", this.detail[0].PrintDate);           // 発行日付
        ...
        private void InputDataToMem(DataTable dataTable)
        {
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                // Detail部          
                this.detail.Add(new Detail(list[0], list[1], list[2], list[3], list[4], list[5], list[6], list[7], list[8], list[9], list[10], list[11]));
            }
        }
