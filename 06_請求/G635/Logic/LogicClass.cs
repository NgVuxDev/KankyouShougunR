...
namespace Shougun.Core.Billing.SeikyuuMeisaihyouShutsuryoku
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// ボタン設定XMLファイルパス
        /// </summary>
        private readonly string buttonInfoXmlPath = "Shougun.Core.Billing.SeikyuuMeisaihyouShutsuryoku.Setting.ButtonSetting.xml";
        ...
        public bool CSV(DTOClass dto)
        {
            ...
            try
            {
                ...
                {
                    string[] csvHead = { "取引先CD", "取引先", "前回請求額", "入金額","調整額", "繰越額", "締",
                                       "今回取引額（税抜）", "消費税","今回取引額", "今回御請求額",
                                       "入金予定日", "請求年月日"};
                    for (int i = 0; i < csvHead.Length; i++)
                    {
                        csvDT.Columns.Add(csvHead[i]);
                    }
                    dtResult = dao.GetPrintDataReportR636(dto);
                    if (0 < dtResult.Rows.Count)
                    {
                        ...
                        for (int i = 0; i < dtResult.Rows.Count; i++)
                        {
                            ...
                            {
                                rowTmp["取引先CD"] = dtResult.Rows[i]["TORIHIKISAKI_CD"].ToString();
                            }

                            if (dtResult.Rows[i]["TORIHIKISAKI_NAME_RYAKU"] != null && !string.IsNullOrEmpty(dtResult.Rows[i]["TORIHIKISAKI_NAME_RYAKU"].ToString()))
                            {
                                rowTmp["取引先"] = dtResult.Rows[i]["TORIHIKISAKI_NAME_RYAKU"].ToString();
                            }

                            string seikyuuKakuteiKbn = dtResult.Rows[i]["SEIKYUU_KEITAI_KBN"] != null ? dtResult.Rows[i]["SEIKYUU_KEITAI_KBN"].ToString() : string.Empty;
                            ...
                            {
                                rowTmp["前回請求額"] = string.Empty;
                            }
                            else
                            {
                                rowTmp["前回請求額"] = this.FormatDecimal(temp);
                            }

                            temp = this.GetDecimal(dtResult.Rows[i]["KONKAI_NYUUKIN_GAKU"]);
                            temp = this.FractionCalc(temp, 3);
                            if ("1".Equals(seikyuuKakuteiKbn))
                            {
                                rowTmp["入金額"] = string.Empty;
                            }
                            else
                            {
                                rowTmp["入金額"] = this.FormatDecimal(temp);
                            }
                            ...
                            //今回取引額(税抜）
                            decimal konkaiUriageGaku = this.GetDecimal(dtResult.Rows[i]["KONKAI_URIAGE_GAKU"]);
                            konkaiUriageGaku = this.FractionCalc(konkaiUriageGaku, 3);
                            rowTmp["今回取引額（税抜）"] = this.FormatDecimal(konkaiUriageGaku);
                            ...
                            //今回御請求額
                            temp = this.GetDecimal(dtResult.Rows[i]["KONKAI_SEIKYU_GAKU"]);
                            temp = this.FractionCalc(temp, 3);
                            rowTmp["今回御請求額"] = this.FormatDecimal(temp);

                            if (dtResult.Rows[i]["NYUUKIN_YOTEI_BI"] != null && !string.IsNullOrEmpty(dtResult.Rows[i]["NYUUKIN_YOTEI_BI"].ToString()))
                            {
                                dt = Convert.ToDateTime(dtResult.Rows[i]["NYUUKIN_YOTEI_BI"].ToString());
                                rowTmp["入金予定日"] = dt.ToString("yyyy/MM/dd");
                            }

                            if (dtResult.Rows[i]["SEIKYUU_DATE"] != null && !string.IsNullOrEmpty(dtResult.Rows[i]["SEIKYUU_DATE"].ToString()))
                            {
                                dt = Convert.ToDateTime(dtResult.Rows[i]["SEIKYUU_DATE"].ToString());
                                rowTmp["請求年月日"] = dt.ToString("yyyy/MM/dd");
                            }
                ...
                else
                {
                    ...
                    dtResult = dao.GetPrintDataReportR637(dto);
                    if (0 < dtResult.Rows.Count)
                    {
                        for (int i = 0; i < dtResult.Rows.Count; i++)
                        {
                            ...
                            decimal konkaiSeikyuGaku = this.GetDecimal(dtResult.Rows[i]["KONKAI_SEIKYU_GAKU"]);
        ...
        public bool Preview(DTOClass dto)
        {
            ...
            try
            {
                ...
                if (dto.SYOSIKI == ConstClass.SHOSHIKI_T)
                {
                    ...
                    if (0 < dtResult.Rows.Count)
                    {
                        ReportInfoR636 reportInfo = new ReportInfoR636(dtResult, dto);
                ...
                else
                {
                    ...
                    if (0 < dtResult.Rows.Count)
                    {
                        ReportInfoR637 reportInfo = new ReportInfoR637(dtResult, dto);
                        using (FormReportPrintPopup popup = new FormReportPrintPopup(reportInfo, this.form.WindowId))
                        {
                            popup.ReportCaption = r_framework.Const.WINDOW_TITLEExt.ToTitleString(this.form.WindowId);
                            popup.ShowDialog();
