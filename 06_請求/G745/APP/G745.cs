...
namespace Shougun.Core.Billing.InxsSeikyuushoHakkou
{
	...
	/// G745 請求書発行
    /// </summary>
	class G745 : r_framework.FormManager.IShougunForm
    {
        /// <summary>
        /// フォーム作成
        /// </summary>
        ...
        public Form CreateForm(params object[] args)
        {
            ...
            var HeaderForm = new Shougun.Core.Billing.InxsSeikyuushoHakkou.APP.UIHeader();

            if (dto != null)
            {
                var callForm = new Shougun.Core.Billing.InxsSeikyuushoHakkou.UIForm(HeaderForm, dto);
                var bbf = new BusinessBaseForm(callForm, HeaderForm) { IsInFormResizable = true };
                return bbf;
            }
            else
            {
                var callForm = new Shougun.Core.Billing.InxsSeikyuushoHakkou.UIForm(HeaderForm);
                var bbf = new BusinessBaseForm(callForm, HeaderForm) { IsInFormResizable = true };
                return bbf;
            }
        }
