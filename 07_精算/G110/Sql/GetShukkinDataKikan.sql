...
WHERE
T_N.TORIHIKISAKI_CD = /*data.SHIHARAI_CD*/
/*IF data.SHIHARAISHIMEBI_FROM != "" && data.SHIHARAISHIMEBI_FROM != null*/
AND CONVERT(DateTime,/*data.SHIHARAISHIMEBI_FROM*/null, 111) <= T_N.DENPYOU_DATE/*END*/
AND T_N.DENPYOU_DATE <= CONVERT(DateTime,/*data.SHIHARAISHIMEBI_TO*/null, 111)
AND T_N.DELETE_FLG = 0 
/*IF data.KYOTEN_CD != 99 */
AND T_N.KYOTEN_CD = /*data.KYOTEN_CD*/ 
/*END*/
AND NOT EXISTS (SELECT T_N.SYSTEM_ID, T_N.SEQ
                FROM T_SEISAN_DETAIL
                WHERE (T_N.SYSTEM_ID = DENPYOU_SYSTEM_ID) 
				AND (T_N.SEQ = DENPYOU_SEQ)
				AND (DENPYOU_SHURUI_CD = '20')
				AND (DELETE_FLG = 0))