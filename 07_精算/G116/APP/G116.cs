...
namespace Shougun.Core.Adjustment.ShiharaiMeisaishoHakko
{
	...
	/// G116 支払明細書発行
    /// </summary>
	class G116 : r_framework.FormManager.IShougunForm
    {
        /// <summary>
        /// フォーム作成
        /// </summary>
        ...
        public Form CreateForm(params object[] args)
        {
            ...
            var HeaderForm = new Shougun.Core.Adjustment.ShiharaiMeisaishoHakko.UIHeader();
            if (dto != null)
            {
                var callForm = new Shougun.Core.Adjustment.ShiharaiMeisaishoHakko.UIForm(HeaderForm, dto);
                var bbf = new BusinessBaseForm(callForm, HeaderForm) { IsInFormResizable = true };
                return bbf;
            }
            else
            {
                var callForm = new Shougun.Core.Adjustment.ShiharaiMeisaishoHakko.UIForm(HeaderForm);
                var bbf = new BusinessBaseForm(callForm, HeaderForm) { IsInFormResizable = true };
                return bbf;
            }
        }
