...
{
    /// <summary>
    /// R387(支払チェック表)を表すクラス・コントロール
    /// </summary>
    public class ReportInfoR387 : ReportInfoBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ReportInfoR387"/> class.
        /// </summary>
        public ReportInfoR387()
        {
            // 帳票出力フルパスフォーム名
            this.OutputFormFullPathName = "./Template/R387-Form.xml";
        ...
        /// を実行する
        /// </summary>
        public void R387_Report(DataTable chouhyouData, DateTime sysDate)
        {
            ...
            for (int i = 0; i < detail.Count; i++)
            {
                ...
                DateTime tempDate = sysDate;
                if (DateTime.TryParse(detail[i].Denpyoudate, out tempDate))
                {
                ...
                row["PHY_TORIHIKI_SHIHARAI_SAKI_VLB"] = detail[i].Torihikisakiname;   // 取引先
        ...
        {
            // ヘッダ部
            this.SetFieldName("FH_TITLE_VLB", "支払チェック表");                  // タイトル
            this.SetFieldName("FH_CORP_RYAKU_NAME_VLB", this.detail[0].CorpRyakuName);              // 会社名
            //this.SetFieldName("FH_PRINT_DATE_VLB", DateTime.Now.ToString());      // 発行日時
        ...
        private void InputDataToMem(DataTable dataTable)
        {
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                // Detail部
                detail.Add(new Detail(list[0], list[1], list[2], list[3], list[4], list[5], list[6], list[7], list[8], list[9], list[10]));
            }
        }
