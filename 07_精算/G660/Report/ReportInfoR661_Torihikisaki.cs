...
namespace Shougun.Core.Adjustment.ShiharaiMeisaiMeisaihyou
{
    ...
    //R661 支払明細明細表（取引先別）
    public class ReportInfoR661_Torihikisaki : ReportInfoBase
    {
       ...
       /// <param name="dto"></param>
        public ReportInfoR661_Torihikisaki(DataTable dataTableInput, DTOClass dto)
            : base(dataTableInput)
        {
           
            this.Title = "支払明細明細表（取引先別）";          
            this.OutputFormFullPathName = "./Template/R661-Form.xml";
        ...
        private void InputDataToMem(DataTable dataTable)
        {
            ...
            // 出金額 合計
            decimal KONKAI_SHUKKIN_GAKU_TOTAL = 0;
            // 調整額 合計
            decimal KONKAI_CHOUSEI_GAKU_TOTAL = 0;
            //// 繰越額 合計
            //decimal KURIKOSHI_GAKU_TOTAL = 0;
            // 今回取引額(税抜) 合計
            decimal KONKAI_SHIHARAI_GAKU_TOTAL = 0;
            // 消費税 合計
            decimal SHOUHIZEI_TOTAL = 0;
            //今回取引額 合計＝ 今回取引額(税抜) 合計 ＋ 消費税 合計
            decimal KONKAI_TORIHIKI_GAKU_TOTAL = 0;
            //// 今回御精算額 合計
            //decimal KONKAI_SEISAN_GAKU_TOTAL = 0;
            foreach (DataRow dtRow in dataTable.Rows)
            {
                string shiharaiKeitaiKbn = dtRow["SHIHARAI_KEITAI_KBN"] != null ? dtRow["SHIHARAI_KEITAI_KBN"].ToString() : string.Empty;
                ...
                //出金額
                temp = this.GetDecimal(dtRow["KONKAI_SHUKKIN_GAKU"]);
                temp = this.FractionCalc(temp, 3);
                if ("1".Equals(shiharaiKeitaiKbn))
                {
                    newRow["PHY_KONKAI_SHUKKIN_GAKU_VLB"] = string.Empty;
                }
                else
                {
                    newRow["PHY_KONKAI_SHUKKIN_GAKU_VLB"] = this.FormatDecimal(temp, 0);
                    KONKAI_SHUKKIN_GAKU_TOTAL += temp;
                }
                ...
                //今回取引額(税抜）
                decimal konkaiShiharaiGaku = this.GetDecimal(dtRow["KONKAI_SHIHARAI_GAKU"]);
                konkaiShiharaiGaku = this.FractionCalc(konkaiShiharaiGaku, 3);
                newRow["PHY_KONKAI_SHIHARAI_GAKU_VLB"] = this.FormatDecimal(konkaiShiharaiGaku, 1);
                KONKAI_SHIHARAI_GAKU_TOTAL += konkaiShiharaiGaku;
                ...
                //今回御精算額
                temp = this.GetDecimal(dtRow["KONKAI_SEISAN_GAKU"]);
                temp = this.FractionCalc(temp, 3);
                newRow["PHY_KONKAI_KURIKOSI_GAKU_VLB"] = this.FormatDecimal(temp, 1);
                //KONKAI_SEISAN_GAKU_TOTAL += temp;

                //出金予定日
                newRow["PHY_SHUKKIN_YOTEI_BI_VLB"] = this.GetStringDate(dtRow["SHUKKIN_YOTEI_BI"]);
                //支払年月日
                newRow["PHY_SEISAN_DATE_VLB"] = this.GetStringDate(dtRow["SEISAN_DATE"]);
            ...
            if (this.chouhyouDataTable.Rows.Count > 0)
            {
                ...
                //出金額 合計
                lastRow["PHN_KONKAI_SHUKKIN_GAKU_TOTAL_VLB"] = this.FormatDecimal(KONKAI_SHUKKIN_GAKU_TOTAL, 1);
                //調整額 合計
                ...
                //今回取引額(税抜）合計
                lastRow["PHN_KONKAI_SHIHARAI_GAKU_TOTAL_VLB"] = this.FormatDecimal(KONKAI_SHIHARAI_GAKU_TOTAL, 1);
                //消費税 合計
                ...
                ////今回御精算額 合計
                //lastRow["PHN_KONKAI_KURIKOSI_GAKU_TOTAL_VLB"] = this.FormatDecimal(KONKAI_SEISAN_GAKU_TOTAL, 1);
            }
        }
        /// <summary>
        /// 
