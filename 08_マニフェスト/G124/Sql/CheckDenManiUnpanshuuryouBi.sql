SELECT
    ...
    CASE
      WHEN R19.UPN_ROUTE_NO = '1' THEN '110'
      WHEN R19.UPN_ROUTE_NO = '2' THEN '111'
      WHEN R19.UPN_ROUTE_NO = '3' THEN '112'
      WHEN R19.UPN_ROUTE_NO = '4' THEN '113'
      WHEN R19.UPN_ROUTE_NO = '5' THEN '114'
    ELSE ''
    END AS CK_KOUMOKU
FROM
    DT_R18 AS R18
    ...
    LEFT JOIN DT_R19_EX AS R19_EX ON R19_EX.KANRI_ID = R19.KANRI_ID AND R19_EX.UPN_ROUTE_NO = R19.UPN_ROUTE_NO AND R19_EX.DELETE_FLG = 0 
    LEFT JOIN M_GYOUSHA AS UNPAN_GYOUSHA ON R19_EX.UPN_GYOUSHA_CD = UNPAN_GYOUSHA.GYOUSHA_CD
WHERE
    (R19.UPN_END_DATE IS NULL OR R19.UPN_END_DATE = '')
AND (ISNULL(R18.HST_SHA_EDI_MEMBER_ID, '') != ISNULL(R19.UPN_SHA_EDI_MEMBER_ID, ''))
/*IF !data.UPN_ROUTE_NO.IsNull && data.UPN_ROUTE_NO != ''*/AND R19.UPN_ROUTE_NO = /*data.UPN_ROUTE_NO*//*END*/
