...
FROM
    ...
    AND UPN.SEQ = TME.SEQ
    INNER JOIN M_GENBA AS MG
    ON  MG.GYOUSHA_CD = UPN.UPN_SAKI_GYOUSHA_CD
    AND MG.GENBA_CD = UPN.UPN_SAKI_GENBA_CD
    AND (MG.CHIIKI_CD IS NULL OR MG.CHIIKI_CD = '')
    AND MG.DELETE_FLG = 0
...
FROM
    ...
    ON TME.HAIKI_KBN_CD = MHK.HAIKI_KBN_CD
WHERE
((UPN.UPN_SAKI_GENBA_CD IS NULL) OR (UPN.UPN_SAKI_GENBA_CD = '') )
AND
TME.DELETE_FLG = 0
/* 20140623 ria EV004852 一覧と抽出条件の変更 start*/
/*IF data.BUNRUI != 5*/
