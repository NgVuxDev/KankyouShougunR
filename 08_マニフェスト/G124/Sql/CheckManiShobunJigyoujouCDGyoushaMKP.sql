SELECT
    ...
    TME.SYSTEM_ID,
    CASE
      WHEN UPN.UPN_ROUTE_NO = '1' THEN '101'
      WHEN UPN.UPN_ROUTE_NO = '2' THEN '102'
      WHEN UPN.UPN_ROUTE_NO = '3' THEN '103'
    ELSE ''
    END AS CK_KOUMOKU
FROM
...
(
    (
        GENBA.GENBA_CD IS NULL
        AND (UPN.UPN_SAKI_GENBA_CD IS NOT NULL OR UPN.UPN_SAKI_GENBA_CD <> '')
        AND (UPN2.UPN_GYOUSHA_CD IS NULL OR UPN2.UPN_GYOUSHA_CD = '')
    )
    OR
    (
        GENBA2.GENBA_CD IS NULL
        AND (TME.TMH_GENBA_CD IS NOT NULL OR TME.TMH_GENBA_CD <> '')
