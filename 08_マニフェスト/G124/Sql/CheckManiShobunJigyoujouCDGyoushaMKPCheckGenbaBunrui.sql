SELECT
    ...
    TME.SYSTEM_ID,
    CASE
      WHEN UPN.UPN_ROUTE_NO = '1' THEN '101_2'
      WHEN UPN.UPN_ROUTE_NO = '2' THEN '102_2'
      WHEN UPN.UPN_ROUTE_NO = '3' THEN '103_2'
    ELSE ''
    END AS CK_KOUMOKU
FROM
    ...
    AND UPN.SEQ = TME.SEQ
/*IF data.UPN_ROUTE_NO == 1*/
    INNER JOIN T_MANIFEST_UPN AS UPN2
    ON  UPN2.SYSTEM_ID = TME.SYSTEM_ID
    AND UPN2.SEQ = TME.SEQ
    AND UPN2.UPN_ROUTE_NO = 2
/*END*/
    LEFT OUTER JOIN M_GENBA AS GENBA
    ON  GENBA.GYOUSHA_CD = UPN.UPN_SAKI_GYOUSHA_CD
    AND GENBA.GENBA_CD = UPN.UPN_SAKI_GENBA_CD
/*IF data.UPN_ROUTE_NO == 1*/
    LEFT OUTER JOIN M_GENBA AS GENBA2
    ON  GENBA2.GYOUSHA_CD = TME.TMH_GYOUSHA_CD
    AND GENBA2.GENBA_CD = TME.TMH_GENBA_CD
/*END*/
...
WHERE
TME.DELETE_FLG = 0
AND ISNULL(GENBA.TSUMIKAEHOKAN_KBN, 0) != 1
AND ISNULL(GENBA.SHOBUN_NIOROSHI_GENBA_KBN, 0) != 1
AND ISNULL(GENBA.SAISHUU_SHOBUNJOU_KBN, 0) != 1
...
/*IF !data.UPN_ROUTE_NO.IsNull && data.UPN_ROUTE_NO != ''*/AND UPN.UPN_ROUTE_NO = /*data.UPN_ROUTE_NO*//*END*/
/*IF data.UPN_ROUTE_NO == 1*/
AND
(
    (
        (UPN.UPN_SAKI_GENBA_CD IS NOT NULL OR UPN.UPN_SAKI_GENBA_CD <> '')
        AND (UPN2.UPN_GYOUSHA_CD IS NULL OR UPN2.UPN_GYOUSHA_CD = '')
    )
    OR
    (
        (TME.TMH_GENBA_CD IS NOT NULL OR TME.TMH_GENBA_CD <> '')
        AND (UPN2.UPN_GYOUSHA_CD IS NOT NULL AND UPN2.UPN_GYOUSHA_CD <> '')
    )
)/*END*/
/*IF data.UPN_ROUTE_NO == 2*/
AND
(
    (
        (UPN.UPN_SAKI_GENBA_CD IS NOT NULL OR UPN.UPN_SAKI_GENBA_CD <> '')
        AND (UPN.UPN_GYOUSHA_CD IS NOT NULL AND UPN.UPN_GYOUSHA_CD <> '')
    )
)/*END*/
/*IF data.JOUKEN == '1'*/
        AND exists (
