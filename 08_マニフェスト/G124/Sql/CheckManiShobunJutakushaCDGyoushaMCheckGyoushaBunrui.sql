...
FROM
    ...
    AND UPN.SEQ = TME.SEQ
    LEFT OUTER JOIN M_GYOUSHA AS GYOUSHA
    ON  GYOUSHA.GYOUSHA_CD = TME.SBN_GYOUSHA_CD
...
WHERE
(
  TME.SBN_GYOUSHA_CD IS NOT NULL
  OR TME.SBN_GYOUSHA_CD <> ''
    )
AND ISNULL(GYOUSHA.SHOBUN_NIOROSHI_GYOUSHA_KBN, 0) != 1
AND UPN.UPN_ROUTE_NO = 1
