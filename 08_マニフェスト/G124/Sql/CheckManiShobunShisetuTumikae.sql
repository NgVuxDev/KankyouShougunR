...
FROM
    ...
    LEFT JOIN (SELECT SYSTEM_ID, SEQ, MAX(UPN_ROUTE_NO) AS 区間数 
		FROM T_MANIFEST_UPN WHERE UPN_SAKI_GENBA_CD <> '' GROUP BY SYSTEM_ID, SEQ) AS UPN_ALL
		ON  UPN_ALL.SYSTEM_ID = TME.SYSTEM_ID
		AND UPN_ALL.SEQ = TME.SEQ
    LEFT JOIN (SELECT SYSTEM_ID, SEQ, Count(*) AS 施設件数, MAX(UPN_ROUTE_NO) AS 施設最終区間  
		FROM T_MANIFEST_UPN WHERE UPN_SAKI_GENBA_CD <> '' AND UPN_SAKI_KBN = 1 GROUP BY SYSTEM_ID, SEQ) AS UPN_SISETU
    ON  UPN_SISETU.SYSTEM_ID = TME.SYSTEM_ID
    AND UPN_SISETU.SEQ = TME.SEQ
...
AND ((UPN_SISETU.SYSTEM_ID IS  NULL)
OR (UPN_SISETU.施設件数 <> 1)
OR (UPN_SISETU.施設最終区間 <> UPN_ALL.区間数))
