...
(
SELECT
          MANI_ETY.HST_GYOUSHA_CD AS HST_GYOUSHA_CD,
		  MANI_ETY.HST_GENBA_CD AS HST_GENBA_CD,
...
--PTN4 排出事業者別 WHERE句 >>
AND MANI_ETY.HST_GYOUSHA_CD IS NOT NULL AND MANI_ETY.HST_GYOUSHA_CD <> ''
AND MANI_ETY.HST_GENBA_CD IS NOT NULL AND MANI_ETY.HST_GENBA_CD <> ''
--PTN4 排出事業者別 WHERE句 <<

-- PTN4 排出事業者別 GROUP BY句 >>
GROUP BY     MANI_ETY.HST_GYOUSHA_CD,MANI_ETY.HST_GENBA_CD,SUBSTRING(CONVERT(VARCHAR, MANI_ETY.KOUFU_DATE, 111),1,7),MANI_DTL.HAIKI_SHURUI_CD, M_HIK_SRI.HAIKI_SHURUI_NAME
-- PTN4 排出事業者別 GROUP BY句 <<
