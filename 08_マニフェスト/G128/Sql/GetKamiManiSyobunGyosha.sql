...
(
SELECT
          MANI_UPN.UPN_SAKI_GYOUSHA_CD AS UPN_SAKI_GYOUSHA_CD,MANI_UPN.UPN_SAKI_GENBA_CD AS UPN_SAKI_GENBA_CD,
...
--PTN3 処分受託者別 WHERE句 >>
AND MANI_UPN.UPN_SAKI_GYOUSHA_CD IS NOT NULL AND MANI_UPN.UPN_SAKI_GYOUSHA_CD <> ''
AND MANI_UPN.UPN_SAKI_GENBA_CD IS NOT NULL AND MANI_UPN.UPN_SAKI_GENBA_CD <> ''
--PTN3 処分受託者別 WHERE句 <<

-- PTN3 処分受託者別 GROUP BY句 >>
GROUP BY     MANI_UPN.UPN_SAKI_GYOUSHA_CD,MANI_UPN.UPN_SAKI_GENBA_CD,SUBSTRING(CONVERT(VARCHAR, MANI_ETY.KOUFU_DATE, 111),1,7),MANI_DTL.HAIKI_SHURUI_CD, M_HIK_SRI.HAIKI_SHURUI_NAME
-- PTN3 処分受託者別 GROUP BY句 <<
...
LEFT JOIN
          M_GYOUSHA AS M_GSH  
  ON        T_SUII.UPN_SAKI_GYOUSHA_CD = M_GSH.GYOUSHA_CD
--現場マスタ JOIN
LEFT JOIN
          M_GENBA AS M_GNB  
  ON        T_SUII.UPN_SAKI_GYOUSHA_CD = M_GNB.GYOUSHA_CD
  AND       T_SUII.UPN_SAKI_GENBA_CD = M_GNB.GENBA_CD
ORDER BY	
	UPN_SAKI_GYOUSHA_CD
,	M_GSH.GYOUSHA_NAME_RYAKU
,	UPN_SAKI_GENBA_CD
