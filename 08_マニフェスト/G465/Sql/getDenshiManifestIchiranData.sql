...
SELECT		--非混廃
	...
	,R18_EX.SEQ											        --SEQ　非表示
	,'' AS DETAIL_SYSTEM_ID										--明細システムID　非表示
	...
	,(R18.HAIKI_DAI_CODE + R18.HAIKI_CHU_CODE + R18.HAIKI_SHO_CODE) AS HAIKI_SHURUI_CD		--廃棄物種類CD　非表示
	...
	,R18.HAIKI_SUU AS HAIKI_SUU						                --廃棄物の数量　表示
	,R18.HAIKI_UNIT_CODE AS HAIKI_UNIT_CD				            --廃棄物の数量単位コード　非表示
	-- 換算用数量と単位CD（非混合電子用、非表示） start　
	,R18.HAIKI_KAKUTEI_SUU AS HAIKI_KAKUTEI_SUU						--廃棄物の確定数量
	,R18.HAIKI_KAKUTEI_UNIT_CODE AS HAIKI_KAKUTEI_UNIT_CODE			--廃棄物の確定数量単位コード　

	,MEMBER_SU1.EDI_PASSWORD AS SU1_UPN_SHA_EDI_PASSWORD            -- 加入者情報マスタ(収集運搬1).EDI_EDI_PASSWORD
	,R19_SU1.UPN_SUU AS SU1_UPN_SUU                                 -- 運搬量(収集運搬1)
	,R19_SU1.UPN_UNIT_CODE AS SU1_UPN_UNIT_CODE                     -- 運搬単位コード(収集運搬1)

	,MEMBER_SU2.EDI_PASSWORD AS SU2_UPN_SHA_EDI_PASSWORD            -- 加入者情報マスタ(収集運搬2).EDI_EDI_PASSWORD
	,R19_SU2.UPN_SUU AS SU2_UPN_SUU                                 -- 運搬量(収集運搬1)
	,R19_SU2.UPN_UNIT_CODE AS SU2_UPN_UNIT_CODE                     -- 運搬単位コード(収集運搬2)

	,MEMBER_SU3.EDI_PASSWORD AS SU3_UPN_SHA_EDI_PASSWORD            -- 加入者情報マスタ(収集運搬3).EDI_EDI_PASSWORD
	,R19_SU3.UPN_SUU AS SU3_UPN_SUU                                 -- 運搬量(収集運搬3)
	,R19_SU3.UPN_UNIT_CODE AS SU3_UPN_UNIT_CODE                     -- 運搬単位コード(収集運搬3)

	,MEMBER_SU4.EDI_PASSWORD AS SU4_UPN_SHA_EDI_PASSWORD            -- 加入者情報マスタ(収集運搬4).EDI_EDI_PASSWORD
	,R19_SU4.UPN_SUU AS SU4_UPN_SUU                                 -- 運搬量(収集運搬4)
	,R19_SU4.UPN_UNIT_CODE AS SU4_UPN_UNIT_CODE                     -- 運搬単位コード(収集運搬4)

	,MEMBER_SU5.EDI_PASSWORD AS SU5_UPN_SHA_EDI_PASSWORD            -- 加入者情報マスタ(収集運搬5).EDI_EDI_PASSWORD
	,R19_SU5.UPN_SUU AS SU5_UPN_SUU                                 -- 運搬量(収集運搬5)
	,R19_SU5.UPN_UNIT_CODE AS SU5_UPN_UNIT_CODE                     -- 運搬単位コード(収集運搬5)

	,R18.RECEPT_SUU AS RECEPT_SUU                                   -- 処分量
	,R18.RECEPT_UNIT_CODE AS RECEPT_UNIT_CODE                       -- 処分単位コード
	-- 換算用数量と単位CD end
	,HAIKI_UNIT.UNIT_NAME_RYAKU AS HAIKI_UNIT_NAME					--廃棄物の数量単位名　表示
	,R18_EX.KANSAN_SUU AS OLD_KANSAN_SUU					    --換算後数量(変更前)　表示
	,R18_EX.GENNYOU_SUU AS OLD_GENNYOU_SUU					    --減容後数量(変更前)　表示
	...
	,R18_EX.HAIKI_NAME_CD										--廃棄物名称CD　非表示
	,R18.HAIKI_NAME AS HAIKI_NAME								--廃棄物名称　表示
	,R18.NISUGATA_CODE AS NISUGATA_CD							--荷姿CD　非表示
	,R18.NISUGATA_NAME AS NISUGATA_NAME							--荷姿名　表示
	,R18_EX.SBN_HOUHOU_CD AS SBN_HOUHOU_CD						--処分方法CD　非表示
	...
	,CASE
		...
		ELSE R18.HAIKI_KAKUTEI_SUU END AS DEN_OLD_KANSAN_SUU	--電マニ換算前（帳票数量）　表示
	,CASE
		WHEN R18.HAIKI_KAKUTEI_SUU IS NULL THEN HAIKI_UNIT.UNIT_NAME_RYAKU
		ELSE HAIKI_KAKUTEI_UNIT.UNIT_NAME_RYAKU END AS DEN_OLD_KANSAN_UNIT_NAME	--換算前単位　表示
	/*END*/

	/*IF searchInfo.MANIFEST_REPORT_SUU_KBN == 2*/
	,R18.HAIKI_SUU AS DEN_OLD_KANSAN_SUU                       --電マニ換算前（帳票数量）　表示
	,HAIKI_UNIT.UNIT_NAME_RYAKU AS DEN_OLD_KANSAN_UNIT_NAME          --換算前単位　表示
	/*END*/
	...
	,CASE 
	 ...
	 ELSE R19_SU5.UPN_SUU END AS DEN_OLD_KANSAN_SUU	            --電マニ換算前（帳票数量）　表示
	...
	,CASE
	 ...
	 ELSE SU5_UPN_UNIT.UNIT_NAME_RYAKU END AS DEN_OLD_KANSAN_UNIT_NAME	   --換算前単位　表示
	/*END*/

	/*IF searchInfo.MANIFEST_REPORT_SUU_KBN == 4*/
	,R18.RECEPT_SUU AS DEN_OLD_KANSAN_SUU                       --電マニ換算前（帳票数量）　表示
	,RECEPT_UNIT.UNIT_NAME_RYAKU AS DEN_OLD_KANSAN_UNIT_NAME          --換算前単位　表示
	/*END*/
