...
namespace Report
{
    ...
    /// を表すクラス・コントロール
    /// </summary>
    public class ReportInfoR691 : Shougun.Core.PaperManifest.InsatsuBusuSettei.Report.ReportInfoBaseG489
    {
        ...
        private string mercury_used_seihin_haikibutu_check = string.Empty;   // 水銀使用製品産業廃棄物
        private string mercury_baijinnado_haikibutu_check = string.Empty;    // 水銀含有ばいじん等
        private string isiwakanado_haikibutu_check = string.Empty;           // 石綿含有産業廃棄物
        private string tokutei_sangyou_haikibutu_check = string.Empty;       // 特定産業廃棄物
        ...
        /// <summary>斜線項目水銀石綿</summary>
        private bool SLASH_MERCURY_FLG = false;                     //斜線項目水銀石綿
        ...
        public ReportInfoR691()
        {
            // 帳票出力フルパスフォーム名
            //this.OutputFormFullPathName = TEMPLATE_PATH + "R691-Form.xml";
            this.OutputFormFullPathName = "Template/R691-Form.xml";
            //this.OutputFormFullPathName = "R691-Form.xml";
        ...
        public void R691_Report(DataTable chouhyouData)
        {
        ...
        protected override void UpdateFieldsStatus()
        {
            ...
            this.SetFieldName("FH_MERCURY_USED_SEIHIN_HAIKIBUTU_CHECK_CTL", this.mercury_used_seihin_haikibutu_check);    // 水銀使用製品産業廃棄物
            this.SetFieldName("FH_MERCURY_BAIJINNADO_HAIKIBUTU_CHECK_CTL", this.mercury_baijinnado_haikibutu_check);      // 水銀含有ばいじん等
            this.SetFieldName("FH_ISIWAKANADO_HAIKIBUTU_CHECK_CTL", this.isiwakanado_haikibutu_check);                    // 石綿含有産業廃棄物
            this.SetFieldName("FH_TOKUTEI_SANGYOU_HAIKIBUTU_CHECK_CTL", this.tokutei_sangyou_haikibutu_check);            // 特定産業廃棄物
            ...
            //水銀石綿
            slashVisivle = this.SLASH_MERCURY_FLG;
            this.SetFieldVisible("FH_PRT_SLASH_MERCURY_BIKOU_FLG", slashVisivle);
            //関連項目は斜線と反対（！）の表示
            this.SetFieldVisible("FH_MERCURY_USED_SEIHIN_HAIKIBUTU_CHECK_CTL", !slashVisivle);
            this.SetFieldVisible("FH_MERCURY_BAIJINNADO_HAIKIBUTU_CHECK_CTL", !slashVisivle);
            this.SetFieldVisible("FH_ISIWAKANADO_HAIKIBUTU_CHECK_CTL", !slashVisivle);
            this.SetFieldVisible("FH_TOKUTEI_SANGYOU_HAIKIBUTU_CHECK_CTL", !slashVisivle);
        ...
        private void InputDataToMem(DataTable dataTable)
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                if (list[0] == "1-1")
                {
                    ...
                    if (list[33] != null && !string.IsNullOrWhiteSpace(list[33]) && list[34] != null && !string.IsNullOrWhiteSpace(list[34]))
                    {
                        this.kamimainh = "H: " + list[33] + "-" + list[34];
                    }

                    if (list[35] != null && !string.IsNullOrWhiteSpace(list[35]) && list[36] != null && !string.IsNullOrWhiteSpace(list[36]))
                    {
                ...
                else if (list[0] == "2-1")
                {
                    ...
                    if (list[14] != null && !string.IsNullOrWhiteSpace(list[14]))
                    {
                ...
                else if (list[0] == "4-1")
                {
                    ...
                    // 水銀使用製品産業廃棄物
                    if (!string.IsNullOrEmpty(list[8]) && bool.Parse(list[8]))
                    {
                        this.mercury_used_seihin_haikibutu_check = "✓";
                    }
                    // 水銀含有ばいじん等
                    if (!string.IsNullOrEmpty(list[9]) && bool.Parse(list[9]))
                    {
                        this.mercury_baijinnado_haikibutu_check = "✓";
                    }
                    // 石綿含有産業廃棄物
                    if (!string.IsNullOrEmpty(list[10]) && bool.Parse(list[10]))
                    {
                        this.isiwakanado_haikibutu_check = "✓";
                    }
                    // 特定産業廃棄物
                    if (!string.IsNullOrEmpty(list[11]) && bool.Parse(list[11]))
                    {
                        this.tokutei_sangyou_haikibutu_check = "✓";
                    }
                }
                else if (list[0] == "7-1")
                {
                    ...
                    this.SLASH_MERCURY_FLG = bool.Parse(list[18]); //斜線項目水銀石綿
                }
            }
