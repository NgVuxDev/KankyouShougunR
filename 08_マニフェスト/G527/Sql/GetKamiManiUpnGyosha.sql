-- マニフェスト推移表 紙マニフェスト 運搬受託者別  --

SELECT
          CASE HAIKI_KBN_CD WHEN 1 THEN '直行' WHEN 2 THEN '建廃' WHEN 3 THEN '積替' ELSE '' END AS HAIKI_KBN_CD
          ,UPN_GYOUSHA_CD
...
(
SELECT
          MANI_ETY.HAIKI_KBN_CD AS HAIKI_KBN_CD,MANI_UPN.UPN_GYOUSHA_CD AS UPN_GYOUSHA_CD,
...
--PTN2 運搬受託者別 WHERE句 >>
AND MANI_UPN.UPN_GYOUSHA_CD IS NOT NULL AND MANI_UPN.UPN_GYOUSHA_CD <> ''
--PTN2 運搬受託者別 WHERE句 <<

-- PTN2 運搬受託者別 GROUP BY句 >>
GROUP BY     MANI_ETY.HAIKI_KBN_CD,MANI_UPN.UPN_GYOUSHA_CD,SUBSTRING(CONVERT(VARCHAR, MANI_ETY.KOUFU_DATE, 111),1,7),MANI_DTL.HAIKI_SHURUI_CD, M_HIK_SRI.HAIKI_SHURUI_NAME
-- PTN2 運搬受託者別 GROUP BY句 <<
...
LEFT JOIN
          M_GYOUSHA AS M_GSH  
  ON        T_SUII.UPN_GYOUSHA_CD = M_GSH.GYOUSHA_CD
  AND       M_GSH.DELETE_FLG = 0

ORDER BY    HAIKI_KBN_CD,UPN_GYOUSHA_CD
			,M_GSH.GYOUSHA_NAME_RYAKU
			,HAIKI_SHURUI_CD
			,KOUFU_YM