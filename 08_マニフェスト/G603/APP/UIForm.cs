...
namespace Shougun.Core.PaperManifest.JissekiHokokuSisetsu
{
    ...
    public partial class UIForm : SuperForm
    {
        ...
        /// </summary>
        public UIForm()
            : base(WINDOW_ID.T_JISSEKIHOKOKU_SISETSU, WINDOW_TYPE.NONE)
        {
        ...
        public void bt_func3_Click(object sender, EventArgs e)
        {
            ...
            {
                LogUtility.DebugMethodStart(sender, e);
                FormManager.OpenFormWithAuth("G134", WINDOW_TYPE.REFERENCE_WINDOW_FLAG, WINDOW_TYPE.NONE);
            }
            catch (Exception ex)
            {
                // 例外エラー
        ...
        {
            LogUtility.DebugMethodStart(sender, e);
            this.messageShowLogic.MessageBoxShow("I022", "「対象期間」「廃棄物区分」「処分事業場抽出条件」");
            var autoCheckLogic = new AutoRegistCheckLogic(this.GetAllControl(), this.GetAllControl());
            base.RegistErrorFlag = autoCheckLogic.AutoRegistCheck();
            if (this.RegistErrorFlag)
            {
        ...
        internal void HOUKOKU_GYOUSHA_CD_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                bool catchErr = false;
                M_GYOUSHA[] results = this.logic.GetGyousha(this.HOUKOKU_GYOUSHA_CD.Text, out catchErr);
                if (catchErr) { return; }
