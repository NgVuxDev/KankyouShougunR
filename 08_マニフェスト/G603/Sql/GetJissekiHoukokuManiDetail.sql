...
(
	...
	,TMD.HAIKI_SHURUI_CD AS DETAIL_HAIKI_SHURUI_CD
	,MHS.HAIKI_SHURUI_NAME AS DETAIL_HAIKI_SHURUI_NAME
...
FROM 
	...
	LEFT JOIN (SELECT TMU1.* FROM T_MANIFEST_ENTRY TME1 INNER JOIN T_MANIFEST_UPN TMU1 ON TMU1.SYSTEM_ID = TME1.SYSTEM_ID AND TMU1.SEQ = TME1.SEQ AND TMU1.UPN_ROUTE_NO = 1 WHERE (TME1.HAIKI_KBN_CD = 1 OR TME1.HAIKI_KBN_CD =2) AND TME1.DELETE_FLG =0 AND TME1.SEQ = (SELECT MAX(SEQ) FROM T_MANIFEST_ENTRY WHERE SYSTEM_ID = TME1.SYSTEM_ID)
	           ...
	           SELECT TMU2.* FROM T_MANIFEST_ENTRY TME2 INNER JOIN T_MANIFEST_UPN TMU2 ON TMU2.SYSTEM_ID = TME2.SYSTEM_ID AND TMU2.SEQ = TME2.SEQ AND TME2.HAIKI_KBN_CD = 3  AND TMU2.UPN_ROUTE_NO = (
	                                                                              SELECT MIN(UPN_ROUTE_NO) FROM T_MANIFEST_UPN WHERE TME2.SYSTEM_ID = SYSTEM_ID AND SEQ = TME2.SEQ AND TME2.HAIKI_KBN_CD = 3 AND UPN_SAKI_KBN = 1) AND TME2.DELETE_FLG = 0) TMU ON TMU.SYSTEM_ID = TME.SYSTEM_ID AND TMD.SEQ = TME.SEQ
	...
	LEFT JOIN T_MANIFEST_RELATION TMR ON  TMR.FIRST_SYSTEM_ID = TMD.DETAIL_SYSTEM_ID AND TMR.DELETE_FLG = 0 AND TMR.FIRST_HAIKI_KBN_CD = TME.HAIKI_KBN_CD
	LEFT JOIN ( SELECT
	...
	LEFT JOIN T_MANIFEST_DETAIL NEXT_TMD ON NEXT_TMD.SYSTEM_ID = NEXT_TME.SYSTEM_ID AND NEXT_TMD.SEQ = NEXT_TME.SEQ AND NEXT_TMD.DETAIL_SYSTEM_ID = NEXT_TME.DETAIL_SYSTEM_ID AND TMR.NEXT_HAIKI_KBN_CD != 4
	...
	LEFT JOIN M_DENSHI_HAIKI_SHURUI NEXT_DEN_MDHS ON NEXT_DEN_MDHS.HAIKI_SHURUI_CD = (NEXT_DTR18.HAIKI_DAI_CODE + NEXT_DTR18.HAIKI_CHU_CODE + NEXT_DTR18.HAIKI_SHO_CODE) AND TMR.NEXT_HAIKI_KBN_CD = 4
...
SELECT 
	...
	,'4' AS HAIKI_KBN_CD
	,DTR18.HAIKI_DAI_CODE + DTR18.HAIKI_CHU_CODE + DTR18.HAIKI_SHO_CODE AS DETAIL_HAIKI_SHURUI_CD
	,DTR18.HAIKI_SHURUI_NAME AS DETAIL_HAIKI_SHURUI_NAME
	,MCS.HOUKOKU_SHISETSU_NAME AS SHORI_SHISETSU_NAME
	,CASE WHEN TMR.NEXT_SYSTEM_ID IS NOT NULL THEN 1 ELSE 0 END AS NEXT_KBN
	,TMR.NEXT_SYSTEM_ID AS NEXT_SYSTEM_ID
	,CASE WHEN TMR.NEXT_SYSTEM_ID IS NOT NULL AND TMR.NEXT_HAIKI_KBN_CD != 4 THEN NEXT_MHS.HOUKOKUSHO_BUNRUI_CD
...
FROM 
	...
	LEFT JOIN DT_R19 DTR19 ON DTR19.KANRI_ID = DTR18.KANRI_ID AND DTR19.SEQ = DTR18.SEQ AND  DTR19.UPN_ROUTE_NO = (SELECT DISTINCT MIN(UPN_ROUTE_NO) FROM DT_R19 WHERE KANRI_ID = DTR19.KANRI_ID AND SEQ = DTR19.SEQ)
	...
	LEFT JOIN DT_R13_EX DTR13EX ON DTR13EX.KANRI_ID = DTR13.KANRI_ID AND DTR13EX.DELETE_FLG = 0
	
	LEFT JOIN M_CHIIKIBETSU_SHISETSU MCS ON MCS.SHOBUN_HOUHOU_CD = DTR18.SBN_HOUHOU_CD AND MCS.CHIIKI_CD = /*data.TEISHUTSU_CHIIKI_CD*/0
	LEFT JOIN M_DENSHI_HAIKI_SHURUI MHS ON MHS.HAIKI_SHURUI_CD = (DTR18.HAIKI_DAI_CODE + DTR18.HAIKI_CHU_CODE + DTR18.HAIKI_SHO_CODE)
	LEFT JOIN M_CHIIKIBETSU_BUNRUI MCB ON MCB.HOUKOKUSHO_BUNRUI_CD = MHS.HOUKOKUSHO_BUNRUI_CD AND MCB.CHIIKI_CD = /*data.TEISHUTSU_CHIIKI_CD*/0

	LEFT JOIN T_MANIFEST_RELATION TMR ON  TMR.FIRST_SYSTEM_ID = DTR18.SYSTEM_ID AND TMR.DELETE_FLG = 0 AND TMR.FIRST_HAIKI_KBN_CD = 4
	LEFT JOIN ( SELECT
					MENTRY.*, MDETAIL.DETAIL_SYSTEM_ID
				FROM
					T_MANIFEST_ENTRY AS MENTRY
	...
	LEFT JOIN (SELECT TMU1.* FROM T_MANIFEST_ENTRY TME1 INNER JOIN T_MANIFEST_UPN TMU1 ON TMU1.SYSTEM_ID = TME1.SYSTEM_ID AND TMU1.SEQ = TME1.SEQ AND TMU1.UPN_ROUTE_NO = 1 WHERE (TME1.HAIKI_KBN_CD = 1 OR TME1.HAIKI_KBN_CD = 2) AND TME1.DELETE_FLG =0 AND TME1.SEQ = (SELECT MAX(SEQ) FROM T_MANIFEST_ENTRY WHERE SYSTEM_ID = TME1.SYSTEM_ID)
	           ...
	           SELECT TMU2.* FROM T_MANIFEST_ENTRY TME2 INNER JOIN T_MANIFEST_UPN TMU2 ON TMU2.SYSTEM_ID = TME2.SYSTEM_ID AND TMU2.SEQ = TME2.SEQ AND TME2.HAIKI_KBN_CD = 3  AND TMU2.UPN_ROUTE_NO = (
	                                                                              SELECT MIN(UPN_ROUTE_NO) FROM T_MANIFEST_UPN WHERE TME2.SYSTEM_ID = SYSTEM_ID AND SEQ = TME2.SEQ AND TME2.HAIKI_KBN_CD = 3 AND UPN_SAKI_KBN = 1) AND TME2.DELETE_FLG = 0 ) NEXT_TMU ON NEXT_TMU.SYSTEM_ID = NEXT_TME.SYSTEM_ID AND NEXT_TMU.SEQ = NEXT_TME.SEQ AND TMR.NEXT_HAIKI_KBN_CD != 4
	LEFT JOIN T_MANIFEST_DETAIL NEXT_TMD ON NEXT_TMD.SYSTEM_ID = NEXT_TME.SYSTEM_ID AND NEXT_TMD.DETAIL_SYSTEM_ID = NEXT_TME.DETAIL_SYSTEM_ID AND NEXT_TMD.SEQ = NEXT_TME.SEQ AND TMR.NEXT_HAIKI_KBN_CD != 4
	LEFT JOIN M_HAIKI_SHURUI NEXT_MHS ON NEXT_MHS.HAIKI_SHURUI_CD = NEXT_TMD.HAIKI_SHURUI_CD AND NEXT_MHS.HAIKI_KBN_CD = TMR.NEXT_HAIKI_KBN_CD AND TMR.NEXT_HAIKI_KBN_CD != 4
	LEFT JOIN M_CHIIKIBETSU_BUNRUI NEXT_MCB ON NEXT_MHS.HOUKOKUSHO_BUNRUI_CD = NEXT_MCB.HOUKOKUSHO_BUNRUI_CD AND NEXT_MCB.CHIIKI_CD = /*data.TEISHUTSU_CHIIKI_CD*/0 AND TMR.NEXT_HAIKI_KBN_CD != 4
	LEFT JOIN M_CHIIKIBETSU_SHOBUN NEXT_MCS ON NEXT_MCS.SHOBUN_HOUHOU_CD = NEXT_TMD.SBN_HOUHOU_CD AND NEXT_MCS.CHIIKI_CD = /*data.TEISHUTSU_CHIIKI_CD*/0 AND TMR.NEXT_HAIKI_KBN_CD != 4
	...
	LEFT JOIN DT_R19 NEXT_DTR19 ON NEXT_DTR19.KANRI_ID = NEXT_DTR19EX.KANRI_ID AND NEXT_DTR19.SEQ = NEXT_DTR19EX.SEQ AND TMR.NEXT_HAIKI_KBN_CD = 4
	LEFT JOIN DT_R18 NEXT_DTR18 ON NEXT_DTR18.KANRI_ID = NEXT_DTR18EX.KANRI_ID AND NEXT_DTR18EX.SEQ = NEXT_DTR18EX.SEQ AND TMR.NEXT_HAIKI_KBN_CD = 4
	LEFT JOIN M_DENSHI_HAIKI_SHURUI NEXT_DEN_MDHS ON NEXT_DEN_MDHS.HAIKI_SHURUI_CD = (DTR18.HAIKI_DAI_CODE + DTR18.HAIKI_CHU_CODE + DTR18.HAIKI_SHO_CODE) AND TMR.NEXT_HAIKI_KBN_CD = 4
	LEFT JOIN M_CHIIKIBETSU_BUNRUI NEXT_DEN_MCB ON NEXT_DEN_MCB.CHIIKI_CD = /*data.TEISHUTSU_CHIIKI_CD*/0 AND NEXT_DEN_MCB.HOUKOKUSHO_BUNRUI_CD = NEXT_DEN_MDHS.HOUKOKUSHO_BUNRUI_CD AND TMR.NEXT_HAIKI_KBN_CD = 4
	LEFT JOIN M_CHIIKIBETSU_SHOBUN NEXT_DEN_MCS ON NEXT_DEN_MCS.SHOBUN_HOUHOU_CD = NEXT_DTR18EX.SBN_HOUHOU_CD AND NEXT_DEN_MCS.CHIIKI_CD = /*data.TEISHUTSU_CHIIKI_CD*/0 AND TMR.NEXT_HAIKI_KBN_CD = 4
	LEFT JOIN M_UNIT MU ON MU.UNIT_CD = (SELECT MANI_KANSAN_KIHON_UNIT_CD FROM M_SYS_INFO) 

	LEFT JOIN M_GENBA HSTMGA ON HSTMGA.GYOUSHA_CD = DTR18.HST_GYOUSHA_CD AND HSTMGA.GENBA_CD = DTR18.HST_GENBA_CD
    LEFT JOIN M_CHIIKI HSTMC ON HSTMC.CHIIKI_CD = HSTMGA.CHIIKI_CD
    LEFT JOIN M_TODOUFUKEN HSTMT ON HSTMT.TODOUFUKEN_CD = HSTMC.TODOUFUKEN_CD
    LEFT JOIN M_GYOUSHA HST_GYOUSHA ON DTR18.HST_GYOUSHA_CD = HST_GYOUSHA.GYOUSHA_CD
...
WHERE 1=1
	...
	/*IF data.JIGYOUJOU_KBN.Value == 1*/
		AND ((DTR18.SBN_END_DATE IS NOT NULL
			...
			AND (ISNULL(DTR18.FIRST_MANIFEST_FLAG,'') = '' OR ISNULL(HST_GYOUSHA.JISHA_KBN, 0) = 0)
		     )
		  --   OR
			 --(
...
select tb.* 
, CASE WHEN tb.NEXT_SYSTEM_ID IS NOT NULL THEN
		(SELECT 
		count(*)
		FROM T_MANIFEST_DETAIL
		WHERE DETAIL_SYSTEM_ID = tb.NEXT_SYSTEM_ID AND SEQ = (SELECT max(SEQ) FROM T_MANIFEST_DETAIL WHERE DETAIL_SYSTEM_ID = tb.NEXT_SYSTEM_ID))
	ELSE 0
END AS DETAIL_COUNT
from tb
