...
namespace Shougun.Core.ElectronicManifest.ShobunShuryouHoukoku
{
    public partial class UIForm : Shougun.Core.Common.IchiranCommon.APP.IchiranSuperForm
    {
        ...
        /// 画面ロジック
        /// </summary>
        private ElectronicManifest.ShobunShuryouHoukoku.LogicClass MILogic;
        ...
        /// </summary>
        public UIForm()
            : base(DENSHU_KBN.SHOBUN_SYUURYOU_HOUKOKU, false)
        {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            else if (Manager.CheckAuthority("G145", WINDOW_TYPE.REFERENCE_WINDOW_FLAG, false))
            {
        ...
        {
            var dtoList = new List<SyobunnShuryouHoukokuIkkatuNyuuryoku.InputInfoDTOCls>();
            foreach (DataGridViewRow row in this.IchiranDgv1.Rows)
            {
                ...
                {
                    var input = new SyobunnShuryouHoukokuIkkatuNyuuryoku.InputInfoDTOCls()
                    {
                        KANRI_ID = row.Cells["管理番号"].Value.ToString(),
                        SEQ = row.Cells["枝番"].Value.ToString(),
                        GYOUSYA_EDI_MEMBER_ID = row.Cells["UPN_SHA_EDI_MEMBER_ID"].Value.ToString(),
                        SAKI_GYOUSYA_EDI_MEMBER_ID = row.Cells["SBN_SHA_MEMBER_ID"].Value.ToString(),
            ...
            var callForm = new Shougun.Core.ElectronicManifest.SyobunnShuryouHoukokuIkkatuNyuuryoku.UIForm();
        ...
        public virtual void bt_func6_Click(object sender, EventArgs e)
        {
            ...
            CSVExp.ConvertCustomDataGridViewToCsv(this.IchiranDgv1, true, true, DENSHU_KBN.SHOBUN_SYUURYOU_HOUKOKU.ToTitleString(), this);
        }
        ...
        public void IchiranDgv1_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            if ("操作CD".Equals(HeaderText)
                || "処分担当者CD".Equals(HeaderText)
                || "報告区分CD".Equals(HeaderText)
                || "報告担当者CD".Equals(HeaderText)
                || "運搬担当者CD".Equals(HeaderText)
                || "車輌番号CD".Equals(HeaderText)
                || "処分終了日".Equals(HeaderText)
                || "廃棄物受領日".Equals(HeaderText)
                || "受入量".Equals(HeaderText)
                || "受入量単位CD".Equals(HeaderText))
            {
            ...
            else if ("処分担当者".Equals(HeaderText)
                || "報告担当者".Equals(HeaderText)
                || "運搬担当者".Equals(HeaderText)
                || "車輌".Equals(HeaderText))
            {
                this.IchiranDgv1.ImeMode = ImeMode.Hiragana;
        ...
        /// <param name="sender">イベントが発生したオブジェクト</param>
        /// <param name="e">イベント引数</param>
        public void IchiranDgv11_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        }
        ...
        private void IchiranDgv1_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                ...
                if (columnName == "操作CD")
                {
                    ...
                    this.SetRowReadOnly(e.RowIndex);
                }
            }
        }
