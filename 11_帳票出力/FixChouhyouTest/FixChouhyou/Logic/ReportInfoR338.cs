...
namespace FixChouhyou
{
    ...
    public class ReportInfoR338 : ReportInfoBase
    {
        ...
        public void CreateSampleData()
        {
            ...
            // 部門名
            dataTableTmp.Columns.Add("BUMON");
            ...
            if (isPrintH)
            {
                ...
                // タイトル名
                rowTmp["TITLE"] = "搬入御請求書,御請求書（控）,計量証明";
                // 担当名
                rowTmp["TANTOU"] = "あいうえおかきくけこさしすせそ";
                // お取引先CD
                rowTmp["TORIHIKISAKICD"] = "123456";
                // お取引先名
                rowTmp["TORIHIKISAKIMEI"] = "あいうえおかきくけこさしすせそたちつてとなにぬねの";
                // お取引先名敬称
                rowTmp["TORIHIKISAKIKEISHOU"] = "あいうえお";
                // 伝票No
                rowTmp["DENPYOUNO"] = "1234567890";
                // 乗員
                rowTmp["JYOUIN"] = "1234";
                // 車番
                rowTmp["SHABAN"] = "1234567890";
                // 伝票日付
                rowTmp["DENPYOUDATE"] = "2013/12/09 12:00:00";
                // 部門名
                rowTmp["BUMON"] = "あいうえおかきくけこ";
            ...
            if (isPrint)
            {
                for (int i = 0; i < 5; i++)
                {
                    ...
                    // No
                    rowTmp["NUMBER"] = ((i + 1) * 1000).ToString();
                    // 総重量
                    ...
                    // 品名CD
                    rowTmp["FHN_HINMEICD"] = "1234567890";
                    // 品名
                    rowTmp["HINMEI"] = "あいうえおかきくけこさしすせそ";
                    // 単価
            ...
            if (isPrint)
            {
                ...
                // 現場名
                rowTmp["GENBA"] = "あいうえおかきくけこさしすせそたちつてとなにぬねの";
                // 正味合計
                ...
                // 拠点
                rowTmp["KYOTEN_NAME"] = "あいうえおかきくけこさしすせそ";
                // 拠点住所1
                rowTmp["KYOTEN_ADDRESS1"] = "あいうえおかきくけこさしすせそたちつてとなにぬねの";
                // 拠点住所2
                rowTmp["KYOTEN_ADDRESS2"] = "あいうえおかきくけこさしすせそたちつてとなにぬねの";
                // 拠点電話
                rowTmp["KYOTEN_TEL"] = "TEL 1234567890123";
                // 拠点FAX
                rowTmp["KYOTEN_FAX"] = "TEL 1234567890123";
        ...
        private void SetChouhyouInfo()
        {
            ...
            for (int pos = 1; pos <= 3; pos++)
            {
                ...
                if (dataTableHeaderTmp.Rows.Count > 0)
                {
                    ...
                    // 部門名
                    index = dataTableHeaderTmp.Columns.IndexOf("BUMON");
                    ctrlName = string.Format("DH_BUMON_CTL{0}", pos);
                    if (!this.IsDBNull(dataTableHeaderTmp.Rows[0].ItemArray[index]))
                    {
                        byteArray = encoding.GetBytes(dataTableHeaderTmp.Rows[0].ItemArray[index].ToString());
                        if (byteArray.Length > 20)
