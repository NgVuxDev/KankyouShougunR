...
namespace Shougun.Core.Common.MeisaihyoSyukeihyoJokenShiteiPopup
{
    ...
    #region - CommonChouhyouR351 -

    /// <summary>帳票Nを表すクラス・コントロール</summary>
    public class CommonChouhyouR351 : CommonChouhyouBase
    {
        #region - Constructors -

        /// <summary>Initializes a new instance of the <see cref="CommonChouhyouR351"/> class.</summary>
        /// <param name="windowID">画面ＩＤ</param>
        public CommonChouhyouR351(WINDOW_ID windowID)
            : base(windowID)
        {
            // 帳票出力フルパスフォーム名
            ...
            // 選択可能な集計項目
            if (windowID == WINDOW_ID.R_KEIRYOU_MEISAIHYOU)
            {   // R351(計量明細表)
        ...
        private void MakeOutDataTable()
        {
            try
            {
                ...
                for (int rowCount = 0; rowCount < this.DataTableMultiSort.DefaultView.Count; rowCount++)
                {
                    ...
                    // 計量番号
                    index = this.InputDataTable[indexTable].Columns.IndexOf("KEIRYOU_NUMBER");
                    ...
                    {   // 伝票番号が変化した
                        ...
                        // 調整重量
                        index = this.InputDataTable[indexTable].Columns.IndexOf("CHOUSEI_JYUURYOU");
                        ...
                        // 正味重量
                        index = this.InputDataTable[indexTable].Columns.IndexOf("NET_TOTAL");
                        shouhizeiTotal = this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]) ? 0 : (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                        ...
                        // 調整重量合計
                        goukeiKingakuTotal += kingakuTotal;

                        // 正味合計
                        souGoukeikingakuTotal += shouhizeiTotal;
                    }
                    else
                    {
                        // 調整重量
                        ...
                        // 調整重量合計
                        goukeiKingakuTotal += this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]) ? 0 : (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                        ...
                        for (itemColumnIndex = 0; itemColumnIndex < syuukeiKoumokuEnableGroupCount; itemColumnIndex++)
                        {
                            ...
                            syuukeiKoumokuKingakuKei[itemColumnIndex] += this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]) ? 0 : (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                        }
                    }
                    ...
                    // 総合計
                    dataRowNew["PHN_GOUKEI_KINGAKU_TOTAL_FLB"] = goukeiKingakuTotal.ToString("#,##0");
                    ...
                    dataRowNew["PHN_GOUKEI_TAX_TOTAL_FLB"] = souGoukeikingakuTotal.ToString("#,##0");
                    ...
                    dataRowNew["PHN_GOUKEI_TOTAL_FLB"] = string.Empty;
                    ...
                    for (itemColumnIndex = 0; itemColumnIndex < syuukeiKoumokuEnableGroupCount; itemColumnIndex++)
                    {
                        switch (itemColumnIndex)
                        {
                            case 0: // 集計項目１
                                ...
                                dataRowNew["PHN_FILL_COND_ID_1_TOTAL_FLB"] = string.Empty;
                            ...
                            case 1: // 集計項目２
                                ...
                                dataRowNew["PHN_FILL_COND_ID_2_TOTAL_FLB"] = string.Empty;
                            ...
                            case 2: // 集計項目３
                                ...
                                dataRowNew["PHN_FILL_COND_ID_3_TOTAL_FLB"] = string.Empty;
