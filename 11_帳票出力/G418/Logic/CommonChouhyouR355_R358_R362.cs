...
namespace Shougun.Core.Common.MeisaihyoSyukeihyoJokenShiteiPopup
{
    ...
    #region - CommonChouhyouR355_R358_R362 -

    /// <summary>帳票Nを表すクラス・コントロール</summary>
    public class CommonChouhyouR355_R358_R362 : CommonChouhyouBase
    {
        #region - Constructors -

        /// <summary>Initializes a new instance of the <see cref="CommonChouhyouR355_R358_R362"/> class.</summary>
        /// <param name="windowID">画面ＩＤ</param>
        public CommonChouhyouR355_R358_R362(WINDOW_ID windowID)
            : base(windowID)
        {
            // 帳票出力フルパスフォーム名
            this.OutputFormFullPathName = CommonChouhyouBase.TemplatePath + "R355_R358_R362-Form.xml";
            ...
            // 選択可能な集計項目
            if (windowID == WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU)
            {   // R355(売上／支払明細表)
                ...
                {
                    new TaishouTable("T_UKEIRE_ENTRY", "T_UKEIRE_DETAIL"),
                    new TaishouTable("T_SHUKKA_ENTRY", "T_SHUKKA_DETAIL"),
                    new TaishouTable("T_UR_SH_ENTRY", "T_UR_SH_DETAIL"),
                };
            }
            else if (windowID == WINDOW_ID.R_URIAGE_MEISAIHYOU || windowID == WINDOW_ID.R_SHIHARAI_MEISAIHYOU)
            {   // R358(売上明細表)またはR362(支払明細表)
                this.SelectEnableSyuukeiKoumokuList = new List<int>()
                {
        ...
        private void MakeOutDataTable()
        {
            try
            {
                ...
                decimal ukeireUriageKingakuTotal = 0;
                decimal ukeireUriageTaxUchi = 0;
                decimal shutsukaUriageAmountTotal = 0;
                decimal shutsukaUriageTaxUchi = 0;
                decimal ukeireUriageTaxSoto = 0;
                decimal shutsukaUriageTaxSoto = 0;

                decimal ukeireShiharaiKingakuTotal = 0;
                decimal shutsukaShiharaiAmountTotal = 0;
                decimal ukeireShiharaiTaxUchi = 0;
                decimal ukeireShiharaiTaxSoto = 0;
                decimal shutsukaShiharaiTaxUchi = 0;
                decimal shutsukaShiharaiTaxSoto = 0;

                decimal uriageShiharaiUriageAmountTotal = 0;
                decimal uriageShiharaiShiharaiAmountTotal = 0;
                decimal uriageShiharaiUriageTaxUchi = 0;
                decimal uriageShiharaiUriageTaxSoto = 0;
                decimal uriageShiharaiShiharaiTaxUchi = 0;
                decimal uriageShiharaiShiharaiTaxSoto = 0;
                ...
                // 消費税計算区分を保持する
                string zei_keisan_kbn = string.Empty;
                ...
                for (int rowCount = 0; rowCount < this.DataTableMultiSort.DefaultView.Count; rowCount++)
                {
                    ...
                    {   // 伝票番号が変化した
                        ...
                        switch (this.WindowID)
                        {
                            case WINDOW_ID.R_URIAGE_MEISAIHYOU:             // R358(売上明細表)
                                ...
                                {
                                    case 0: // 受入入力
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_ZEI_KEISAN_KBN_CD");
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_UCHI");
                                        ukeireUriageTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_SOTO");
                                        ukeireUriageTaxSoto = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_KINGAKU_TOTAL");
                                        ukeireUriageKingakuTotal = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);

                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = ukeireUriageKingakuTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = ukeireUriageKingakuTotal - ukeireUriageTaxUchi;
                                        }
                                        taxTotal = ukeireUriageTaxSoto + ukeireUriageTaxUchi;
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                                    ...
                                    case 1: // 出荷入力
                                        ...
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_UCHI");
                                        shutsukaUriageTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_SOTO");
                                        shutsukaUriageTaxSoto = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_AMOUNT_TOTAL");
                                        shutsukaUriageAmountTotal = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);

                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = shutsukaUriageAmountTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = shutsukaUriageAmountTotal - shutsukaUriageTaxUchi;
                                        }
                                        taxTotal = shutsukaUriageTaxSoto + shutsukaUriageTaxUchi;
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                                    ...
                                    case 2: // 売上支払入力
                                        ...
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_UCHI");
                                        uriageShiharaiShiharaiTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_SOTO");
                                        uriageShiharaiShiharaiTaxSoto = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_AMOUNT_TOTAL");
                                        uriageShiharaiShiharaiAmountTotal = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);

                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = uriageShiharaiShiharaiAmountTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = uriageShiharaiShiharaiAmountTotal - uriageShiharaiShiharaiTaxUchi;
                                        }
                                        taxTotal = uriageShiharaiShiharaiTaxSoto + uriageShiharaiShiharaiTaxUchi;
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                            ...
                            case WINDOW_ID.R_SHIHARAI_MEISAIHYOU:           // R362(支払明細表)
                                ...
                                {
                                    case 0: // 受入入力
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_ZEI_KEISAN_KBN_CD");
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_UCHI");
                                        ukeireShiharaiTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_SOTO");
                                        ukeireShiharaiTaxSoto = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_KINGAKU_TOTAL");
                                        ukeireShiharaiKingakuTotal = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);

                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = ukeireShiharaiKingakuTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = ukeireShiharaiKingakuTotal - ukeireShiharaiTaxUchi;
                                        }
                                        taxTotal = ukeireShiharaiTaxSoto + ukeireShiharaiTaxUchi;
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                                    ...
                                    case 1: // 出荷入力
                                        ...
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_UCHI");
                                        shutsukaShiharaiTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_SOTO");
                                        shutsukaShiharaiTaxSoto = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_AMOUNT_TOTAL");
                                        shutsukaShiharaiAmountTotal = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);

                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = shutsukaShiharaiAmountTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = shutsukaShiharaiAmountTotal - shutsukaShiharaiTaxUchi;
                                        }
                                        taxTotal = shutsukaShiharaiTaxSoto + shutsukaShiharaiTaxUchi;
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                            ...
                            case WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU:    // R355(売上／支払明細表)
                                switch (indexTable)
                                {
                                    case 0: // 受入入力
                                        ...
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = ukeireUriageKingakuTotal - ukeireShiharaiKingakuTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = (ukeireUriageKingakuTotal - ukeireUriageTaxUchi) - (ukeireShiharaiKingakuTotal - ukeireShiharaiTaxUchi);
                                        }
                                        taxTotal = (ukeireUriageTaxSoto + ukeireUriageTaxUchi) - (ukeireShiharaiTaxSoto + ukeireShiharaiTaxUchi);
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                                    ...
                                    case 1: // 出荷入力
                                        ...
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = shutsukaUriageAmountTotal - shutsukaShiharaiAmountTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = (shutsukaUriageAmountTotal - shutsukaUriageTaxUchi) - (shutsukaShiharaiAmountTotal - shutsukaShiharaiTaxUchi);
                                        }
                                        taxTotal = (shutsukaUriageTaxSoto + shutsukaUriageTaxUchi) - (shutsukaShiharaiTaxSoto + shutsukaShiharaiTaxUchi);
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                                    ...
                                    case 2: // 売上支払入力
                                        ...
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_UCHI");
                                        uriageShiharaiUriageTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_SOTO");
                                        uriageShiharaiUriageTaxSoto = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_AMOUNT_TOTAL");
                                        uriageShiharaiUriageAmountTotal = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_ZEI_KEISAN_KBN_CD");
                                        zei_keisan_kbn = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_UCHI");
                                        uriageShiharaiShiharaiTaxUchi = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                        ...
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            kingakuTotal = uriageShiharaiUriageAmountTotal - uriageShiharaiShiharaiAmountTotal;
                                        }
                                        else
                                        {
                                            kingakuTotal = (uriageShiharaiUriageAmountTotal - uriageShiharaiUriageTaxUchi) - (uriageShiharaiShiharaiAmountTotal - uriageShiharaiShiharaiTaxUchi);
                                        }
                                        taxTotal = (uriageShiharaiUriageTaxSoto + uriageShiharaiUriageTaxUchi) - (uriageShiharaiShiharaiTaxSoto + uriageShiharaiShiharaiTaxUchi);
                                        if ("2" == zei_keisan_kbn)
                                        {
                                            total = kingakuTotal;
                                        }
                        ...
                        for (itemColumnIndex = 0; itemColumnIndex < syuukeiKoumokuEnableGroupCount; itemColumnIndex++)
                        {
                            ...
                            if ("2" != zei_keisan_kbn)
                            {
                                syuukeiKoumokuShouhizei[itemColumnIndex] += taxTotal;
                            }
                            syuukeiKoumokuGoukeiTotal[itemColumnIndex] += total;
                        }
                    }
                    ...
                    if (!this.IsDBNull(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        dataRowNew["PHY_DENPYOU_DATE_FLB"] = ((DateTime)this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]).ToString("yyyy/MM/dd");
                    }
                    else
                    {
                    ...
                    for (int i = 0; i < this.SelectChouhyouOutKoumokuDepyouList.Count; i++, itemColumnIndex++)
                    {
                        ...
                        switch (fieldName)
                        {
                            case "TAX_SOTO":
                            case "TAX_UCHI":
                            case "HINMEI_TAX_SOTO":
                            case "HINMEI_TAX_UCHI":
                                isDefault = true;
                        ...
                        if (isDefault)
                        {
                            if ((WINDOW_ID.R_URIAGE_MEISAIHYOU != this.WindowID && WINDOW_ID.R_SHIHARAI_MEISAIHYOU != this.WindowID && WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU != this.WindowID)
                                && ("TAX_SOTO" == fieldName || "TAX_UCHI" == fieldName || "HINMEI_TAX_SOTO" == fieldName || "HINMEI_TAX_UCHI" == fieldName || "SHIHARAI_TAX_SOTO" == fieldName || "SHIHARAI_TAX_UCHI" == fieldName))
                            {
                                if ("2" == zei_keisan_kbn)
                                {
                                    code = string.Empty;
                                ...
                                {
                                    index = this.InputDataTable[indexTable].Columns.IndexOf(fieldName);
                                    code = string.Empty;
                                    codeData = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                                }
                            }
                            else if ("URIAGE_KINGAKU_TOTAL" == fieldName || "URIAGE_AMOUNT_TOTAL" == fieldName || "URIAGE_TAX_SOTO" == fieldName || "URIAGE_TAX_UCHI" == fieldName)
                            {
                                if (WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU == this.WindowID)
                                {
                                    var ur_sh_calc_base_kbn = this.MSysInfo.UR_SH_CALC_BASE_KBN.ToString();
                                    index = this.InputDataTable[indexTable].Columns.IndexOf(fieldName);
                                    code = string.Empty;
                                    if ("1" == ur_sh_calc_base_kbn)
                                    {
                                        codeData = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]);
                                    }
                                    else
                                    {
                                        codeData = this.ConvertNull2Zero(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]) * -1;
                                    }
                                }
                                else
                                {
                                ...
                                // 売上明細票 or 売上／支払明細表 - 税計算区分「2:請求毎」の場合は消費税を出力しない
                                if (WINDOW_ID.R_URIAGE_MEISAIHYOU == this.WindowID || WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU == this.WindowID)
                                {
                                    if ("2" == zei_keisan_kbn && ("URIAGE_TAX_SOTO" == fieldName || "URIAGE_TAX_UCHI" == fieldName))
                                    {
                                        code = String.Empty;
                                        codeData = String.Empty;
                                    }
                            ...
                            else if ("SHIHARAI_KINGAKU_TOTAL" == fieldName || "SHIHARAI_AMOUNT_TOTAL" == fieldName || "SHIHARAI_TAX_SOTO" == fieldName || "SHIHARAI_TAX_UCHI" == fieldName)
                            {
                                if (WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU == this.WindowID)
                                {
                                    var ur_sh_calc_base_kbn = this.MSysInfo.UR_SH_CALC_BASE_KBN.ToString();
                                ...
                                // 支払明細票 or 売上／支払明細表 - 税計算区分「2:請求毎」の場合は消費税を出力しない
                                if (WINDOW_ID.R_SHIHARAI_MEISAIHYOU == this.WindowID || WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU == this.WindowID)
                                {
                                    if ("2" == zei_keisan_kbn && ("SHIHARAI_TAX_SOTO" == fieldName || "SHIHARAI_TAX_UCHI" == fieldName))
                                    {
                                        code = String.Empty;
                                        codeData = String.Empty;
                                    }
                                    ...
                                    {   // 通常
                                        ...
                                        {   // CD（コードを含む）
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    if (null == this.MTorihikiKbnList)
                                                    {
                                                        // 取引区分リストがなければ取得する
                                                        var mTorihikiKbn = new M_TORIHIKI_KBN();
                                                        var mTorihikiKbnList = this.MasterTorihikiKbnDao.GetAllValidData(mTorihikiKbn);
                                                        this.MTorihikiKbnList = new List<M_TORIHIKI_KBN>(mTorihikiKbnList);
                                                    }

                                                    codeData = string.Empty;
                                                    if (0 < this.MTorihikiKbnList.Count)
                                                    {
                                                        codeData = this.MTorihikiKbnList.Where(x => x.TORIHIKI_KBN_CD.ToString() == code.ToString())
                                                                                        .Select(x => x.TORIHIKI_KBN_NAME)
                        ...
                        {   // 金額系
                            ...
                            decimal dataTmp = this.ConvertNull2Zero(codeData);
                    ...
                    if (!this.IsDBNull(this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        dataRowNew["PHY_ROW_NO_FLB"] = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }
                    else
                    {
                    ...
                    for (int i = 0; i < this.SelectChouhyouOutKoumokuMeisaiList.Count; i++, itemColumnIndex++)
                    {
                        ...
                        if (isDefault)
                        {
                            if ("TAX_SOTO" == fieldName || "TAX_UCHI" == fieldName || "HINMEI_TAX_SOTO" == fieldName || "HINMEI_TAX_UCHI" == fieldName)
                            {
                                if ("3" != zei_keisan_kbn)
                                {
                                    code = string.Empty;
                                ...
                                else
                                {
                                    if (WINDOW_ID.R_URIAGE_SHIHARAI_MEISAIHYOU == this.WindowID)
                                    {
                                        ...
                                        var denpyou_kbn_cd = this.InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index].ToString();
                                        if (denpyou_kbn_cd == "1")
                                        {
                                            index = this.InputDataTable[indexTable].Columns.IndexOf(fieldName);
                                            code = string.Empty;
                    ...
                    dataRowNew["PHY_KINGAKU_TOTAL_FLB"] = kingakuTotal;
                    if ("2" != zei_keisan_kbn)
                    {
                        dataRowNew["PHY_TAX_TOTAL_FLB"] = taxTotal.ToString("#,##0");
                    }
                    else
                    {
                        dataRowNew["PHY_TAX_TOTAL_FLB"] = string.Empty;
                    }
                    dataRowNew["PHY_TOTAL_FLB"] = total;
                    ...
                    for (itemColumnIndex = 0; itemColumnIndex < syuukeiKoumokuEnableGroupCount; itemColumnIndex++)
                    {
                        switch (itemColumnIndex)
                        {
                            case 0: // 集計項目１
                                ...
                                dataRowNew["PHN_FILL_COND_ID_1_KINGAKU_TOTAL_FLB"] = syuukeiKoumokuKingakuKei[itemColumnIndex];
                            ...
                            case 1: // 集計項目２
                                ...
                                dataRowNew["PHN_FILL_COND_ID_2_KINGAKU_TOTAL_FLB"] = syuukeiKoumokuKingakuKei[itemColumnIndex];
                            ...
                            case 2: // 集計項目３
                                ...
                                dataRowNew["PHN_FILL_COND_ID_3_KINGAKU_TOTAL_FLB"] = syuukeiKoumokuKingakuKei[itemColumnIndex];
    ...
    #endregion - CommonChouhyouR355_R358_R362 -
