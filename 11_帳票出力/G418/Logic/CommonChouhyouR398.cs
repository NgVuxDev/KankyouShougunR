...
namespace Shougun.Core.Common.MeisaihyoSyukeihyoJokenShiteiPopup
{
    ...
    #region - CommonChouhyouR398 -

    /// <summary>運賃明細票表すクラス・コントロール</summary>
    public class CommonChouhyouR398 : CommonChouhyouBase
    {
        #region - Constructors -

        /// <summary>Initializes a new instance of the <see cref="CommonChouhyouR398"/> class.</summary>
        /// <param name="windowID">画面ＩＤ</param>
        public CommonChouhyouR398(WINDOW_ID windowID)
            : base(windowID)
        {
            // 帳票出力フルパスフォーム名
            ...
            // 選択可能な集計項目
            if (windowID == WINDOW_ID.R_UNNCHIN_MEISAIHYOU)
            {   // R398(運賃明細表)
                ...
                {
                    0, 1, 2, 4, 5, 6, 7, 8, 10, 11, 15, 16, 17, 18, 19
                };

                // 対象テーブルリスト
                this.TaishouTableList = new List<TaishouTable>()
                {
                    new TaishouTable("T_UNCHIN_ENTRY", "T_UNCHIN_DETAIL"),  // 運賃
                };
            }
        ...
        private void MakeOutDataTable()
        {
            try
            {
                ...
                decimal denpyouGotoKingakuTotal = 0;
                decimal denpyouGotoTax = 0;
                ...
                for (int rowCount = 0; rowCount < this.DataTableMultiSort.DefaultView.Count; rowCount++)
                {
                    ...
                    // 伝票番号
                    index = this.InputDataTable[indexTable].Columns.IndexOf("DENPYOU_NUMBER");
                    ...
                    {   // 伝票番号が変化した
                        ...
                        // 金額計算処理
                        this.KingakuCalculate(indexTable, indexTableRow, ref denpyouGotoKingakuTotal, ref denpyouGotoTax);
                        kingakuTotal = denpyouGotoKingakuTotal;
                        shouhizeiTotal = denpyouGotoTax;
                        ...
                        // 合計金額
                        goukeiKingakuTotal = kingakuTotal + shouhizeiTotal;
                    ...
                    else
                    {
                        ...
                        // 金額総合計
                        souKingakuTotal += denpyouGotoKingakuTotal;
                        decimal kingaku = denpyouGotoKingakuTotal;

                        // 消費税金額

                        // 消費税総合計
                        souShouhizeiTotal += denpyouGotoTax;
                        decimal shouhizei = denpyouGotoTax;

                        // 合計金額
                        goukeiKingakuTotal = kingakuTotal + shouhizeiTotal;

                        // 合計金額総合計
                        souGoukeikingakuTotal = souKingakuTotal + souShouhizeiTotal;
                        ...
                        for (itemColumnIndex = 0; itemColumnIndex < syuukeiKoumokuEnableGroupCount; itemColumnIndex++)
                        {
                            ...
                            syuukeiKoumokuKingakuKei[itemColumnIndex] += kingaku;
                            syuukeiKoumokuShouhizei[itemColumnIndex] += shouhizei;
                            syuukeiKoumokuGoukeiTotal[itemColumnIndex] += kingaku + shouhizei;
                        }
                    }
        ...
        /// <summary>金額計算処理を実行する</summary>
        /// <param name="indexTable">テーブルインデックスを表す数値</param>
        /// <param name="indexTableRow">テーブル行を表す数値</param>
        /// <param name="denpyouGotoKingakuTotal">伝票毎合計金額を表す数値</param>
        /// <param name="denpyouGotoTax">伝票毎消費税を表す数値</param>
        private void KingakuCalculate(int indexTable, int indexTableRow, ref decimal denpyouGotoKingakuTotal, ref decimal denpyouGotoTax)
        {
            try
            {
                int index;
                ...
                // 確定区分
                index = this.InputDataTable[indexTable].Columns.IndexOf("KAKUTEI_KBN");
                kakuteiKubun = (short)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                ...
                {   // 確定
                    ...
                    // 伝票毎金額計 =
                    // ([運賃入力].[売上金額合計] ＋ [運賃入力].[品名別売上金額合計] ＋
                    // [運賃入力].[支払金額合計] ＋ [運賃入力].[品名別支払金額合計])

                    index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_KINGAKU_TOTAL");
                    decimal uriageKingakuGoukei = 0;
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        uriageKingakuGoukei = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    decimal hinmeibetsuUriageKingakuGoukei = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_URIAGE_KINGAKU_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuUriageKingakuGoukei = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    decimal shiharaiKingakuGoukei = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_KINGAKU_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        shiharaiKingakuGoukei = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    decimal hinmeibetsuShiharaiKingakuGoukei = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_SHIHARAI_KINGAKU_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuShiharaiKingakuGoukei = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    denpyouGotoKingakuTotal = uriageKingakuGoukei + hinmeibetsuUriageKingakuGoukei + shiharaiKingakuGoukei + hinmeibetsuShiharaiKingakuGoukei;

                    // 伝票毎消費税計 =

                    // ([運賃入力].[売上伝票毎消費税外税] ＋ [運賃入力].[売上伝票毎消費税内税] ＋
                    // [運賃入力].[売上明細毎消費税外税合計] ＋ [運賃入力].[売上明細毎消費税内税合計] ＋
                    // [運賃入力].[品名別売上消費税外税合計] ＋ [運賃入力].[品名別売上消費税内税合計] ＋

                    // [運賃入力].[支払伝票毎消費税外税] ＋ [運賃入力].[支払伝票毎消費税内税] ＋
                    // [運賃入力].[支払明細毎消費税外税合計] ＋ [運賃入力].[支払明細毎消費税内税合計] ＋
                    // [運賃入力].[品名別支払消費税外税合計] ＋ [運賃入力].[品名別支払消費税内税合計])

                    // 売上伝票毎消費税外税
                    decimal uriageDenpyougotoTaxSoto = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_SOTO");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        uriageDenpyougotoTaxSoto = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 売上伝票毎消費税内税
                    decimal uriageDenpyougotoTaxUchi = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_UCHI");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        uriageDenpyougotoTaxUchi = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 売上明細毎消費税外税合計
                    decimal uriageMeisaigotoTaxSotoTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_SOTO_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        uriageMeisaigotoTaxSotoTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 売上明細毎消費税内税合計
                    decimal uriageMeisaigotoTaxUchiTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("URIAGE_TAX_UCHI_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        uriageMeisaigotoTaxUchiTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 品名別売上消費税外税合計
                    decimal hinmeibetsuUriageTaxSotoTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_URIAGE_TAX_SOTO_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuUriageTaxSotoTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 品名別売上消費税内税合計
                    decimal hinmeibetsuUriageTaxUchiTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_URIAGE_TAX_UCHI_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuUriageTaxUchiTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 支払伝票毎消費税外税
                    decimal shiharaiDenpyougotoTaxSoto = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_SOTO");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        shiharaiDenpyougotoTaxSoto = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 支払伝票毎消費税内税
                    decimal shiharaiDenpyougotoTaxUchi = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_UCHI");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        shiharaiDenpyougotoTaxUchi = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 支払明細毎消費税外税合計
                    decimal shiharaiMeisaigotoTaxSotoTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_SOTO_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        shiharaiMeisaigotoTaxSotoTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 支払明細毎消費税内税合計
                    decimal shiharaiMeisaigotoTaxUchiTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("SHIHARAI_TAX_UCHI_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        shiharaiMeisaigotoTaxUchiTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 品名別支払消費税外税合計
                    decimal hinmeibetsuShiharaiTaxSotoTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_SHIHARAI_TAX_SOTO_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuShiharaiTaxSotoTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    // 品名別支払消費税内税合計
                    decimal hinmeibetsuShiharaiTaxUchiTotal = 0;
                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_SHIHARAI_TAX_UCHI_TOTAL");
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuShiharaiTaxUchiTotal = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    denpyouGotoTax = uriageDenpyougotoTaxSoto + uriageDenpyougotoTaxUchi +
                                    uriageMeisaigotoTaxSotoTotal + uriageMeisaigotoTaxUchiTotal +
                                    hinmeibetsuUriageTaxSotoTotal + hinmeibetsuUriageTaxUchiTotal +
                                    shiharaiDenpyougotoTaxSoto + shiharaiDenpyougotoTaxUchi +
                                    shiharaiMeisaigotoTaxSotoTotal + shiharaiMeisaigotoTaxUchiTotal +
                                    hinmeibetsuShiharaiTaxSotoTotal + hinmeibetsuShiharaiTaxUchiTotal;
                ...
                {   // 未確定
                    ...
                    // 伝票毎金額計 =
                    // ([運賃明].[金額] ＋ [運賃明].[品名別金額]): ※ 明細全て加算

                    index = this.InputDataTable[indexTable].Columns.IndexOf("KINGAKU");
                    decimal kingaku = 0;
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        kingaku = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_KINGAKU");
                    decimal hinmeibetsuKingaku = 0;
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuKingaku = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    denpyouGotoKingakuTotal = kingaku + hinmeibetsuKingaku;

                    // 伝票毎消費税計 =
                    // ([運賃明細].[消費税外税] ＋　[運賃明細].[消費税内税] ＋
                    // [運賃明細].[品名別消費税外税] ＋ [運賃明細].[品名別消費税内税]): ※ 明細全て加算

                    index = this.InputDataTable[indexTable].Columns.IndexOf("TAX_SOTO");
                    decimal taxSoto = 0;
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        taxSoto = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }
                    ...
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        taxUchi = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_TAX_SOTO");
                    decimal hinmeibetsuTaxSoto = 0;
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuTaxSoto = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    index = this.InputDataTable[indexTable].Columns.IndexOf("HINMEI_TAX_UCHI");
                    decimal hinmeibetsuTaxUchi = 0;
                    if (!this.IsDBNull(InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index]))
                    {
                        hinmeibetsuTaxUchi = (decimal)InputDataTable[indexTable].Rows[indexTableRow].ItemArray[index];
                    }

                    denpyouGotoTax = taxSoto + taxUchi + hinmeibetsuTaxSoto + hinmeibetsuTaxUchi;
