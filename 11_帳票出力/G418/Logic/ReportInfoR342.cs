...
namespace Shougun.Core.Common.MeisaihyoSyukeihyoJokenShiteiPopup
{
    ...
    #region - ReportInfoR342 -

    /// <summary>受付明細表(R342)のレポート情報を表すクラス・コントロール</summary>
    internal class ReportInfoR342 : ReportInfoCommon
    {
        #region - Constructors -

        /// <summary>Initializes a new instance of the <see cref="ReportInfoR342" /> class.</summary>
        /// <param name="windowID">画面ＩＤ</param>
        /// <param name="dataTable">データーテーブル</param>
        /// <param name="commonChouhyouBase">共通帳票</param>
        public ReportInfoR342(WINDOW_ID windowID, DataTable dataTable, CommonChouhyouBase commonChouhyouBase)
        ...
        protected override void UpdateFieldsStatus()
        {
            try
            {
                ...
                this.SetFieldName("PHY_DENPYOU_NUMBER_FLB", "受付番号");
                index = this.DataTableList["Header"].Columns.IndexOf("DENPYOU_NUMBER_LABEL");
                this.DataTableList["Header"].Rows[0][index] = "受付番号";

                this.SetFieldName("PHY_DENPYOU_DATE_FLB", "受付日");
                index = this.DataTableList["Header"].Columns.IndexOf("DENPYOU_DATE_LABEL");
                this.DataTableList["Header"].Rows[0][index] = "受付日";
                ...
                //this.SetFieldName("PHY_URIAGE_SHURUI_FLB", "売上種類");
