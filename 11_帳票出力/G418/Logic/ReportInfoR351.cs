...
namespace Shougun.Core.Common.MeisaihyoSyukeihyoJokenShiteiPopup
{
    ...
    #region - ReportInfoR351 -

    /// <summary>計量明細表(R351)のレポート情報を表すクラス・コントロール</summary>
    internal class ReportInfoR351 : ReportInfoCommon
    {
        #region - Constructors -

        /// <summary>Initializes a new instance of the <see cref="ReportInfoR351" /> class.</summary>
        /// <param name="windowID">画面ＩＤ</param>
        /// <param name="dataTable">データーテーブル</param>
        /// <param name="chusyutsuJoken">抽出条件</param>
        public ReportInfoR351(WINDOW_ID windowID, DataTable dataTable, CommonChouhyouBase commonChouhyouBase)
        ...
        protected override void UpdateFieldsStatus()
        {
            try
            {
                ...
                this.SetFieldName("PHY_DENPYOU_NUMBER_FLB", "計量番号");
                index = this.DataTableList["Header"].Columns.IndexOf("DENPYOU_NUMBER_LABEL");
                this.DataTableList["Header"].Rows[0][index] = "計量番号";
                ...
                for (int i = 0; i < this.ComChouhyouBase.SelectChouhyouOutKoumokuDepyouList.Count; i++, itemColumnIndex++)
                {
                    ...
                    switch (itemColumnIndex)
                    {
                        case 0: // 帳票出力可能項目１番目
                            ...
                            //index = this.DataTableList["Header"].Columns.IndexOf("OUTPUT_DENPYOU_1_CD_LABEL");
                            //this.DataTableList["Header"].Rows[0][index] = chouhyouOutKoumoku.;
                ...
                this.DataTableList["Header"].Rows[0][index] = "調整合計(㎏)";

                // 正味合計ラベル
                index = this.DataTableList["Header"].Columns.IndexOf("TAX_TOTAL_LABEL");
                this.DataTableList["Header"].Rows[0][index] = "正味合計(㎏)";

                this.SetGroupVisible("GROUP6", true, false);
                ...
                this.SetFieldName("PHY_KINGAKU_TOTAL_FLB", "調整合計(㎏)");
                this.SetFieldName("PHY_TAX_TOTAL_FLB", "正味合計(㎏)");
