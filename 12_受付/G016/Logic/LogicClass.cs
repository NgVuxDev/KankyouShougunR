...
namespace Shougun.Core.Reception.UketsukeSyukkaNyuuryoku
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// 受付（出荷）入力のDao
        /// </summary>
        private T_UKETSUKE_SK_ENTRYDao daoUketsukeEntry;

        /// <summary>
        /// 受付（出荷）明細のDao
        /// </summary>
        private T_UKETSUKE_SK_DETAILDao daoUketsukeDetail;
        ...
        /// 受付出荷入力Entityを格納
        /// </summary>
        private List<T_UKETSUKE_SK_ENTRY> insEntryEntityList = new List<T_UKETSUKE_SK_ENTRY>();

        /// <summary>
        /// 受付出荷明細Entityを格納
        /// </summary>
        private List<T_UKETSUKE_SK_DETAIL> insDetailEntityList = new List<T_UKETSUKE_SK_DETAIL>();

        /// <summary>
        /// 受付出荷入力を削除Entity
        /// </summary>
        private T_UKETSUKE_SK_ENTRY delEntryEntity;
        ...
        internal LogicClass(UIForm targetForm)
        {
            try
            {
                ...
                // 受付（出荷）入力のDao
                this.daoUketsukeEntry = DaoInitUtility.GetComponent<T_UKETSUKE_SK_ENTRYDao>();
                // 受付（出荷）明細のDao
                this.daoUketsukeDetail = DaoInitUtility.GetComponent<T_UKETSUKE_SK_DETAILDao>();
                // システム情報Dao
        ...
        internal bool WindowInit()
        {
            ...
            try
            {
                ...
                this.RirekeShow(); //CongBinh 20210713 #152803
            }
        ...
        private void EnterEventInit()
        {
            try
            {
                ...
                this.form.UNPAN_GYOUSHA_CD.Enter += this.form.Control_Enter;
                //荷積業者
                this.form.NIZUMI_GYOUSHA_CD.Enter -= this.form.Control_Enter;
                this.form.NIZUMI_GYOUSHA_CD.Enter += this.form.Control_Enter;
                //荷積現場
                this.form.NIZUMI_GENBA_CD.Enter -= this.form.Control_Enter;
                this.form.NIZUMI_GENBA_CD.Enter += this.form.Control_Enter;
                //受付番号
                ...
                this.form.UNTENSHA_CD.Enter += this.form.Control_Enter;
                // 補助員
                this.form.HOJOIN_CD.Enter -= this.form.Control_Enter;
                this.form.HOJOIN_CD.Enter += this.form.Control_Enter;
        ...
        internal bool DisplayInit(out bool catchErr)
        {
            ...
            try
            {
                ...
                this.EnterEventInit();
                this.form.btnSayouDate.Enabled = false;//CongBinh 20210713 #152803 

                switch (this.form.WindowType)
                {
                    case WINDOW_TYPE.NEW_WINDOW_FLAG:
                        this.form.btnSayouDate.Enabled = true;//CongBinh 20210713 #152803 
                        // 新規モードの場合
                        // 諸口関連項目のReadonly初期設定
                        ...
                        if (!this.CheckNizumiGyoushaShokuchi())
                        {
                            catchErr = false;
                            return ret;
                        }
                ...
                this.form.ChangeNizumiDateEnabled();
            ...
            try
            {
                ...
                foreach (var detaiControlName in inputDetailControlNames)
                {
                    ...
                    if (detaiControlName.Equals("HINMEI_CD"))
                    {
                        ...
                        //{
                        //    // ポップアップ設定
                        //    ((DgvCustomTextBoxColumn)dgvDetail.Columns[detaiControlName]).PopupWindowId = WINDOW_ID.M_HINMEI;
                    ...
                    else if (detaiControlName.Equals("UNIT_CD"))
                    {
                        ...
                        //    existCheck.CheckMethodName = "単位マスタコードチェックandセッティング";
                        //    Collection<SelectCheckDto> excitChecks = new Collection<SelectCheckDto>();
                        //    excitChecks.Add(existCheck);
                        //    ((DgvCustomTextBoxColumn)dgvDetail.Columns[detaiControlName]).FocusOutCheckMethod = excitChecks;
        ...
        private void InitDataForNewMode()
        {
            ...
            // 荷積日
            this.form.NIZUMI_DATE.Value = parentForm.sysDate.Date;
        ...
        internal void CheckUnpanGyoushaCdForDisplay()
        {
            ...
            //if (!short.TryParse(string.Format("{0:#,0}", this.form.UNPAN_GYOUSHA_CD.Text), out unpanGyoushaCd))
            //{
            //    return;
            //}
        ...
        private int GetGroupNumber(long groupNumber)
        {
            ...
            try
            {
                ...
                string selectStr = "SELECT COUNT(SYSTEM_ID) AS CNT FROM T_UKETSUKE_SK_ENTRY";
        ...
        internal void SetValueToForm()
        {
            try
            {
                ...
                // 荷積業者
                this.form.NIZUMI_GYOUSHA_CD.Text = this.ChgDBNullToValue(dt.Rows[0]["NIZUMI_GYOUSHA_CD"], string.Empty).ToString();
                this.form.NIZUMI_GYOUSHA_NAME.Text = this.ChgDBNullToValue(dt.Rows[0]["NIZUMI_GYOUSHA_NAME"], string.Empty).ToString();

                // 荷積現場
                this.form.NIZUMI_GENBA_CD.Text = this.ChgDBNullToValue(dt.Rows[0]["NIZUMI_GENBA_CD"], string.Empty).ToString();
                this.form.NIZUMI_GENBA_NAME.Text = this.ChgDBNullToValue(dt.Rows[0]["NIZUMI_GENBA_NAME"], string.Empty).ToString();
                ...
                // 荷積日
                this.form.NIZUMI_DATE.Value = this.ChgDBNullToValue(dt.Rows[0]["NIZUMI_DATE"], null);
        ...
        internal bool CheckGyousha()
        {
            try
            {
                ...
                //20150609 #10700 現場に登録されている運転者指示事項をセット hoanghm start
                ...
                //20150609 #10700 現場に登録されている運転者指示事項をセット hoanghm end
        ...
        internal bool ErrorCheckGyousha()
        {
            ...
            if (!String.IsNullOrEmpty(gyoushaCd))
            {
                ...
                {
                    // nullはFalseとして扱う
                    if (false == gyousha.GYOUSHAKBN_SHUKKA.IsTrue)
                    {
                        // 業者名、業者電話設定
                        this.form.GYOUSHA_NAME.Text = String.Empty;
                        this.form.GYOSHA_TEL.Text = String.Empty;
        ...
        internal bool CheckUnpanGyoushaCd()
        {
            ...
            try
            {
                ...
                if (false == gyoushaEntity.GYOUSHAKBN_SHUKKA.IsTrue)
                {
                    this.msgLogic.MessageBoxShow("E058");
                    this.isInputError = true;
                    return returnVal;
        ...
        internal bool CheckNizumiGyoushaCd()
        {
            ...
            try
            {
                ...
                // Readonly初期化
                this.SetCtrlReadonly(this.form.NIZUMI_GYOUSHA_NAME, true);
                ...
                // 業者情報取得
                bool catchErr = true;
                var gyoushaEntity = this.GetGyousha(this.form.NIZUMI_GYOUSHA_CD.Text, out catchErr);
                if (!catchErr)
                {
                    return returnVal;
                }
                if (gyoushaEntity == null)
                {
                    ...
                    this.msgLogic.MessageBoxShow("E020", "荷積業者");
                ...
                // 区分
                // 20151026 BUNN #12040 STR
                if (!gyoushaEntity.HAISHUTSU_NIZUMI_GYOUSHA_KBN.IsTrue && !gyoushaEntity.UNPAN_JUTAKUSHA_KAISHA_KBN.IsTrue)
                // 20151026 BUNN #12040 END
                {
                    // 背景色変更
                    this.form.NIZUMI_GYOUSHA_CD.IsInputErrorOccured = true;
                    this.msgLogic.MessageBoxShow("E062", "荷積業者");
                    this.isInputError = true;
                    return returnVal;
                }
                ...
                // 変更ありの場合
                if (!this.form.dicControl.ContainsKey("NIZUMI_GYOUSHA_CD") ||
                    !this.form.dicControl["NIZUMI_GYOUSHA_CD"].Equals(this.form.NIZUMI_GYOUSHA_CD.Text))
                {
                    // 荷積業者名を設定
                    // 20151021 katen #13337 品名手入力に関する機能修正 start
                    if (gyoushaEntity.SHOKUCHI_KBN.IsTrue)
                ...
                {
                    // 荷積業者名編集可
                    this.SetCtrlReadonly(this.form.NIZUMI_GYOUSHA_NAME, false);
                }
        ...
        internal bool ChechNizumiGenbaCd()
        {
            ...
            try
            {
                ...
                // Readonly初期化
                this.SetCtrlReadonly(this.form.NIZUMI_GENBA_NAME, true);
                ...
                if (string.IsNullOrEmpty(this.form.NIZUMI_GYOUSHA_CD.Text))
                {
                    ...
                    this.msgLogic.MessageBoxShow("E051", "荷積業者");
                ...
                //// 荷積現場CDで現場情報取得（複数）
                //var genbaEntityList = this.GetGenba(this.form.NIZUMI_GENBA_CD.Text);
                //if (genbaEntityList == null)
                //{
                //    this.msgLogic.MessageBoxShow("E020", "荷積現場");
                //    this.form.NIZUMI_GENBA_CD.Text = string.Empty;
                //    // 処理終了
                //    return returnVal;
                //}
                ...
                genbaEntity = GetGenba(this.form.NIZUMI_GENBA_CD.Text, this.form.NIZUMI_GYOUSHA_CD.Text, out catchErr);
                if (!catchErr)
                {
                    return returnVal;
                }
                ...
                // 排出事業場区分、積替え保管区分、荷積降現場区分チェック
                // 20151026 BUNN #12040 STR
                if (!genbaEntity.HAISHUTSU_NIZUMI_GENBA_KBN.IsTrue && !genbaEntity.TSUMIKAEHOKAN_KBN.IsTrue)
                // 20151026 BUNN #12040 END
                {
                    // 背景色変更
                    this.form.NIZUMI_GENBA_CD.IsInputErrorOccured = true;
                    // 一致するデータがないのでエラー
                    this.msgLogic.MessageBoxShow("E062", "荷積現場");
                    this.isInputError = true;
                    // 処理終了
                    return returnVal;
                }
                ...
                if (null == gyousha)
                {
                    ...
                    this.isInputError = true;
                    this.form.SetFocusControl(this.form.NIZUMI_GYOUSHA_CD);
                ...
                // 変更ありの場合
                if (!this.form.dicControl.ContainsKey("NIZUMI_GENBA_CD") ||
                    !this.form.dicControl["NIZUMI_GENBA_CD"].Equals(this.form.NIZUMI_GENBA_CD.Text))
                {
                    // 荷積現場情報設定
                    // 20151021 katen #13337 品名手入力に関する機能修正 start
                ...
                {
                    // 荷積業者名編集可
                    this.SetCtrlReadonly(this.form.NIZUMI_GENBA_NAME, false);
                }
        ...
        internal bool IsPrint(out bool catchErr)
        {
            ...
            {
                LogUtility.DebugMethodEnd(retResult, catchErr);
            }
        }
        #endregion
                ...
                switch (this.form.WindowType)
                {
                    ...
                    case WINDOW_TYPE.NEW_WINDOW_FLAG:
                        ...
                        if (this.form.ListSagyouBi == null || this.form.ListSagyouBi.Count <= 1)
                        {
                            ...
                            {
                                // 受付出荷入力Entityを作成
                                T_UKETSUKE_SK_ENTRY entryEntity = this.CreateEntryEntity(ref systemID, i);
                    ...
                    case WINDOW_TYPE.UPDATE_WINDOW_FLAG:
                        ...
                        T_UKETSUKE_SK_ENTRY updEntryEntity = this.CreateEntryEntity(ref updSystemID);
                    ...
                    case WINDOW_TYPE.DELETE_WINDOW_FLAG:
                        ...
                        T_UKETSUKE_SK_ENTRY delEntryEntity = this.CreateEntryEntity(ref delSystemID);
        ...
        private void CreateDelEntryEntity()
        {
            try
            {
                ...
                // 初期化
                this.delEntryEntity = new T_UKETSUKE_SK_ENTRY();
                ...
                // 作成と更新情報設定
                var dbLogic = new DataBinderLogic<r_framework.Entity.T_UKETSUKE_SK_ENTRY>(this.delEntryEntity);
        ...
        /// <returns>T_UKETSUKE_SK_ENTRY</returns>
        private T_UKETSUKE_SK_ENTRY CreateEntryEntity(ref string systemID, int no = 1)
        {

            T_UKETSUKE_SK_ENTRY entryEntity = new T_UKETSUKE_SK_ENTRY();
            ...
            try
            {
                ...
                // 作成と更新情報設定
                var dbLogic = new DataBinderLogic<r_framework.Entity.T_UKETSUKE_SK_ENTRY>(entryEntity);
                ...
                if (!string.IsNullOrEmpty(this.form.NIZUMI_GYOUSHA_CD.Text))
                {
                    entryEntity.NIZUMI_GYOUSHA_CD = this.form.NIZUMI_GYOUSHA_CD.Text;
                    entryEntity.NIZUMI_GYOUSHA_NAME = this.form.NIZUMI_GYOUSHA_NAME.Text;
                }

                // 荷積現場
                if (!string.IsNullOrEmpty(this.form.NIZUMI_GENBA_CD.Text))
                {
                    entryEntity.NIZUMI_GENBA_CD = this.form.NIZUMI_GENBA_CD.Text;
                    entryEntity.NIZUMI_GENBA_NAME = this.form.NIZUMI_GENBA_NAME.Text;
                }
                ...
                // 荷積日
                if (this.form.NIZUMI_DATE.Value != null)
                {
                    entryEntity.NIZUMI_DATE = ((DateTime)this.form.NIZUMI_DATE.Value).ToShortDateString();
                }
        ...
        private void CreateDetailEntity(SqlInt64 systemID, int SEQ, SqlInt64 uketsukeNumber)
        {
            try
            {
                ...
                {
                    T_UKETSUKE_SK_DETAIL detailEntity = new T_UKETSUKE_SK_DETAIL();

                    DataGridViewRow row = this.dgvDetail.Rows[i];

                    // 受付出荷入力テーブルのSYSTEM_ID
                    ...
                    //if (row.Cells["ROW_NO"].Value == null || string.IsNullOrEmpty(row.Cells["ROW_NO"].Value.ToString().Replace(",", "").Replace(".", "")))
                    //{
                    //    detailEntity.ROW_NO = 0;
                    //}
                    //else
                    //{
                    //    detailEntity.ROW_NO = (SqlInt16)int.Parse(row.Cells["ROW_NO"].Value.ToString().Replace(",", "").Replace(".", ""));
                    //}
                    // 品名CD（必須項目）
        ...
        internal bool CreateEntityForUpdPrintFlg()
        {
            ...
            {
                LogUtility.Error("CreateEntityForUpdPrintFlg", ex);
                this.msgLogic.MessageBoxShow("E245", "");
                ret = false;
            }
            finally
        ...
        internal bool Print()
        {
            ...
            try
            {
                ...
                foreach (T_UKETSUKE_SK_ENTRY entity in this.insEntryEntityList)
                {
                    // printクラスを呼出
                    ...
                    //FormReport formReport = new FormReport(reportInfoR345_R350, WINDOW_ID.UKETSUKE_SHUKKA);
        ...
        public bool RegistData()
        {
            try
            {
                ...
                using (Transaction tran = new Transaction())
                {
                    ...
                    // 受付出荷明細レコードをループ
                    foreach (T_UKETSUKE_SK_DETAIL entity in this.insDetailEntityList)
                    {
                        // 登録処理を行う
        ...
        /// </summary>
        /// <returns>
        /// false   伝種区分が、２．出荷　３．売上支払　９．共通　以外の場合
        /// true    伝種区分が、２．出荷　３．売上支払　９．共通　の場合
        /// </returns>
        internal bool CheckHinmeiDensyu()
        {
            LogUtility.DebugMethodStart();
            ...
            // 伝種区分が、２．出荷　３．売上支払　９．共通　の場合
            if (targetHimei.DENSHU_KBN_CD == ConstClass.DENSHU_KBN_CD_SHUKKA ||
        ...
        internal bool CalcTanka(DataGridViewRow targetRow)
        {
            ...
            try
            {
                ...
                var oldTanka = targetRow.Cells["TANKA"].Value == null ? string.Empty : targetRow.Cells["TANKA"].Value.ToString(); // MAILAN #158989 START
                ...
                denshuKbnList.Add(DENSHU_KBN.SHUKKA);
                denshuKbnList.Add(DENSHU_KBN.URIAGE_SHIHARAI);
                denshuKbnList.Add(DENSHU_KBN.KYOUTSUU);

                var updateTanka = string.Empty; // MAILAN #158989 START
                // 個別品名単価から取得
                ...
                if (kobetsuHinmeiTanka == null)
                {
                    ...
                    if (kihonHinmeiTanka != null)
                    {
                        ...
                        updateTanka = kihonHinmeiTanka.TANKA.Value.ToString(); // MAILAN #158989 START
                    }
                    else
                    {
                        ...
                        updateTanka = string.Empty; // MAILAN #158989 START
                    }
                }
                else
                {
                    decimal.TryParse(Convert.ToString(kobetsuHinmeiTanka.TANKA.Value), out tanka);
                    updateTanka = kobetsuHinmeiTanka.TANKA.Value.ToString(); // MAILAN #158989 START
                }
        ...
        internal bool CalcAllDetailAndTotal()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < this.form.dgvDetail.Rows.Count - 1; i++)
                {
                    ...
                    {
                        ret = false;
                        this.ResetTankaCheck(); // MAILAN #158989 START
                        return ret;
                    }
        ...
        /// 車輌CD入力チェック
        /// </summary>
        internal bool CheckSharyou(bool isRireki = false)//CongBinh 20210713 #152803
        {
            bool ret = true;
            try
            {
                LogUtility.DebugMethodStart(isRireki);//CongBinh 20210713 #152803
                ...
                var mSharyouList = this.sharyouDao.GetAllValidDataForGyoushaKbn(keyEntity, "2", sagyouDate, true, false, false).Cast<M_SHARYOU>();
                ...
                {
                    // 該当が1件
                    SetSharyou(mSharyouList.FirstOrDefault(), isRireki);//CongBinh 20210713 #152803
                }
                else
                {
                    if (!String.IsNullOrEmpty(inputUnpanGyoushaCd) && !String.IsNullOrEmpty(inputShashuCd))
                    {
                        ...
                        else if (mSharyouList.Where(s => s.GYOUSHA_CD == inputUnpanGyoushaCd && s.SHASYU_CD == inputShashuCd).Count() == 1)
                        {
                            ...
                            SetSharyou(mSharyouList.Where(s => s.GYOUSHA_CD == inputUnpanGyoushaCd && s.SHASYU_CD == inputShashuCd).FirstOrDefault(), isRireki);//CongBinh 20210713 #152803
                        }
                        else
                        {
                            // 複数が該当
                    ...
                    else if (!String.IsNullOrEmpty(inputUnpanGyoushaCd) && String.IsNullOrEmpty(inputShashuCd))
                    {
                        ...
                        {
                            // 運搬業者CDの条件で該当が1件
                            SetSharyou(mSharyouList.Where(s => s.GYOUSHA_CD == inputUnpanGyoushaCd).FirstOrDefault(), isRireki);//CongBinh 20210713 #152803
                        }
                        else
                        {
                            // 複数が該当
                    ...
                    else if (String.IsNullOrEmpty(inputUnpanGyoushaCd) && !String.IsNullOrEmpty(inputShashuCd))
                    {
                        ...
                        {
                            // 車種CDの条件で該当が1件
                            SetSharyou(mSharyouList.Where(s => s.SHASYU_CD == inputShashuCd).FirstOrDefault(), isRireki);//CongBinh 20210713 #152803
                        }
                        else
                        {
                            // 複数が該当
        ...
        private void SetSharyou(M_SHARYOU sharyou, bool isRireki)//CongBinh 20210713 #152803
        {
            this.form.SHARYOU_NAME.Text = sharyou.SHARYOU_NAME_RYAKU;
            //CongBinh 20210713 #152803 S
            if (!isRireki)
        ...
        internal bool CheckHaishaJokyoCd(string haishaJokyoCd, out bool catchErr)
        {
            ...
            {
                LogUtility.Error("CheckHaishaJokyoCd", ex1);
                this.msgLogic.MessageBoxShow("E093", "");
                catchErr = false;
            }
            catch (Exception ex)
        ...
        private bool CheckNizumiGyoushaShokuchi()
        {
            if (!string.IsNullOrEmpty(this.form.NIZUMI_GYOUSHA_CD.Text))
            {
                bool catchErr = true;
                var gyousha = this.GetGyousha(this.form.NIZUMI_GYOUSHA_CD.Text, out catchErr);
                if (!catchErr)
                {
                    return false;
                }
                if (gyousha != null)
                {
                    if (gyousha.UNPAN_JUTAKUSHA_KAISHA_KBN.IsTrue || gyousha.SHOBUN_NIOROSHI_GYOUSHA_KBN.IsTrue)
                    {
                        ...
                        //this.form.NIZUMI_GYOUSHA_NAME.Tag = (bool)gyousha.SHOKUCHI_KBN ? string.Format(ConstClass.Hint.ZENKAKU, Strings.StrConv(this.form.NIZUMI_GYOUSHA_NAME.MaxLength.ToString(), VbStrConv.Wide)) : string.Empty;
                        this.SetCtrlReadonly(this.form.NIZUMI_GYOUSHA_NAME, !(bool)gyousha.SHOKUCHI_KBN);
                    }
                }
            }
            return true;
        ...
        private void CheckNizumiGenbaShokuchi()
        {
            ...
            {
                var genbaEntityList = this.GetGenbaList(this.form.NIZUMI_GYOUSHA_CD.Text, this.form.NIZUMI_GENBA_CD.Text);
                ...
                if (genbaEntityList != null && genbaEntityList.Length > 0)
                {
                    ...
                    if (genba.TSUMIKAEHOKAN_KBN.IsTrue || genba.SHOBUN_NIOROSHI_GENBA_KBN.IsTrue || genba.SAISHUU_SHOBUNJOU_KBN.IsTrue)
                    {
                        ...
                        //this.form.NIZUMI_GENBA_NAME.Tag = (bool)genba.SHOKUCHI_KBN ? string.Format(ConstClass.Hint.ZENKAKU, Strings.StrConv(this.form.NIZUMI_GENBA_NAME.MaxLength.ToString(), VbStrConv.Wide)) : string.Empty;
                        this.SetCtrlReadonly(this.form.NIZUMI_GENBA_NAME, !(bool)genba.SHOKUCHI_KBN);
                    }
                }
            }
        }
        ...
        internal List<mapDtoList> createMapboxDto()
        {
            try
            {
                ...
                else
                {
                    ...
                    sb.AppendFormat(" FROM M_GYOUSHA AS GYO ");
                    sb.AppendFormat(" LEFT JOIN M_GENBA GEN ON GYO.GYOUSHA_CD = GEN.GYOUSHA_CD ");
                    sb.AppendFormat(" LEFT JOIN M_TODOUFUKEN TDF ON GEN.GENBA_TODOUFUKEN_CD = TDF.TODOUFUKEN_CD ");
                    sb.AppendFormat(" WHERE GYO.DELETE_FLG = 0 ");
                    sb.AppendFormat(" AND GEN.DELETE_FLG = 0 ");
        ...
        public bool CreateMobileEntity()
        {
            ...
            try
            {
                ...
                foreach (DataRow tableRow in this.ResultTable.Rows)
                {
                    ...
                    DataTable ShukkaUketukeDetailData = this.daoUketsukeDetail.GetShukkaUketukeDetailData(SYSTEM_ID, SEQ);
        ...
        internal void RirekeSet()
        {
            ...
            else if (row[ConstClass.CELL_NAME_RIREKI_KBN].Value != null && row[ConstClass.CELL_NAME_RIREKI_KBN].Value.Equals("3"))
            {
                ...
                if (row[ConstClass.CELL_NAME_RIREKE_SS].Value != null && !string.IsNullOrEmpty(row[ConstClass.CELL_NAME_RIREKE_SS].Value.ToString()))
                {
                    ...
                    if (hinmeiTmp == null || (hinmeiTmp != null && hinmeiTmp.DENSHU_KBN_CD == ConstClass.DENSHU_KBN_CD_UKEIRE))
                    {
        ...
        internal string[] SmsParamListSetting()
        {
            ...
            if(this.form.WindowType == WINDOW_TYPE.NEW_WINDOW_FLAG)
            {
                T_UKETSUKE_SK_ENTRY entity = this.insEntryEntityList.FirstOrDefault();

                smsparamList[0] = ConstClass.DENPYOU_SHURUI_SK;
