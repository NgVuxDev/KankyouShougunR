...
namespace Shougun.Core.Reception.UketukeiIchiran.CustomControls_Ex2
{
    public delegate void CheckBoxClickedHandler2(bool state);
    public class DataGridViewCheckBoxHeaderCellEventArgs2 : EventArgs
    {
        bool _bChecked;
        public DataGridViewCheckBoxHeaderCellEventArgs2(bool bChecked)
        {
            _bChecked = bChecked;
        }
        public bool Checked
    ...
    public class DataGridViewCheckBoxHeaderCell2 : DataGridViewColumnHeaderCell
    {
        ...
        public DataGridViewCheckBoxHeaderCell2()
        {
            _idxCol = 0;
            _checked = false;
            _isCanCheck = true;
        }
        public DataGridViewCheckBoxHeaderCell2(int idxCol, bool isCanCheck)
        {
            _idxCol = idxCol;
            _checked = false;
            _isCanCheck = isCanCheck;
        }
        public DataGridViewCheckBoxHeaderCell2(int idxCol, int positionCheck)
        {
            _idxCol = idxCol;
            _positionCheck = positionCheck;
            _checked = false;
        }
        public DataGridViewCheckBoxHeaderCell2(int idxCol, string title)
        {
            _idxCol = idxCol;
            _title = title;
            _checked = false;
                ...
                && p.Y >= checkBoxLocation.Y && p.Y <= checkBoxLocation.Y + checkBoxSize.Height)
            {
                if (_isCanCheck)
                {
                    ...
                    foreach (DataGridViewRow dgvRow in this.DataGridView.Rows)
                    {
                        ...
                        {
                            // 緯度があるデータにチェックを付ける
                            if (dgvRow.Cells[ConstCls.HIDDEN_GENBA_LATITUDE].Value.ToString() != string.Empty)
                            {
