...
namespace Shougun.Core.Allocation.TeikiHaisyaIchiran
{
    ...
    public class ReportInfoR450_R551 : ReportInfoBase
    {
        ...
        private const int ConstMaxDispDetail2RowCount = 8;

        /// <summary>横Detail2ヘッダの最大表示行数を保持するフィールド</summary>
        private const int ConstMaxDispHinmeiColCount = 20;
        ...
        {
            this.strSystemDate = this.getDBDateTime().ToString() + "　発行";
        ...
        protected override void CreateDataTableInfo()
        {
            ...
            switch (this.OutputType)
            {
                case OutputTypeDef.Vertical:    // 縦出力
                    ...
                    // Page No
                    this.dataTable.Columns.Add("GROUP_PAGE");
                    // No
                    this.dataTable.Columns.Add("PHN_NO_FLB");
                    // 収集時間
                    this.dataTable.Columns.Add("PHN_SHUUSHUU_JIKAN_FLB");
                    // 業者CD
                    this.dataTable.Columns.Add("PHN_GYOUSHA_CD_FLB");
                    // 業者名
                    this.dataTable.Columns.Add("PHN_GYOUSHA_MEI_FLB");
                    // 現場CD
                    this.dataTable.Columns.Add("PHN_GENBA_CD_FLB");
                    // 現場名
                    this.dataTable.Columns.Add("PHN_GENBA_MEI_FLB");
                    // 品名CD
                    this.dataTable.Columns.Add("PHN_HINMEI_CD_FLB");
                    // 品名
                    this.dataTable.Columns.Add("PHN_HINMEI_MEI_FLB");
                    // 収集量
                    this.dataTable.Columns.Add("PHN_SUURYOU_FLB");
                    // 単位
                    this.dataTable.Columns.Add("PHN_UNIT_FLB");
                    // 換算収集量
                    this.dataTable.Columns.Add("PHN_KANSANSUURYOU_FLB");
                    // 換算単位
                    this.dataTable.Columns.Add("PHN_KANSANUNIT_FLB");
                    // 備考
                    this.dataTable.Columns.Add("PHN_BIKOU_FLB");
                    // 荷降No
                    this.dataTable.Columns.Add("PHN_NIOROSHI_NUMBER_FLB");
                    ...
                    detail1Page = (int)Math.Ceiling((decimal)detail1MaxCount / ConstMaxDispDetail1VRowCount);
                    detail2Page = (int)Math.Ceiling((decimal)detail2MaxCount / ConstMaxDispDetail2VRowCount);
                    ...
                    {
                        for (i = 0; i < ConstMaxDispDetail1VRowCount; i++)
                        {
                            row = this.dataTable.NewRow();
                            row["GROUP_PAGE"] = pageNo.ToString();
                            if (i == 0)
                            {
                                row["PHN_NO_FLB"] = "***";
                            }
                    ...
                    for (i = 0; i < dataTableDetail1Tmp.Rows.Count; i++)
                    {
                        row = this.dataTable.Rows[i];
                        // No
                        ...
                        ctrlName = "PHN_SHUUSHUU_JIKAN_FLB";
                        if (!this.IsDBNull(dataTableDetail1Tmp.Rows[i].ItemArray[index]))
                        {
                            row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index];
                        }
                        ...
                        ctrlName = "PHN_GYOUSHA_MEI_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty).SubStringByByte(40);
                        // 現場CD
                        ...
                        ctrlName = "PHN_GENBA_MEI_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty).SubStringByByte(40);
                        // 品名CD
                        index = dataTableDetail1Tmp.Columns.IndexOf("HINMEI_CD");
                        ctrlName = "PHN_HINMEI_CD_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty);
                        // 品名
                        index = dataTableDetail1Tmp.Columns.IndexOf("HINMEI_MEI");
                        ctrlName = "PHN_HINMEI_MEI_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty).SubStringByByte(20);
                        // 収集量
                        ...
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty).SubStringByByte(6);
                        // 換算収集量
                        index = dataTableDetail1Tmp.Columns.IndexOf("KANSANSUURYOU");
                        ctrlName = "PHN_KANSANSUURYOU_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty);
                        // 換算単位
                        index = dataTableDetail1Tmp.Columns.IndexOf("KANSANUNIT");
                        ctrlName = "PHN_KANSANUNIT_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty).SubStringByByte(6);
                        // 備考
                        index = dataTableDetail1Tmp.Columns.IndexOf("BIKOU");
                        ctrlName = "PHN_BIKOU_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty).SubStringByByte(40);
                        // 荷降No
                        index = dataTableDetail1Tmp.Columns.IndexOf("NIOROSHI_NUMBER");
                        ctrlName = "PHN_NIOROSHI_NUMBER_FLB";
                        row[ctrlName] = dataTableDetail1Tmp.Rows[i].ItemArray[index].ConvertToString(string.Empty);
                    }
                    // 荷降
                    for (int pageNo = 1; pageNo <= maxPage; pageNo++)
                    {
                        var lastRowOfPage = this.dataTable.AsEnumerable().Where(r => r["GROUP_PAGE"].ConvertToString() == pageNo.ToString()).LastOrDefault();
                        if (lastRowOfPage != null)
                        {
                            var nifuRows = dataTableDetail2Tmp.AsEnumerable().Skip((pageNo - 1) * ConstMaxDispDetail2VRowCount).Take(ConstMaxDispDetail2VRowCount);
                            if (nifuRows.Any())
                            {
                                for (int n = 0; n < nifuRows.Count(); n++)
                                {
                                    rowNo = n + 1;
                                    DataRow nifuRow = nifuRows.ElementAt(n);
                                    //No
                                    index = nifuRow.Table.Columns.IndexOf("NO2");
                                    ctrlName = string.Format("PHN_NO_{0}_FLB", rowNo);
                                    if (!this.IsDBNull(nifuRow.ItemArray[index]))
                                    {
                                        lastRowOfPage[ctrlName] = nifuRow.ItemArray[index];
                                    }
                                    // 荷降現場CD
                                    index = nifuRow.Table.Columns.IndexOf("NIOROSHIGENBA_CD");
                                    ...
                                    // 荷降現場名
                                    index = nifuRow.Table.Columns.IndexOf("NIOROSHIGENBA_MEI");
                                    ctrlName = string.Format("PHN_NIOROSHIGENBA_MEI_{0}_FLB", rowNo);
                                    lastRowOfPage[ctrlName] = nifuRow.ItemArray[index].ConvertToString(string.Empty).SubStringByByte(40);
                                    // 計量値（重量）
                                    index = nifuRow.Table.Columns.IndexOf("KEIRYOUCHI");
                                    ...
                                    // 搬入時間
                                    index = nifuRow.Table.Columns.IndexOf("HANNYUU_JIKAN");
                ...
                case OutputTypeDef.Holizontal:  // 横出力
                    ...
                    {
                        // 収集量(N) / 換算収集量(N)
                        ctrlName = string.Format("PHN_SUURYOU{0}_FLB", j);
                        this.dataTable.Columns.Add(ctrlName);
                        //// 換算収集量(N)
                    ...
                    //荷降
                    detail1Page = (int)Math.Ceiling((decimal)detail1MaxCount / ConstMaxDispDetail1RowCount);
                    //業者&現場
                    detail2Page = (int)Math.Ceiling((decimal)detail2MaxCount / ConstMaxDispDetail2RowCount);
                    //int intHinmeiCount = (dataTableDetail2Tmp.Columns.Count - 6) / 2;
                    int intHinmeiCount = dataTableDetail2Tmp.Columns.Count - 6;
                    //品名&単位
                    int maxLoopCount = (int)Math.Ceiling((decimal)(intHinmeiCount - 1) / ConstMaxDispHinmeiColCount);
                    ...
                    for (int pageNo = 1; pageNo <= maxPage; pageNo++)
                    {
                        ...
                        //    if (i == 0)
                        //    {
                        //        row["PHN_GYOUSHA_CD_FLB"] = "***";
                        //        row["PHN_GYOUSHA_MEI_FLB"] = "**********";
                        //    }
                        //    this.dataTable.Rows.Add(row);
                        //}
                        ...
                        //            ctrlName = string.Format("PHN_HINMEI{0}_FLB", i);
                        //            if (index > -1 && !this.IsDBNull(dataTableDetail2HeaderTmp.Rows[0].ItemArray[index]))
                        //            {
                        ...
                        //                    index = dataTableFooterTmp.Columns.IndexOf(string.Format("SUURYOU_GOUKEI{0}", j + intCount));
                        //                    ctrlName = string.Format("PHN_SUURYOU_GOUKEI{0}_FLB", j);
                        //                    if (index > -1 && !this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                        //                    {
                        //                        row[ctrlName] = dataTableFooterTmp.Rows[0].ItemArray[index];
                        //                    }
                        //                    else
                    ...
                    int hPageNo = 0;
                    //Loop by 業者&現場 Pages
                    for (j = 1; j <= detail2Page; j++)
                    {
                        //Loop by 品名&単位 Pages
                        for (i = 1; i <= maxLoopCount; i++)
                        {
                            hPageNo++;
                            var gyoushaRows = dataTableDetail2Tmp.AsEnumerable().Skip((j - 1) * ConstMaxDispDetail2RowCount).Take(ConstMaxDispDetail2RowCount);
                            //ページごとに詳細を追加 (Add detail per page)
                            for (int k = 0; k < ConstMaxDispDetail2RowCount; k++)
                            {
                                row = this.dataTable.NewRow();
                                row["GROUP_PAGE"] = hPageNo.ToString();
                                if (k == 0)
                                {
                                    row["PHN_GYOUSHA_CD_FLB"] = "***";
                                    row["PHN_GYOUSHA_MEI_FLB"] = "**********";
                                ...
                                this.dataTable.Rows.Add(row);
                                if (gyoushaRows.Any() && gyoushaRows.Count() > k)
                                {
                                    // 業者CD
                                    index = gyoushaRows.ElementAt(k).Table.Columns.IndexOf("GYOUSHA_CD");
                                    ctrlName = "PHN_GYOUSHA_CD_FLB";
                                    row[ctrlName] = gyoushaRows.ElementAt(k).ItemArray[index];

                                    // 業者名
                                    index = gyoushaRows.ElementAt(k).Table.Columns.IndexOf("GYOUSHA_MEI");
                                    ctrlName = "PHN_GYOUSHA_MEI_FLB";
                                    row[ctrlName] = gyoushaRows.ElementAt(k).ItemArray[index].SubStringByByte(40);

                                    // 現場CD
                                    index = gyoushaRows.ElementAt(k).Table.Columns.IndexOf("GENBA_CD");
                                    ctrlName = "PHN_GENBA_CD_FLB";
                                    row[ctrlName] = gyoushaRows.ElementAt(k).ItemArray[index];

                                    // 現場名
                                    index = gyoushaRows.ElementAt(k).Table.Columns.IndexOf("GENBA_MEI");
                                    ctrlName = "PHN_GENBA_MEI_FLB";
                                    row[ctrlName] = gyoushaRows.ElementAt(k).ItemArray[index].SubStringByByte(40);
                                    // 収集量(n)
                                    for (int h = 1; h <= ConstMaxDispHinmeiColCount; h++)
                                    {
                                        int begin = (ConstMaxDispHinmeiColCount * (i - 1));
                                        index = gyoushaRows.ElementAt(k).Table.Columns.IndexOf(string.Format("SUURYOU{0}", h + begin));
                                        ctrlName = string.Format("PHN_SUURYOU{0}_FLB", h);
                                        if (index > -1)
                                        {
                                            if (!this.IsDBNull(gyoushaRows.ElementAt(k).ItemArray[index]))
                                            {
                                                row[ctrlName] = gyoushaRows.ElementAt(k).ItemArray[index];
                                            }
                                            //else
                                    ...
                                    // 備考
                                    index = gyoushaRows.ElementAt(k).Table.Columns.IndexOf("BIKOU");
                                    ctrlName = "PHN_BIKOU_FLB";
                                    row[ctrlName] = gyoushaRows.ElementAt(k).ItemArray[index].SubStringByByte(40);
                                }
                            }
                            ...
                            //荷降
                            var firstRowOfPage = this.dataTable.AsEnumerable().Where(r => r["GROUP_PAGE"].ConvertToString() == hPageNo.ToString()).FirstOrDefault();
                            if (firstRowOfPage != null)
                            {
                                var nifuRows = dataTableDetail1Tmp.AsEnumerable().Skip((j - 1) * ConstMaxDispDetail1RowCount).Take(ConstMaxDispDetail1RowCount);
                                if (nifuRows.Any())
                                {
                                    for (int n = 0; n < nifuRows.Count(); n++)
                                    {
                                        ...
                                        if (!this.IsDBNull(nifuRow.ItemArray[index]))
                                        {
                                            firstRowOfPage[ctrlName] = nifuRow.ItemArray[index];
                                        }
                                        // 荷降現場CD
                                        index = nifuRow.Table.Columns.IndexOf("NIOROSHIGENBA_CD");
                                        ctrlName = string.Format("PHN_NIOROSHIGENBA_CD_{0}_FLB", rowNo);
                                        ...
                                        index = nifuRow.Table.Columns.IndexOf("NIOROSHIGENBA_MEI");
                                        ctrlName = string.Format("PHN_NIOROSHIGENBA_MEI_{0}_FLB", rowNo);
                                        firstRowOfPage[ctrlName] = nifuRow.ItemArray[index].ConvertToString(string.Empty).SubStringByByte(40);
                                        // 計量値（重量）
                                        index = nifuRow.Table.Columns.IndexOf("KEIRYOUCHI");
                                        ctrlName = string.Format("PHN_KEIRYOUCHI_{0}_FLB", rowNo);
                                        if (!this.IsDBNull(nifuRow.ItemArray[index]))
                            ...
                            {
                                //Header row
                                var headerRow = this.dataTable.AsEnumerable().Where(r => hPageNo.ToString().Equals(r["GROUP_PAGE"])).FirstOrDefault();
                                for (int h = 1; h <= ConstMaxDispHinmeiColCount; h++)
                                {
                                    int begin = (ConstMaxDispHinmeiColCount * (i - 1));
                                    // 品名
                                    index = dataTableDetail2HeaderTmp.Columns.IndexOf(string.Format("HINMEI{0}", h + begin));
                                    ctrlName = string.Format("PHN_HINMEI{0}_FLB", h);
                                    if (index > -1)
                                    {
                                        headerRow[ctrlName] = dataTableDetail2HeaderTmp.Rows[0].ItemArray[index].SubStringByByte(20);
                                    }
                                    // 単位
                                    index = dataTableDetail2HeaderTmp.Columns.IndexOf(string.Format("UNIT{0}", h + begin));
                                    ctrlName = string.Format("PHN_UNIT{0}_FLB", h);
                                    if (index > -1)
                                    {
                                        headerRow[ctrlName] = dataTableDetail2HeaderTmp.Rows[0].ItemArray[index].SubStringByByte(6);
                                    }
                                }
                            }
                            //収集量合計
                            ...
                            {
                                //フッター列 (Footer row)
                                var footerRow = this.dataTable.AsEnumerable().Where(r => hPageNo.ToString().Equals(r["GROUP_PAGE"])).LastOrDefault();
                                for (int h = 1; h <= ConstMaxDispHinmeiColCount; h++)
                                {
                                    int begin = (ConstMaxDispHinmeiColCount * (i - 1));
                                    index = dataTableFooterTmp.Columns.IndexOf(string.Format("SUURYOU_GOUKEI{0}", h + begin));
                                    ctrlName = string.Format("PHN_SUURYOU_GOUKEI{0}_FLB", h);
                                    if (index > -1)
                                    {
                                        footerRow[ctrlName] = dataTableFooterTmp.Rows[0].ItemArray[index];
                                    }
                                }
                            }
                        }
                    }
                    //荷降のページ番号が詳細ページよりも多い場合 (Page is empty detai, only contain 荷降)
                    if (maxLoopCount < detail1Page)
                    {
                        int addPage = detail1Page - maxLoopCount;
                        for (int p = 1; p <= addPage; p++)
                        {
                            hPageNo++;
                            for (int k = 0; k < ConstMaxDispDetail2RowCount; k++)
                            {
                            ...
                            if (firstRowOfPage != null)
                            {
                                var nifuRows = dataTableDetail1Tmp.AsEnumerable().Skip(((maxLoopCount + p) - 1) * ConstMaxDispDetail1RowCount).Take(ConstMaxDispDetail1RowCount);
                                if (nifuRows.Any())
                                {
                                    for (int n = 0; n < nifuRows.Count(); n++)
                                    {
        ...
        private string GetSqlStringHeaderDetai1And2TeikiHaishaV()
        {
            ...
            stringBuilder.Append("T_TEIKI_HAISHA_SHOUSAI.NIOROSHI_NUMBER AS NIOROSHI_NUMBER, ");

            stringBuilder.Append("T_TEIKI_HAISHA_SHOUSAI.UNIT_CD, ");
            stringBuilder.Append("M_UNIT.UNIT_NAME_RYAKU, ");
            stringBuilder.Append("ISNULL (T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_MOBILE_OUTPUT_FLG, 0) AS KANSAN_UNIT_MOBILE_OUTPUT_FLG, ");
            stringBuilder.Append("CASE WHEN ISNULL (T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_MOBILE_OUTPUT_FLG, 0) = 0 THEN NULL ");
            stringBuilder.Append("ELSE T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_CD ");
            stringBuilder.Append("END KANSAN_UNIT_CD, ");

            stringBuilder.Append("CASE WHEN ISNULL (T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_MOBILE_OUTPUT_FLG, 0) = 0 THEN NULL ");
            stringBuilder.Append("ELSE M_KANSAN_UNIT.UNIT_NAME_RYAKU ");
            stringBuilder.Append("END KANSAN_UNIT_NAME_RYAKU ");

            stringBuilder.Append("FROM ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY ");
            stringBuilder.Append("LEFT JOIN T_TEIKI_HAISHA_DETAIL ");
            ...
            stringBuilder.Append("ON M_SHARYOU.GYOUSHA_CD = T_TEIKI_HAISHA_ENTRY.UNPAN_GYOUSHA_CD AND T_TEIKI_HAISHA_ENTRY.SHARYOU_CD = M_SHARYOU.SHARYOU_CD ");
            ...
            stringBuilder.Append("ON M_KANSAN_UNIT.UNIT_CD = T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_CD ");
            ...
            stringBuilder.Append("ORDER BY T_TEIKI_HAISHA_DETAIL.ROW_NUMBER, ");
            stringBuilder.Append("T_TEIKI_HAISHA_SHOUSAI.HINMEI_CD");
        ...
        private string GetSqlStringHeaderDetai1And2TeikiHaishaH()
        {
            ...
            stringBuilder.Append("SELECT ");
            stringBuilder.Append("TEIKI_HAISHA.SYSTEM_ID ");
            stringBuilder.Append(",TEIKI_HAISHA.SEQ ");
            stringBuilder.Append(",TEIKI_HAISHA.SAGYOU_DATE ");
            stringBuilder.Append(",TEIKI_HAISHA.HAISHA_NO ");
            stringBuilder.Append(",TEIKI_HAISHA.UNTENSHA_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.SHAIN_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.SHASHU_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.SHASHU_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.COURSE_NAME_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.COURSE_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.SHARYOU_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.SHARYOU_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.ROUND_NO ");
            stringBuilder.Append(",TEIKI_HAISHA.GYOUSHA_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.GYOUSHA_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.GENBA_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.GENBA_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.MEISAI_BIKOU ");
            stringBuilder.Append(",TEIKI_HAISHA.HINMEI_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.UNIT_NAME_RYAKU ");
            stringBuilder.Append(",TEIKI_HAISHA.HINMEI_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.UNIT_CD ");
            stringBuilder.Append(",SUM (TEIKI_HAISHA.STANDARD_VALUE) AS STANDARD_VALUE_TOTAL ");
            stringBuilder.Append("FROM ( ");
            stringBuilder.Append("SELECT ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY.SYSTEM_ID AS SYSTEM_ID ");
            stringBuilder.Append(",T_TEIKI_HAISHA_ENTRY.SEQ AS SEQ ");
            ...
            stringBuilder.Append(",T_TEIKI_HAISHA_ENTRY.SHARYOU_CD AS SHARYOU_CD ");
            stringBuilder.Append(",M_SHARYOU.SHARYOU_NAME_RYAKU AS SHARYOU_NAME_RYAKU ");
            stringBuilder.Append(",T_TEIKI_HAISHA_DETAIL.ROUND_NO AS ROUND_NO ");
            stringBuilder.Append(",T_TEIKI_HAISHA_DETAIL.GYOUSHA_CD AS GYOUSHA_CD ");
            stringBuilder.Append(",M_GYOUSHA.GYOUSHA_NAME_RYAKU AS GYOUSHA_NAME_RYAKU ");
            stringBuilder.Append(",T_TEIKI_HAISHA_DETAIL.GENBA_CD AS GENBA_CD ");
            stringBuilder.Append(",M_GENBA.GENBA_NAME_RYAKU AS GENBA_NAME_RYAKU ");
            ...
            stringBuilder.Append(",0 AS STANDARD_VALUE ");
            stringBuilder.Append(",T_TEIKI_HAISHA_SHOUSAI.TEIKI_HAISHA_NUMBER ");
            stringBuilder.Append(",T_TEIKI_HAISHA_DETAIL.ROW_NUMBER ");
            stringBuilder.Append("FROM ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY ");
            ...
            stringBuilder.Append(",T_TEIKI_HAISHA_SHOUSAI.HINMEI_CD ");
            stringBuilder.Append(",M_HINMEI.HINMEI_NAME_RYAKU AS HINMEI_NAME_RYAKU ");
            stringBuilder.Append(",T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_CD AS UNIT_CD ");
            stringBuilder.Append(",M_UNIT_2.UNIT_NAME_RYAKU AS UNIT_NAME_RYAKU ");
            stringBuilder.Append(",T_TEIKI_HAISHA_SHOUSAI.KANSANCHI AS STANDARD_VALUE ");
            stringBuilder.Append(",T_TEIKI_HAISHA_SHOUSAI.TEIKI_HAISHA_NUMBER ");
            stringBuilder.Append(",T_TEIKI_HAISHA_DETAIL.ROW_NUMBER ");
            stringBuilder.Append("FROM ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY ");
            ...
            stringBuilder.Append("WHERE ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY.DELETE_FLG = 0 ");
            stringBuilder.Append("AND T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_MOBILE_OUTPUT_FLG = 1 ");
            stringBuilder.Append("AND	T_TEIKI_HAISHA_ENTRY.TEIKI_HAISHA_NUMBER = " + this.TeikiHaishaNo + " ");
            stringBuilder.Append(") TEIKI_HAISHA ");
            ...
            stringBuilder.Append(",TEIKI_HAISHA.HINMEI_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.UNIT_CD ");
            stringBuilder.Append(",TEIKI_HAISHA.ROW_NUMBER ");
            stringBuilder.Append("ORDER BY TEIKI_HAISHA.ROW_NUMBER ");
        ...
        private string GetSqlStringHinmeiTaniTeikiHaisha()
        {
            ...
            stringBuilder.Append("SELECT ");
            stringBuilder.Append("HINMEI_CD, ");
            stringBuilder.Append("HINMEI_UNIT.HINMEI_NAME_RYAKU, ");
            stringBuilder.Append("HINMEI_UNIT.UNIT_CD, ");
            stringBuilder.Append("HINMEI_UNIT.UNIT_NAME_RYAKU1 ");
            stringBuilder.Append("FROM ( ");
            stringBuilder.Append("SELECT ");
            stringBuilder.Append("M_HINMEI.HINMEI_NAME_RYAKU AS HINMEI_NAME_RYAKU, ");
            ...
            stringBuilder.Append("SELECT ");
            stringBuilder.Append("M_HINMEI.HINMEI_NAME_RYAKU AS HINMEI_NAME_RYAKU, ");
            stringBuilder.Append("M_UNIT_2.UNIT_NAME_RYAKU AS UNIT_NAME_RYAKU1, ");
            stringBuilder.Append("T_TEIKI_HAISHA_SHOUSAI.HINMEI_CD, ");
            stringBuilder.Append("T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_CD AS UNIT_CD ");
            stringBuilder.Append("FROM ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY ");
            ...
            stringBuilder.Append("AND T_TEIKI_HAISHA_SHOUSAI.KANSAN_UNIT_MOBILE_OUTPUT_FLG = 1 ");
            stringBuilder.Append("AND T_TEIKI_HAISHA_ENTRY.TEIKI_HAISHA_NUMBER = " + this.TeikiHaishaNo + " ");
            stringBuilder.Append(") HINMEI_UNIT ");
            stringBuilder.Append("ORDER BY HINMEI_UNIT.HINMEI_CD, HINMEI_UNIT.UNIT_CD ");
            return stringBuilder.ToString();
        }

        /// <summary>
        /// ハイフン出力用の現場で必須な品名をチェックする情報SQL
        /// </summary>
        /// <returns>SQL文</returns>
        private string GetSqlStringCheckGenbaHinmeiTeikiHaisha()
        {
            StringBuilder stringBuilder = new StringBuilder();

            stringBuilder.Append("SELECT ");
            stringBuilder.Append("THD.GYOUSHA_CD ");
            stringBuilder.Append(",THD.GENBA_CD ");
            stringBuilder.Append(",THS.HINMEI_CD ");
            stringBuilder.Append(",THS.UNIT_CD ");
            stringBuilder.Append(",THS.KANSAN_UNIT_CD ");
            stringBuilder.Append(",THS.DENPYOU_KBN_CD ");
            stringBuilder.Append(",THS.KANSAN_UNIT_MOBILE_OUTPUT_FLG ");
            stringBuilder.Append("FROM ");
            stringBuilder.Append("T_TEIKI_HAISHA_ENTRY THE ");
            stringBuilder.Append("INNER JOIN T_TEIKI_HAISHA_DETAIL THD ");
            stringBuilder.Append("ON THE.SYSTEM_ID = THD.SYSTEM_ID AND THE.SEQ = THD.SEQ ");
            stringBuilder.Append("LEFT JOIN T_TEIKI_HAISHA_SHOUSAI THS ");
            stringBuilder.Append("ON THE.SYSTEM_ID = THS.SYSTEM_ID ");
            stringBuilder.Append("AND THE.SEQ = THS.SEQ ");
            stringBuilder.Append("AND THD.DETAIL_SYSTEM_ID = THS.DETAIL_SYSTEM_ID ");
            stringBuilder.Append("WHERE ");
            stringBuilder.Append("THE.DELETE_FLG = 0 ");
            stringBuilder.Append("AND THE.TEIKI_HAISHA_NUMBER = " + this.TeikiHaishaNo + " ");
        ...
        private void GetDatabaseData()
        {
            ...
            DataTable dataTableCheckGenbaHinmeiTmp = null;

            /*
             * 定期配車実績入力後に数値が入った帳票出力は現在行わない
             * また、真レイアウトには未対応（未修正）なので使用する場合は要修正
             */
            if (!this.isTeikiJitsuseki)
            {   // 定期配車
                ...
                {   // 横
                    ...
                    sql = this.GetSqlStringCheckGenbaHinmeiTeikiHaisha();
                    dataTableCheckGenbaHinmeiTmp = this.dao.GetDateForStringSql(sql);
                }
            }
            else
            {   // 定期実績
            ...
            if (dataTableHinmeiTaniTmp != null)
            {
                if (dataTableHinmeiTaniTmp.Rows.Count != 0)
                {
                    for (intIdex = 0; intIdex < dataTableHinmeiTaniTmp.Rows.Count; intIdex++)
                    {
                        ...
                        index = dataTableHinmeiTaniTmp.Columns.IndexOf("HINMEI_CD");
                        ...
                        index = dataTableHinmeiTaniTmp.Columns.IndexOf("UNIT_CD");
                        if (!this.IsDBNull(dataTableHinmeiTaniTmp.Rows[intIdex].ItemArray[index]))
                        {
                            //shushu_tani = (string)dataTableHinmeiTaniTmp.Rows[intIdex].ItemArray[index];
                            shushu_tani = dataTableHinmeiTaniTmp.Rows[intIdex].ItemArray[index].ToString();
                        }
                        else
                        {
                        ...
                        if (index > -1 && !this.IsDBNull(dataTableHinmeiTaniTmp.Rows[intIdex].ItemArray[index]))
                        {
            ...
            {   // 縦
                ...
                //// 発行日時
                rowTmp["PRINT_DATE"] = this.strSystemDate;
                ...
                if (dataTableHeaderDetai1And2Tmp.Rows.Count != 0)
                {
                    ...
                    {   // 定期実績
                        ...
                        if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[0].ItemArray[index]))
                        {
                            ...
                            {
                                rowTmp["SHUKKO_METER"] = Convert.ToDecimal(dataTableHeaderDetai1And2Tmp.Rows[0].ItemArray[index]).ToString(jyuuryouFormat);
                            }
                        }
                        else
                        {
                    ...
                    {   // 定期実績
                        ...
                        if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[0].ItemArray[index]))
                        {
                            ...
                            {
                                rowTmp["KIKO_METER"] = Convert.ToDecimal(dataTableHeaderDetai1And2Tmp.Rows[0].ItemArray[index]).ToString(jyuuryouFormat);
                            }
                        }
                        else
                        {
                ...
                for (int i = 0; i < dataTableHeaderDetai1And2Tmp.Rows.Count; i++)
                {
                    ...
                    // No
                    var indexGyoushaCd = dataTableHeaderDetai1And2Tmp.Columns.IndexOf("GYOUSHA_CD");
                    var indexGenbaCd = dataTableHeaderDetai1And2Tmp.Columns.IndexOf("GENBA_CD");
                    if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[indexGyoushaCd]) && !this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[indexGenbaCd]))
                    {
                        rowTmp["NO"] = (i + 1).ToString();
                    }
                    else
                    {
                        rowTmp["NO"] = String.Empty;
                    }
                    ...
                    {   // 定期実績
                        ...
                        if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]))
                        {
                            ...
                            {
                                rowTmp["SUURYOU"] = Convert.ToDecimal(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]).ToString(format);
                            }
                        }
                        else
                        {
                    ...
                    {   // 定期実績
                        ...
                        if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]))
                        {
                            ...
                            rowTmp["KANSANSUURYOU"] = Convert.ToDecimal(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]).ToString(format);
                        }
                        else
                        {
                            rowTmp["KANSANSUURYOU"] = string.Empty;
                ...
                for (int i = 0; i < dataTableNioroshiMeisaiTmp.Rows.Count; i++)
                {
                    ...
                    {   // 定期実績
                        ...
                        if (!this.IsDBNull(dataTableNioroshiMeisaiTmp.Rows[i].ItemArray[index]))
                        {
                            rowTmp["KEIRYOUCHI"] = Convert.ToDecimal(dataTableNioroshiMeisaiTmp.Rows[i].ItemArray[index]).ToString(jyuuryouFormat);
                        }
                        else
                        {
                            rowTmp["KEIRYOUCHI"] = string.Empty;
            ...
            {   // 横
                ...
                {
                    // 回数
                    index = dataTableHeaderDetai1And2Tmp.Columns.IndexOf("ROUND_NO");
                    roundNo = dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index].ToString();
                    ...
                    key = roundNo + "_" + gyoushaCD + "_" + genbaCD;
                    ...
                    // 単位名
                    index = dataTableHeaderDetai1And2Tmp.Columns.IndexOf("UNIT_CD");
                    if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]))
                    {
                        shushu_tani = dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index].ToString();
                    }
                    else
                    {
                        shushu_tani = string.Empty;
                    ...
                    // 単位換算名
                    index = dataTableHeaderDetai1And2Tmp.Columns.IndexOf("KANSAN_UNIT_NAME_RYAKU");
                    if (index > -1 && !this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]))
                    {
                    ...
                    if (!this.isTeikiJitsuseki)
                    {
                        if (!string.IsNullOrEmpty(hinmei) ||
                            !string.IsNullOrEmpty(shushu_tani) ||
                            !string.IsNullOrEmpty(kansan_tani))
                        {
                            // 定期配車
                            string val = this.CheckUnitFromGenbaHinmei(dataTableCheckGenbaHinmeiTmp, gyoushaCD, genbaCD, hinmei, shushu_tani);
                            suuryou.SyuusyuuRyou = val;
                        }
                    ...
                    {   // 定期実績
                        ...
                        if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]))
                        {
                            ...
                            suuryou.SyuusyuuRyou = Convert.ToDecimal(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]).ToString(format);
                        }
                        else
                        {
                        ...
                        if (!this.IsDBNull(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]))
                        {
                            ...
                            suuryou.KansanSuuryou = Convert.ToDecimal(dataTableHeaderDetai1And2Tmp.Rows[i].ItemArray[index]).ToString(format);
                        }
                        else
                        {
                            suuryou.KansanSuuryou = string.Empty;
                ...
                for (int i = 0; i < dataTableHeaderDetai1And2Tmp.Rows.Count; i++)
                {
                    ...
                    {
                        // 収集量(N) / 収集量：換算値(N)
                        rowTmp[string.Format("SUURYOU{0}", j)] = "－";
                        ...
                        {   // 定期実績
                            ...
                            if (!this.IsDBNull(dataTableHinmeiTaniTmp.Rows[j].ItemArray[index]))
                            {
                                ...
                                else
                                {
                                    ...
                                    rowTmp[string.Format("SUURYOU_GOUKEI{0}", j + 1)] = Convert.ToDecimal(dataTableHinmeiTaniTmp.Rows[j].ItemArray[index]).ToString(format);
                                }
                            }
                            else
                            {
                            ...
                            if (!this.IsDBNull(dataTableHinmeiTaniTmp.Rows[j].ItemArray[index]))
                            {
                                ...
                                else
                                {
                                    ...
                                    rowTmp[string.Format("KANSANSUURYOU_GOUKEI{0}", j + 1)] = Convert.ToDecimal(dataTableHinmeiTaniTmp.Rows[j].ItemArray[index]).ToString(format);
                                }
                            }
                            else
                            {
        ...
        #region 定期配車 - 横：ハイフン出力チェック
        /// <summary>
        /// (定期配車 - 横フォーマット用) 品名 - 単位必須チェック
        /// 現場に属する品名と単位の組み合わせにおいて、記載の必須/必須ではないかによって帳票出力用「ハイフン」を返す
        /// </summary>
        /// <param name="dataTable">チェック用データテーブル</param>
        /// <param name="gyoushaCd">業者CD</param>
        /// <param name="genbaCd">現場CD</param>
        /// <param name="hinmeiCd">品名CD</param>
        /// <param name="unitCd">単位CD</param>
        /// <returns>記載必須：空文字　記載が必須ではない：ハイフン</returns>
        private string CheckUnitFromGenbaHinmei(DataTable dataTable, string gyoushaCd, string genbaCd, string hinmeiCd, string unitCd)
        {
            string result_Hyphen = "－";
            string result = string.Empty;

            if (dataTable == null ||
                dataTable.Rows.Count == 0)
            {
                return result_Hyphen;
            }

            // 検索条件
            StringBuilder select = new StringBuilder();
            select.Append("GYOUSHA_CD = '" + gyoushaCd + "' ");
            select.Append("AND GENBA_CD = '" + genbaCd + "' ");
            select.Append("AND HINMEI_CD = '" + hinmeiCd + "' ");
            select.Append("AND (UNIT_CD = " + unitCd + " OR KANSAN_UNIT_CD = " + unitCd + ")");

            // 検索
            DataRow[] dataRows = dataTable.Select(select.ToString());
            ...
            int unitCount = 0;
            int kansanUnitCount = 0;
            for (int i = 0; i < dataRows.Length; i++)
            {
                DataRow row = dataRows[i];

                // 単位
                if (row["UNIT_CD"] != null && unitCd.Equals(row["UNIT_CD"].ToString()))
                {
                    unitCount++;
                }

                // 換算後単位
                if (row["KANSAN_UNIT_CD"] != null && unitCd.Equals(row["KANSAN_UNIT_CD"].ToString()))
                {
                    kansanUnitCount++;
                }
            ...
            else
            {
                ...
                select.Append("AND GENBA_CD = '" + genbaCd + "' ");
                select.Append("AND HINMEI_CD = '" + hinmeiCd + "' ");
                select.Append("AND KANSAN_UNIT_CD = " + unitCd + " ");
                select.Append("AND KANSAN_UNIT_MOBILE_OUTPUT_FLG = 1");
                DataRow[] dataRows_kansan = dataTable.Select(select.ToString());

                // 記載必須である換算後単位が1件でも存在する場合は空文字
                ...
                {
                    result = result_Hyphen;
                }
            }

            return result;
        }
        #endregion 定期配車 - 横：ハイフン出力チェック
        ...
        private DateTime getDBDateTime()
        {
            ...
            GET_SYSDATEDao dao = r_framework.Utility.DaoInitUtility.GetComponent<GET_SYSDATEDao>();//DBサーバ日付を取得するため作成したDao
