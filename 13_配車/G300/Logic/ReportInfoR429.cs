...
namespace Shougun.Core.Allocation.Teikihaisyajissekihyou
{
    ...
    public class ReportInfoR429 : ReportInfoBase
    {
        ...
        protected override void CreateDataTableInfo()
        {
            ...
            var sortedList = DataTablePageList.OrderBy((o) => o.Key).ToDictionary(d => d.Key);

            int count = 0;
            int count2 = 0;
            Dictionary<string, int> result = new Dictionary<string, int>();
            foreach (string keyName1 in sortedList.Keys)
            {
                keysName2 = keyName1.Split('_');
                if (gyoushaCD + genbaCD != keysName2[0].ToString() + keysName2[1].ToString())
                {
                    count = 0;
                }
                dataTableDetailTmp = this.DataTablePageList[keyName1]["Detail"];
                count = count + ((int)Math.Ceiling((decimal)(dataTableDetailTmp.Columns.Count - 1) / ConstMaxHinmeiCount));
                if (result.ContainsKey(keysName2[0].ToString() + keysName2[1].ToString()))
                {
                    result.Remove(keysName2[0].ToString() + keysName2[1].ToString());
                }
                result.Add(keysName2[0].ToString() + keysName2[1].ToString(), count);
                gyoushaCD = keysName2[0].ToString();
                genbaCD = keysName2[1].ToString();
            }
            ...
            foreach (string keyName1 in sortedList.Keys)
            {
                ...
                dateTime = DateTime.Parse(keysName2[2]);
                gyoushaCD = keysName2[0].ToString();
                genbaCD = keysName2[1].ToString();
                ...
                for (int colRoop = 0; colRoop < roopCount; colRoop++)
                {
                    ...
                    for (y = 0; y < ConstMaxDay; y++)
                    {
                        ...
                        // 年月分
                        row["PHN_TITLE_DATE_YY_FLB"] = string.Format("{0}年{1}月分", dateTime.Year, dateTime.Month);

                        // 業者CD
                        row["PHN_GYOUSHA_CD_FLB"] = keysName2[0];
                        // 現場CD
                        row["PHN_GENBA_CD_FLB"] = keysName2[1];
                        ...
                        // 頁数
                        row["PHN_PAGE_FLB"] = count2.ToString() + " / " + result[gyoushaCD + genbaCD].ToString() + "頁";
                        #endregion - Footer -
        ...
        private void SetChouhyouInfo()
        {
            try
            {
                ...
                foreach (string keyName1 in this.DataTablePageList.Keys)
                {
                    ...
                    // 発行日付
                    this.SetFieldName("PHY_PRINT_DATE_VLB", this.getDBDateTime().ToString("yyyy/MM/dd HH:mm:ss") + " 発行");
