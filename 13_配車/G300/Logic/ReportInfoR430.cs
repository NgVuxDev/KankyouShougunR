...
namespace Shougun.Core.Allocation.Teikihaisyajissekihyou
{
    ...
    /// <summary>(R430)定期配車実績表(年報)を表すクラス・コントロール</summary>
    public class ReportInfoR430 : ReportInfoBase
    {
        #region - Fields -
        ...
        /// <summary>最大品名数を保持するフィールド</summary>
        private const int ConstSampleHinmeiCount = -1;

        /// <summary>最大年数を保持するフィールド</summary>
        private const int ConstMaxYear = 12;
        ...
        /// <summary>Initializes a new instance of the <see cref="ReportInfoR430"/> class.</summary>
        /// <param name="windowID">ウィンドウＩＤ</param>
        public ReportInfoR430(WINDOW_ID windowID)
        {
        ...
        public void CreateSampleData()
        {
            ...
            for (int year = 1; year <= 1; year++)
            {
                ...
                {
                    for (int gyoushaCD = 1; gyoushaCD <= 3; gyoushaCD++)
                    {
                        for (int genbaCD = 1; genbaCD <= 4; genbaCD++)
                        {
                            if (month == 1)
                            {
                                tmp = string.Format("{0}_{1}_{2}_{3}_{4}_{5}", year, month, year, 12, gyoushaCD, genbaCD);
                            }
                            else
                            {
                                tmp = string.Format("{0}_{1}_{2}_{3}_{4}_{5}", year, month, year + 1, month - 1, gyoushaCD, genbaCD);
                            }
                            ...
                            if (isPrint)
                            {
                                for (int i = 0; i < 12; i++)
                                {
                                    ...
                                    // 日付
                                    rowTmp["DATE"] = year.ToString() + "/" + (i + 1).ToString();
        ...
        protected override void CreateDataTableInfo()
        {
            ...
            // 頁数
            this.dataTable.Columns.Add("PHN_PAGE_FLB");
            ...
            foreach (string keyName1 in this.DataTablePageList.Keys)
            {
                ...
                if (keysName2.Length != 6)
                {   // 仕様外
                    return;
                }

                yearStart = int.Parse(keysName2[0]);
                monthStart = int.Parse(keysName2[1]);
                yearEnd = int.Parse(keysName2[2]);
                monthEnd = int.Parse(keysName2[3]);
                gyoushaCD = keysName2[4];
                genbaCD = keysName2[5];
                ...
                int roopCount = (int)Math.Ceiling((decimal)(dataTableDetailTmp.Columns.Count - 1) / ConstMaxHinmeiCount);

                for (int colRoop = 0; colRoop < roopCount; colRoop++)
                {
                    isDateForceChange = !isDateForceChange;

                    for (y = 0; y < ConstMaxYear; y++)
                    {
                        ...
                        // 年 月分
                        row["PHN_TITLE_DATE_YY_FLB"] = string.Format("{0}年{1}月　～　{2}年{3}月分", yearStart, monthStart, yearEnd, monthEnd);

                        // 業者CD
                        row["PHN_GYOUSHA_CD_FLB"] = keysName2[4];
                        // 現場CD
                        row["PHN_GENBA_CD_FLB"] = keysName2[5];
                        ...
                        // 頁数
                        row["PHN_PAGE_FLB"] = (colRoop + 1).ToString() + " / " + roopCount.ToString() + "頁";
                        #endregion - Footer -
