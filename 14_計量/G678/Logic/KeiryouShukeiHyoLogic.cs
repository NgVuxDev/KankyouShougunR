...
namespace Shougun.Core.Scale.KeiryouShukeiHyo
{
    ...
    /// 計量集計表ロジッククラス
    /// </summary>
    internal class KeiryouShukeiHyoLogic : IBuisinessLogic
    {
        /// <summary>
        /// 計量集計表画面クラス
        /// </summary>
        private KeiryouShukeiHyoUIForm form;
        ...
        /// 集計データを取得・設定します
        /// </summary>
        internal List<KeiryouData> ShukeiDataList { get; private set; }

        /// <summary>
        /// 集計したデータを取得・設定します
        /// </summary>
        internal List<KeiryouData> ShukeiDataSummaryList { get; private set; }

        /// <summary>
        /// 集計表帳票出力用DTOリストを取得・設定します
        /// </summary>
        internal List<KeiryouShukeiHyoReportDto> ShukeiHyoReportDtoList { get; private set; }
        ...
        /// コンストラクタ
        /// </summary>
        /// <param name="targetForm">計量集計表画面クラス</param>
        public KeiryouShukeiHyoLogic(KeiryouShukeiHyoUIForm targetForm)
        {
        ...
        public int Search()
        {
            ...
            try
            {
                var dao = DaoInitUtility.GetComponent<KeiryouShukeiHyoDao>();

                this.ShukeiDataList = new List<KeiryouData>();
                this.ShukeiDataList.AddRange(dao.GetUriageShukeiHyoDataKeiryou(this.form.FormDataDto));
        ...
        internal bool CSVPrint()
        {
            try
            {
                ...
                {
                    var reportLogic = new KeiryouShukeiHyoReportLogic();
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (this.errmessage.MessageBoxShow("C013") == DialogResult.Yes)
                    {
                        ...
                        // CSV出力
                        csvExport.ConvertDataTableToCsv(csvDT, true, true, "計量集計表", this.form);
                    }
            }
            catch (Exception ex)
            {
        ...
        private void CreateReportDtoList()
        {
            ...
            this.ShukeiHyoReportDtoList = this.ShukeiDataList.Select(u => new KeiryouShukeiHyoReportDto()
                                                             {
                                                                 COLUMN_1 = column1,
                                                                 COLUMN_2 = column2,
                                                                 COLUMN_3 = column3,
                                                                 ...
                                                                 NET_JYUURYOU = this.ConvertToDecimal(this.GetValue(u, "NET_JYUURYOU")),
                                                                 SUURYOU = this.ConvertToDecimal(this.GetValue(u, "SUURYOU")),
        ...
        private String GetColumnName(String cdColumnName)
        {
            ...
            if (cdColumnName.Equals("KIHON_KEIRYOU"))
            {
                ret = "KIHON_KEIRYOU_NAME";
            }
            else
            {
        ...
        private void CreateSummaryData()
        {
            ...
            this.ShukeiHyoReportDtoList = this.ShukeiHyoReportDtoList.GroupBy(u => new { u.CD_1, u.CD_2, u.CD_3, u.CD_4, u.CD_5, u.UNIT_CD })
                                                                     .Select(u => new KeiryouShukeiHyoReportDto()
                                                                     {
        ...
        private void CreateJoukenFieldData()
        {
            this.ShukeiHyoReportDtoList.ForEach(u =>
            {
                u.TITLE = "計量集計表（" + this.form.FormDataDto.Pattern.PATTERN_NAME + "）";
                //u.TITLE = this.form.FormDataDto.Pattern.PATTERN_NAME;
            ...
            jouken1.AppendLine("　[伝票区分] " + this.form.FormDataDto.DenpyouKbnName);
            ...
            jouken1.AppendLine("　[基本計量] " + this.form.FormDataDto.DenpyouShuruiName);
            ...
            if (!string.IsNullOrEmpty(this.form.FormDataDto.UriageTorihikiKbnName))
            {
                jouken1.AppendLine("　[請求取引区分] " + this.form.FormDataDto.UriageTorihikiKbnName);
            }
            if (!string.IsNullOrEmpty(this.form.FormDataDto.ShiharaiTorihikiKbnName))
            {
                jouken1.AppendLine("　[支払取引区分] " + this.form.FormDataDto.ShiharaiTorihikiKbnName);
            }
        ...
        private ButtonSetting[] CreateButtonInfo()
        {
            try
            {
                ...
                return buttonSetting.LoadButtonSetting(thisAssembly, KeiryouShukeiHyoConst.BUTTON_SETTING_XML);
            }
            catch (Exception e)
            {
                LogUtility.Error(e.Message, e);
