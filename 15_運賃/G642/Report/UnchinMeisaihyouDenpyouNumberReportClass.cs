...
namespace Shougun.Core.Carriage.UnchinMeisaihyouDto
{
    ...
    /// 運賃明細表作成クラス（伝票番号順）
    /// </summary>
    internal class UnchinMeisaihyouDenpyouNumberReportClass
    {
        /// <summary>
        /// 会社情報エンティティ
        /// </summary>
        ...
        /// 帳票テンプレートのパス
        /// </summary>
        private static readonly string OutputFormFullPathName = "./Template/R645-Form.xml";
        ...
        /// デフォルトコンストラクタ
        /// </summary>
        internal UnchinMeisaihyouDenpyouNumberReportClass()
        {
        ...
        private DataTable EditChouhyouDataTable(DataTable dt, DtoClass dto, LogicClass.Format format)
        {
            ...
            // 書式設定後の伝種区分
            //ret.Columns.Add("FH_DENSHU_KBN_NAME_RYAKU_VLB");
            // 書式設定後の正味
            ...
            foreach (DataRow row in ret.Rows)
            {
                ...
                // 伝種区分の書式設定
                if (denshuKbn == row["DENSHU_KBN_CD"].ToString())
                {
                    row["DENSHU_KBN_NAME_RYAKU"] = string.Empty;
                }
                else
                {
                    denshuKbn = row["DENSHU_KBN_CD"].ToString();
                }
