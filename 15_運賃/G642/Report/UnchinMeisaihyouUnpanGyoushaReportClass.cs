...
namespace Shougun.Core.Carriage.UnchinMeisaihyouDto
{
    ...
    /// 運賃明細表作成クラス（運搬業者CD順）
    /// </summary>
    internal class UnchinMeisaihyouUnpanGyoushaReportClass
    {
        /// <summary>
        /// 会社情報エンティティ
        /// </summary>
        ...
        /// 帳票テンプレートのパス
        /// </summary>
        private static readonly string OutputFormFullPathName = "./Template/R643-Form.xml";
        ...
        /// デフォルトコンストラクタ
        /// </summary>
        internal UnchinMeisaihyouUnpanGyoushaReportClass()
        {
        ...
        internal void CreateReport(DataTable dt, DtoClass dto, LogicClass.Format format)
        {
            ...
            // グループの表示制御
            reportInfo.SetGroupVisible("UNPANGYOUSHA", true, dto.IsGroupUnpanGyousha);
        ...
        private DataTable EditChouhyouDataTable(DataTable dt, DtoClass dto, LogicClass.Format format)
        {
            ...
            // 運搬業者合計（正味）
            ret.Columns.Add("GF_UPN_GYOUSHA_NET_JYUURYOU_VLB");
            // 総合計（正味）
            ret.Columns.Add("GF_ALL_NET_JYUURYOU_VLB");
            // 伝票合計（金額）
            ret.Columns.Add("GF_DENPYOU_KINGAKU_VLB");
            // 運搬業者合計（金額）
            ret.Columns.Add("GF_UPN_GYOUSHA_KINGAKU_VLB");
            // 総合計（金額）
            ...
            // データ整理
            // 正味重量
            decimal? numNetJyuuryou = null;
            decimal? upnNetJyuuryou = null;
            decimal? allNetJyuuryou = null;
            // 金額
            decimal? numKingaku = null;
            decimal? upnKingaku = null;
            decimal? allKingaku = null;
            // 伝票
            string num = ret.Rows[0]["DENPYOU_NUMBER"].ToString(); // 伝票番号
            string upn = ret.Rows[0]["UNPAN_GYOUSHA_CD"].ToString(); // 運搬業者
            ...
            foreach (DataRow row in ret.Rows)
            {
                ...
                if (1 == dto.Order)
                {
                    row["FH_TITLE_VLB"] += ConstClass.SORT_UNPAN_GYOUSHA_CD_SUB_TITLE;
                }
                else if (2 == dto.Order)
                {
                ...
                // 運搬業者毎リセット
                if (upn != row["UNPAN_GYOUSHA_CD"].ToString())
                {
                    upnNetJyuuryou = null;
                    upnKingaku = null;

                    upn = row["UNPAN_GYOUSHA_CD"].ToString();
                }
                ...
                if (!Convert.IsDBNull(row["NET_JYUURYOU"]))
                {
                    ...
                    if (tmpNetJyuuryou != decimal.Zero || !format.JYUURYOU_EMPTY_ZERO)
                    {
                        ...
                        upnNetJyuuryou = (upnNetJyuuryou ?? decimal.Zero) + tmpNetJyuuryou;
                ...
                if (!Convert.IsDBNull(row["KINGAKU"]))
                {
                    ...
                    if (tmpKingaku != decimal.Zero || !format.KINGAKU_EMPTY_ZERO)
                    {
                        ...
                        upnKingaku = (upnKingaku ?? decimal.Zero) + tmpKingaku;
                ...
                row["GF_UPN_GYOUSHA_NET_JYUURYOU_VLB"] = upnNetJyuuryou.HasValue ? upnNetJyuuryou.Value.ToString(format.ZERO_FORMAT_DECIMAL) : string.Empty;
                ...
                row["GF_UPN_GYOUSHA_KINGAKU_VLB"] = upnKingaku.HasValue ? upnKingaku.Value.ToString(format.ZERO_FORMAT_INTEGER) : string.Empty;
