...
namespace Shougun.Core.PayByProxy.DainoDenpyoHakkou_invoice.Report
{
    #region - Class -

    /// <summary>(R765)仕切書を表すクラス・コントロール</summary>
    public class ReportInfoR765 : ReportInfoBase
    {
        #region - Fields -
        ...
        /// <summary>Initializes a new instance of the <see cref="ReportInfoR765"/> class.</summary>
        /// <param name="windowID">ウィンドウＩＤ</param>
        public ReportInfoR765(WINDOW_ID windowID)
        {
        ...
        private void SetChouhyouInfo()
        {
            ...
            for (int pos = 1; pos <= 3; pos++)
            {
                ...
                if (dataTableFooterTmp.Rows.Count > 0)
                {
                    ...
                    //追加ラベル
                    //上段御請求額
                    index = dataTableFooterTmp.Columns.IndexOf("SOUSAI_LBL1");
                    ctrlName = string.Format("DF_SKY_SEISANGAKU_FLB{0}", pos);
                    if (!this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                    {
                        this.SetFieldName(ctrlName, (string)dataTableFooterTmp.Rows[0].ItemArray[index]);
                    ...
                    //上段差引残高
                    index = dataTableFooterTmp.Columns.IndexOf("SOUSAI_LBL2");
                    ctrlName = string.Format("DF_SKY_SASHIHIKIZANDAKA_FLB{0}", pos);
                    if (!this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                    {
                        this.SetFieldName(ctrlName, (string)dataTableFooterTmp.Rows[0].ItemArray[index]);
                    }
                    ...
                    //下段税率
                    index = dataTableFooterTmp.Columns.IndexOf("KAZEI_LBL");
                    ctrlName = string.Format("DF_SHR_ZENKAI_ZANDAKA_FLB{0}", pos);
                    if (!this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                    {
                        this.SetFieldName(ctrlName, (string)dataTableFooterTmp.Rows[0].ItemArray[index]);
                    }
                    ...
                    if (pos != 3)
                    {
                        index = dataTableFooterTmp.Columns.IndexOf("TORIHIKI_LBL");
                        ctrlName = string.Format("DH_TORIHIKISAKI_FLB{0}", pos);
                        if (!this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                        {
                            this.SetFieldName(ctrlName, (string)dataTableFooterTmp.Rows[0].ItemArray[index]);
                        }
                    ...
                    //登録番号
                    index = dataTableFooterTmp.Columns.IndexOf("TOUROKU_NO");
                    ctrlName = string.Format("DF_CORP_TOUROKU_CTL{0}", pos);
                    if (!this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                    {
                        this.SetFieldName(ctrlName, "登録番号：" + (string)dataTableFooterTmp.Rows[0].ItemArray[index]);
                    }
                    else
                    {   // Null
                        this.SetFieldName(ctrlName, string.Empty);
