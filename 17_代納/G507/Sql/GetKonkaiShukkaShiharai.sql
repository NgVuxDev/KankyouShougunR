...
ON
    ...
    -- 支払時は支払
	AND dainoShukkaEntry.SHIHARAI_TORIHIKI_KBN_CD = 2                       -- 1:現金／2:掛け ※代納は2固定
...
ON
	...
	AND dainoShukkaDetail.DENPYOU_KBN_CD = 2 								-- 1:売上(請求時)／2:支払(支払) ※変更箇所
...
WHERE
	...
	NOT EXISTS (
				...
				WHERE
					...
					AND seisanDetail.DENPYOU_SYSTEM_ID = dainoShukkaDetail.SYSTEM_ID
					AND seisanDetail.DENPYOU_SEQ = dainoShukkaDetail.SEQ
					/*IF meisaiChecked == true*/
					AND seisanDetail.DETAIL_SYSTEM_ID = dainoShukkaDetail.DETAIL_SYSTEM_ID
					/*END*/
