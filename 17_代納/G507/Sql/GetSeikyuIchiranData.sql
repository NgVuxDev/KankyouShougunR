...
/*IF densyuKbn == '1' && systemID != null*/AND UE.SYSTEM_ID <> /*systemID*//*END*/
  AND UE.URIAGE_TORIHIKI_KBN_CD <> '1'
/*IF endDay != null*/AND CONVERT(DATETIME, CONVERT(nvarchar, UE.URIAGE_DATE, 111), 120) <= /*endDay*//*END*/
  AND NOT EXISTS ( 
    SELECT
      1
    FROM
 	  T_SEIKYUU_DENPYOU SEIE INNER JOIN T_SEIKYUU_DETAIL SEID ON SEIE.SEIKYUU_NUMBER = SEID.SEIKYUU_NUMBER AND SEID.DELETE_FLG = '0'
...
UNION ALL (
	SELECT DISTINCT
		...
		THEN
			(CASE WHEN ISNULL(KD.HINMEI_ZEI_KBN_CD, 0) = 0 THEN 
				(CASE WHEN SE.URIAGE_ZEI_KEISAN_KBN_CD = 3 THEN 
					(CASE SE.URIAGE_ZEI_KBN_CD WHEN 1 THEN KD.TAX_SOTO WHEN 2 THEN KD.TAX_UCHI ELSE 0 END)
				ELSE
					0 
				END)
			ELSE
		...
		THEN
			(CASE WHEN ISNULL(KD.HINMEI_ZEI_KBN_CD, 0) = 0 THEN 
				(CASE WHEN SE.URIAGE_ZEI_KEISAN_KBN_CD = 3 THEN 
					(CASE WHEN SE.URIAGE_ZEI_KBN_CD = 1 THEN KD.TAX_SOTO ELSE 0 END)
				ELSE
					0 
				END)
			ELSE
		...
		CASE WHEN SE.KENSHU_MUST_KBN = 1
		THEN
			KD.HINMEI_ZEI_KBN_CD
		ELSE
			SD.HINMEI_ZEI_KBN_CD
		END
		AS HINMEI_ZEI_KBN_CD,
		SE.URIAGE_SHOUHIZEI_RATE
	FROM
	...
	/*IF densyuKbn == '2' && systemID != null*/AND SE.SYSTEM_ID <> /*systemID*//*END*/
	    AND SE.URIAGE_TORIHIKI_KBN_CD <> '1'
...
UNION ALL (
	...
	/*IF densyuKbn == '3' && systemID != null*/AND U_SE.SYSTEM_ID <> /*systemID*//*END*/
	    AND U_SE.URIAGE_TORIHIKI_KBN_CD <> '1'
...
UNION ALL (
	...
	/*IF startCD != null*/AND NE.TORIHIKISAKI_CD >= /*startCD*//*END*/
	/*IF endCD != null*/AND NE.TORIHIKISAKI_CD <= /*endCD*//*END*/
...
ORDER BY TORIHIKISAKI_CD, URIAGE_SHOUHIZEI_RATE, MEISAI_DATE, DENPYOU_NUMBER, DENSHU_KBN
