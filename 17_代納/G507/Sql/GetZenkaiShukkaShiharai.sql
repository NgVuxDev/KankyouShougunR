...
ON
    ...
    -- 出荷テーブルの支払時は支払フィールドを見る
	AND zenkaiDainoShukkaEntry.SHIHARAI_TORIHIKI_KBN_CD = 2                       -- 1:現金／2:掛け ※代納は2固定
...
ON
	...
	AND zenkaiDainoShukkaDetail.DENPYOU_KBN_CD = 2 								-- 1:売上(請求時)／2:支払(支払) ※変更箇所
...
WHERE
	...
	NOT EXISTS (
				...
				WHERE
					...
					AND seisanDetail.DENPYOU_SYSTEM_ID = zenkaiDainoShukkaEntry.SYSTEM_ID
					AND seisanDetail.DENPYOU_SEQ = zenkaiDainoShukkaEntry.SEQ
					/*IF meisaiChecked == true */
					AND seisanDetail.DETAIL_SYSTEM_ID = zenkaiDainoShukkaDetail.DETAIL_SYSTEM_ID
					/*END*/
