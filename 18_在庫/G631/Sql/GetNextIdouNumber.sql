...
WHERE IDOU_NUMBER = (
  ...
  CASE 
	...
	END AS IDOU_NUMBER
  FROM
   (SELECT MIN(IDOU_NUMBER) AS IDOU_NUMBER
   FROM T_ZAIKO_IDOU_ENTRY 
   WHERE DELETE_FLG = 0 /*IF idouNumber != ""*/ AND IDOU_NUMBER > /*idouNumber*/0/*END*/)TMP
  LEFT JOIN T_ZAIKO_IDOU_ENTRY TNTE ON TNTE.IDOU_NUMBER = (SELECT MIN(IDOU_NUMBER) FROM T_ZAIKO_IDOU_ENTRY WHERE DELETE_FLG = 0))
AND DELETE_FLG = 0