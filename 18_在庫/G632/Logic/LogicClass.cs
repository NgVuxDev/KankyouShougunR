...
namespace Shougun.Core.Stock.ZaikoIdouIchiran
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        /// ボタン設定格納ファイル
        /// </summary>
        private readonly string ButtonInfoXmlPath = "Shougun.Core.Stock.ZaikoIdouIchiran.Setting.ButtonSetting.xml";
        ...
        internal readonly string SELECT =
            "  T_ZAIKO_IDOU_ENTRY.IDOU_NUMBER AS 移動番号" +
            ", T_ZAIKO_IDOU_ENTRY.IDOU_DATE AS '入力日付'" +
            ", T_ZAIKO_IDOU_ENTRY.GYOUSHA_CD AS '業者CD'" +
            ", T_ZAIKO_IDOU_ENTRY.GYOUSHA_NAME AS '業者名'" +
            ", T_ZAIKO_IDOU_ENTRY.GENBA_CD AS '現場CD'" +
            ", T_ZAIKO_IDOU_ENTRY.GENBA_NAME AS '現場名'" +
            ", T_ZAIKO_IDOU_ENTRY.ZAIKO_HINMEI_CD AS '在庫品名CD'" +
            ", T_ZAIKO_IDOU_ENTRY.ZAIKO_HINMEI_NAME AS '在庫品名'" +
            ", T_ZAIKO_IDOU_ENTRY.IDOU_BEFORE_ZAIKO_RYOU AS '移動前在庫量'" +
            ", T_ZAIKO_IDOU_ENTRY.IDOU_RYOU_GOUKEI AS '移動量合計'" +
            ", T_ZAIKO_IDOU_ENTRY.IDOU_AFTER_ZAIKO_RYOU AS '移動後在庫量'" +
            ", T_ZAIKO_IDOU_ENTRY.CREATE_USER AS '作成者'" +
            ", T_ZAIKO_IDOU_ENTRY.CREATE_DATE AS '作成日時'" +
            ", T_ZAIKO_IDOU_ENTRY.CREATE_PC AS '作成PC'";
        ...
        internal bool WindowInit()
        {
            ...
            try
            {
                ...
                this.headForm.lb_title.Text = "在庫移動一覧";
                this.allControl = this.form.allControl;
                //this.form.customSortHeader1.Location = new System.Drawing.Point(3, 118);
        ...
        private void HeaderInit()
        {
            ...
            var parentForm = (BusinessBaseForm)this.form.Parent;
            parentForm.Text = r_framework.Dto.SystemProperty.CreateWindowTitle("在庫移動一覧");
        ...
        public void EventInit()
        {
            ...
            footer.bt_func5.Click += new System.EventHandler(this.bt_func5_Click);          // 複写
        ...
        private void bt_func2_Click(object sender, EventArgs e)
        {
            ...
            FormManager.OpenFormWithAuth("G631", WINDOW_TYPE.NEW_WINDOW_FLAG);
        ...
        private void bt_func3_Click(object sender, EventArgs e)
        {
            ...
            string idouNumber = Convert.ToString(row.Cells["移動番号"].Value);
            FormManager.OpenFormWithAuth("G631", WINDOW_TYPE.REFERENCE_WINDOW_FLAG, WINDOW_TYPE.REFERENCE_WINDOW_FLAG, idouNumber);
        ...
        private void bt_func5_Click(object sender, EventArgs e)
        {
            ...
            string idouNumber = Convert.ToString(row.Cells["移動番号"].Value);
            FormManager.OpenFormWithAuth("G631", WINDOW_TYPE.NEW_WINDOW_FLAG, WINDOW_TYPE.NEW_WINDOW_FLAG, idouNumber);
        ...
        private void bt_func6_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (msgLogic.MessageBoxShow("C012") == DialogResult.Yes)
                {
                    ...
                    exp.ConvertCustomDataGridViewToCsv(this.form.customDataGridView1, true, true, "在庫移動一覧", this.form);
                }
            }
            LogUtility.DebugMethodEnd();
        }
        ...
        private void bt_func10_Click(object sender, EventArgs e)
        {
            ...
            {
                // アラートを表示し、画面遷移しない
                MessageBoxShowLogic mgsLogic = new MessageBoxShowLogic();
                mgsLogic.MessageBoxShow("E076");
            }
            else
            {
                // ソート設定ダイアログを呼び出す
        ...
        public void MakeSearchCondition()
        {
            ...
            sql.Append(" WHERE ");
            sql.Append(" T_ZAIKO_IDOU_ENTRY.DELETE_FLG = 0 ");

            // 日付
            if (!string.IsNullOrEmpty(this.form.HIDUKE_FROM.Text))
            {
                sql.Append(" AND T_ZAIKO_IDOU_ENTRY.IDOU_DATE >= '" + DateTime.Parse(this.form.HIDUKE_FROM.Value.ToString()).ToShortDateString() + " 00:00:00' ");
            }
            if (!string.IsNullOrEmpty(this.form.HIDUKE_TO.Text))
            {
                sql.Append(" AND T_ZAIKO_IDOU_ENTRY.IDOU_DATE <= '" + DateTime.Parse(this.form.HIDUKE_TO.Value.ToString()).ToShortDateString() + " 23:59:59' ");
            }
            // 業者
            if (!string.IsNullOrEmpty(this.form.GYOUSHA_CD.Text))
            {
                sql.Append(" AND T_ZAIKO_IDOU_ENTRY.GYOUSHA_CD = '" + this.form.GYOUSHA_CD.Text + "' ");
            }
            // 現場
            if (!string.IsNullOrEmpty(this.form.GENBA_CD.Text))
            {
                sql.Append(" AND T_ZAIKO_IDOU_ENTRY.GENBA_CD = '" + this.form.GENBA_CD.Text + "' ");
            }
            // 在庫品名
            if (!string.IsNullOrEmpty(this.form.ZAIKO_HINMEI_CD.Text))
            {
                sql.Append(" AND T_ZAIKO_IDOU_ENTRY.ZAIKO_HINMEI_CD = '" + this.form.ZAIKO_HINMEI_CD.Text + "' ");
            }

            #region ORDERBY句

            sql.Append(" ORDER BY IDOU_NUMBER ASC");
