...
using System.Data;
using Shougun.Core.BusinessManagement.MitsumoriNyuryoku.Utility;
...
namespace Shougun.Core.BusinessManagement.MitsumoriNyuryoku.Accessor
{
    ...
    public class DBAccessor
    {
        ...
        //20250416
        DAO.IT_MITSUMORI_DETAIL_2Dao bikoDetailDao;

        /// <summary>
        /// IT_MITSUMORI_ENTRYDao
        /// </summary>
        public Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IT_MITSUMORI_ENTRYDao mitsumoriEntryDao;

        /// <summary>
        /// IM_HIKIAI_TORIHIKISAKIDao
        /// </summary>
        public Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_TORIHIKISAKIDao hikiaiTorihikisakiDao;

        /// <summary>
        /// IM_HIKIAI_TORIHIKISAKI_SEIKYUUDao
        /// </summary>
        public Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_TORIHIKISAKI_SEIKYUUDao hikiaiTorihikisakiSeikyuuDao;

        /// <summary>
        /// IM_HIKIAI_TORIHIKISAKI_SHIHARAIDao
        /// </summary>
        public Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_TORIHIKISAKI_SHIHARAIDao hikiaiTorihikisakiShiharaiDao;

        /// <summary>
        /// IM_HIKIAI_GYOUSHADao
        /// </summary>
        public Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_GYOUSHADao hikiaiGyoushaDao;

        /// <summary>
        /// IM_HIKIAI_GENBADao
        /// </summary>
        public Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_GENBADao hikiaiGenbaDao;
        ...
        /// IM_BUSHODao
        /// </summary>
        r_framework.Dao.IM_BUSHODao bushoDao;

        /// <summary>
        /// IM_GYOUSHADao
        /// </summary>
        public r_framework.Dao.IM_GYOUSHADao gyoushaDao;

        /// <summary>
        /// IM_GENBADao
        /// </summary>
        public r_framework.Dao.IM_GENBADao genbaDao;
        ...
        /// IM_TORIHIKISAKIDao
        /// </summary>
        public r_framework.Dao.IM_TORIHIKISAKIDao torihikisakiDao;
        ...
        public DBAccessor()
        {
            ...
            //20250416
            this.bikoDetailDao = DaoInitUtility.GetComponent<DAO.IT_MITSUMORI_DETAIL_2Dao>();
            ...
            this.hikiaiTorihikisakiSeikyuuDao = DaoInitUtility.GetComponent<Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_TORIHIKISAKI_SEIKYUUDao>();
            this.hikiaiTorihikisakiShiharaiDao = DaoInitUtility.GetComponent<Shougun.Core.BusinessManagement.MitsumoriNyuryoku.DAO.IM_HIKIAI_TORIHIKISAKI_SHIHARAIDao>();
            ...
            this.bushoDao = DaoInitUtility.GetComponent<r_framework.Dao.IM_BUSHODao>();
        ...
        /// <param name="tirihikisakiCd"></param>
        /// <returns></returns>
        public M_HIKIAI_TORIHIKISAKI_SEIKYUU GetHikiaiTorihikisakiSeikyuuEntry(string torihikisakiCd)
        {
            if (string.IsNullOrEmpty(torihikisakiCd))
            {
                return null;
            ...
            M_HIKIAI_TORIHIKISAKI_SEIKYUU keyEntity = new M_HIKIAI_TORIHIKISAKI_SEIKYUU();
            keyEntity.TORIHIKISAKI_CD = torihikisakiCd;
            var torihikisaki = this.hikiaiTorihikisakiSeikyuuDao.GetAllValidData(keyEntity);
            if (torihikisaki == null || torihikisaki.Length < 1)
            {
                return null;
            }
        ...
        /// <param name="tirihikisakiCd"></param>
        /// <returns></returns>
        public M_HIKIAI_TORIHIKISAKI_SHIHARAI GetHikiaiTorihikisakiShiharaiEntry(string torihikisakiCd)
        {
            if (string.IsNullOrEmpty(torihikisakiCd))
            {
                return null;
            ...
            M_HIKIAI_TORIHIKISAKI_SHIHARAI keyEntity = new M_HIKIAI_TORIHIKISAKI_SHIHARAI();
            keyEntity.TORIHIKISAKI_CD = torihikisakiCd;
            var torihikisaki = this.hikiaiTorihikisakiShiharaiDao.GetAllValidData(keyEntity);
            if (torihikisaki == null || torihikisaki.Length < 1)
            {
                return null;
            }
        ...
        /// <returns></returns>
        public M_HIKIAI_GYOUSHA GetHikiaiGyoushaEntry(string gyoushaCd)
        {
        ...
        {
            return this.genbaDao.Update(entity);
        }
        ...
        {
            return this.hikiaiGenbaDao.Update(entity);
        }
        ...
        //20250416
        public T_MITSUMORI_DETAIL_2[] GetBikoDetail(SqlInt64 entrySysId, SqlInt32 entrySeq)
        {
            T_MITSUMORI_DETAIL_2 entity = new T_MITSUMORI_DETAIL_2();
            entity.SYSTEM_ID = entrySysId;
            entity.SEQ = entrySeq;
            return bikoDetailDao.GetDataForEntity(entity);
        }

        public int InsertBikoDetail(T_MITSUMORI_DETAIL_2[] entitys)
        {
            int returnint = 0;

            foreach (var bikoDetail in entitys)
            {
                returnint = returnint + this.bikoDetailDao.Insert(bikoDetail);
            }

            return returnint;
        }

        public int UpdateBikoDetail(T_MITSUMORI_DETAIL_2[] entitys)
        {
            int returnint = 0;

            foreach (var bikoDetail in entitys)
            {
                returnint = returnint + this.bikoDetailDao.Update(bikoDetail);
            }
        ...
        /// 自動でWHERE句を生成するためこのメソッドでは指定する必要はない。
        /// </summary>
        /// <param name="busho">BUSHO_CD</param>
        /// <returns></returns>
        public M_BUSHO[] GetDataByCodeForBusho(string busho)
        {
            if (string.IsNullOrEmpty(busho))
            {
                return null;
            }

            M_BUSHO keyEntity = new M_BUSHO();
            keyEntity.BUSHO_CD = busho;
            return this.bushoDao.GetAllValidData(keyEntity);
        }
        ...
        /// <param name="tirihikisakiCd"></param>
        /// <returns></returns>
        public M_TORIHIKISAKI GetTorihikisaki(string torihikisakiCd, SqlDateTime mitsumoriDate)
        {
            if (string.IsNullOrEmpty(torihikisakiCd))
            {
                return null;
            ...
            if (torihikisaki[0].TEKIYOU_BEGIN.IsNull && torihikisaki[0].TEKIYOU_END.IsNull)
            {
                ...
                && mitsumoriDate.CompareTo(torihikisaki[0].TEKIYOU_END) <= 0)
            {
                return torihikisaki[0];
            }
            else if (!torihikisaki[0].TEKIYOU_BEGIN.IsNull && torihikisaki[0].TEKIYOU_END.IsNull
                    && mitsumoriDate.CompareTo(torihikisaki[0].TEKIYOU_BEGIN) >= 0)
            {
                return torihikisaki[0];
            }
            else if (!torihikisaki[0].TEKIYOU_BEGIN.IsNull && !torihikisaki[0].TEKIYOU_END.IsNull
        ...
        /// <param name="gyoushaCd"></param>
        /// <returns></returns>
        public M_GYOUSHA GetGyousha(string gyoushaCd, SqlDateTime mitsumoriDate)
        {
            if (string.IsNullOrEmpty(gyoushaCd))
            {
                return null;
            ...
            else
            {
                if (gyousha[0].TEKIYOU_BEGIN.IsNull && gyousha[0].TEKIYOU_END.IsNull)
                {
                    ...
                    && mitsumoriDate.CompareTo(gyousha[0].TEKIYOU_END) <= 0)
                {
                    return gyousha[0];
                }
                else if (!gyousha[0].TEKIYOU_BEGIN.IsNull && gyousha[0].TEKIYOU_END.IsNull
                        && mitsumoriDate.CompareTo(gyousha[0].TEKIYOU_BEGIN) >= 0)
                {
                    return gyousha[0];
                }
                else if (!gyousha[0].TEKIYOU_BEGIN.IsNull && !gyousha[0].TEKIYOU_END.IsNull
        ...
        /// <returns></returns>
        public M_GENBA[] GetGenba(string genbaCd, SqlDateTime mitsumoriDate)
        {
            ...
            foreach (M_GENBA entity in genba)
            {
                if (entity.TEKIYOU_BEGIN.IsNull && entity.TEKIYOU_END.IsNull)
                {
                    ...
                    && mitsumoriDate.CompareTo(entity.TEKIYOU_END) <= 0)
                {
                    returnVal.Add(entity);
                }
                else if (!entity.TEKIYOU_BEGIN.IsNull && entity.TEKIYOU_END.IsNull
                        && mitsumoriDate.CompareTo(entity.TEKIYOU_BEGIN) >= 0)
                {
                    returnVal.Add(entity);
                }
                else if (!entity.TEKIYOU_BEGIN.IsNull && !entity.TEKIYOU_END.IsNull
        ...
        /// <param name="genbaCd"></param>
        /// <returns></returns>
        public M_GENBA GetGenba(string gyoushaCd, string genbaCd, SqlDateTime mitsumoriDate)
        {
            ...
            if (genba[0].TEKIYOU_BEGIN.IsNull && genba[0].TEKIYOU_END.IsNull)
            {
                ...
                && mitsumoriDate.CompareTo(genba[0].TEKIYOU_END) <= 0)
            {
                return genba[0];
            }
            else if (!genba[0].TEKIYOU_BEGIN.IsNull && genba[0].TEKIYOU_END.IsNull
                    && mitsumoriDate.CompareTo(genba[0].TEKIYOU_BEGIN) >= 0)
            {
                return genba[0];
            }
            else if (!genba[0].TEKIYOU_BEGIN.IsNull && !genba[0].TEKIYOU_END.IsNull
        ...
        /// <param name="tirihikisakiCd"></param>
        /// <returns></returns>
        public M_HIKIAI_GENBA GetHikiaiGenbaEntry(string gyoushaCd, string genbaCd, SqlDateTime mitsumoriDate, string hikiaiGyoushaUseFlg)
        {
            if (string.IsNullOrEmpty(gyoushaCd) || string.IsNullOrEmpty(genbaCd))
            {
                return null;
            ...
            {
                keyEntity.HIKIAI_GYOUSHA_USE_FLG = SqlBoolean.Parse(hikiaiGyoushaUseFlg);
            }
            else
            {
                keyEntity.HIKIAI_GYOUSHA_USE_FLG = SqlBoolean.Parse("1");
            }
        ...
        /// <param name="gyoushaCd"></param>
        /// <returns></returns>
        public M_GENBA[] GetGenbaByGyousha(string gyoushaCd, SqlDateTime mitsumoriDate)
        {
            if (string.IsNullOrEmpty(gyoushaCd))
            {
                return null;
        ...
        /// <param name="gyoushaCd"></param>
        /// <returns></returns>
        public M_HIKIAI_GENBA[] GetGenbaByGyousha_hiki(string gyoushaCd, SqlDateTime mitsumoriDate)
        {
            if (string.IsNullOrEmpty(gyoushaCd))
            {
                return null;
            ...
            var returnVal = new List<M_HIKIAI_GENBA>();
            foreach (M_HIKIAI_GENBA entity in genba)
            {
                if (entity.TEKIYOU_BEGIN.IsNull && entity.TEKIYOU_END.IsNull)
                {
                    returnVal.Add(entity);
