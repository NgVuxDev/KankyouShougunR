...
namespace Shougun.Core.BusinessManagement.JuchuuMokuhyouKensuuNyuuryoku
{
    ...
    /// ビジネスロジック
    /// </summary>
    internal class JuchuuMokuhyouLogic : IBuisinessLogic
    {
        #region 内部定数
        ...
        /// ボタン設定格納ファイル
        /// </summary>
        private const string ButtonInfoXmlPath = "Shougun.Core.BusinessManagement.JuchuuMokuhyouKensuuNyuuryoku.Setting.ButtonSetting.xml";
        ...
        /// DTO
        /// </summary>
        private JuchuuMokuhyouDto dto;

        /// <summary>
        /// DAO
        /// </summary>
        private JuchuuMokuhyouDao dao;

        /// <summary>
        /// Form
        /// </summary>
        private JuchuuMokuhyouForm form;

        /// <summary>
        /// 更新データリスト
        /// </summary>
        List<T_JUCHU_M_KENSU> juchuKensuuListUpdate;

        /// <summary>
        /// 挿入データリスト
        /// </summary>
        List<T_JUCHU_M_KENSU> juchuKensuuListInsert;
        ...
        /// </summary>
        DataTable jinendoTable = new DataTable();
        ...
        /// 例：期首月５月の場合１月～４月
        /// </summary>
        List<string> jinendoMonthArray = new List<string>();
        ...
        /// コンストラクタ
        /// </summary>
        public JuchuuMokuhyouLogic(JuchuuMokuhyouForm targetForm)
        {
            LogUtility.DebugMethodStart(targetForm);

            this.form = targetForm;
            this.dto = new JuchuuMokuhyouDto();
            this.dao = DaoInitUtility.GetComponent<JuchuuMokuhyouDao>();
        ...
        internal bool WindowInit()
        {
            ...
            try
            {
                ...
                // 権限チェック
                if (!r_framework.Authority.Manager.CheckAuthority("G464", WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        #region 指定年度及び次年度分のリストを取得・設定します
        /// <summary>
        /// 指定年度及び次年度分のリストを取得・設定します
        /// </summary>
        internal void SetMonthArray()
        {
            ...
            {
                this.jinendoMonthArray.Add("MONTH" + i.ToString());
            }
        }
        #endregion
        ...
        void bt_func8_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // 20140714 chinchisi EV003037_年度に必須チェックがかからない start
                var parentForm = (BusinessBaseForm)this.form.Parent;
                string nendo = CorpInfoUtility.GetCurrentYear(parentForm.sysDate, (short)this.kishuMonth).ToString();
                // 年度取得
                if (this.form.dtp_nendo.Text != "" && !string.IsNullOrEmpty(this.form.dtp_nendo.Text))
                {
                    nendo = this.form.dtp_nendo.Text;
                }
                else
                {
                    this.form.dtp_nendo.Text = nendo;
                }
                // 20140714 chinchisi EV003037_年度に必須チェックがかからない end
                ...
                if (this.kishuMonth != 1)
                {
                    ...
                    this.jinendoTable = this.dao.GetDispDataForEntity(this.dto);
                    this.dto.NENDO = (Convert.ToInt16(this.dto.NENDO) - 1).ToString();
                    ...
                    for (int i = 0; i < jinendoTable.Rows.Count; i++)
                    {
                        foreach (var s in this.jinendoMonthArray)
                        {
                            table.Rows[i][s] = this.jinendoTable.Rows[i][s];
                        }
                    }
                ...
                // DGVへ値をセット
                bool catchErr = false;
                SetDataGridView(table, this.form.customDataGridView1, true, out catchErr);
                if (catchErr) { return; }
                ...
                if (msgLogic.MessageBoxShow("C046", "登録") == DialogResult.Yes)
                {
                    ...
                    if (this.kishuMonth != 1)
                    {
                        ...
                        foreach (var s in this.MonthArray)
                        {
                            ...
                            jinendoTable.Columns[s].ReadOnly = false;
                        }
                        ...
                        for (int i = 0; i < this.form.customDataGridView1.Rows.Count; i++)
                        {
                            ...
                            {
                                this.jinendoTable.Rows[i][jinendoMonthArray[monthArrayCount]] = SqlInt32.Parse(this.form.customDataGridView1.Rows[i].Cells["MONTH" + j.ToString()].Value.ToString().Replace(",", "")).Value;
                                monthArrayCount++;
                            }
                        }
                        ...
                        foreach (var s in this.MonthArray)
                        {
                            ...
                            jinendoTable.Columns[s].ReadOnly = true;
                        }

                        //データベースに登録および更新を行う。
                        var jinendoDGV = this.form.customDataGridView1;

                        // 指定年度を登録および更新
                        bool catchErr = false;
                        SetDataGridView(this.nendoTable, this.form.customDataGridView1, false, out catchErr);
                        if (catchErr) { return; }
                        bool ret = this.Data_InsertUpdate(this.form.customDataGridView1, this.form.dtp_nendo.Text);

                        // 次年度を登録および更新
                        var jinendoDGV2 = SetDataGridView(this.jinendoTable, jinendoDGV, false, out catchErr);
                        if (catchErr) { return; }
                        ret = this.Data_InsertUpdate(jinendoDGV2, (Convert.ToInt16(this.form.dtp_nendo.Text) + 1).ToString());

                        if (ret)
                        {
                            // ポップアップが上がっている時に表示が次年度
        ...
        private bool Data_InsertUpdate(CustomDataGridView CDGV, string tourokuNendo)
        {
            ...
            try
            {
                ...
                {
                    //updata
                    if (juchuKensuuListUpdate != null && juchuKensuuListUpdate.Count() > 0)
                    {
                        foreach (T_JUCHU_M_KENSU updateJuchuKensu in juchuKensuuListUpdate)
                        {
                            int updateCount = this.dao.Update(updateJuchuKensu);
                        }
                    }
                    //insert
                    if (juchuKensuuListInsert != null && juchuKensuuListInsert.Count() > 0)
                    {
                        foreach (T_JUCHU_M_KENSU insertJuchuKensu in juchuKensuuListInsert)
                        {
                            int insertCount = this.dao.Insert(insertJuchuKensu);
                        }
                    }
                    tran.Commit();
                }
        ...
        private void GetMeisaiIchiranData(CustomDataGridView CDGV, string tourokuNendo)
        {
            ...
            juchuKensuuListUpdate = new List<T_JUCHU_M_KENSU>();
            juchuKensuuListInsert = new List<T_JUCHU_M_KENSU>();
            //■■■■■■■■■■■■■■■■■■■■■■■■■
            ...
            {
                T_JUCHU_M_KENSU dispDataUpdate = new T_JUCHU_M_KENSU();
                T_JUCHU_M_KENSU dispDataInsert = new T_JUCHU_M_KENSU();
                ...
                if (!string.IsNullOrEmpty(dgvRow.Cells[17].Value.ToString()))
                {
                    ...
                    //NEW
                    var WHO = new DataBinderLogic<T_JUCHU_M_KENSU>(dispDataUpdate);
                    ...
                    dispDataUpdate.MONTH_KENSU_01 = SqlInt32.Parse(dgvRow.Cells[4].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_02 = SqlInt32.Parse(dgvRow.Cells[5].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_03 = SqlInt32.Parse(dgvRow.Cells[6].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_04 = SqlInt32.Parse(dgvRow.Cells[7].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_05 = SqlInt32.Parse(dgvRow.Cells[8].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_06 = SqlInt32.Parse(dgvRow.Cells[9].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_07 = SqlInt32.Parse(dgvRow.Cells[10].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_08 = SqlInt32.Parse(dgvRow.Cells[11].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_09 = SqlInt32.Parse(dgvRow.Cells[12].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_10 = SqlInt32.Parse(dgvRow.Cells[13].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_11 = SqlInt32.Parse(dgvRow.Cells[14].Value.ToString().Replace(",", ""));
                    dispDataUpdate.MONTH_KENSU_12 = SqlInt32.Parse(dgvRow.Cells[15].Value.ToString().Replace(",", ""));
                    //設定項目------------------
                    ...
                    //■■■■■■■■■■■■■■■■■■■■■■■■■■
                    juchuKensuuListUpdate.Add(dispDataUpdate);
                }
                if (!string.IsNullOrEmpty(dgvRow.Cells[17].Value.ToString()))
                {
                    //■■■■■■■■■■■■■■■■
                    ...
                    //NEW
                    var WHO = new DataBinderLogic<T_JUCHU_M_KENSU>(dispDataInsert);
                    ...
                    dispDataInsert.MONTH_KENSU_01 = SqlInt32.Parse(dgvRow.Cells[4].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_02 = SqlInt32.Parse(dgvRow.Cells[5].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_03 = SqlInt32.Parse(dgvRow.Cells[6].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_04 = SqlInt32.Parse(dgvRow.Cells[7].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_05 = SqlInt32.Parse(dgvRow.Cells[8].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_06 = SqlInt32.Parse(dgvRow.Cells[9].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_07 = SqlInt32.Parse(dgvRow.Cells[10].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_08 = SqlInt32.Parse(dgvRow.Cells[11].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_09 = SqlInt32.Parse(dgvRow.Cells[12].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_10 = SqlInt32.Parse(dgvRow.Cells[13].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_11 = SqlInt32.Parse(dgvRow.Cells[14].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_12 = SqlInt32.Parse(dgvRow.Cells[15].Value.ToString().Replace(",", ""));
                    //設定項目------------------
                    ...
                    juchuKensuuListInsert.Add(dispDataInsert);
                }
                else
                {
                    //■■■■■■■■
                    ...
                    dispDataInsert.SHAIN_CD = dgvRow.Cells[2].Value.ToString();
                    dispDataInsert.SHAIN_NAME = dgvRow.Cells[3].Value.ToString();
                    dispDataInsert.MONTH_KENSU_01 = Int32.Parse(dgvRow.Cells[4].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_02 = Int32.Parse(dgvRow.Cells[5].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_03 = Int32.Parse(dgvRow.Cells[6].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_04 = Int32.Parse(dgvRow.Cells[7].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_05 = Int32.Parse(dgvRow.Cells[8].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_06 = Int32.Parse(dgvRow.Cells[9].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_07 = Int32.Parse(dgvRow.Cells[10].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_08 = Int32.Parse(dgvRow.Cells[11].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_09 = Int32.Parse(dgvRow.Cells[12].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_10 = Int32.Parse(dgvRow.Cells[13].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_11 = Int32.Parse(dgvRow.Cells[14].Value.ToString().Replace(",", ""));
                    dispDataInsert.MONTH_KENSU_12 = Int32.Parse(dgvRow.Cells[15].Value.ToString().Replace(",", ""));
                    //設定項目------------------
                    ...
                    Int16 densyuKbnCd = Int16.Parse(DENSHU_KBN.JYUCYUU_MOKUHYOU.GetHashCode().ToString());
        ...
        /// <returns></returns>
        CustomDataGridView SetDataGridView(DataTable table, CustomDataGridView DataGridView, bool kishuMonthSortFlg, out bool catchErr)
        {
            ...
            {
                LogUtility.DebugMethodEnd(DataGridView, catchErr);
            }
            return DataGridView;
        }
