...
{
    public partial class PatternIchiranForm : SuperForm
    {
        #region フィールド
        /// <summary>
        /// 画面ロジック
        /// </summary>
        private PatternIchiranLogic Logic;
        ...
        public PatternIchiranForm(String paramIn_ShaInCd, string paramIn_DenshuKb)
            : base(WINDOW_ID.C_PATTERN_ICHIRAN, WINDOW_TYPE.ICHIRAN_WINDOW_FLAG)
        {
            ...
            this.Logic = new PatternIchiranLogic(this);
        ...
        {
            //画面呼び出す
            var callForm = new Shougun.Core.Common.IchiranCommon.APP.IchiranSyuForm(this.DenshuKbn, "");
            var callHeader = new Shougun.Core.Common.IchiranCommon.APP.IchiranSyuHeader();
        ...
        private void PatternIchiran_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewCell cell = this.PatternIchiran[e.ColumnIndex, e.RowIndex];
                if (cell is r_framework.CustomControl.DataGridCustomControl.DgvCustomCheckBoxCell)
                {
                    if (e.ColumnIndex == 1)
                    {
                        for (int i = 0; i < this.PatternIchiran.Rows.Count; i++)
                        {
                            if (e.RowIndex != i)
                            {
                                this.PatternIchiran[1, i].Value = false;
                            }
                        }
                    }
                }
