...
{
    class NumberReceiptAccess : IMasterDataAccess
    {
        /// <summary>
        /// 日連番
        /// </summary>
        private IS_NUMBER_RECEIPTDao Dao;
        ...
        public NumberReceiptAccess(ICustomControl control, object[] obj, object[] sendParam)
        {
            this.CheckControl = control;
            this.Param = obj;
            this.SendParam = sendParam;
            Dao = DaoInitUtility.GetComponent<IS_NUMBER_RECEIPTDao>();
        }
        ...
        public bool IsOverCDLimit(S_NUMBER_RECEIPT data, out int maxPlusKeyValue)
        {
            maxPlusKeyValue = -1;

            // 基本データ型の場合、Exceptionでエラーハンドリングしたくないので
            // メソッド内で判定
            int maxKey = this.Dao.GetMaxKey(data);
