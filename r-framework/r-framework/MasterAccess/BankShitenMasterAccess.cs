...
namespace r_framework.MasterAccess
{
    ...
    /// 銀行支店マスタアクセスクラス
    /// </summary>
    public class BankShitenMasterAccess : IMasterDataAccess
    {
        /// <summary>
        /// 銀行支店マスタのDao
        /// </summary>
        ...
        /// CodeCheckDao
        /// </summary>
        private IM_BANK_SHITEN_FOR_CODECHECKDao codeCheckDao;
        /// <summary>
        /// Entity
        /// </summary>
        ...
        /// コンストラクタ
        /// </summary>
        public BankShitenMasterAccess(ICustomControl control, object[] obj, object[] sendParam)
        {
            this.CheckControl = control;
            this.Param = obj;
            this.SendParam = sendParam;
            Dao = DaoInitUtility.GetComponent<IM_BANK_SHITENDao>();
            codeCheckDao = DaoInitUtility.GetComponent<IM_BANK_SHITEN_FOR_CODECHECKDao>();
        }
        ...
        public string CodeCheck()
        {
            ...
            // No2994-->
            string errorMessage = string.Empty;
            if (string.IsNullOrEmpty(entity.BANK_CD))
            {
                //コードが存在しない場合エラー
                var messageUtil = new MessageUtility();
                errorMessage = messageUtil.GetMessage("E012").MESSAGE;
                errorMessage = String.Format(errorMessage, "振込銀行");
                return errorMessage;
            }
            // No2994<--

            entity.BANK_SHITEN_CD = CheckControl.GetResultText();

            var returnEntitys = codeCheckDao.GetAllValidDataForCodeCheck(entity);
            ...
            if (returnEntitys.Length == 0)
            {
                ...
                errorMessage = String.Format(errorMessage, "銀行支店");
            }
            else
            {
        ...
        public string RegistCodeCheck(bool presenceFlag)
        {
            ...
            else
            {
                if (returnEntitys.Length != 0)
                {
                    ...
                    errorMessage = String.Format(errorMessage, "銀行CD、銀行支店CD、口座種類、口座番号");
                }
            }
            return errorMessage;
        }
        ...
        public virtual void CodeDataSetting()
        {
            ...
            controlUtil.setCheckDate((M_BANK_SHITEN_FOR_CODECHECK)Entity);
        }
    }
}
