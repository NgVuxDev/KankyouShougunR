...
{
    public class KeiryouChouseiMasterAccess : IMasterDataAccess
    {
        /// <summary>
        /// Dao
        /// </summary>
        ...
        /// コンストラクタ
        /// </summary>
        public KeiryouChouseiMasterAccess(ICustomControl control, object[] obj, object[] sendParam)
        {
            this.CheckControl = control;
            this.Param = obj;
            this.SendParam = sendParam;
            Dao = DaoInitUtility.GetComponent<IM_KEIRYOU_CHOUSEIDao>();
        }
        ...
        public string CodeCheck()
        {
            ...
            M_KEIRYOU_CHOUSEI entity = new M_KEIRYOU_CHOUSEI();
            if (SendParam != null)
            {
            ...
            if (returnEntitys.Length == 0)
            {
                ...
                errorMessage = String.Format(errorMessage, "計量調整");
            }
            else
            {
        ...
        public string RegistCodeCheck(bool presenceFlag)
        {
            ...
            else
            {
                if (returnEntitys.Length != 0)
                {
                    ...
                    errorMessage = String.Format(errorMessage, "取引先CD、業者CD、現場CD、品名CD、単位CD");
                }
            }
            return errorMessage;
        }
        ...
        public virtual void CodeDataSetting()
        {
            ...
            controlUtil.setCheckDate((M_KEIRYOU_CHOUSEI)Entity);
        }
        ...
        /// <returns></returns>
        private bool HasEmptyKey(M_KEIRYOU_CHOUSEI data)
        {
            if (data == null
                || string.IsNullOrEmpty(data.TORIHIKISAKI_CD)
                || string.IsNullOrEmpty(data.GYOUSHA_CD)
                || string.IsNullOrEmpty(data.GENBA_CD)
                || string.IsNullOrEmpty(data.HINMEI_CD)
                || data.UNIT_CD.IsNull)
            {
                return true;
            }
