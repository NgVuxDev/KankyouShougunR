...
{
    public class ManifestKansanMasterAccess : IMasterDataAccess
    {
        /// <summary>
        /// 取引先マスタのDao
        /// </summary>
        ...
        /// コンストラクタ
        /// </summary>
        public ManifestKansanMasterAccess(ICustomControl control, object[] obj, object[] sendParam)
        {
            this.CheckControl = control;
            this.Param = obj;
            this.SendParam = sendParam;
            Dao = DaoInitUtility.GetComponent<IM_MANIFEST_KANSANDao>();
        }
        ...
        public string CodeCheck()
        {
            ...
            if (returnEntitys.Length == 0)
            {
                ...
                errorMessage = String.Format(errorMessage, "マニフェスト換算値");
            }
            else
            {
        ...
        public string RegistCodeCheck(bool presenceFlag)
        {
            ...
            else
            {
                if (returnEntitys.Length != 0)
                {
                    ...
                    errorMessage = String.Format(errorMessage, "報告書分類CD、廃棄物名称CD、単位CD、荷姿CD");
                }
            }
            return errorMessage;
        }
        ...
        public virtual void CodeDataSetting()
        {
            ...
            controlUtil.setCheckDate((M_MANIFEST_KANSAN)Entity);
        }
        ...
        /// <returns></returns>
        private bool HasEmptyKey(M_MANIFEST_KANSAN data)
        {
            if (data == null
                || string.IsNullOrEmpty(data.HOUKOKUSHO_BUNRUI_CD)
                || string.IsNullOrEmpty(data.HAIKI_NAME_CD)
                || data.UNIT_CD.IsNull
                || string.IsNullOrEmpty(data.NISUGATA_CD))
            {
                return true;
            }
            else
