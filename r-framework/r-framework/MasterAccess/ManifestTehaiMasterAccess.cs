...
{
    public class ManifestTehaiMasterAccess : IMasterDataAccess
    {
        /// <summary>
        /// 取引先マスタのDao
        /// </summary>
        private IM_MANIFEST_TEHAIDao Dao;
        /// <summary>
        /// Entity
        /// </summary>
        ...
        /// コンストラクタ
        /// </summary>
        public ManifestTehaiMasterAccess(ICustomControl control, object[] obj, object[] sendParam)
        {
            this.CheckControl = control;
            this.Param = obj;
            this.SendParam = sendParam;
            Dao = DaoInitUtility.GetComponent<IM_MANIFEST_TEHAIDao>();
        }
        ...
        private string CodeCheck()
        {
            ...
            entity.MANIFEST_TEHAI_CD = Int16.Parse(CheckControl.GetResultText());
            ...
            if (returnEntitys.Length == 0)
            {
                ...
                errorMessage = String.Format(errorMessage, "マニフェスト手配");
            }
            else
            {
        ...
        public virtual void CodeDataSetting()
        {
            ...
            controlUtil.setCheckDate((M_MANIFEST_TEHAI)Entity);
        }
    }
}
