...
{
    /// <summary>
    /// チェックメソッドの情報をXMLより取得するクラス
    /// </summary>
    public class CheckMethodSetting
    {
        /// <summary>
        /// メソッド情報の格納クラス
        /// </summary>
        ...
        private static Dictionary<string, CheckMethodSettingDto> _methodSettingList = new Dictionary<string, CheckMethodSettingDto>(); //キャッシュするように変更
        ...
        public CheckMethodSetting()
        {
            lock (_methodSettingList)
            {
                if (_methodSettingList.Count == 0)
                {
                    ...
                    using (var resourceStream = thisAssembly.GetManifestResourceStream("r_framework.Setting.CheckMethodSetting.xml"))
                    {
