SELECT
	...
	SEI.SEIKYUU_NUMBER AS SEID_SEIKYUU_NUMBER
FROM
...
LEFT JOIN T_SEIKYUU_DETAIL AS SEID ON
...
LEFT JOIN T_SEIKYUU_DENPYOU AS SEI ON
	SEI.SEIKYUU_NUMBER = SEID.SEIKYUU_NUMBER
    /*IF QueryFlg == 0*/
        /*IF startDay != null*/
        AND CONVERT (DATETIME ,CONVERT (NVARCHAR ,SEI.SEIKYUU_DATE ,111),120) >= CONVERT (DATETIME ,CONVERT (NVARCHAR ,/*startDay*/'2013/11/01 0:00:00' ,111),120)
        /*END*/
    /*END*/
    /*IF endDay != null*/
    AND CONVERT (DATETIME ,CONVERT (NVARCHAR ,SEI.SEIKYUU_DATE ,111),120) <= CONVERT (DATETIME ,CONVERT (NVARCHAR ,/*endDay*/'2013/11/30 0:00:00' ,111),120)
    /*END*/
WHERE
    /*IF QueryFlg == 1*/
	SEI.SEIKYUU_NUMBER IS NULL AND
    /*END*/
	(UD.DENPYOU_KBN_CD = 1) AND (UE.DELETE_FLG = 0) AND (UE.TAIRYUU_KBN = 0)  
/*IF startCD != '' && startCD != null*/AND UE.TORIHIKISAKI_CD >= /*startCD*//*END*/
/*IF endCD != '' && startCD != null*/AND UE.TORIHIKISAKI_CD <= /*endCD*//*END*/
/*IF tyuusyutuKBN == 1*/
...
UNION ALL (
	...
	/*IF endDay != null*/AND CONVERT(DATETIME, CONVERT(nvarchar, SE.URIAGE_DATE, 111), 120) <= /*endDay*//*END*/
	/*END*/
...
UNION ALL (
	...
	LEFT JOIN (
		...
		) SD ON ((SE.SYSTEM_ID = SD.SYSTEM_ID) AND (SE.SEQ = SD.SEQ))
	LEFT JOIN
		dbo.M_TORIHIKI_KBN ON SE.URIAGE_TORIHIKI_KBN_CD = dbo.M_TORIHIKI_KBN.TORIHIKI_KBN_CD
	LEFT JOIN
		dbo.M_UNIT ON SD.UNIT_CD = dbo.M_UNIT.UNIT_CD
	...
	/*IF endDay != null*/AND CONVERT(DATETIME, CONVERT(nvarchar, SE.KENSHU_URIAGE_DATE, 111), 120) <= /*endDay*//*END*/
	/*END*/
	/*IF kakuteiKBN == 1*/AND SE.KAKUTEI_KBN = 1/*END*/
	/*IF torihikiKBN == 2*/AND SE.URIAGE_TORIHIKI_KBN_CD = 1/*END*/
	/*IF torihikiKBN == 1*/AND SE.URIAGE_TORIHIKI_KBN_CD = 2/*END*/
...
UNION ALL (
	...
	/*IF endDay != null*/AND CONVERT(DATETIME, CONVERT(nvarchar, U_SE.URIAGE_DATE, 111), 120) <= /*endDay*//*END*/
	/*END*/
...
UNION ALL (
	SELECT
		...
		'' AS GYOUSHA_NAME,
		'' AS GENBA_NAME,
        ND.ROW_NUMBER AS MEISAI_ROW_NO,
		'' AS HINMEI_CD,
	...
	LEFT JOIN T_SEIKYUU_DETAIL AS SEID ON
		...
		AND SEID.DENPYOU_SYSTEM_ID = ND.SYSTEM_ID
		AND SEID.DENPYOU_SEQ = ND.SEQ
		AND SEID.DETAIL_SYSTEM_ID = ND.DETAIL_SYSTEM_ID
		AND SEID.DELETE_FLG = 0
	LEFT JOIN T_SEIKYUU_DENPYOU AS SEI ON
		SEI.SEIKYUU_NUMBER = SEID.SEIKYUU_NUMBER
        /*IF QueryFlg == 0*/
	...
	/*IF endDay != null*/AND CONVERT(DATETIME, CONVERT(nvarchar, NE.DENPYOU_DATE, 111), 120) <= /*endDay*//*END*/
	/*IF shimebi != '' && shimebi != null*/AND (MTSE.SHIMEBI1 = /*shimebi*/
	OR MTSE.SHIMEBI2 = /*shimebi*/
	OR MTSE.SHIMEBI3 = /*shimebi*/)/*END*/)
/*END*/
