...
namespace Shougun.Function.ShougunCSCommon.Utility
{
	...
	public static class CommonCalc
	{
        ...
        public static decimal FractionCalc(decimal kingaku, int calcCD, short hasuKeta)
        {
            ...
            {
                case fractionType.CEILING:
                    returnVal = (Math.Ceiling(Math.Abs(kingaku) * hasuKetaCoefficient) / hasuKetaCoefficient) * sign;
                    break;
                case fractionType.FLOOR:
                    returnVal = (Math.Floor(Math.Abs(kingaku) * hasuKetaCoefficient) / hasuKetaCoefficient) * sign;
                    break;
                case fractionType.ROUND:
                    returnVal = (Math.Round(Math.Abs(kingaku) * hasuKetaCoefficient, MidpointRounding.AwayFromZero) / hasuKetaCoefficient) * sign;
                    break;
                default:
                    // NOTHING
                    break;
