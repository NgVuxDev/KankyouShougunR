...
using Shougun.Core.SalesPayment.UkeireNyuuryoku;
using Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO;
...
namespace Shougun.SalesPayment.UkeireNyuuryoku.Accessor
{
    /// <summary>
    /// DBAccessするためのクラス
    ///
    ...
    public class DBAccessor
    {
        ...
        // No.4578-->
        // 20150411 在庫品名振分追加 Start
        /// <summary>
        /// 在庫品名振分
        /// </summary>
        TZHHClass tzhhDao;
        // 20150411 在庫品名振分追加 End
        // No.4578<--
        ...
        // No.4578-->
        // 20150412 go 既存IM在庫品名DAOを利用するように改修 Start
        //MZHClass mzhDao;
        // 20150412 go 既存IM在庫品名DAOを利用するように改修 End
        ...
        public DBAccessor()
        {
            ...
            this.tcrDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TCRClass>();
            this.tcreDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TCREClass>();
            this.mtSeiDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.MTSeiClass>();
            this.mtShihaDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.MTShihaClass>();
            this.tzudDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TZUDClass>();
            // No.4578-->
            // 20150411 go 在庫品名振分処理追加 Start
            this.tzhhDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TZHHClass>();
            // 20150411 go 在庫品名振分処理追加 End
            // No.4578<--
            this.tumeDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TUMEClass>();
            this.tumdDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TUMDClass>();
            this.shashuDao = DaoInitUtility.GetComponent<r_framework.Dao.IM_SHASHUDao>();
            this.tkeDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TKEClass>();
            this.tkdDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TKDClass>();
            this.nisugataDao = DaoInitUtility.GetComponent<r_framework.Dao.IM_NISUGATADao>();
            // No.4578-->
            // 20150412 go 在庫品名DAO削除 Start
            //this.mzhDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.MZHClass>();
            // 20150412 go 在庫品名DAO削除 End
            // 20150412 go 在庫比率DAO追加 Start
            this.mzhiDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.MZHIClass>();
            // 20150412 go 在庫比率DAO追加 End
            // No.4578<--
            this.tuseDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TUSEClass>();
            this.tusdDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TUSDClass>();
            this.mkkDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.MKKClass>();
            this.tueDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TUEClass>();
            this.mcDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.MCClass>();
            ...
            this.tkujDao = DaoInitUtility.GetComponent<Shougun.Core.SalesPayment.UkeireNyuuryoku.DAO.TKUJEClass>();
        }
        #endregion
        ...
        internal DTOUketukeKeiryoClass GetKeiryou(string keiryouNum)
        {
            ...
            //コンテナCD
            //if (!retval.keiryouEntryEntity.CONTENA_SOUSA_CD.Equals(SqlInt16.Null))
            //{
            //    M_CONTENA_JOUKYOU contenaDto = this.contenaJoukyouDao.GetDataByCd(retval.keiryouEntryEntity.CONTENA_SOUSA_CD.ToString());
            //    if (contenaDto != null)
            //    {
            //        retval.strContenaName = contenaDto.CONTENA_JOUKYOU_NAME_RYAKU;
            //    }
            //}
            //マニ種類CD
            ...
            for (int i = 0; i < retval.detailCount; i++)
            {
                ...
                //荷姿単位CD
                //if (!retval.keiryouDetailEntity[i].NISUGATA_UNIT_CD.Equals(SqlInt16.Null))
                //{
                //    M_UNIT unitDto = this.unitDao.GetDataByCd(int.Parse(retval.keiryouDetailEntity[i].NISUGATA_UNIT_CD.ToString()));
                //    if (unitDto != null)
                //    {
                //        retval.strNisugataTaniName.Add(i, unitDto.UNIT_NAME_RYAKU);
                //    }
                //    else
                //    {
                //        retval.strNisugataTaniName.Add(i, "");
                //    }
                //}
            }
        ...
        {
            //// 値を配列に変換する
            //List<T_ZAIKO_UKEIRE_DETAIL>[] values = new List<T_ZAIKO_UKEIRE_DETAIL>[targetEntities.Values.Count];
            //targetEntities.Values.CopyTo(values, 0);

            //foreach (List<T_ZAIKO_UKEIRE_DETAIL> entityList in values)
        ...
        /// </summary>
        /// <param name="targetEntityLists"></param>
        //internal void UpdateZaikoUkeireDetail(List<List<T_ZAIKO_UKEIRE_DETAIL>> targetEntity)
        internal void UpdateZaikoUkeireDetails(Dictionary<T_UKEIRE_DETAIL, List<T_ZAIKO_UKEIRE_DETAIL>> targetEntityLists)
        {
            //foreach (List<T_ZAIKO_UKEIRE_DETAIL> entityLst in targetEntity)
            foreach (List<T_ZAIKO_UKEIRE_DETAIL> entityList in targetEntityLists.Values)
            {
                foreach (T_ZAIKO_UKEIRE_DETAIL entity in entityList)
                {
