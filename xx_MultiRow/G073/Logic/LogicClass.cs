...
using Shougun.Core.SalesManagement.ShiharaiMotocho.Accessor;
...
namespace Shougun.Core.SalesManagement.ShiharaiMotocho.Logic
{
    /// <summary>
    /// ビジネスロジック
    /// </summary>
    internal class LogicClass
    {
        ...
        /// 支払元帳画面Form
        /// </summary>
        private ShiharaiMotocho.APP.UIForm form;
        /// <summary>
        /// 範囲条件指定結果
        /// </summary>
        ...
        internal LogicClass(ShiharaiMotocho.APP.UIForm targetForm)
        {
        ...
        internal bool Print()
        {
            ...
            try
            {
                ...
                if (this.list != null)
                {
                    ...
                    if (table != null)
                    {
                        // 取引先請求情報により、出力する帳票の切り替え

                        // 現在表示されている一覧をレポート情報として生成
                        ReportInfoBase reportInfo = new ReportInfoBase(table);
                        reportInfo.Create(Const.UIConstans.OutputFormFullPathName, "ShiharaiMotochoReport", table);
            ...
            try
            {
                ...
                if (this.list != null)
                {
                    ...
                    if (msgLogic.MessageBoxShow("C012") == DialogResult.Yes)			// CSV出力しますか？
                    {
                        ...
                        // 支払番号項目削除対応(#2402)
                        suryouTaniKirihanasi.Columns.Remove("支払番号");
                        ...
                        CSVExport.ConvertCustomDataGridViewToCsv(dgv, true, true, WINDOW_TITLEExt.ToTitleString(WINDOW_ID.T_SHIHARAI_MOTOCHO), this.form);
        ...
        internal bool ShowPopUp(MotochoHaniJokenPopUp.Const.UIConstans.DispType type)
        {
            ...
            try
            {
                ...
                {
                    this.form.DENPYOU_DATE_LABEL.Text = "支払日付";
                }
        ...
        internal bool UpdateWindowShow()
        {
            ...
            try
            {
                ...
                if (DenpyouNum != -1)
                {
                    ...
                    switch (denshuKbn)
                    {
                        ...
                        case 20:
                            // 出金入力画面を開く
                            var HeaderBaseForm = new Shougun.Core.ReceiptPayManagement.Syukinnyuryoku.UIHeader();//.HeaderSample();
                            Shougun.Core.ReceiptPayManagement.Syukinnyuryoku.UIForm callForm = null;

                            // 修正モードの権限チェック
                            if (Manager.CheckAuthority("G090", WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                            {
                                callForm = new Shougun.Core.ReceiptPayManagement.Syukinnyuryoku.UIForm(HeaderBaseForm, WINDOW_TYPE.UPDATE_WINDOW_FLAG, DenpyouNum.ToString(), null);
                            }
                            else if (Manager.CheckAuthority("G090", WINDOW_TYPE.REFERENCE_WINDOW_FLAG, false))
                            {
                                callForm = new Shougun.Core.ReceiptPayManagement.Syukinnyuryoku.UIForm(HeaderBaseForm, WINDOW_TYPE.REFERENCE_WINDOW_FLAG, DenpyouNum.ToString(), null);
                            }
                            else
                            {
                                // 修正モードの権限なしのアラームを上げる
            ...
            {
                LogUtility.Error("UpdateWindowShow", ex1);
                this.errmessage.MessageBoxShow("E093", "");
                ret = false;
            }
            catch (Exception ex)
        ...
        private DataTable CreateReportData()
        {
            ...
            reportTable.Columns.Add("SHIHARAI_KINGAKU_STR", typeof(string));
            reportTable.Columns.Add("SHOUHIZEI_CALC_STR", typeof(string));
            reportTable.Columns.Add("SHUKKIN_KINGAKU_STR", typeof(string));
            ...
            foreach (DataRow row in reportTable.Rows)
            {
                ...
                // 20151030 katen #12048 「システム日付」の基準作成、適用 start
                row["HAKKOU_DATE"] = this.getDBDateTime();
                // 20151030 katen #12048 「システム日付」の基準作成、適用 end
                ...
                else if (param.TyuusyutuKBN == 2)   // 抽出方法が「2.支払日付：締日」の場合
                {
                    row["DATE_NAME"] = "支払日付";
                }
                ...
                row["SHIHARAI_KINGAKU_STR"] = string.Format("{0:#,0}", row["SHIHARAI_KINGAKU"]);
                ...
                row["SHUKKIN_KINGAKU_STR"] = string.Format("{0:#,0}", row["SHUKKIN_KINGAKU"]);
