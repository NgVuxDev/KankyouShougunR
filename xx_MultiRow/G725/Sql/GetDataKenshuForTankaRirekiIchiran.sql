...
FROM
	(SELECT
		...
		,KS.HINMEI_CD AS HINMEI_CD_COL
		,KS.HINMEI_NAME AS HINMEI_NAME_COL
		,KS.UNIT_CD AS UNIT_CD_COL
		,MU.UNIT_NAME_RYAKU AS UNIT_NAME_COL
		,KS.TANKA AS TANKA_COL

	FROM T_SHUKKA_ENTRY AS SE
		LEFT JOIN T_SHUKKA_DETAIL AS SD ON SE.SYSTEM_ID = SD.SYSTEM_ID AND SE.SEQ = SD.SEQ
		LEFT JOIN T_KENSHU_DETAIL AS KS ON SE.SYSTEM_ID = KS.SYSTEM_ID AND SE.SEQ = KS.SEQ AND SD.DETAIL_SYSTEM_ID = KS.DETAIL_SYSTEM_ID
		LEFT JOIN M_HINMEI AS HM ON KS.HINMEI_CD = HM.HINMEI_CD
		LEFT JOIN M_UNIT AS MU ON KS.UNIT_CD = MU.UNIT_CD
		LEFT JOIN M_SHURUI AS MS ON HM.SHURUI_CD = MS.SHURUI_CD

	WHERE SE.DELETE_FLG = 0
	AND ISNULL(KS.HINMEI_CD, '') <> ''
	...
	/*IF dto.DENPYOU_KBN != null && dto.DENPYOU_KBN != ''*/
		/*IF dto.DENPYOU_KBN == '1'*/
			AND KS.DENPYOU_KBN_CD = 1
		/*END*/
		/*IF dto.DENPYOU_KBN == '2'*/
			AND KS.DENPYOU_KBN_CD = 2
		/*END*/
	/*END*/
	...
	/*IF dto.HINMEI_CD != null && dto.HINMEI_CD != ''*/
		AND KS.HINMEI_CD = /*dto.HINMEI_CD*/'000000'
	/*END*/
